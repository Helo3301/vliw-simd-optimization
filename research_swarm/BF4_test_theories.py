"""
BF4: Breadth-First Agent 4 - Testing 10 branch computation theories
Baseline: 1,548 cycles (A1 with R10 skip)

Theories to test:
31. Use shift instead of FMA: (idx << 1) | 1 | bit
32. Use subtract: idx*2 + 2 - (1 - bit)
33. Precompute both children: left=2*idx+1, right=2*idx+2, select
34. Store 2*idx instead of idx between rounds
35. Store idx+1 instead of idx (offset addressing)
36. Use vselect for branch instead of arithmetic
37. Compute bit during previous hash stage
38. Batch branch computation for 2 rounds at once
39. Use lookup table for small idx values
40. Skip branch computation in round 0 (idx=0 known)
"""
...
class KernelBuilderBF4:
    """BF4: Testing branch optimization theories"""
    def __init__(self, theory_num):
        self.theory_num = theory_num
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_branch_baseline(desk_idx):
            """Baseline: 3 ops - FMA then add bit"""
            d = desks[desk_idx]
            self.emit("valu", ("&", d['tmp1'], d['val'], v_one))
            self.emit("valu", ("multiply_add", d['idx'], d['idx'], v_two, v_one))
            self.emit("valu", ("+", d['idx'], d['idx'], d['tmp1']))

        def emit_branch_theory31(desk_idx):
            """Theory 31: Use shift instead of FMA: (idx << 1) | 1 | bit
            3 ops: shift, and bit, or shift with 1, add bit
            Actually: shift, or 1, add bit = 3 ops (same count but different path)
            """
            d = desks[desk_idx]
            self.emit("valu", ("&", d['tmp1'], d['val'], v_one))  # bit
            self.emit("valu", ("<<", d['idx'], d['idx'], v_one))  # idx << 1
            self.emit("valu", ("|", d['idx'], d['idx'], v_one))   # | 1
            self.emit("valu", ("+", d['idx'], d['idx'], d['tmp1']))  # + bit
            # 4 ops - worse

        def emit_branch_theory32(desk_idx):
            """Theory 32: Use subtract: idx*2 + 2 - (1 - bit)
            = idx*2 + 1 + bit (same as baseline math)
            But rearranged: idx*2 + 2 - 1 + bit
            Can use: FMA(idx, 2, 2) then add bit-1
            Hmm: bit is 0 or 1, so 1-bit is 1 or 0
            idx*2 + 2 - (1-bit) = idx*2 + 1 + bit (verified)
            """
            d = desks[desk_idx]
            self.emit("valu", ("&", d['tmp1'], d['val'], v_one))  # bit (0 or 1)
            self.emit("valu", ("multiply_add", d['idx'], d['idx'], v_two, v_two))  # idx*2 + 2
            self.emit("valu", ("-", d['tmp2'], v_one, d['tmp1']))  # 1 - bit (1 or 0)
            self.emit("valu", ("-", d['idx'], d['idx'], d['tmp2']))  # idx*2 + 2 - (1-bit)
            # 4 ops - worse

        def emit_branch_theory33(desk_idx):
            """Theory 33: Precompute both children, select
            left = 2*idx + 1 (if bit=0)
            right = 2*idx + 2 (if bit=1)
            Using vselect: 4 ops minimum
            """
            d = desks[desk_idx]
            self.emit("valu", ("&", d['tmp1'], d['val'], v_one))  # bit
            self.emit("valu", ("multiply_add", d['tmp2'], d['idx'], v_two, v_one))  # left = 2*idx + 1
            self.emit("valu", ("+", d['idx'], d['tmp2'], v_one))  # right = 2*idx + 2
            self.emit("flow", ("vselect", d['idx'], d['tmp1'], d['idx'], d['tmp2']))  # select based on bit
            # 4 ops - worse

        def emit_branch_theory36(desk_idx):
            """Theory 36: Use vselect for branch instead of arithmetic
            Same as theory 33 essentially - compute both, select
            """
            d = desks[desk_idx]
            self.emit("valu", ("&", d['tmp1'], d['val'], v_one))  # bit
            self.emit("valu", ("multiply_add", d['tmp2'], d['idx'], v_two, v_one))  # left = 2*idx + 1
            self.emit("valu", ("+", d['addr'], d['tmp2'], v_one))  # right = 2*idx + 2 (using addr as temp)
            self.emit("flow", ("vselect", d['idx'], d['tmp1'], d['addr'], d['tmp2']))  # select
            # 4 ops - worse

        # For theories 34, 35, 37, 38, 39, 40 - these require structural changes
        ...
        # Select branch implementation based on theory
        if self.theory_num == 31:
            emit_branch = emit_branch_theory31
        elif self.theory_num == 32:
            emit_branch = emit_branch_theory32
        elif self.theory_num == 33:
            emit_branch = emit_branch_theory33
        elif self.theory_num == 36:
            emit_branch = emit_branch_theory36
        else:
            emit_branch = emit_branch_baseline
        ...
        def emit_rounds_0_1_2_fused(group_desks):
            ...
            if self.theory_num == 40:
                # Theory 40: Skip branch computation in round 0 (idx=0 known)
                # After round 0, idx is either 1 or 2
                # bit0 = val & 1, idx = 1 + bit0
                for d in group_desks:
                    desk = desks[d]
                    self.emit("valu", ("&", desk['bit0'], desk['val'], v_one))
                    self.emit("valu", ("+", desk['idx'], v_one, desk['bit0']))
                    # 2 ops vs 3 ops in baseline!
            else:
                for d in group_desks:
                    desk = desks[d]
                    self.emit("valu", ("&", desk['bit0'], desk['val'], v_one))
        ...
        def emit_round_10_optimized(group_desks):
            """Round 10: Skip branch computation, directly set idx=0 since all indices wrap"""
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
            for d in group_desks:
        ...
        return len(self.slots), valu_count, len(self.instrs)


def test_theory(theory_num, check=False, verbose=False):
    """Test a single theory and return results"""
    random.seed(123)
    ...
    try:
        kb = KernelBuilderBF4(theory_num)
        slots, valu_ops, _ = kb.build_kernel(forest.height, len(forest.values), len(inp.indices), 16)
        ...
        for i, ref_mem in enumerate(reference_kernel2(mem, value_trace)):
            ...
            if check:
                if machine.mem[inp_values_p : inp_values_p + len(inp.values)] != ref_mem[inp_values_p : inp_values_p + len(inp.values)]:
                    return theory_num, None, None, "FAILED_CORRECTNESS"

        return theory_num, machine.cycle, slots, "PASS"
    except Exception as e:
        return theory_num, None, None, f"ERROR: {str(e)[:50]}"


if __name__ == "__main__":
    print("BF4: Testing Branch Computation Theories")
    print("=" * 60)
    print(f"Baseline: 1,548 cycles")
    print()

    theories = [31, 32, 33, 36, 40]  # Theories with direct implementations

    results = []
    for theory in theories:
        num, cycles, slots, status = test_theory(theory, check=True)
        delta = cycles - 1548 if cycles else None
        results.append((num, cycles, slots, status, delta))

        if cycles:
            print(f"Theory {num}: {cycles} cycles ({delta:+d}), {slots} slots - {status}")
        else:
            print(f"Theory {num}: {status}")

    print()
    print("=" * 60)
    print("Summary:")
    print("=" * 60)
    for num, cycles, slots, status, delta in results:
        if cycles:
            verdict = "BETTER" if delta < 0 else "SAME" if delta == 0 else "WORSE"
            print(f"  Theory {num}: {delta:+d} cycles - {verdict}")
        else:
            print(f"  Theory {num}: {status}")
