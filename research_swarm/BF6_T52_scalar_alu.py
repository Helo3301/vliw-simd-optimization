"""
Theory 52: Move setup ops to scalar ALU (12 slots)
The scalar ALU has 12 slots per cycle. We should use scalar ALU more aggressively
for setup computations to free up VALU slots for the main computation.
"""
...
class KernelBuilderT52:
    """T52: Maximize scalar ALU usage for setup"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Batch all const loads and use parallel scalar ALU for address computation
        ...
        NUM_PRELOADED = 7
        v_tree = []
        # Pre-compute all 7 addresses with scalar ALU in parallel
        tree_addrs = [self.alloc_scratch(f"tree_addr_{i}") for i in range(NUM_PRELOADED)]
        # Emit all address computations using scalar ALU (12 slots available)
        c_zero = self.scratch_const(0)
        for i in range(NUM_PRELOADED):
            self.emit("alu", ("+", tree_addrs[i], self.scratch["forest_values_p"], self.scratch_const(i)))

        for i in range(NUM_PRELOADED):
            v_node = self.alloc_vec(f"v_tree_{i}")
            v_tree.append(v_node)
            self.emit("load", ("load", tmp_scalar, tree_addrs[i]))
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            # Batch all const loads
            for d in range(NUM_DESKS):
                self.emit("load", ("const", offset_regs[d], tile_offset + d * VLEN))

            # Use scalar ALU for all 32 address computations (12 slots per cycle)
            for d in range(NUM_DESKS):
...
if __name__ == "__main__":
    try:
        ...
        print(f"T52 RESULT: {cycles} cycles (baseline 1548)")
        print(f"T52 DELTA: {1548 - cycles}")
