...
Precompute masks for common selection patterns to avoid
computing them dynamically during execution.
"""
...
def test_theory():
    ...
    Selection masks in baseline come from:
    - val & 1 (bit extraction)

    This is data-dependent (val changes after each hash).
    Cannot precompute since val is different for each input.

    Theory N/A - masks depend on dynamic data.
    """
    return 1548
...
if __name__ == "__main__":
    cycles = test_theory()
    print(f"Theory 87 (Precompute masks): {cycles} cycles")
    print(f"Delta from baseline (1548): {cycles - 1548}")
    print("Note: Theory N/A - masks depend on dynamic hash values")
