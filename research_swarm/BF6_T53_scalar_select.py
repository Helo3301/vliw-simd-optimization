"""
Theory 53: Use scalar select instead of vselect
The scalar select in FLOW engine can handle conditions when they're uniform.
However, in tree traversal the conditions are typically divergent so this
is unlikely to help. Testing anyway.
"""
...
class KernelBuilderT53:
    """T53: Scalar select - can't replace vselect (divergent conditions)
    This is essentially the baseline with no change."""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        # This is the baseline - scalar select cannot replace vselect
        # because the conditions are per-lane divergent
...
if __name__ == "__main__":
    try:
        ...
        print(f"T53 RESULT: {cycles} cycles (baseline 1548)")
        print(f"T53 DELTA: {1548 - cycles}")
        print("T53 NOTE: Scalar select cannot replace vselect (divergent per-lane conditions)")
