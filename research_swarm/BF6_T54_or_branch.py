"""
Theory 54: Exploit OR instruction for branch
Use OR to combine bit computations for branch decisions.
"""
...
class KernelBuilderT54:
    """T54: Try OR for branch computation
    Branch is: idx = 2*idx + 1 + bit
    OR doesn't help here - stick with baseline"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_branch_with_or(desk_idx):
            """Try: idx = (2*idx + 1) | bit  -- equivalent to + when bit is 0/1 and idx*2+1 is odd"""
            d = desks[desk_idx]
...
if __name__ == "__main__":
    try:
        ...
        print(f"T54 RESULT: {cycles} cycles (baseline 1548)")
        print(f"T54 DELTA: {1548 - cycles}")
