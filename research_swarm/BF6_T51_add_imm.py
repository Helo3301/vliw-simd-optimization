"""
Theory 51: Use add_imm for constant additions (FLOW engine)
The add_imm instruction in FLOW engine can add an immediate to a register.
This frees up ALU slots for other operations.
"""
...
class KernelBuilderT51:
    """T51: Use add_imm for constant additions"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Use add_imm instead of ALU for constant address offsets
        ...
        # Load index 0 first, then use add_imm for sequential loads
        self.emit("load", ("const", tmp_scalar, 1))  # Start with index 1
        self.emit("load", ("load", self.scratch["n_nodes"], tmp_scalar))

        # Use add_imm to increment address for sequential loads
        self.emit("flow", ("add_imm", tmp_scalar, tmp_scalar, 3))  # tmp_scalar = 1 + 3 = 4
        self.emit("load", ("load", self.scratch["forest_values_p"], tmp_scalar))
        self.emit("flow", ("add_imm", tmp_scalar, tmp_scalar, 1))  # tmp_scalar = 5
        self.emit("load", ("load", self.scratch["inp_indices_p"], tmp_scalar))
        self.emit("flow", ("add_imm", tmp_scalar, tmp_scalar, 1))  # tmp_scalar = 6
        self.emit("load", ("load", self.scratch["inp_values_p"], tmp_scalar))
        ...
        for i in range(NUM_PRELOADED):
            ...
            # Use add_imm for address computation instead of ALU
            if i == 0:
                self.emit("load", ("load", tmp_scalar, self.scratch["forest_values_p"]))
            else:
                self.emit("flow", ("add_imm", tmp_addr, self.scratch["forest_values_p"], i))
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for d in range(NUM_DESKS):
                # Use add_imm for address computation
                self.emit("flow", ("add_imm", addr_tmp[d*2], self.scratch["inp_indices_p"], tile_offset + d * VLEN))
                self.emit("flow", ("add_imm", addr_tmp[d*2+1], self.scratch["inp_values_p"], tile_offset + d * VLEN))
...
if __name__ == "__main__":
    try:
        ...
        print(f"T51 RESULT: {cycles} cycles (baseline 1548)")
        print(f"T51 DELTA: {1548 - cycles}")
