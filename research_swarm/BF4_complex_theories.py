"""
BF4: Complex theories that require structural changes
...
class KernelBuilderTheory37:
    """Theory 37: Compute bit during previous hash stage

    The idea: The last hash stage computes:
        val = val ^ hash_const ^ (val >> shift)
    We need bit = val & 1 afterward.

    Can we pipeline this? The bit depends on the final hash result,
    so we can't truly compute it early. But we CAN try to overlap
    the AND operation with the hash computation by exploiting that
    the scheduler might place it better.

    Actually, the & 1 is already quite cheap (1 cycle) and happens
    right after hash. The real question is: can we FUSE it?

    Observation: The final hash stage XORs create the LSB.
    The LSB of (a ^ b ^ c) is (a[0] ^ b[0] ^ c[0]).
    We could theoretically compute this incrementally...
    but that requires 2+ extra ops.

    Let's try: emit the AND in parallel with the last hash XOR
    by reordering.
    """
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_hash_with_bit_pipeline(group_desks):
            """Hash with AND 1 pipelined into last stage"""
            ...
            # Stage 5: Emit XOR and AND interleaved to pipeline
            for d in group_desks:
                desk = desks[d]
            ...
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['tmp1'], desk['tmp2']))
            # AND 1 immediately after - scheduler should place well
            for d in group_desks:
                desk = desks[d]
        ...
        def emit_branch_using_precomputed_bit(desk_idx):
            """Use bit already computed in tmp1"""
            d = desks[desk_idx]
            self.emit("valu", ("multiply_add", d['idx'], d['idx'], v_two, v_one))
            self.emit("valu", ("+", d['idx'], d['idx'], d['tmp1']))
        ...
        def emit_rounds_0_1_2_fused(group_desks):
            ...
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            emit_hash_with_bit_pipeline(group_desks)  # Hash + compute bit
            for d in group_desks:
                emit_branch_using_precomputed_bit(d)
        ...
        def emit_rounds_11_12_13_fused(group_desks):
            ...
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            emit_hash_with_bit_pipeline(group_desks)
            for d in group_desks:
                emit_branch_using_precomputed_bit(d)

        def emit_gather_round_with_pipeline(group_desks):
            """Gather round with pipelined bit computation"""
            for d in group_desks:
                desk = desks[d]
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            GROUP_SIZE = 4
            all_groups = [list(range(g * GROUP_SIZE, (g + 1) * GROUP_SIZE)) for g in range(4)]

            for group_desks in all_groups:
                emit_rounds_0_1_2_fused(group_desks)
                # Use pipelined version for gather rounds
                for _rnd in range(3, 10):
                    emit_gather_round_with_pipeline(group_desks)
        ...
        for i, ref_mem in enumerate(reference_kernel2(mem, value_trace)):
            ...
            if check:
                if machine.mem[inp_values_p : inp_values_p + len(inp.values)] != ref_mem[inp_values_p : inp_values_p + len(inp.values)]:
                    return 37, None, None, "FAILED_CORRECTNESS"

        return 37, machine.cycle, slots, "PASS"
    except Exception as e:
        import traceback
        traceback.print_exc()
        return 37, None, None, f"ERROR: {str(e)[:50]}"


if __name__ == "__main__":
    print("BF4: Testing Complex Theories")
    print("=" * 60)
    print(f"Baseline: 1,548 cycles")
    print()

    # Test Theory 37
    num, cycles, slots, status = test_theory37(check=True)
    if cycles:
        delta = cycles - 1548
        print(f"Theory 37: {cycles} cycles ({delta:+d}), {slots} slots - {status}")
    else:
        print(f"Theory 37: {status}")
