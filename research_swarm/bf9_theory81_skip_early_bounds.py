"""
Theory 81: Skip bounds check in rounds 0-9 (can't exceed n_nodes)

In early rounds (0-9), the index mathematically cannot exceed n_nodes (2047).
Skipping the bounds check saves 2 ops per round per desk.
"""
...
from problem import (
    Engine, SLOT_LIMITS, VLEN, N_CORES, SCRATCH_SIZE, Machine, Tree, Input,
...
class KernelBuilder81:
    """Theory 81: Skip bounds check in early rounds"""
    def __init__(self):
        self.slots = []
        self.scratch = {}
        self.scratch_debug = {}
    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        ...
        # THEORY 81: emit_branch_no_bounds - skip bounds check for rounds 0-9
        def emit_branch_no_bounds(desk_idx):
            """Branch without bounds check (for rounds 0-9 where idx < n_nodes guaranteed)"""
            d = desks[desk_idx]
            self.emit("valu", ("&", d['tmp1'], d['val'], v_one))
            self.emit("valu", ("multiply_add", d['idx'], d['idx'], v_two, v_one))
            self.emit("valu", ("+", d['idx'], d['idx'], d['tmp1']))
        ...
        def emit_rounds_0_1_2_fused(group_desks):
            ...
            emit_hash_interleaved(group_desks)
            for d in group_desks:
                emit_branch_no_bounds(d)  # NO bounds check
        ...
        # THEORY 81: No bounds check in gather rounds 3-9
        def emit_gather_round_no_bounds(group_desks):
            for d in group_desks:
                desk = desks[d]
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                ...
                # Rounds 3-9: NO bounds check
                for _rnd in range(3, 10):
                    emit_gather_round_no_bounds(group_desks)
...
def test_theory():
    ...
    value_trace = {}
    machine = Machine(mem, kb.instrs, DebugInfo(scratch_map=kb.scratch_debug), n_cores=N_CORES, value_trace=value_trace)
...
if __name__ == "__main__":
    cycles = test_theory()
    print(f"Theory 81 (Skip early bounds): {cycles} cycles")
