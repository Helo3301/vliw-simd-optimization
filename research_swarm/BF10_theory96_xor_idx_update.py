"""
Theory 96: Use XOR for idx update instead of arithmetic
Hypothesis: XOR operations might have better scheduling characteristics than
multiply-add for the branch index computation.

Currently: idx = idx*2 + 1 + bit  (multiply_add + add)
Try: idx = (idx << 1) ^ bit ^ 1   (shift + xor + xor)

This is mathematically different but let's see if it's faster in cycles.
"""
...
class KernelBuilderT96:
    """Theory 96: XOR-based idx update (baseline - will break correctness)"""
    def __init__(self):
...
if __name__ == "__main__":
    ...
    print(f"Theory 96 - XOR idx update (baseline): {cycles} cycles")
