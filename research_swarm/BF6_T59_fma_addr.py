"""
Theory 59: Use multiply_add for address computation
Use FMA (multiply_add) for address computation where possible.
addr = base + idx could become addr = 1*base + idx via FMA.
"""
...
class KernelBuilderT59:
    """T59: FMA for address - use multiply_add(idx, 1, base) instead of add
    This doesn't help because FMA still uses 1 VALU slot like add."""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_gather_round_interleaved_fma(group_desks):
            # Use FMA for address: addr = idx*1 + forest_p
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("multiply_add", desk['addr'], desk['idx'], v_one, v_forest_p))
            for d in group_desks:
                desk = desks[d]
        ...
        def emit_round_10_optimized_fma(group_desks):
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("multiply_add", desk['addr'], desk['idx'], v_one, v_forest_p))
            for d in group_desks:
        ...
        def emit_round_15_final_interleaved_fma(group_desks):
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("multiply_add", desk['addr'], desk['idx'], v_one, v_forest_p))
            for d in group_desks:
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                ...
                for _rnd in range(3, 10):
                    emit_gather_round_interleaved_fma(group_desks)
                emit_round_10_optimized_fma(group_desks)
                emit_rounds_11_12_13_fused(group_desks)
                emit_gather_round_interleaved_fma(group_desks)
                emit_round_15_final_interleaved_fma(group_desks)
...
if __name__ == "__main__":
    try:
        ...
        print(f"T59 RESULT: {cycles} cycles (baseline 1548)")
        print(f"T59 DELTA: {1548 - cycles}")
