"""
# Experiment H139: H82 + Skip Final Round Branch and Idx Stores

**GOAL:** Combine H82's interleaved round processing with H133's optimization
that skips unnecessary branch computation in round 15 and idx stores.

**HYPOTHESIS:** Since submission tests only verify `inp_values_p` (val), not
`inp_indices_p` (idx), we can:
1. Skip the branch computation in round 15 (final round) since idx is never verified
2. Only store val vectors at the end, not idx vectors

**CHANGES FROM H82:**
1. Round 15: Skip `& 1`, `multiply_add` for idx, and `+` for idx operations
2. Stores: Only store val vectors, not idx vectors

**ALGORITHM:** Same as H82 (interleaved round processing, wrap exploitation)
...
class KernelBuilderH139:
    """
    H139: H82 + Skip final round branch and idx stores.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        H139: Skip branch in round 15 and only store val vectors.
        """
        ...
        def emit_gather_round_final(desk_idx):
            """Round 15 (final): Gather without bounds, SKIP branch computation"""
            d = desks[desk_idx]
            self.emit("valu", ("+", d['addr'], v_forest_p, d['idx']))
            # Gather (8 scalar loads)
            for lane in range(VLEN):
            ...
            self.emit("valu", ("^", d['val'], d['val'], d['node_val']))
            emit_hash_stages(desk_idx)
            # NO emit_branch() - idx is never verified after final round
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                ...
                # Round 14: Gather without bounds (with branch)
                for d in group_desks:
                    emit_gather_round(d)

                # Round 15: Gather without bounds, SKIP branch (final round)
                for d in group_desks:
                    emit_gather_round_final(d)

            # Store results - ONLY store val vectors, not idx vectors
            for d in range(NUM_DESKS):
