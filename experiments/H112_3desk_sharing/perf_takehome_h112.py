"""
# Experiment H112: 3-Desk Temp Sharing

**GOAL:** Change from 4-desk temp sharing to 3-desk temp sharing.

**HYPOTHESIS:** H105 uses 4-desk temp sharing. What if we try 3-desk sharing?

With 7 preloaded nodes (not 15), we have more scratch room:
- H105: Uses 16 shared temps (128 slots) for 4-desk sharing
- 3-desk sharing needs: ceil(32/3) = 11 groups * 2 temps = 22 temps = 176 slots
- But we process in stride-3 groups: desks 0,3,6... then 1,4,7... then 2,5,8...

**SCRATCH CALCULATION:**
- Base from H105: ~1,431 slots
- Remove 4-desk temps: -128 slots
- Add 3-desk temps (22 temps): +176 slots
- Total: ~1,479 slots - FITS in 1,536!
...
class KernelBuilderH112:
    """
    H112: 3-desk temp sharing (instead of 4-desk in H105).
    Process groups of non-conflicting desks (stride by 3).
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel using 32 desks per tile with 3-desk temp sharing.
        """
        ...
        # H112 CHANGE: SHARED TEMP REGISTERS - 3-desk sharing
        # Every 3 consecutive desks share 2 temps
        # 32 desks / 3 = 11 groups (rounding up), each needs 2 temps = 22 temps total
        NUM_SHARED_TEMPS = 22  # 11 groups * 2 temps
        ...
        def get_temps(desk_idx):
            ...
            3 desks share 2 temps to avoid conflicts when processing in stride-3 groups.
            """
            group = desk_idx // 3  # 11 groups total (0-2, 3-5, etc.)
            t1_idx = group * 2
            t2_idx = group * 2 + 1
        ...
        print(f"  - H112: 3-desk temp sharing (22 shared temps = {NUM_SHARED_TEMPS * VLEN} slots)")
        print(f"  - Changed from 4-desk (16 temps) to 3-desk (22 temps) sharing")

        # Pause before main computation
        self.emit("flow", ("pause",))

        # H112 CHANGE: 3-way desk groupings for temp conflict avoidance
        # With 3-desk sharing, we need to process desks in groups that don't share temps
        group_0 = list(range(0, NUM_DESKS, 3))   # [0, 3, 6, 9, 12, 15, 18, 21, 24, 27, 30]
        group_1 = list(range(1, NUM_DESKS, 3))   # [1, 4, 7, 10, 13, 16, 19, 22, 25, 28, 31]
        group_2 = list(range(2, NUM_DESKS, 3))   # [2, 5, 8, 11, 14, 17, 20, 23, 26, 29]
