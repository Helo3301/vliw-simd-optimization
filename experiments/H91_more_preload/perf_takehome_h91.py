"""
# Experiment H91: Expanded Tree Preloading

**HYPOTHESIS:** Currently H85 preloads tree nodes 0-14 (levels 0-3). Rounds 0-2 and 11-13 use
arithmetic selection from these. Rounds 3-9 and 14-15 use gather. What if we preload MORE
tree nodes (0-30, level 4) and extend arithmetic selection to rounds 3 and 14?

**FINDINGS:**
1. Preloading 31 tree nodes (0-30) adds ~17 cycles overhead vs 15 nodes
2. 8-way arithmetic selection for indices 7-14 requires ~20 VALU operations with 3-level
   dependency chain, which is SLOWER than gather (8 loads + 2 VALU ops)
3. Gather uses 2 load slots per cycle, achieving 8 loads in 4 cycles
4. 8-way arithmetic has ~10+ cycles critical path due to dependencies

**IMPLEMENTATION:**
This file implements the hypothesis as specified, with 8-way arithmetic selection.

**RESULT:** 2051 cycles (WORSE than H85's 1898 cycles)
- The 8-way selection adds significant overhead
- Gather is more efficient for 8+ way selection due to load parallelism

**CONCLUSION:** The hypothesis is REJECTED. Expanded tree preloading with arithmetic
selection does NOT improve performance. Gather is more efficient for level 3+ nodes.
"""
...
class KernelBuilderH91:
    """
    H91: Expanded tree preloading with 8-way arithmetic selection for rounds 3 and 14.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel with expanded tree preloading (0-30) and 8-way selection.
        """
        ...
        v_seven = self.scratch_vconst(7, "v_seven")  # For 8-way selection base offset
        ...
        # ===== EXPANDED TREE PRELOADING: nodes 0-30 (31 total, levels 0-4) =====
        NUM_PRELOADED = 31
        v_tree = []
        ...
        # Precompute differences for 2-way selections (rounds 1, 12)
        v_diff_1_2 = self.alloc_vec("v_diff_1_2")  # tree[2] - tree[1]
        self.emit("valu", ("-", v_diff_1_2, v_tree[2], v_tree[1]))

        # Precompute differences for 4-way selections (rounds 2, 13)
        ...
        # Precompute differences for 8-way selections (rounds 3, 14)
        ...
        # SHARED TEMP REGISTERS - Use 16 vectors
        NUM_SHARED_TEMPS = 16
        ...
        print(f"  - Preloaded {NUM_PRELOADED} tree nodes ({NUM_PRELOADED * VLEN} slots)")
        ...
        def emit_8way_select(desk_idx):
            """8-way arithmetic selection for indices in {7-14}.

            sel = idx - 7 gives {0..7}
            3-level binary selection using precomputed diffs.
            """
            d = desks[desk_idx]
            tmp1, tmp2 = get_temps(desk_idx)

            # Step 1: Compute pair0 and pair1 (first quad)
            self.emit("valu", ("-", tmp1, d['idx'], v_seven))  # tmp1 = sel
            self.emit("valu", ("&", tmp2, tmp1, v_one))  # tmp2 = bit0

            # pair0 = tree[7] + bit0 * (tree[8] - tree[7])
            self.emit("valu", ("multiply_add", d['node_val'], tmp2, v_diff_7_8, v_tree[7]))
            # pair1 = tree[9] + bit0 * (tree[10] - tree[9])
            self.emit("valu", ("multiply_add", d['addr'], tmp2, v_diff_9_10, v_tree[9]))

            # Step 2: Compute quad0 = pair0 + bit1 * (pair1 - pair0)
            self.emit("valu", (">>", tmp1, tmp1, v_one))  # tmp1 = sel >> 1
            self.emit("valu", ("&", tmp1, tmp1, v_one))  # tmp1 = bit1

            self.emit("valu", ("-", tmp2, d['addr'], d['node_val']))  # tmp2 = pair1 - pair0
            self.emit("valu", ("multiply_add", d['node_val'], tmp1, tmp2, d['node_val']))  # node_val = quad0

            # Step 3: Compute pair2 and pair3 (second quad)
            self.emit("valu", ("-", tmp2, d['idx'], v_seven))  # tmp2 = sel
            self.emit("valu", ("&", tmp2, tmp2, v_one))  # tmp2 = bit0

            # pair2 = tree[11] + bit0 * (tree[12] - tree[11])
            self.emit("valu", ("multiply_add", d['addr'], tmp2, v_diff_11_12, v_tree[11]))  # addr = pair2

            # pair3 = tree[13] + bit0 * (tree[14] - tree[13])
            self.emit("valu", ("multiply_add", tmp2, tmp2, v_diff_13_14, v_tree[13]))  # tmp2 = pair3

            # Step 4: Compute quad1 = pair2 + bit1 * (pair3 - pair2)
            self.emit("valu", ("-", tmp1, d['idx'], v_seven))  # tmp1 = sel
            self.emit("valu", (">>", tmp1, tmp1, v_one))  # tmp1 = sel >> 1
            self.emit("valu", ("&", tmp1, tmp1, v_one))  # tmp1 = bit1

            self.emit("valu", ("-", tmp2, tmp2, d['addr']))  # tmp2 = pair3 - pair2
            self.emit("valu", ("multiply_add", d['addr'], tmp1, tmp2, d['addr']))  # addr = quad1

            # Step 5: Final selection using bit2
            self.emit("valu", ("-", tmp1, d['idx'], v_seven))  # tmp1 = sel
            self.emit("valu", (">>", tmp1, tmp1, v_two))  # tmp1 = bit2

            self.emit("valu", ("-", tmp2, d['addr'], d['node_val']))  # tmp2 = quad1 - quad0
            self.emit("valu", ("multiply_add", d['node_val'], tmp1, tmp2, d['node_val']))  # node_val = result
        ...
        def emit_tile(tile_idx):
            ...
            # Round 3: Indices in {7-14}, use 8-way arithmetic selection
            for d in range(NUM_DESKS):
                emit_8way_select(d)
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))
                emit_hash_stages(d)
            ...
            # Round 14: Indices in {7-14} after wrap, use 8-way arithmetic selection
            for d in range(NUM_DESKS):
                emit_8way_select(d)
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))
                emit_hash_stages(d)
            ...
            # Round 15: Gather without bounds (indices 15-30, all < 2047)
            for d in range(NUM_DESKS):
                self.emit("valu", ("+", desks[d]['addr'], v_forest_p, desks[d]['idx']))
                for lane in range(VLEN):
                    self.emit("load", ("load", desks[d]['node_val'] + lane, desks[d]['addr'] + lane))
