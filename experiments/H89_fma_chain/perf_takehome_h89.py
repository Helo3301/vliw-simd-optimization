"""
# Experiment H89: Split Branch Computation for Better Overlap

**GOAL:** Improve cycle count by overlapping branch computation with hash stages.

**FINDING:** Replacing FMA with shift+add actually INCREASES cycles (3 ops vs 1 FMA).
However, a different optimization works: splitting the branch computation.

**KEY OPTIMIZATION:**
The branch computation `idx = idx * 2 + 1 + (val & 1)` can be split:
- Early: `idx = idx * 2 + 1` - only depends on idx, can run during hash
- Late: `idx = idx + (val & 1)` - depends on hash result

By computing the early part (`idx * 2 + 1`) right after XOR with node value,
it can overlap with the hash computation which doesn't depend on idx.

**APPROACH:**
1. Keep FMA for hash stages 0, 2, 4 (more efficient than shift+add)
2. Split branch: emit `idx = idx * 2 + 1` early, before hash stages
3. Emit `idx += val & 1` after hash completes

**BASED ON:** H85 (1,898 cycles)
**RESULT:** 1,894 cycles (4 cycle improvement)
"""
...
class KernelBuilderH89:
    """
    H89: Restructure hash FMA chains using shifts + adds.

    Replace multiply_add in hash stages 0, 2, 4 with shift + add sequences
    to potentially break dependency chains.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        Uses shift+add instead of FMA for hash stages 0, 2, 4.
        """
        ...
        # Hash constants - use FMA for stages 0, 2, 4 and reorder operations for stages 1, 3, 5
        # Stage 0: val = val * 4097 + const
        # Stage 2: val = val * 33 + const
        # Stage 4: val = val * 9 + const
        ...
        for hi, (op1, val1, op2, op3, val3) in enumerate(HASH_STAGES):
            ...
            if hi in FMA_MULTIPLIERS:
                # For FMA stages, store the multiplier
        ...
        # SHARED TEMP REGISTERS - Use 16 vectors (128 slots)
        ...
        print(f"  - H89: Split branch computation for better overlap with hash")

        # Pause before main computation
        self.emit("flow", ("pause",))

        # Helper: emit hash stages using standard approach
        def emit_hash_stages(desk_idx):
            ...
            # Stage 1: val = (val ^ const) ^ (val >> 19)
            self.emit("valu", ("^", tmp1, d['val'], v_hash_consts[1]))
            self.emit("valu", (">>", tmp2, d['val'], v_hash_shifts[1]))
            self.emit("valu", ("^", d['val'], tmp1, tmp2))

            # Stage 2: val = val * 33 + const
            self.emit("valu", ("multiply_add", d['val'], d['val'], v_fma_mult[2], v_hash_consts[2]))

            # Stage 3: val = (val + const) ^ (val << 9)
            self.emit("valu", ("+", tmp1, d['val'], v_hash_consts[3]))
            self.emit("valu", ("<<", tmp2, d['val'], v_hash_shifts[3]))
            self.emit("valu", ("^", d['val'], tmp1, tmp2))

            # Stage 4: val = val * 9 + const
            self.emit("valu", ("multiply_add", d['val'], d['val'], v_fma_mult[4], v_hash_consts[4]))

            # Stage 5: val = (val ^ const) ^ (val >> 16)
            self.emit("valu", ("^", tmp1, d['val'], v_hash_consts[5]))
            self.emit("valu", (">>", tmp2, d['val'], v_hash_shifts[5]))
            self.emit("valu", ("^", d['val'], tmp1, tmp2))

        # Interleaved hash stages - emit stage-by-stage across all desks
        # This allows the scheduler to pack more operations per cycle
        def emit_hash_stages_interleaved(desk_indices):
            """Emit hash stages for multiple desks in an interleaved manner."""
            # Stage 0: FMA for all desks first
            for di in desk_indices:
                d = desks[di]
                self.emit("valu", ("multiply_add", d['val'], d['val'], v_fma_mult[0], v_hash_consts[0]))

            # Stage 1: Two parallel ops, then combine
            for di in desk_indices:
                d = desks[di]
                tmp1, tmp2 = get_temps(di)
                self.emit("valu", ("^", tmp1, d['val'], v_hash_consts[1]))
                self.emit("valu", (">>", tmp2, d['val'], v_hash_shifts[1]))
            for di in desk_indices:
                d = desks[di]
            ...
            # Stage 2: FMA for all desks
            for di in desk_indices:
                d = desks[di]
                self.emit("valu", ("multiply_add", d['val'], d['val'], v_fma_mult[2], v_hash_consts[2]))

            # Stage 3: Two parallel ops, then combine
            for di in desk_indices:
                d = desks[di]
                tmp1, tmp2 = get_temps(di)
                self.emit("valu", ("+", tmp1, d['val'], v_hash_consts[3]))
            ...
            # Stage 4: FMA for all desks
            for di in desk_indices:
                d = desks[di]
                self.emit("valu", ("multiply_add", d['val'], d['val'], v_fma_mult[4], v_hash_consts[4]))

            # Stage 5: Two parallel ops, then combine
            for di in desk_indices:
                d = desks[di]
                tmp1, tmp2 = get_temps(di)
                self.emit("valu", ("^", tmp1, d['val'], v_hash_consts[5]))
        ...
        # Split branch into two parts: idx computation (can start early) and combination (needs hash result)
        def emit_branch_early(desk_idx):
            """Emit idx = idx * 2 + 1, which only depends on idx not val."""
            d = desks[desk_idx]
            self.emit("valu", ("multiply_add", d['idx'], d['idx'], v_two, v_one))

        def emit_branch_late(desk_idx):
            """Emit idx = idx + (val & 1), which needs hash result."""
            d = desks[desk_idx]
            tmp1, _ = get_temps(desk_idx)
            self.emit("valu", ("&", tmp1, d['val'], v_one))
            self.emit("valu", ("+", d['idx'], d['idx'], tmp1))
        ...
        def emit_tile(tile_idx):
            ...
            # Use split branch: emit idx*2+1 early (can overlap with hash), then finish later
            for d in range(NUM_DESKS):
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], v_tree[0]))
                emit_branch_early(d)  # idx = idx * 2 + 1 can start immediately
                emit_hash_stages(d)
                emit_branch_late(d)   # idx += val & 1 needs hash result
