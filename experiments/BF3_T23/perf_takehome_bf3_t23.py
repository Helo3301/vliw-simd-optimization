"""
BF3 Theory 23: Combine stages 0+2 into single FMA chain

Stages 0 and 2 both use FMA patterns. Can we combine them to reduce ops?
Stage 0: val = (val * 4097) + const0  (where 4097 = 1 + 2^12)
Stage 2: val = (val * 33) + const2    (where 33 = 1 + 2^5)

This tests whether we can skip the intermediate stages 1 and do both together.
Expected: INCORRECT - stages cannot be skipped as they modify val.
"""
...
class KernelBuilderT23:
    """T23: Combine stages 0+2 - try fused FMA chain"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # NEW: Try combined multiplier 4097 * 33 = 135201 for stages 0+2 combined
        v_fma_combined_02 = self.scratch_vconst(135201, "v_fma_combined_02")
        ...
        # MODIFIED HASH: Try to combine stages 0+2 into single FMA
        # NOTE: This is WRONG mathematically because stage 1 modifies val
        def emit_hash_interleaved_combined_02(group_desks):
            # Combined stage 0+2: val = val * 135201 + combined_const
            # NOTE: This skips stage 1 which is WRONG
            combined_const = (0x7ED55D16 * 33 + 0x165667B1) % (2**32)
            v_comb_const = self.scratch_vconst(combined_const, "v_comb_const")

            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("multiply_add", desk['val'], desk['val'], v_fma_combined_02, v_comb_const))
            # Stage 3
            for d in group_desks:
                desk = desks[d]
        ...
        def emit_rounds_0_1_2_fused(group_desks):
            ...
            emit_hash_interleaved_combined_02(group_desks)
            for d in group_desks:
                desk = desks[d]
...
if __name__ == "__main__":
    ...
    print(f"T23 Combined 0+2 FMA: Cycles={cycles}, Correct={correct}")
