"""
C5-5: Optimized 4-Way Selection

Current 4-way selection (rounds 2, 13):
  idx in {3, 4, 5, 6}
  offset = 2*bit0 + bit1
  select tree[3 + offset]

Current implementation uses:
  - low = tree[3] + bit1 * diff_3_4  (1 FMA)
  - high = tree[5] + bit1 * diff_5_6 (1 FMA)
  - result = vselect(bit0, high, low) (1 FLOW)

Total: 2 VALU + 1 FLOW

Alternative 1: All VALU (no vselect)
  - low = tree[3] + bit1 * diff_3_4
  - high = tree[5] + bit1 * diff_5_6
  - diff = high - low (1 VALU)
  - result = low + bit0 * diff (1 FMA)

Total: 4 VALU + 0 FLOW (tested in C5-4, made it worse)

Alternative 2: Index-based direct lookup
  Compute offset = 2*bit0 + bit1, then use idx to gather from tree.
  But this requires gather operations, which are expensive.

Alternative 3: Use bit arithmetic to combine selections
  Notice: all tree values 3-6 can be precomputed as diffs from tree[3]
    d3 = tree[3] - tree[3] = 0
    d4 = tree[4] - tree[3]
    d5 = tree[5] - tree[3]
    d6 = tree[6] - tree[3]

  result = tree[3] + d_offset where d_offset = d[3 + 2*bit0 + bit1]

  We need to select d_offset based on bit0, bit1.

  d_offset = d3 + bit1*(d4-d3) when bit0=0
           = d5 + bit1*(d6-d5) when bit0=1

  Since d3=0 and d4-d3=d4:
    when bit0=0: d_offset = bit1*d4
    when bit0=1: d_offset = d5 + bit1*(d6-d5)

  Combined: d_offset = bit0*(d5 + bit1*d65) + (1-bit0)*bit1*d4
                     = bit0*d5 + bit0*bit1*d65 + bit1*d4 - bit0*bit1*d4
                     = bit1*d4 + bit0*(d5 - bit1*d4 + bit1*d65)
                     = bit1*d4 + bit0*(d5 + bit1*(d65 - d4))

  This is getting complicated. Let me try a different approach.

Alternative 4: Direct formula with precomputed coefficients
  result = tree[3] + offset where offset = sum of weighted diffs

  When offset=0: tree[3]
  When offset=1: tree[4] = tree[3] + d4
  When offset=2: tree[5] = tree[3] + d5
  When offset=3: tree[6] = tree[3] + d6

  result = tree[3] + bit1*d4 + bit0*(d5 - bit1*d4 + bit1*(d6-d5))
         = tree[3] + bit1*d4 + bit0*d5 - bit0*bit1*d4 + bit0*bit1*d65
         = tree[3] + bit1*d4*(1-bit0) + bit0*d5 + bit0*bit1*d65

  Let me verify:
    bit0=0, bit1=0: tree[3] + 0 = tree[3] OK
    bit0=0, bit1=1: tree[3] + d4*1 = tree[4] OK
    bit0=1, bit1=0: tree[3] + d5 = tree[5] OK
    bit0=1, bit1=1: tree[3] + 0 + d5 + d65 = tree[3] + d5 + d6-d5 = tree[3] + d6 = tree[6] OK

  So: result = tree[3] + bit1*(1-bit0)*d4 + bit0*d5 + bit0*bit1*d65

  Operations needed:
    1. (1-bit0) = NOT bit0 conceptually, but we only have {0,1}, so 1-bit0 works
       inv_bit0 = 1 - bit0 (1 VALU SUB)
    2. bit1 * inv_bit0 (1 VALU MUL)
    3. term1 = #2 * d4 (1 VALU MUL) or FMA
    4. term2 = bit0 * d5 (1 VALU MUL)
    5. bit0 * bit1 (1 VALU MUL)
    6. term3 = #5 * d65 (1 VALU MUL)
    7. result = tree[3] + term1 + term2 + term3 (2 VALU ADD)

  Total: 8 VALU ops. Much worse than current 2 VALU + 1 FLOW.

Let me try yet another approach: Keep the current structure but optimize
the surrounding code.

Actually, let me check if the vselect is really on the critical path.
If vselect runs in parallel with other VALU ops, it costs nothing.

Let me just test the baseline again to understand the timing better.
"""
...
class KernelBuilderC5_5:
    """C5-5: Optimized 4-way selection - trying different arrangements"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_rounds_11_12_13_fused(group_desks):
            """Rounds 11+12+13 fused with direct bit tracking"""
            for d in group_desks:
                emit_xor_with_node(d, v_tree[0])
            emit_hash_interleaved(group_desks)
            for d in group_desks:
