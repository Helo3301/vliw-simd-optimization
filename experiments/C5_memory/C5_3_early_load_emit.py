"""
C5-3: Early Load Emission (Load Before Hash)

Hypothesis: Currently we emit:
  1. XOR
  2. Hash (12 VALU ops)
  3. Branch
  4. Compute address
...
If we instead emit loads BEFORE the hash (using current idx before branch):
  1. Compute address from CURRENT idx (will be wrong half the time)
  2. Loads for CURRENT idx
  3. XOR
  4. Hash
  5. Branch
  6. Next round uses loaded data

Wait, this doesn't work because the idx changes EACH round.

Let me think differently: What if we reorder to emit loads between hash stages?

Actually, let me re-examine the problem. The theoretical analysis shows:
- VALU bound: 1514 cycles
- LOAD bound: 1321 cycles
- Current B4-2: 1558 cycles

VALU is the bottleneck! Memory access pattern changes won't help because
loads are NOT on the critical path - VALU is.

The gap of 1558-1514=44 cycles is pure scheduling inefficiency.

Let me try a different approach: Emit loads interleaved with hash stages
to allow better scheduling overlap.
"""
...
class KernelBuilderC5_3:
    """C5-3: Interleave loads with hash stages for better overlap"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # NEW: Emit loads interleaved with hash stages for better scheduling
        def emit_gather_round_with_early_loads(group_desks):
            """Gather round: emit loads earlier, interleaved with hash"""
            # Compute addr FIRST (before XOR/hash)
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))

            # Emit ALL loads immediately after addr computation
            # This gives maximum time for loads to complete during subsequent hash
            for d in group_desks:
                desk = desks[d]
                for lane in range(VLEN):
                    self.emit("load", ("load", desk['node_val'] + lane, desk['addr'] + lane))

            # XOR and hash (loads can complete during this time)
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
        ...
        def emit_round_10_with_early_loads(group_desks):
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
            for d in group_desks:
        ...
        def emit_round_15_final_with_early_loads(group_desks):
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
            for d in group_desks:
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                ...
                # Rounds 3-9: use early load emission
                for _rnd in range(3, 10):
                    emit_gather_round_with_early_loads(group_desks)

                emit_round_10_with_early_loads(group_desks)
                emit_rounds_11_12_13_fused(group_desks)
                emit_gather_round_with_early_loads(group_desks)  # Round 14
                emit_round_15_final_with_early_loads(group_desks)
