"""
C5-1: Batch Consecutive Loads for Better Scheduling

Hypothesis: Emitting all gather loads together (before XOR/hash) instead of
interleaved with other operations might allow better overlap with preceding
hash computations from different desks.

Current pattern (per group):
  - Compute addr
  - Load 8 lanes
  - XOR
  - Hash
...
New pattern (per group):
  - Compute all addrs for all desks in group
  - Emit ALL loads for all desks
  - Then all XORs
  - Then all hashes
  - Then all branches

This allows loads to begin immediately after addr is ready, potentially
overlapping with prior hash stages from earlier desks.
"""
...
class KernelBuilderC5_1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # NEW: Batched gather round - compute all addrs first, then all loads
        def emit_gather_round_batched(group_desks):
            """Emit gather round with batched loads"""
            # Step 1: Compute ALL addresses for all desks
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))

            # Step 2: Emit ALL loads for all desks (back-to-back)
            for d in group_desks:
                desk = desks[d]
        ...
        def emit_round_10_batched(group_desks):
            """Round 10 with batched loads"""
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
            for d in group_desks:
        ...
        def emit_round_15_final_batched(group_desks):
            """Round 15 (final) with batched loads"""
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
            for d in group_desks:
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                ...
                # Rounds 3-9: gather rounds with batched loads
                for _rnd in range(3, 10):
                    emit_gather_round_batched(group_desks)

                # Round 10: last before wrap
                emit_round_10_batched(group_desks)
                ...
                emit_round_15_final_batched(group_desks)
