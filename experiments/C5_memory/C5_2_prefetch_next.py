"""
C5-2: Prefetch Tree Nodes for Next Round During Current Hash

Hypothesis: During the hash computation (which takes many cycles), we know
that the NEXT node address will be computed from current idx. We can compute
both possible next addresses (idx*2+1 and idx*2+2) and prefetch both nodes
during the hash.

After the hash completes and we determine the branch bit, we select which
prefetched node to use.

This could hide memory latency by overlapping loads with compute.

Analysis:
- Hash takes ~12-16 VALU operations per desk
- Each gather round has 8 loads per desk
- With 2 loads/cycle, that's 4 cycles minimum per desk
- If we can overlap those 4 cycles with hash compute...

Implementation:
1. Before hash: compute possible next addresses (idx*2+1, idx*2+2)
2. During hash: emit prefetch loads for both children
3. After hash: use vselect to pick correct prefetched node

Cost: 2x the loads per gather round
Benefit: Loads can overlap with hash compute
"""
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        for d in range(NUM_DESKS):
            desk = {
                ...
                # NEW: prefetch buffers for next round
                'pf_addr_left': self.alloc_vec(f"v_pf_addr_left_{d}"),
                'pf_addr_right': self.alloc_vec(f"v_pf_addr_right_{d}"),
                'pf_node_left': self.alloc_vec(f"v_pf_node_left_{d}"),
                'pf_node_right': self.alloc_vec(f"v_pf_node_right_{d}"),
            }
            desks.append(desk)
        ...
        def emit_gather_round_interleaved(group_desks):
            """Standard gather round (no prefetch)"""
            for d in group_desks:
                desk = desks[d]
