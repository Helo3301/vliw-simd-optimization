"""
C5-4: Replace vselect with VALU Operations

Hypothesis: The FLOW engine has only 1 slot/cycle, while VALU has 6.
Currently we use vselect for 4-way selection in rounds 2 and 13.

vselect(dest, cond, a, b) = a if cond else b

This can be replaced with VALU multiply:
  result = a * cond + b * (1 - cond)
  = a * cond + b - b * cond
  = (a - b) * cond + b

If cond is {0, 1}, then:
  multiply_add(dest, cond, (a - b), b) gives the same result!

This trades 1 FLOW operation for 2 VALU operations (subtract + FMA).
Since VALU has more slots, this might improve parallel scheduling.

Analysis:
- B4-2 has 8 vselects per tile (4 desks * 2 rounds with 4-way select * 1 vselect each)
- With 2 tiles: 16 vselects total
- FLOW is 1/cycle, so 16 cycles consumed by vselects
- Converting to VALU: 16 * 2 = 32 VALU ops
- At 6 VALU/cycle, that's ~5.3 cycles if perfectly scheduled
- Potential savings: ~10 cycles

However, VALU is already the bottleneck! Adding VALU ops makes it WORSE.

Let me reconsider... Actually, looking at the code more carefully, the vselect
is selecting between two already-computed values. The (a - b) computation
would add overhead.

Actually, the analysis shows this would HURT performance, not help.
Let me try something else entirely.
"""
...
class KernelBuilderC5_4:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Precompute diff between high and low pairs for VALU-based select
        v_diff_56_34 = self.alloc_vec("v_diff_56_34")
        self.emit("valu", ("-", v_diff_56_34, v_tree[5], v_tree[3]))  # Base difference
        ...
        for d in range(NUM_DESKS):
            desk = {
                ...
                'tmp3': self.alloc_vec(f"v_tmp3_{d}"),  # Extra temp for vselect replacement
            }
            desks.append(desk)
        ...
        def emit_rounds_0_1_2_fused_valu_select(group_desks):
            """Rounds 0+1+2 fused with VALU-based select instead of vselect"""
            # === Round 0 ===
            for d in group_desks:
            ...
            # === Round 2 ===
            # Replace vselect with VALU operations
            # Original: vselect(node_val, bit0, tmp2, node_val) where:
            #   node_val = tree[3] + bit1 * diff_3_4 (low pair)
            #   tmp2 = tree[5] + bit1 * diff_5_6 (high pair)
            # Select: result = tmp2 if bit0 else node_val = high if bit0 else low
            #
            # VALU equivalent: result = low + bit0 * (high - low)
            for d in group_desks:
                desk = desks[d]
                # bit1 is in tmp1
                ...
                # VALU-based select: result = low + bit0 * (high - low)
                self.emit("valu", ("-", desk['tmp3'], desk['tmp2'], desk['node_val']))  # high - low
                self.emit("valu", ("multiply_add", desk['node_val'], desk['bit0'], desk['tmp3'], desk['node_val']))  # low + bit0 * (high - low)
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            emit_hash_interleaved(group_desks)
        ...
        def emit_rounds_11_12_13_fused_valu_select(group_desks):
            """Rounds 11+12+13 fused with VALU-based select"""
            # === Round 11 ===
            for d in group_desks:
                emit_xor_with_node(d, v_tree[0])
            emit_hash_interleaved(group_desks)
            ...
            for d in group_desks:
                ...
                # VALU-based select
                self.emit("valu", ("-", desk['tmp3'], desk['tmp2'], desk['node_val']))
                self.emit("valu", ("multiply_add", desk['node_val'], desk['bit0'], desk['tmp3'], desk['node_val']))
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            emit_hash_interleaved(group_desks)
