"""
# Experiment H137: H82 + Reduced Tree Preload

**GOAL:** Reduce unnecessary preloading by only loading tree nodes 0-6.

**HYPOTHESIS:** Nodes 7-14 are never used because:
- Rounds 0-2 and 11-13 use arithmetic selection with nodes 0-6
- Rounds 3-10 and 14-15 use gather (load from memory, not preloaded)

By reducing NUM_PRELOADED from 15 to 7, we save 8 preload operations (8 loads + 8 broadcasts).

**ALGORITHM:** Same as H82 (interleaved round processing with wrap exploitation)
**SCHEDULING:** Automatic via _schedule_slots()

**BASELINE:** H82 achieved 1,656 cycles (89.2x speedup)
"""
...
class KernelBuilderH137:
    """
    H137: H82 + Reduced tree preload (nodes 0-6 only instead of 0-14).
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        # Preload tree nodes 0-6 only (H137: reduced from 15 to 7)
