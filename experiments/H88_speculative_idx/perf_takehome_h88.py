"""
# Experiment H88: Speculative Next-Round Index Computation

**HYPOTHESIS:** The next-round index is predictable: new_idx = 2*idx + 1 + (hash & 1).
We know 2*idx + 1 (the speculative base), and the +0 or +1 depends on hash LSB.
By computing the speculative index BEFORE the hash completes, we can overlap this
computation with hash stages, reducing the critical path.

**KEY CHANGE:**
1. After XOR but BEFORE hash stages, compute in-place: idx = 2*idx + 1 (speculative)
2. After hash completes, extract bit and finalize: idx = idx + (val & 1)

This allows the multiply_add to run in parallel with early hash stages instead of
waiting for hash to complete.

**ALGORITHM:** Same as H85 (shared temps, wrap exploitation, no bounds on R11-15)
**SCHEDULING:** Automatic via _schedule_slots()

**RESULT:** 1,894 cycles (4 cycles improvement over H85's 1,898 cycles)
"""
...
class KernelBuilderH88:
    """
    H88: Speculative next-round index computation.
    Compute spec_idx = 2*idx + 1 BEFORE hash, then add bit after hash.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        # H88: NO separate spec_idx - compute in-place in idx
        NUM_DESKS = 16
        desks = []
        for d in range(NUM_DESKS):
            desk = {
                ...
                # NO tmp1/tmp2 or spec_idx here - compute spec_idx in-place
            }
            desks.append(desk)

        # SHARED TEMP REGISTERS - Use 16 vectors (128 slots) instead of 32 (256 slots)
        # Each pair of 2 consecutive desks shares temps (0-1, 2-3, 4-5, etc.)
        # This saves 128 slots while minimizing scheduler conflicts
        NUM_SHARED_TEMPS = 16
        shared_temps = [self.alloc_vec(f"v_shared_tmp_{i}") for i in range(NUM_SHARED_TEMPS)]

        # Track which temp to use for each desk
        # Give each pair of desks unique temps
        ...
        # H88: Split branch into speculative and finalize phases
        # Compute spec_idx in-place in idx register
        def emit_speculative_idx(desk_idx):
            """Compute speculative index BEFORE hash: idx = 2*idx + 1 (in-place)"""
            d = desks[desk_idx]
            self.emit("valu", ("multiply_add", d['idx'], d['idx'], v_two, v_one))

        def emit_finalize_idx(desk_idx):
            """Finalize index AFTER hash: idx = idx + (val & 1)"""
            d = desks[desk_idx]
        ...
        def emit_branch(desk_idx):
            """Legacy branch for compatibility - calls both phases"""
            emit_speculative_idx(desk_idx)
            emit_finalize_idx(desk_idx)
        ...
        def emit_tile(tile_idx):
            ...
            # H88: Emit speculative idx BEFORE hash, finalize AFTER
            for d in range(NUM_DESKS):
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], v_tree[0]))
                emit_speculative_idx(d)  # H88: spec_idx = 2*idx + 1 (overlaps with hash)
                emit_hash_stages(d)
                emit_finalize_idx(d)  # H88: idx = spec_idx + bit
            ...
            for d in range(NUM_DESKS):
                ...
                emit_speculative_idx(d)  # H88: spec_idx = 2*idx + 1
                emit_hash_stages(d)
                emit_finalize_idx(d)  # H88: idx = spec_idx + bit
            ...
            # H88: Emit speculative idx BEFORE hash, finalize AFTER, then bounds
            for d in range(NUM_DESKS):
                self.emit("valu", ("+", desks[d]['addr'], v_forest_p, desks[d]['idx']))
                for lane in range(VLEN):
                    self.emit("load", ("load", desks[d]['node_val'] + lane, desks[d]['addr'] + lane))
