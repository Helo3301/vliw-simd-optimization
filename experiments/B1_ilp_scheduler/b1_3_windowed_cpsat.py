"""
B1-3: Windowed CP-SAT Scheduler

Schedule small windows (100-200 slots) optimally with CP-SAT, then stitch together.
The key insight is that while the full problem is too large (11,776 slots),
smaller windows can be solved optimally and then combined.

Window overlap allows us to capture some cross-window optimization.
"""
...
def _schedule_slots_cpsat_bounded(slots: list[tuple[str, tuple]],
                                   earliest_start: dict[int, int],
                                   time_limit_seconds: float = 5.0) -> tuple[list[dict[str, list[tuple]]], dict[int, int]]:
    """
    CP-SAT scheduler with bounded start times from previous window.
    Returns (schedule, finish_times) where finish_times[addr] is when each addr is ready.
    """
    if not slots:
        return [], {}
    ...
    # Compute earliest possible cycle
    earliest = [0] * n_slots
    last_write = {}
    last_read = {}

    for i in range(n_slots):
        # Consider earliest from previous window
        for addr in slot_reads[i]:
            if addr in earliest_start:
                earliest[i] = max(earliest[i], earliest_start[addr])
    ...
    ) if engine_counts else 0

    horizon = max(max_earliest + 1, resource_bound) + 30
    ...
    for i in range(n_slots):
        for addr in slot_reads[i]:
            if addr in last_write:
                model.Add(cycle_vars[i] >= cycle_vars[last_write[addr]] + 1)

        for addr in slot_writes[i]:
            if addr in last_write:
                model.Add(cycle_vars[i] >= cycle_vars[last_write[addr]] + 1)
            if addr in last_read:
                model.Add(cycle_vars[i] >= cycle_vars[last_read[addr]])
    ...
    # Track finish times for each address
    finish_times = {}
    ...
    for i, (engine, slot) in enumerate(slots):
        ...
        # Update finish times
        for addr in slot_writes[i]:
            finish_times[addr] = c + 1  # Ready next cycle

    return [c for c in cycles if c], finish_times
...
def _schedule_slots_windowed(slots: list[tuple[str, tuple]], window_size: int = 150) -> list[dict[str, list[tuple]]]:
    """
    Schedule using windowed CP-SAT with overlapping windows.
    """
    if not slots:
        return []

    if len(slots) <= window_size * 2:
        # Small enough for direct CP-SAT
        result, _ = _schedule_slots_cpsat_bounded(slots, {}, time_limit_seconds=30.0)
        if result is not None:
            return result
        return _schedule_slots_greedy(slots)
    ...
    i = 0
    while i < len(slots):
        end = min(i + window_size, len(slots))
        window_slots = slots[i:end]

        result, finish_times = _schedule_slots_cpsat_bounded(
            window_slots, earliest_start, time_limit_seconds=3.0
        )

        if result is None:
            # Fallback to greedy for this window
            result = _schedule_slots_greedy(window_slots)

        # Offset cycles and merge
        for cycle_dict in result:
            while len(all_cycles) <= base_cycle:
                all_cycles.append({})

            for engine, engine_slots in cycle_dict.items():
                # Check slot limits
                limit = SLOT_LIMITS[engine]
                current = all_cycles[base_cycle].setdefault(engine, [])
                remaining = engine_slots
        ...
        # Update earliest start for next window
        if finish_times:
            for addr, finish in finish_times.items():
                earliest_start[addr] = base_cycle - len(result) + finish

        i = end

    return [c for c in all_cycles if c]


class KernelBuilderB1_3:
    """B1-3: Windowed CP-SAT Scheduler"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        for i, phase in enumerate(phases):
            if phase:
                if len(phase) > 300:
                    print(f"Phase {i}: {len(phase)} slots, windowed CP-SAT scheduling...")
                    phase_instrs = _schedule_slots_windowed(phase, window_size=100)
                else:
                    print(f"Phase {i}: {len(phase)} slots, CP-SAT scheduling...")
                    result, _ = _schedule_slots_cpsat_bounded(phase, {}, time_limit_seconds=30.0)
                    if result is None:
