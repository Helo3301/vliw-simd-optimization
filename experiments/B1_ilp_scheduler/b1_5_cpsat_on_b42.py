"""
B1-5: CP-SAT Analysis on B4-2 (Reduced Operation Set)

Apply CP-SAT scheduler analysis to the B4-2 implementation which has fewer
operations due to round fusion. With fewer slots, CP-SAT might have a better
chance at finding improvements.

This also serves as a final analysis to understand if ILP can help further.
"""
...
def analyze_schedule(slots: list[tuple[str, tuple]], schedule: list[dict[str, list[tuple]]]) -> dict:
    """Analyze schedule quality."""
    total_cycles = len(schedule)

    slots_by_engine = defaultdict(int)
    for engine, _ in slots:
        slots_by_engine[engine] += 1

    theoretical_min = {}
    for engine, count in slots_by_engine.items():
        limit = SLOT_LIMITS[engine]
        theoretical_min[engine] = (count + limit - 1) // limit

    utilization = {}
    for engine, count in slots_by_engine.items():
        limit = SLOT_LIMITS[engine]
        utilization[engine] = count / (total_cycles * limit) * 100

    return {
        'total_cycles': total_cycles,
        'slots_by_engine': dict(slots_by_engine),
        'theoretical_min': theoretical_min,
        'utilization': utilization,
        'bottleneck_engine': max(theoretical_min, key=theoretical_min.get),
        'bottleneck_min_cycles': max(theoretical_min.values()),
        'gap_to_theoretical': total_cycles - max(theoretical_min.values()),
    }


class KernelBuilderB1_5:
    """B1-5: CP-SAT analysis on B4-2's reduced op set"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        for i, phase in enumerate(phases):
            if phase:
                ...
                analysis = analyze_schedule(phase, phase_instrs)
                print(f"  Cycles: {analysis['total_cycles']}")
                print(f"  Slots by engine: {analysis['slots_by_engine']}")
                ...
                print(f"  Bottleneck: {analysis['bottleneck_engine']} ({analysis['bottleneck_min_cycles']} min cycles)")
                print(f"  Gap to theoretical: {analysis['gap_to_theoretical']} cycles ({analysis['gap_to_theoretical']/analysis['bottleneck_min_cycles']*100:.1f}%)")
...
def do_kernel_test(forest_height: int, rounds: int, batch_size: int, seed: int = 123,
    ...
    print("vs B4-2 (1558): ", 1558 - machine.cycle)
