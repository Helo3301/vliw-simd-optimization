"""
B1-1: CP-SAT Scheduler for VLIW Architecture

Replace the greedy list scheduler with Google OR-Tools CP-SAT solver.
This should find optimal or near-optimal instruction schedules that
minimize total cycles while respecting:
1. Data dependencies (read-after-write, write-after-write, write-after-read)
2. Resource constraints (slot limits per engine per cycle)

Based on H143d implementation.
"""
...
def _schedule_slots_cpsat(slots: list[tuple[str, tuple]], time_limit_seconds: float = 30.0) -> list[dict[str, list[tuple]]]:
    ...
    # For each slot, track which addresses it reads and writes
    slot_reads = []
    slot_writes = []
    slot_engines = []
    ...
    earliest = [0] * n_slots
    last_write = {}  # addr -> (slot_idx, cycle)
    last_read = {}   # addr -> (slot_idx, cycle)

    for i in range(n_slots):
        # Read-after-write: must wait for previous write to complete
        for addr in slot_reads[i]:
            if addr in last_write:
                earliest[i] = max(earliest[i], last_write[addr][1] + 1)

        # Write-after-write and write-after-read: must wait for previous access
    ...
    # Estimate upper bound on cycles
    max_earliest = max(earliest) if earliest else 0

    # Count slots per engine to estimate resource-constrained bound
    ...
    horizon = max(max_earliest + 1, resource_bound) + 100  # Add some slack

    # Create CP-SAT model
    model = cp_model.CpModel()

    # Decision variables: cycle[i] = cycle when slot i executes
    cycle_vars = [model.NewIntVar(earliest[i], horizon, f'cycle_{i}') for i in range(n_slots)]

    # Objective: minimize makespan (max cycle)
    ...
    for i in range(n_slots):
        ...
        for addr in slot_writes[i]:
            if addr in last_read:
                ...
                # Write can happen same cycle as read (results take effect at end of cycle)
                model.Add(cycle_vars[i] >= cycle_vars[prev_idx])

        # Update tracking (use latest writer/reader for each address)
        for addr in slot_reads[i]:
            last_read[addr] = i
        for addr in slot_writes[i]:
            last_write[addr] = i

    # Resource constraints using interval variables and cumulative constraint
    # For each engine, at most SLOT_LIMITS[engine] slots can execute per cycle
    ...
    for engine, slot_indices in engine_slots.items():
        ...
        # Create interval variables (all have duration 1)
        intervals = []
        demands = []
    ...
    if status not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        print(f"CP-SAT failed with status {status}, falling back to greedy")
...
class KernelBuilderB1_1:
    """B1-1: CP-SAT Scheduler"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        for i, phase in enumerate(phases):
            if phase:
                ...
                phase_instrs = _schedule_slots_cpsat(phase, time_limit_seconds=60.0)
