"""
B1-4: Analyze Scheduling Bottleneck

Before trying more ILP approaches, let's analyze exactly where the greedy
scheduler is suboptimal. We'll compute:
1. Theoretical minimum cycles based on total slots per engine
2. Actual utilization per engine
3. Identify cycles with poor utilization

This will help us understand if ILP can help at all.
"""
...
def _schedule_slots_greedy_analyzed(slots: list[tuple[str, tuple]]) -> tuple[list[dict[str, list[tuple]]], dict]:
    """Greedy scheduler with detailed analysis."""
    ...
    # Track why each slot was placed where
    slot_placement = []  # (earliest_due_to_deps, placed_at, reason)
    ...
    def find_cycle(engine: str, earliest: int) -> tuple[int, str]:
        cycle = earliest
    ...
    for engine, slot in slots:
        ...
        dep_reason = None
        for addr in reads:
            if ready_time[addr] > earliest:
                earliest = ready_time[addr]
                dep_reason = f"RAW on {addr}"
        for addr in writes:
            if last_write[addr] + 1 > earliest:
                earliest = last_write[addr] + 1
                dep_reason = f"WAW on {addr}"
            if last_read[addr] > earliest:
                earliest = last_read[addr]
                dep_reason = f"WAR on {addr}"

        cycle, resource_reason = find_cycle(engine, earliest)
        ensure_cycle(cycle)
        cycles[cycle].setdefault(engine, []).append(slot)
        usage[cycle][engine] += 1

        if resource_reason == "resource":
            slot_placement.append((earliest, cycle, f"{engine} full"))
        else:
            slot_placement.append((earliest, cycle, dep_reason or "start"))
    ...
    # Analyze results
    analysis = {
        'total_cycles': len([c for c in cycles if c]),
        'slots_by_engine': defaultdict(int),
        'theoretical_min': {},
        'utilization': {},
    ...
    for engine, slot in slots:
        analysis['slots_by_engine'][engine] += 1

    for engine, count in analysis['slots_by_engine'].items():
        limit = SLOT_LIMITS[engine]
        analysis['theoretical_min'][engine] = (count + limit - 1) // limit

    total_cycles = analysis['total_cycles']
    for engine, count in analysis['slots_by_engine'].items():
        limit = SLOT_LIMITS[engine]
        analysis['utilization'][engine] = count / (total_cycles * limit) * 100

    for earliest, placed, reason in slot_placement:
        if placed > earliest:
            analysis['delay_cycles'] += (placed - earliest)
            if "full" in str(reason):
                analysis['resource_delays'] += 1
            analysis['delay_reasons'][reason] += 1

    return [c for c in cycles if c], analysis


class KernelBuilderAnalyze:
    """Analyze scheduling bottleneck"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        for i, phase in enumerate(phases):
            if phase:
                print(f"\n=== Phase {i}: {len(phase)} slots ===")
                phase_instrs, analysis = _schedule_slots_greedy_analyzed(phase)
                print(f"  Cycles: {analysis['total_cycles']}")
                print(f"  Slots by engine: {dict(analysis['slots_by_engine'])}")
                print(f"  Theoretical min by engine: {analysis['theoretical_min']}")
                print(f"  Utilization: {dict((k, f'{v:.1f}%') for k,v in analysis['utilization'].items())}")
                print(f"  Total delay cycles: {analysis['delay_cycles']}")
                print(f"  Slots delayed by resource: {analysis['resource_delays']}")
                if analysis['delay_reasons']:
                    print(f"  Top delay reasons:")
                    sorted_reasons = sorted(analysis['delay_reasons'].items(), key=lambda x: -x[1])[:10]
        ...
        print(f"\nTotal slots: {len(self.slots)}, Cycles: {len(self.instrs)}")
