"""
Theory 42: Prefetch next round's tree nodes during hash

Hypothesis: Computing the next round's addresses during the hash computation
allows loads to be issued earlier, potentially hiding memory latency.
...
def _slot_rw(engine: str, slot: tuple) -> tuple[list[int], list[int]]:
    ...
    if engine == "alu":
        ...
        if op == "load":
            ...
            dest, addr, lane = slot[1], slot[2], slot[3]
            reads = [addr + lane]
            writes = [dest + lane]
        else:
...
class KernelBuilderT42:
    """T42: Prefetch next round's tree nodes during hash"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        for d in range(NUM_DESKS):
            desk = {
                ...
                'next_addr': self.alloc_vec(f"v_next_addr_{d}"),  # T42: extra reg for prefetch
            }
            desks.append(desk)
        ...
        # T42: Gather with prefetch of next round's addresses during hash
        def emit_gather_round_prefetch(group_desks, is_last=False):
            for d in group_desks:
                desk = desks[d]
            ...
            # Interleave hash with branch computation and next addr prefetch
            # Hash stage 0
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("multiply_add", desk['val'], desk['val'], v_fma_mult[0], v_hash_consts[0]))
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                ...
                for _rnd in range(3, 10):
                    emit_gather_round_prefetch(group_desks)
