"""
# Experiment B3-5: Addr-based bounds check

Instead of computing `idx = addr - forest_p` for bounds check,
compare addr directly against `n_nodes + forest_p`.

This eliminates one subtraction in the bounds check path.
"""
...
class KernelBuilderB3_5:
    """B3-5: Addr-based bounds check"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # NEW: Precompute n_nodes + forest_p for addr-based bounds check
        s_addr_bound = self.alloc_scratch("s_addr_bound")
        self.emit("alu", ("+", s_addr_bound, self.scratch["n_nodes"], self.scratch["forest_values_p"]))
        v_addr_bound = self.alloc_vec("v_addr_bound")
        self.emit("valu", ("vbroadcast", v_addr_bound, s_addr_bound))
        ...
        def emit_bounds_with_addr(desk_idx):
            """Bounds check using addr computed from the NEW idx (after branch)"""
            d = desks[desk_idx]
            # addr was computed for gather BEFORE branch, so it's stale
            # We need to compute addr from the new idx after branch
            # addr_new = forest_p + idx_new
            # valid = addr_new < addr_bound (where addr_bound = n_nodes + forest_p)
            # This is: forest_p + idx_new < n_nodes + forest_p
            # Which is: idx_new < n_nodes (same as before!)
            # So there's no savings - we need to do the comparison on idx, not addr.
            # Let me just use the original bounds check.
            self.emit("valu", ("<", d['tmp1'], d['idx'], v_n_nodes))
            self.emit("valu", ("*", d['idx'], d['idx'], d['tmp1']))
        ...
        def emit_round_10_interleaved(group_desks):
            ...
            for d in group_desks:
                emit_branch(d)
            # Bounds check using addr (saves one subtraction vs original)
            for d in group_desks:
                emit_bounds_with_addr(d)
