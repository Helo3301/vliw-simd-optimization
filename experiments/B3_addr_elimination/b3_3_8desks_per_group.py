"""
# Experiment B3-3: Larger groups to better amortize addr computation

The kernel is load-bound (2 loads/cycle vs 6 VALU/cycle).
With 4 desks per group, we have 32 loads per gather (16 cycles).
With 8 desks per group, we have 64 loads per gather (32 cycles).

But more desks per group means more VALU ops can overlap with loads.
The addr computation (1 VALU per desk) might get "hidden" better.

Let's try 8 desks per group instead of 4.
"""
...
class KernelBuilderB3_3:
    """B3-3: 8 desks per group for better load/VALU overlap"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            # CHANGED: 8 desks per group instead of 4
            GROUP_SIZE = 8
