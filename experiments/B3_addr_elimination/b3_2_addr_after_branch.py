"""
# Experiment B3-2: Compute addr after branch for better overlap

The idea: Instead of computing addr at the start of gather round,
compute it right after branch (at end of previous round).
This way the addr computation can overlap with the loads and other
operations of the next round.

For rounds that need gather (3-9, 10, 14, 15):
- After branch in round N-1, immediately compute addr for round N

This doesn't reduce op count, but may improve ILP scheduling.
"""
...
class KernelBuilderB3_2:
    """B3-2: Compute addr after branch for better ILP"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_branch_with_addr(desk_idx):
            """Branch followed immediately by addr computation"""
            d = desks[desk_idx]
            self.emit("valu", ("&", d['tmp1'], d['val'], v_one))
            self.emit("valu", ("multiply_add", d['idx'], d['idx'], v_two, v_one))
            self.emit("valu", ("+", d['idx'], d['idx'], d['tmp1']))
            # Immediately compute addr for next round's gather
        ...
        def emit_round_2_interleaved_vselect(group_desks):
            ...
            for d in group_desks:
                emit_hash_stage_5(d)
            # Branch WITH addr computation for next gather round
            for d in group_desks:
                emit_branch_with_addr(d)

        def emit_gather_round_interleaved(group_desks, compute_addr_for_next=True):
            # Gather loads using precomputed addr
            for d in group_desks:
                desk = desks[d]
            ...
            for d in group_desks:
                emit_hash_stage_5(d)
            # Branch with addr for next round
            if compute_addr_for_next:
                for d in group_desks:
                    emit_branch_with_addr(d)
            else:
                for d in group_desks:
        ...
        def emit_round_13_interleaved_vselect(group_desks):
            """Round 13 = Round 2 but computes addr for round 14"""
            # 4-way selection setup
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("-", desk['tmp1'], desk['idx'], v_three))
            ...
            for d in group_desks:
                emit_hash_stage_5(d)
            # Branch WITH addr computation for round 14
            for d in group_desks:
                emit_branch_with_addr(d)

        def emit_round_14_interleaved(group_desks):
            """Round 14 gather, computes addr for round 15"""
            # Gather loads using precomputed addr
            for d in group_desks:
                desk = desks[d]
                for lane in range(VLEN):
            ...
            for d in group_desks:
                emit_hash_stage_5(d)
            # Branch WITH addr computation for round 15
            for d in group_desks:
                emit_branch_with_addr(d)
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                ...
                emit_round_2_interleaved_vselect(group_desks)  # Computes addr for round 3

                # Rounds 3-9: gather rounds, each computes addr for next
                for rnd in range(3, 10):
                    compute_next = (rnd < 9)  # Round 9 doesn't need addr for round 10
                    emit_gather_round_interleaved(group_desks, compute_addr_for_next=compute_next)

                # Round 9 computed addr, but round 10 needs it too
                # Actually need to compute addr before round 10
                for d in group_desks:
                    desk = desks[d]
                ...
                emit_round_13_interleaved_vselect(group_desks)  # Round 13, computes addr for 14
                emit_round_14_interleaved(group_desks)  # Round 14, computes addr for 15
