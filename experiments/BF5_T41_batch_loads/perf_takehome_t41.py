"""
Theory 41: Batch all loads at start of each group

Hypothesis: Moving all gather loads to the beginning of each group enables
better load pipelining and may reduce memory access latency stalls.
...
class KernelBuilderT41:
    """T41: Batch all loads at start of each group"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # T41: BATCHED LOADS - compute all addresses first, then load all at once
        def emit_gather_round_batched(group_desks):
            # Batch step 1: Compute ALL addresses first
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
            # Batch step 2: Issue ALL loads
            for d in group_desks:
                desk = desks[d]
                for lane in range(VLEN):
                    self.emit("load", ("load", desk['node_val'] + lane, desk['addr'] + lane))
            # Batch step 3: Process ALL XORs
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            emit_hash_interleaved(group_desks)
        ...
        def emit_round_10_optimized(group_desks):
            """Round 10: Skip branch computation, directly set idx=0"""
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
            for d in group_desks:
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                ...
                emit_gather_round_batched(group_desks)  # Round 14
