"""
# Experiment H7H10: Combined Interleaving + vselect Bypass

This combines the two best optimizations:
- H7 (6,203 cycles): Aggressive cross-desk interleaving
- H10 (7,611 cycles): vselect bypass via VALU multiply

**Key H7 optimization:**
Software pipeline where desks are at different stages - overlapping gather
for one desk while hashing another, maximizing VALU/Load unit utilization.

**Key H10 optimization:**
Replace flow-unit vselect with VALU multiply:
- vselect(dest, cond, idx, v_zero) -> dest = idx * cond
- cond is 0 or 1 from bounds check (<)
- idx * 1 = idx (when in bounds)
- idx * 0 = 0 (when out of bounds)

**Savings from H10 applied to H7:**
- Before: 4 separate vselect operations = 4 flow-unit cycles
- After: 4 VALU multiply operations = can pack into 1-2 cycles
- Estimated savings: 2-3 cycles per loop iteration

**Expected:** ~5,900-6,100 cycles (H7's 6,203 minus ~100-300 cycles)
"""
...
class KernelBuilderH7H10:
    """
    H7H10: Aggressive Cross-Desk Interleaving + vselect Bypass

    Combines:
    - H7's software pipeline where desks operate at different stages
    - H10's vselect-to-multiply optimization for bounds checking
    ...
    - Stage 11: Bounds check & VALU multiply (was vselect)
    ...
    def build_kernel(
        ...
        """
        H7H10: Combined interleaving + vselect bypass kernel.
        """
        ...
        # Original H7: "flow": [("vselect", desks[0]['idx'], desks[0]['tmp1'], desks[0]['idx'], v_zero)]
        # H10 optimization: idx = idx * cond (where cond is 0 or 1)
        self.instrs.append({
            "valu": [
        ...
        # Original H7: "flow": [("vselect", desks[1]['idx'], desks[1]['tmp1'], desks[1]['idx'], v_zero)]
        self.instrs.append({
            "valu": [
                # H10 optimization: vselect -> multiply
                ("*", desks[1]['idx'], desks[1]['idx'], desks[1]['tmp1']),
        ...
        # Original H7: "flow": [("vselect", desks[2]['idx'], desks[2]['tmp1'], desks[2]['idx'], v_zero)]
        self.instrs.append({
            "valu": [
                # H10 optimization: vselect -> multiply
                ("*", desks[2]['idx'], desks[2]['idx'], desks[2]['tmp1']),
        ...
        # Original H7: self.add("flow", ("vselect", desks[3]['idx'], desks[3]['tmp1'], desks[3]['idx'], v_zero))
        # Key insight: Using VALU multiply instead of flow vselect allows us to overlap
        # with ALU operations that compute store addresses for desk3
        self.instrs.append({
            "valu": [
                # H10 optimization: vselect -> multiply
                ("*", desks[3]['idx'], desks[3]['idx'], desks[3]['tmp1']),
