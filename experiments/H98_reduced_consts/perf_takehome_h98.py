"""
# Experiment H98: Reduced Constants Analysis

**GOAL:** Analyze and potentially reduce the number of vconst allocations and broadcasts
by finding duplicate constants or mathematical relationships between hash stage constants.

**HYPOTHESIS:** Some hash constants may be the same or related, allowing us to reduce
the number of vconst allocations and broadcasts, saving scratch space and init cycles.

**ANALYSIS RESULTS:**
HASH_STAGES val1 constants (all unique):
  Stage 0: 0x7ED55D16 = 2127912214
  Stage 1: 0xC761C23C = 3345072700
  Stage 2: 0x165667B1 = 374761393
  Stage 3: 0xD3A2646C = 3550635116
  Stage 4: 0xFD7046C5 = 4251993797
  Stage 5: 0xB55A4F09 = 3042594569

HASH_STAGES val3 (shift) constants (all unique):
  Stage 0: 12  (FMA stage, uses mult 4097 = 1 + 2^12)
  Stage 1: 19
  Stage 2: 5   (FMA stage, uses mult 33 = 1 + 2^5)
  Stage 3: 9
  Stage 4: 3   (FMA stage, uses mult 9 = 1 + 2^3)
  Stage 5: 16

**KEY FINDING:** v_fma_mult[4] = 9 and v_hash_shifts[3] = 9 share the same value!
The scratch_vconst() function already deduplicates by value, so this is already optimized.

**CURRENT UNIQUE VECTOR CONSTANTS (15 total):**
  Basic: 0, 1, 2, 3 (4 constants)
  Hash val1: 6 unique constants (stages 0-5)
  FMA mults: 4097, 33 (stage 4's 9 is already counted in shifts)
  Shifts: 19, 9, 16 (3 unique, stage 3's 9 shared with FMA mult 4)
  Total: 4 + 6 + 2 + 3 = 15 unique vector constants

**CONCLUSION:** The H96 implementation already achieves optimal constant deduplication.
This experiment verifies the deduplication and explores if any further optimizations
are possible through constant reorganization.

**SCRATCH BUDGET (1536 slots):** Same as H96
- Scalars/init: ~20
- Vector constants: 15 * 8 = 120 (already optimal)
- Tree preloaded: 15 * 8 = 120
- Tree diffs: 3 * 8 = 24
- 32 desks (idx, val, node_val, addr): 32 * 4 * 8 = 1024
- Shared temps (16 vectors, 4 desks share): 16 * 8 = 128
- Offset regs: 32
- Addr tmp: 4
- Total: ~1472/1536
...
class KernelBuilderH98:
    """
    H98: Reduced Constants Analysis - verify deduplication is working optimally.
    Same as H96 but with explicit documentation of constant sharing.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        # Note: FMA_MULTIPLIERS[4] = 9 which equals HASH_STAGES[3][4] = 9 (shift value)
        # The scratch_vconst deduplication handles this automatically
        ...
        print(f"Unique scalar constants: {len(self.const_map)}")
        print(f"Unique vector constants: {len(self.vconst_map)}")

        # Verify the 9 deduplication
        if 9 in self.vconst_map:
            v_nine_addr = self.vconst_map[9]
            print(f"Value 9 is shared (addr={v_nine_addr}): v_fma_mult[4]={v_fma_mult[4]}, v_hash_shifts[3]={v_hash_shifts[3]}")
            assert v_fma_mult[4] == v_hash_shifts[3], "Deduplication failed for value 9!"
