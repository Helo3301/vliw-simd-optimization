"""
BF8 Theory 76: Emit ops for all 16 desks before scheduling
Hypothesis: Global scheduling across all desks may find more parallelism.
"""
...
class KernelBuilderBF8_76:
    """BF8 Theory 76: Emit all 16 desks before scheduling (global approach)"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # THEORY 76: Emit ALL desks for each operation type globally
        ALL_DESKS = list(range(NUM_DESKS))

        def emit_hash_all(desk_indices):
            # Stage 0 - FMA for ALL desks
            for d in desk_indices:
                desk = desks[d]
                self.emit("valu", ("multiply_add", desk['val'], desk['val'], v_fma_mult[0], v_hash_consts[0]))
            # Stage 1 - XOR/shift for ALL
            for d in desk_indices:
                desk = desks[d]
        ...
        def emit_branch_all(desk_indices):
            for d in desk_indices:
                desk = desks[d]
        ...
        def emit_rounds_0_1_2_all(desk_indices):
            # Round 0
            for d in desk_indices:
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], v_tree[0]))
            emit_hash_all(desk_indices)
            for d in desk_indices:
                desk = desks[d]
            ...
            for d in desk_indices:
                ...
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            emit_hash_all(desk_indices)
            emit_branch_all(desk_indices)

        def emit_rounds_11_12_13_all(desk_indices):
            # Round 11
            for d in desk_indices:
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], v_tree[0]))
            emit_hash_all(desk_indices)
        ...
        def emit_gather_round_all(desk_indices):
            for d in desk_indices:
                desk = desks[d]
        ...
        def emit_round_10_all(desk_indices):
            for d in desk_indices:
                desk = desks[d]
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
            for d in desk_indices:
        ...
        def emit_round_15_all(desk_indices):
            for d in desk_indices:
                desk = desks[d]
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
            for d in desk_indices:
        ...
        def emit_tile_all16(tile_idx):
            ...
            # THEORY 76: Process ALL 16 desks together
            emit_rounds_0_1_2_all(ALL_DESKS)
            for _rnd in range(3, 10):
                emit_gather_round_all(ALL_DESKS)
            emit_round_10_all(ALL_DESKS)
            emit_rounds_11_12_13_all(ALL_DESKS)
            emit_gather_round_all(ALL_DESKS)
            emit_round_15_all(ALL_DESKS)
