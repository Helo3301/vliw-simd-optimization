"""
# Experiment H108: 20 Desks with Per-Desk Temps

**GOAL:** Reduce temp conflicts by giving each desk its own tmp1 and tmp2.

**HYPOTHESIS:** 32 desks with shared temps causes conflicts. What if we used 20 desks with per-desk temps?
- 20 desks * 4 vectors (idx, val, node, addr) * 8 = 640 slots
- 20 desks * 2 temps * 8 = 320 slots
- Total desk storage: 960 slots
- Plus init (~300 slots) = ~1260 slots total (fits!)

**APPROACH:**
- Process in 2 tiles: first 160 elements (20*8), second 96 elements (12*8)
- Second tile uses only 12 of the 20 desks
...
class KernelBuilderH108:
    """
    H108: 20 desks with per-desk temps - eliminates temp conflicts.
    Process in 2 tiles: 160 elements (20*8) + 96 elements (12*8).
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel using 20 desks with per-desk temps.
        Process in 2 tiles: first 160 elements, second 96 elements.
        """
        ...
        # Preload only tree nodes 0-6 (same as H105)
        NUM_PRELOADED = 7
        v_tree = []
        ...
        # H108: Allocate 20 desks with per-desk temps (6 vectors each)
        NUM_DESKS = 20
        desks = []
        for d in range(NUM_DESKS):
            desk = {
                ...
                'tmp1': self.alloc_vec(f"v_tmp1_{d}"),  # Per-desk temp 1
                'tmp2': self.alloc_vec(f"v_tmp2_{d}"),  # Per-desk temp 2
            }
            desks.append(desk)

        # Offset addresses for 20 desks
        ...
        print(f"  - H108: 20 desks with per-desk temps (6 vectors each)")
        print(f"  - Processing 2 tiles: 160 elements (20*8) + 96 elements (12*8)")

        # Pause before main computation
        self.emit("flow", ("pause",))

        def emit_hash_stages_desks(desk_range):
            ...
            # Stage 1 (3 ops) - per-desk temps mean no conflicts
            for d in desk_range:
        ...
        def emit_hash_stages_single(desk_idx):
        ...
        def emit_tile(num_active_desks, offset_base):
            """Emit all operations for a tile with specified number of active desks."""
            desk_range = range(num_active_desks)

            # Load offsets for active desks
            for d in desk_range:
                self.emit("load", ("const", offset_regs[d], offset_base + d * VLEN))
            ...
            emit_hash_stages_desks(desk_range)
            for d in desk_range:
                emit_branch(d)
        ...
        # Emit Tile 1: First 160 elements (20 desks * 8)
        emit_tile(20, 0)

        # Emit Tile 2: Remaining 96 elements (12 desks * 8)
        # NO pause between tiles - they must complete in the same machine run
        emit_tile(12, 160)
