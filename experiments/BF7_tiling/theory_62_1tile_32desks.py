"""
Theory 62: 1 tile of 32 desks (if scratch fits)
Maximizes ILP by having all data in registers at once
"""
...
class KernelBuilder:
    ...
    def alloc_scratch(self, name=None, length=1):
        ...
        if self.scratch_ptr > SCRATCH_SIZE:
            raise MemoryError(f"Scratch overflow: {self.scratch_ptr} > {SCRATCH_SIZE}")
        return addr
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # CHANGED: 32 desks in a single tile
        NUM_DESKS = 32
        desks = []
        try:
            ...
            print(f"SCRATCH OVERFLOW: {e}")
            print(f"32 desks require more scratch than available ({SCRATCH_SIZE})")
            return
...
def do_kernel_test(forest_height: int, rounds: int, batch_size: int, seed: int = 123,
    ...
    if not hasattr(kb, 'instrs') or not kb.instrs:
        print("CYCLES: N/A (scratch overflow)")
        return None
