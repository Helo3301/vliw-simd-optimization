"""
Theory 65: Tile by round (8 rounds per tile)
Instead of tiling by batch, tile by rounds: process all desks for rounds 0-7, then 8-15
"""
...
class KernelBuilder:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # 16 desks processing all batch elements in tile 0
        NUM_DESKS = 16
        desks = []
        ...
        # Process 2 batch tiles of 16 desks each, but split by rounds
        def emit_round_tile_batch(batch_tile, round_start, round_end):
            """Process a tile of batch elements for a range of rounds"""
            tile_offset = batch_tile * NUM_DESKS * VLEN
            ...
            all_groups = [list(range(g * GROUP_SIZE, (g + 1) * GROUP_SIZE)) for g in range(num_full_groups)]

            # Process only the specified rounds
            for group_desks in all_groups:
                for rnd in range(round_start, round_end):
                    if rnd == 0:
                        emit_rounds_0_1_2_fused(group_desks)
                    elif rnd == 1 or rnd == 2:
                        pass  # handled in fused 0-1-2
                    elif rnd == 10:
                        emit_round_10_optimized(group_desks)
                    elif rnd == 11:
                        # Round 11 is first of fused 11-12-13, but we need to handle
                        # partial rounds properly
                        if round_end > 13:
                            # We can do the full fused version
                            pass  # skip, handled with 11-12-13 fused
                        else:
                            emit_gather_round_interleaved(group_desks)
                    elif rnd == 12 or rnd == 13:
                        pass  # handled in fused 11-12-13 if present
                    elif rnd == 15:
        ...
        # Round tile 0: rounds 0-7 for all batch elements
        emit_round_tile_batch(0, 0, 8)
        emit_round_tile_batch(1, 0, 8)

        # Round tile 1: rounds 8-15 for all batch elements
        emit_round_tile_batch(0, 8, 16)
        emit_round_tile_batch(1, 8, 16)
...
if __name__ == "__main__":
    do_kernel_test(10, 16, 256, check=False)  # Note: round tiling changes semantics
