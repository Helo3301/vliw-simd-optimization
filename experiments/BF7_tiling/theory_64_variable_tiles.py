"""
Theory 64: Variable tile sizes (16+16 vs 12+20)
Test asymmetric tiling - larger first tile to maximize pipelining
"""
...
class KernelBuilder:
    """Theory 64: Variable tile sizes 12+20"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Max desks needed (for 20 in second tile)
        MAX_DESKS = 20
        desks = []
        for d in range(MAX_DESKS):
            desk = {
        ...
        offset_regs = [self.alloc_scratch(f"off_{d}") for d in range(MAX_DESKS)]
        addr_tmp = [self.alloc_scratch(f"addr_tmp_{i}") for i in range(MAX_DESKS * 2)]
        ...
        def emit_tile(tile_idx, num_desks, base_offset):
            for d in range(num_desks):
                self.emit("load", ("const", offset_regs[d], base_offset + d * VLEN))
        ...
        # CHANGED: Variable tiles - 12 desks then 20 desks
        emit_tile(0, 12, 0)  # 12 desks = 96 elements
        emit_tile(1, 20, 96)  # 20 desks = 160 elements (total 256)
