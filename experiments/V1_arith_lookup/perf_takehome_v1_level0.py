"""
# Experiment V1b: Level-0 Only Arithmetic Lookup

This is a simplified version that specifically tests arithmetic lookup
for level 0 only (the root node, index 0).

Since all inputs start at index 0, the first round ALWAYS uses level 0.
For level 0, there's only 1 node value, so we can simply broadcast it
instead of doing any gather at all!

**Expected savings for round 0:**
- Standard gather: 4 cycles (8 loads at 2/cycle) + address computation
- Level 0 optimization: 0 cycles - just XOR with pre-broadcast value!

This is essentially "constant folding" for the root node.
"""
...
class KernelBuilderV1Level0:
    """
    V1b: Optimized for level 0 (root node).

    Since all inputs start at index 0 and the root value never changes,
    we preload tree[0] and broadcast it once. The first round of every
    batch iteration can skip the gather entirely!
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def build_kernel(
        ...
        # ============================================================
        # V1b: Pre-load root value (tree[0]) once
        # ============================================================
        root_val_scalar = self.alloc_scratch("root_val_scalar")
        v_root_val = self.alloc_scratch("v_root_val", VLEN)

        # Load tree[0] - the root value
        self.add("load", ("load", root_val_scalar, self.scratch["forest_values_p"]))
        # Broadcast to vector for XOR
        self.add("valu", ("vbroadcast", v_root_val, root_val_scalar))
