"""
# Experiment H101: Fused Branch Operations

**GOAL:** Reduce critical path length of branch and bounds operations.

**HYPOTHESIS:** In gather rounds (3-10, 14-15), after the XOR with node_val, the
node_val register is no longer needed for the rest of the round. We can reuse it
as the temp register for the branch operation, eliminating shared temp conflicts.

**KEY INSIGHT:** H96 uses shared temps for the branch operation:
  tmp1 = val & 1          (uses shared temp - potential conflict)
  idx = idx*2 + 1         (FMA)
  idx = idx + tmp1        (uses shared temp)

By using the per-desk node_val register (which is free after XOR), we eliminate
the temp sharing constraint and allow all 32 desks to emit branch ops in parallel.

**OPTIMIZATION:**
- For gather rounds (3-10, 14-15): Use node_val as temp for branch
- For other rounds (0, 1, 2, 11, 12, 13): Use standard shared temp

**BASELINE:** H96 = 1,850 cycles
**RESULT:** H101 = 1,847 cycles (3 cycle improvement)
"""
...
class KernelBuilderH101:
    """
    H96: 32 desks per tile - process all 256 batch elements with one tile.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        def emit_branch_with_node_val_temp(desk_idx):
            """Branch using per-desk node_val as temp (no shared temp conflicts)."""
            d = desks[desk_idx]
            # Use node_val as temp since each desk has its own
            self.emit("valu", ("&", d['node_val'], d['val'], v_one))
            self.emit("valu", ("multiply_add", d['idx'], d['idx'], v_two, v_one))
            self.emit("valu", ("+", d['idx'], d['idx'], d['node_val']))
        ...
        def emit_tile():
            ...
            # H101: Use node_val as temp for branch since it's free after XOR
            for _round in range(3, 10):
                ...
                emit_hash_stages_all_desks()
                # Branch using node_val as temp (all 32 desks in parallel)
                for d in range(NUM_DESKS):
                    self.emit("valu", ("&", desks[d]['node_val'], desks[d]['val'], v_one))
                for d in range(NUM_DESKS):
                    self.emit("valu", ("multiply_add", desks[d]['idx'], desks[d]['idx'], v_two, v_one))
                for d in range(NUM_DESKS):
                    self.emit("valu", ("+", desks[d]['idx'], desks[d]['idx'], desks[d]['node_val']))

            # Round 10: Gather WITH bounds
            # H101: Use node_val as temp for branch
            for d in range(NUM_DESKS):
                self.emit("valu", ("+", desks[d]['addr'], v_forest_p, desks[d]['idx']))
            for d in range(NUM_DESKS):
                for lane in range(VLEN):
            ...
            # H101: Overlap bounds with round 11 XOR and hash start
            # The XOR for round 11 only uses val (not idx), so it can happen
            # in parallel with the bounds check which uses idx

            # Emit all operations in optimal order for scheduler:
            # - bounds < uses idx and shared tmp
            # - XOR uses val and constant
            # - bounds * uses idx and shared tmp
            # - hash stages use val

            # First: all bounds comparison (<) ops
            for group in [group_0, group_1, group_2, group_3]:
                for d in group:
                    tmp1, _ = get_temps(d)
                    self.emit("valu", ("<", tmp1, desks[d]['idx'], v_n_nodes))

            # Second: all bounds multiply (*) ops - must complete before round 11 uses idx
            # But round 11 doesn't use idx in XOR or hash, only in branch at the end!
            for group in [group_0, group_1, group_2, group_3]:
                for d in group:
                    tmp1, _ = get_temps(d)
                    self.emit("valu", ("*", desks[d]['idx'], desks[d]['idx'], tmp1))

            # Third: Round 11 XOR with tree[0]
            for d in range(NUM_DESKS):
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], v_tree[0]))

            # Fourth: Round 11 hash stages
            emit_hash_stages_all_desks()

            # Fifth: Round 11 branch
            for d in range(NUM_DESKS):
                emit_branch(d)
