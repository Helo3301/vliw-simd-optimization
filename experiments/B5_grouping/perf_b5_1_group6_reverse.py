"""
# Experiment B5-1: Combine GROUP_SIZE=6 + reverse ordering

From prior results:
- H142: GROUP_SIZE=6 gave 1,644 cycles (-1 vs baseline)
- H144-G: Reverse group + desk order gave 1,635 cycles (-10)

This experiment combines both strategies:
- GROUP_SIZE=6 (instead of 4)
- Reverse processing order for groups and desks within groups
"""
...
class KernelBuilderB5_1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            # KEY CHANGE: GROUP_SIZE=6 + reverse ordering
            GROUP_SIZE = 6

            # Build groups with remaining desks
            all_groups = []
            for g in range(NUM_DESKS // GROUP_SIZE):
                # Reverse desk order within group
                group = list(range(g * GROUP_SIZE, (g + 1) * GROUP_SIZE))
                all_groups.append(list(reversed(group)))

            # Handle remaining desks (16 % 6 = 4 remaining)
            remaining_start = (NUM_DESKS // GROUP_SIZE) * GROUP_SIZE
            if remaining_start < NUM_DESKS:
                remaining = list(range(remaining_start, NUM_DESKS))
                all_groups.append(list(reversed(remaining)))
