"""
# Experiment B5-5: Variable group sizes per round type

Hypothesis: Different round types may benefit from different group sizes:
- Gather rounds (3-10, 14, 15): Smaller groups to reduce load contention
- Pure arithmetic rounds (0, 11): Larger groups for better ILP

Try GROUP_SIZE=8 for arithmetic-only rounds, GROUP_SIZE=2 for gather rounds.
"""
...
class KernelBuilderB5_5:
    """B5-5: Variable group sizes per round type"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Use GROUP_SIZE=8 for arithmetic-heavy rounds (larger groups for ILP)
        def emit_round_0_large_group(desk_list):
            for d in desk_list:
        ...
        def emit_round_1_large_group(desk_list):
            for d in desk_list:
                desk = desks[d]
        ...
        def emit_round_2_large_group(desk_list):
            for d in desk_list:
                desk = desks[d]
        ...
        # Use GROUP_SIZE=2 for gather rounds (smaller groups to reduce load contention)
        def emit_gather_round_small_group(desk_list):
            for d in desk_list:
                desk = desks[d]
        ...
        def emit_round_10_small_group(desk_list):
            for d in desk_list:
                desk = desks[d]
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
            for d in desk_list:
        ...
        def emit_round_11_large_group(desk_list):
            for d in desk_list:
                emit_xor_with_node(d, v_tree[0])
            for d in desk_list:
                emit_hash_stage_0(d)
        ...
        def emit_round_15_small_group(desk_list):
            for d in desk_list:
                desk = desks[d]
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
            for d in desk_list:
        ...
        def emit_tile_variable_groups(tile_idx):
            ...
            # Large groups (8 desks) for arithmetic-heavy rounds
            LARGE_GROUP_SIZE = 8
            large_groups = [list(range(i * LARGE_GROUP_SIZE, (i + 1) * LARGE_GROUP_SIZE)) for i in range(NUM_DESKS // LARGE_GROUP_SIZE)]

            # Small groups (2 desks) for gather rounds
            SMALL_GROUP_SIZE = 2
            small_groups = [list(range(i * SMALL_GROUP_SIZE, (i + 1) * SMALL_GROUP_SIZE)) for i in range(NUM_DESKS // SMALL_GROUP_SIZE)]

            # Round 0: large groups
            for group in large_groups:
                emit_round_0_large_group(group)

            # Round 1: large groups
            for group in large_groups:
                emit_round_1_large_group(group)

            # Round 2: large groups
            for group in large_groups:
                emit_round_2_large_group(group)

            # Rounds 3-9: small groups for gather
            for _rnd in range(3, 10):
                for group in small_groups:
                    emit_gather_round_small_group(group)

            # Round 10: small groups for gather + bounds
            for group in small_groups:
                emit_round_10_small_group(group)

            # Round 11: large groups
            for group in large_groups:
                emit_round_11_large_group(group)

            # Round 12 (same as 1): large groups
            for group in large_groups:
                emit_round_1_large_group(group)

            # Round 13 (same as 2): large groups
            for group in large_groups:
                emit_round_2_large_group(group)

            # Round 14: small groups for gather
            for group in small_groups:
                emit_gather_round_small_group(group)

            # Round 15: small groups for gather (final)
            for group in small_groups:
                emit_round_15_small_group(group)

            for d in range(NUM_DESKS):
                self.emit("store", ("vstore", addr_tmp[d*2], desks[d]['idx']))
                self.emit("store", ("vstore", addr_tmp[d*2+1], desks[d]['val']))

        emit_tile_variable_groups(0)
        emit_tile_variable_groups(1)
