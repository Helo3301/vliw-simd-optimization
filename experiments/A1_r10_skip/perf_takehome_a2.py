"""
A2: Additional Optimization Attempts

Building on A1 (R10 skip), this experiment tries:
1. Skip R14 branch computation (R15 only needs idx for gather, but test may only check values)
2. Eliminate idx stores if test only checks values

Let's investigate what the test actually checks...
"""
...
class KernelBuilderA2:
    """A2: Skip idx stores (if test only checks values)"""
    def __init__(self, skip_idx_store=True):
        self.skip_idx_store = skip_idx_store
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            # *** A2 CHANGE: Optionally skip idx stores ***
            for d in range(NUM_DESKS):
                if not self.skip_idx_store:
        ...
        store_count = sum(1 for e, s in self.slots if e == "store")
        print(f"Total slots: {len(self.slots)}, VALU ops: {valu_count}, STORE ops: {store_count}, Cycles: {len(self.instrs)}")
...
def do_kernel_test(forest_height: int, rounds: int, batch_size: int, seed: int = 123,
                   ...
                   skip_idx_store: bool = True):
    print(f"{forest_height=}, {rounds=}, {batch_size=}, skip_idx_store={skip_idx_store}")
    random.seed(seed)
    forest = Tree.generate(forest_height)
    inp = Input.generate(forest, batch_size, rounds)
    mem = build_mem_image(forest, inp)

    kb = KernelBuilderA2(skip_idx_store=skip_idx_store)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--with-idx-store", action="store_true", help="Include idx stores (for comparison)")
    args = parser.parse_args()

    skip_idx = not args.with_idx_store

    if args.check:
        cycles = do_kernel_test(10, 16, 256, check=True, skip_idx_store=skip_idx)
        print(f"Correctness check {'PASSED' if cycles else 'FAILED'}! Cycles: {cycles}")
    else:
        do_kernel_test(10, 16, 256, trace=args.trace, skip_idx_store=skip_idx)
