"""
A3: Deeper Operation Analysis

Let me analyze where cycles are being spent and try to find more optimizations.

Key insight: A1 saved 128 ops but only 10 cycles.
This means ops were overlapped with loads.

Strategy: Analyze scheduling patterns to find inefficiencies.
"""
...
def _schedule_slots_with_stats(slots: list[tuple[str, tuple]]) -> tuple[list[dict[str, list[tuple]]], dict]:
    """Schedule slots and return statistics about utilization"""
    ...
    for engine, slot in slots:
        ...
        # Track if this was delayed beyond minimum
        if cycle > earliest:
            delays.append((engine, slot, earliest, cycle, cycle - earliest))
    ...
    stats = {
        ...
        "valu_ops": sum(1 for e, s in slots if e == "valu"),
        "load_ops": sum(1 for e, s in slots if e == "load"),
        "store_ops": sum(1 for e, s in slots if e == "store"),
        "flow_ops": sum(1 for e, s in slots if e == "flow"),
        "alu_ops": sum(1 for e, s in slots if e == "alu"),
        "delays": delays,
    }

    # Compute per-cycle utilization
    valu_per_cycle = []
    load_per_cycle = []
    for c in cycles:
        if c:
            valu_per_cycle.append(usage[cycles.index(c)].get("valu", 0))
            load_per_cycle.append(usage[cycles.index(c)].get("load", 0))

    stats["avg_valu_util"] = sum(valu_per_cycle) / len(valu_per_cycle) if valu_per_cycle else 0
    stats["avg_load_util"] = sum(load_per_cycle) / len(load_per_cycle) if load_per_cycle else 0
    stats["max_valu_util"] = max(valu_per_cycle) if valu_per_cycle else 0
    stats["valu_full_cycles"] = sum(1 for v in valu_per_cycle if v == 6)

    return [c for c in cycles if c], stats


def _schedule_slots(slots: list[tuple[str, tuple]]) -> list[dict[str, list[tuple]]]:
    result, _ = _schedule_slots_with_stats(slots)
    return result


class KernelBuilderA3:
    """A3: Analysis and optimization"""
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        if self.analyze:
            ...
            for i, phase in enumerate(phases):
                if phase:
                    phase_instrs, stats = _schedule_slots_with_stats(phase)
            ...
            for i, stats in enumerate(all_stats):
                ...
                print(f"  VALU ops: {stats['valu_ops']} ({stats['avg_valu_util']:.2f} avg util, {stats['valu_full_cycles']} full cycles)")
                print(f"  LOAD ops: {stats['load_ops']} ({stats['avg_load_util']:.2f} avg util)")
                print(f"  Theoretical VALU min: {stats['valu_ops'] / 6:.1f} cycles")
                print(f"  Theoretical LOAD min: {stats['load_ops'] / 2:.1f} cycles")
                print(f"  Scheduling overhead: {stats['total_cycles'] - max(stats['valu_ops'] / 6, stats['load_ops'] / 2):.1f} cycles")
        else:
            for i, phase in enumerate(phases):
                if phase:
        ...
        print(f"\nTotal slots: {len(self.slots)}, VALU ops: {valu_count}, Cycles: {len(self.instrs)}")
...
def do_kernel_test(forest_height: int, rounds: int, batch_size: int, seed: int = 123,
    ...
    kb = KernelBuilderA3(analyze=analyze)
    ...
    print("vs A1 baseline (1548): ", 1548 - machine.cycle)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--analyze", action="store_true", help="Analyze scheduling")
    args = parser.parse_args()

    if args.check:
        cycles = do_kernel_test(10, 16, 256, check=True, analyze=args.analyze)
        print(f"Correctness check PASSED! Cycles: {cycles}")
    else:
        do_kernel_test(10, 16, 256, trace=args.trace, analyze=args.analyze)
