...
Building on H7H10 (5,947 cycles), this experiment optimizes the branch computation
by replacing MUL + ADD with a single FMA (multiply_add) instruction.

**Branch Computation Pattern:**
Current (4 ops per desk):
    branch = val & 1      # AND
    idx = idx * 2         # MUL
    idx = idx + 1         # ADD (separate cycle)
    idx = idx + branch    # ADD

Optimized (3 ops per desk):
    branch = val & 1                     # AND
    idx = multiply_add(idx, v_two, v_one) # FMA: idx*2 + 1
    idx = idx + branch                   # ADD

**Savings Analysis:**
- 4 desks * 1 VALU op saved = 4 VALU ops per iteration
- Total iterations = (256 / 8) * 16 / 4 = 128 iterations
- 4 ops * 128 iterations = 512 VALU ops saved
- Desk3's +1 operation is in its own cycle, so we save 1 cycle per iteration
- Expected: ~128 cycles savings (1 per iteration)

**Expected:** ~5,819 cycles (5,947 - 128)
"""
...
class KernelBuilderH11:
    """
    H11: Branch Computation FMA Optimization

    Builds on H7H10 by using FMA to combine branch MUL + ADD:
    - Before: idx = idx * 2; (later) idx = idx + 1
    - After:  idx = multiply_add(idx, v_two, v_one)  # idx*2 + 1
    ...
    def build_kernel(
        ...
        """
        H11: Branch FMA Optimization kernel.
        """
        ...
        # H11 CHANGE: Desk3 gather 4-5 + desk0 branch prep (AND, FMA) + desk1 hash3 combine + desk2 hash1 prep
        # Original: ("*", desks[0]['idx'], desks[0]['idx'], v_two) followed by separate ADD +1
        # H11: Use FMA to combine MUL and ADD: idx = idx*2 + 1
        self.instrs.append({
            "load": [
            ...
            "valu": [
                ...
                ("multiply_add", desks[0]['idx'], desks[0]['idx'], v_two, v_one),  # H11: FMA replaces MUL
                ("^", desks[1]['val'], desks[1]['tmp1'], desks[1]['tmp2']),
        ...
        # H11 CHANGE: Desk3 gather 6-7 + desk0 branch (add tmp1 ONLY - +1 now in FMA) + desk1 hash4 + desk2 hash1 combine
        # Original had: ("+", desks[0]['idx'], desks[0]['idx'], v_one) - REMOVED (in FMA now)
        self.instrs.append({
            "load": [
            ...
            "valu": [
                ("+", desks[0]['idx'], desks[0]['idx'], desks[0]['tmp1']),  # H11: Only add branch bit now
                ("multiply_add", desks[1]['val'], desks[1]['val'], v_fma_mult[4], v_hash_consts[4]),
                ("^", desks[2]['val'], desks[2]['tmp1'], desks[2]['tmp2']),
            ],
        })

        # H11 CHANGE: desk0 branch add moved to previous cycle, so this cycle just has hash ops
        # Original had: ("+", desks[0]['idx'], desks[0]['idx'], desks[0]['tmp1']) - REMOVED (moved earlier)
        self.instrs.append({
            "valu": [
                # Desk1 hash5 prep
        ...
        # H11 CHANGE: Replace desk0 vselect with VALU multiply + desk1 branch prep with FMA
        # Original: ("*", desks[1]['idx'], desks[1]['idx'], v_two) followed by separate ADD +1
        # H11: Use FMA for desk1: idx = idx*2 + 1
        self.instrs.append({
            "valu": [
                # H10 optimization: vselect -> multiply
                ("*", desks[0]['idx'], desks[0]['idx'], desks[0]['tmp1']),
                # desk1 branch prep with FMA
                ("&", desks[1]['tmp1'], desks[1]['val'], v_one),
                ("multiply_add", desks[1]['idx'], desks[1]['idx'], v_two, v_one),  # H11: FMA replaces MUL
                # desk2 hash3 combine
                ("^", desks[2]['val'], desks[2]['tmp1'], desks[2]['tmp2']),
                # desk3 hash1 prep
        ...
        # H11 CHANGE: Desk1 branch (add tmp1 ONLY - +1 now in FMA) + desk2 hash4 + desk3 hash1 combine
        # Original had: ("+", desks[1]['idx'], desks[1]['idx'], v_one) - REMOVED (in FMA now)
        self.instrs.append({
            "valu": [
                ("+", desks[1]['idx'], desks[1]['idx'], desks[1]['tmp1']),  # H11: Only add branch bit now
                ("multiply_add", desks[2]['val'], desks[2]['val'], v_fma_mult[4], v_hash_consts[4]),
                ("^", desks[3]['val'], desks[3]['tmp1'], desks[3]['tmp2']),
            ],
        })

        # H11 CHANGE: Desk1 branch add moved to previous cycle, so just hash ops
        # Original had: ("+", desks[1]['idx'], desks[1]['idx'], desks[1]['tmp1']) - REMOVED (moved earlier)
        self.instrs.append({
            "valu": [
        ...
        # H11 CHANGE: Replace desk1 vselect with VALU multiply + desk2 branch prep with FMA
        # Original: ("*", desks[2]['idx'], desks[2]['idx'], v_two) followed by separate ADD +1
        # H11: Use FMA for desk2: idx = idx*2 + 1
        self.instrs.append({
            "valu": [
                # H10 optimization: vselect -> multiply
                ("*", desks[1]['idx'], desks[1]['idx'], desks[1]['tmp1']),
                # desk2 branch prep with FMA
                ("&", desks[2]['tmp1'], desks[2]['val'], v_one),
                ("multiply_add", desks[2]['idx'], desks[2]['idx'], v_two, v_one),  # H11: FMA replaces MUL
                # desk3 hash3 combine
                ("^", desks[3]['val'], desks[3]['tmp1'], desks[3]['tmp2']),
            ],
        })

        # H11 CHANGE: Desk2 branch (add tmp1 ONLY - +1 now in FMA) + desk3 hash4
        # Original had: ("+", desks[2]['idx'], desks[2]['idx'], v_one) - REMOVED (in FMA now)
        self.instrs.append({
            "valu": [
                ("+", desks[2]['idx'], desks[2]['idx'], desks[2]['tmp1']),  # H11: Only add branch bit now
                ("multiply_add", desks[3]['val'], desks[3]['val'], v_fma_mult[4], v_hash_consts[4]),
            ],
        })

        # H11 CHANGE: Desk2 branch add moved to previous cycle, so just hash ops
        # Original had: ("+", desks[2]['idx'], desks[2]['idx'], desks[2]['tmp1']) - REMOVED (moved earlier)
        self.instrs.append({
            "valu": [
        ...
        # H11 CHANGE: Replace desk2 vselect with VALU multiply + desk3 branch prep with FMA
        # Original: ("*", desks[3]['idx'], desks[3]['idx'], v_two) followed by separate ADD +1
        # H11: Use FMA for desk3: idx = idx*2 + 1
        self.instrs.append({
            "valu": [
                # H10 optimization: vselect -> multiply
                ("*", desks[2]['idx'], desks[2]['idx'], desks[2]['tmp1']),
                # desk3 branch prep with FMA
                ("&", desks[3]['tmp1'], desks[3]['val'], v_one),
                ("multiply_add", desks[3]['idx'], desks[3]['idx'], v_two, v_one),  # H11: FMA replaces MUL
            ],
        })

        # H11 CHANGE: Desk3 branch (add tmp1 ONLY - +1 now in FMA)
        # Original had a separate cycle for +1 which is now ELIMINATED
        self.instrs.append({
            "valu": [
                ("+", desks[3]['idx'], desks[3]['idx'], desks[3]['tmp1']),  # H11: Only add branch bit now
            ],
        })
