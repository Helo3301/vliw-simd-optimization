...
Based on H7's aggressive interleaving (6,203 cycles with 4 desks), NOT T4's sequential
approach (9,792 cycles).

**Strategy:** Run H7's proven 4-desk interleaving pattern twice per loop iteration.
This processes 8 desks total per iteration while maintaining the validated interleaving
structure that gives 36.7% improvement.

**Result:** 6,334 cycles - slightly worse than H7's 6,203 due to:
- Sequential processing of two 4-desk blocks doesn't provide additional ILP
- Extra overhead for loading 8 desks' data
- Store phase at end is longer (8 stores vs 4)

**Conclusion:** 8-desk interleaving doesn't improve over 4-desk because:
- The bottleneck is the gather phase (2 loads/cycle for each desk's 8 lanes)
- More desks don't help if they can't overlap their gather phases efficiently
- True interleaving of 8 desks would require overlapping stores with gathers,
  but store and load engines operate independently
"""
...
class KernelBuilderH19:
    """
    H19: 8-Desk Extreme ILP using H7's interleaving pattern twice.

    Each loop iteration processes 8 desks by running the 4-desk pattern twice.
    This reduces loop overhead while maintaining the proven interleaving structure.
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def emit_4desk_block(self, desks, offset_regs, addr_tmp, v_fma_mult, v_hash_consts,
                         v_hash_shifts, v_zero, v_one, v_two, v_n_nodes, base_offset_idx):
        """
        Emit H7-style interleaving for 4 desks.
        base_offset_idx: 0 for desks 0-3, 4 for desks 4-7
        """
        # Gather addresses computation
        self.instrs.append({
            "valu": [
        ...
        # Desk0 vselect + desk1 branch prep + desk2 hash3 combine + desk3 hash1 prep
        self.instrs.append({
        ...
        # Desk1 vselect + desk2 branch prep + desk3 hash3 combine
        self.instrs.append({
        ...
        # Desk2 vselect + desk3 branch prep
        self.instrs.append({
        ...
        self.instrs.append({
            "alu": [
                ("+", addr_tmp[0], self.scratch["inp_indices_p"], offset_regs[base_offset_idx + 0]),
                ("+", addr_tmp[1], self.scratch["inp_values_p"], offset_regs[base_offset_idx + 0]),
                ("+", addr_tmp[2], self.scratch["inp_indices_p"], offset_regs[base_offset_idx + 1]),
                ("+", addr_tmp[3], self.scratch["inp_values_p"], offset_regs[base_offset_idx + 1]),
                ("+", addr_tmp[4], self.scratch["inp_indices_p"], offset_regs[base_offset_idx + 2]),
                ("+", addr_tmp[5], self.scratch["inp_values_p"], offset_regs[base_offset_idx + 2]),
            ],
        })
        self.instrs.append({
            "alu": [
                ("+", addr_tmp[6], self.scratch["inp_indices_p"], offset_regs[base_offset_idx + 3]),
                ("+", addr_tmp[7], self.scratch["inp_values_p"], offset_regs[base_offset_idx + 3]),
            ],
        })

        # Store all 4 desks (2 stores per cycle)
    ...
    def build_kernel(
        ...
        """
        H19: 8-desk interleaving kernel using H7 pattern twice per iteration.
        """
        ...
        # Use 8 desks for interleaving (two groups of 4)
        NUM_DESKS = 8
        ...
        # === MAIN LOOP with 8-DESK processing (two 4-desk blocks) ===
        main_loop_start = len(self.instrs)

        # PHASE 1: Calculate all 8 offsets
        self.instrs.append({
            "alu": [
        ...
        # Load all 8 desks' indices and values
        # First compute addresses for all loads
        self.instrs.append({
            "alu": [
        ...
        # Compute addresses for desks 4-7
        self.instrs.append({
            "alu": [
                ("+", addr_tmp[0], self.scratch["inp_indices_p"], offset_regs[4]),
                ("+", addr_tmp[1], self.scratch["inp_values_p"], offset_regs[4]),
                ("+", addr_tmp[2], self.scratch["inp_indices_p"], offset_regs[5]),
                ("+", addr_tmp[3], self.scratch["inp_values_p"], offset_regs[5]),
                ("+", addr_tmp[4], self.scratch["inp_indices_p"], offset_regs[6]),
                ("+", addr_tmp[5], self.scratch["inp_values_p"], offset_regs[6]),
                ("+", addr_tmp[6], self.scratch["inp_indices_p"], offset_regs[7]),
                ("+", addr_tmp[7], self.scratch["inp_values_p"], offset_regs[7]),
            ],
        })

        # Load desks 4-7
        self.instrs.append({"load": [("vload", desks[4]['idx'], addr_tmp[0]), ("vload", desks[4]['val'], addr_tmp[1])]})
        self.instrs.append({"load": [("vload", desks[5]['idx'], addr_tmp[2]), ("vload", desks[5]['val'], addr_tmp[3])]})
        self.instrs.append({"load": [("vload", desks[6]['idx'], addr_tmp[4]), ("vload", desks[6]['val'], addr_tmp[5])]})
        self.instrs.append({"load": [("vload", desks[7]['idx'], addr_tmp[6]), ("vload", desks[7]['val'], addr_tmp[7])]})

        # Process first 4 desks using H7 pattern
        self.emit_4desk_block(desks[0:4], offset_regs, addr_tmp, v_fma_mult, v_hash_consts,
                              v_hash_shifts, v_zero, v_one, v_two, v_n_nodes, 0)

        # Process second 4 desks using H7 pattern
        self.emit_4desk_block(desks[4:8], offset_regs, addr_tmp, v_fma_mult, v_hash_consts,
                              v_hash_shifts, v_zero, v_one, v_two, v_n_nodes, 4)
