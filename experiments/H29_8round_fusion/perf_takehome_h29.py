"""
# Experiment H29: 8-Round Fusion for VLIW SIMD Tree Traversal

This experiment tests if increasing fusion depth from 2 rounds (C4) to 8 rounds
shows proportional performance gains.

**Hypothesis:** By keeping idx and val in registers for 8 consecutive rounds,
we eliminate 75% of vload/vstore pairs compared to processing 1 round at a time,
and 60% compared to C4's 2-round fusion.

**Key Changes from C4:**
1. Process 8 rounds per iteration instead of 2
2. Keep idx and val in registers for all 8 rounds before storing
3. Total iterations reduced from 64 to 16: (256 elements * 16 rounds) / (4 desks * 8 elements * 8 rounds) = 16

**Expected Results:**
- If theory correct: ~2,800-3,200 cycles (30-40% improvement over C4's 4,667)
- If theory wrong: ~4,400-4,600 cycles (marginal improvement)
"""
...
class KernelBuilderH29:
    """
    H29: 8-Round Fusion Kernel

    Processes 8 consecutive rounds per memory access cycle:
    - load -> round0 -> round1 -> round2 -> round3 -> round4 -> round5 -> round6 -> round7 -> store
    - Eliminates intermediate vload/vstore pairs
    - Total iterations = (batch_size // VLEN) * (rounds // 8) // NUM_DESKS = 16
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def emit_single_round(self, desks, v_hash_consts, v_hash_shifts, v_fma_mult, v_one, v_two, v_n_nodes, round_num, is_last_round=False):
        """
        Emit instructions for a single round of tree traversal for all 4 desks.
        The idx and val are already in registers from the previous round.

        For each desk, we need to:
        1. Compute gather address from idx
        2. Gather node values
        3. XOR val with node values
        4. Apply 6 hash stages to val
        5. Compute branch: idx = idx * 2 + 1 + (val & 1)
        6. Bounds check: if idx >= n_nodes, idx = 0
        """
        # Step 1: Broadcast forest_values_p and add idx to get gather addresses
        self.instrs.append({
            "valu": [
        ...
        # Desk1 gather 2-3 + desk0 hash stage 0 (FMA)
        self.instrs.append({
            "load": [
        ...
        # Desk3 gather 6-7 + desk0 branch (add tmp1) + desk1 hash4 + desk2 hash1 combine
        self.instrs.append({
            "load": [
        ...
        # desk0 branch done, desk3 XOR + remaining hashes
        self.instrs.append({
            "valu": [
        ...
        # Desk0 vselect bypass + desk1 branch prep + more hashes
        self.instrs.append({
            "valu": [
                ("*", desks[0]['idx'], desks[0]['idx'], desks[0]['tmp1']),
    ...
    def build_kernel(
        ...
        # H29 CHANGE: Process 8 rounds per iteration
        # Total iterations = (batch_size // VLEN) * (rounds // 8) // NUM_DESKS
        # = (256 // 8) * (16 // 8) // 4 = 32 * 2 // 4 = 16
        ROUNDS_PER_ITER = 8
        total_iterations = (batch_size // VLEN) * (rounds // ROUNDS_PER_ITER) // NUM_DESKS
        ...
        # ============================================================
        # H29: Process 8 rounds without intermediate stores
        # ============================================================

        for round_num in range(ROUNDS_PER_ITER):
            self.emit_single_round(
                desks, v_hash_consts, v_hash_shifts, v_fma_mult,
                v_one, v_two, v_n_nodes, round_num,
                is_last_round=(round_num == ROUNDS_PER_ITER - 1)
            )

        # ============================================================
        # After all 8 rounds: Store results
        # ============================================================
