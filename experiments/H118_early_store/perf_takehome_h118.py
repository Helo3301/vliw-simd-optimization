"""
# Experiment H118: Early Store Address Computation

**GOAL:** Overlap store preparation with final round operations to reduce cycles.

**HYPOTHESIS:** After round 15's hash+branch, we store idx and val. But the stores depend
on the final val. What if we started computing store addresses earlier (during the last
hash stages) while hash is still computing?

The key insight:
- Store addresses only depend on offset_regs and inp_indices_p/inp_values_p
- These are constants that don't change during computation
- We can compute store addresses during round 15's hash stages
- Start storing desks as soon as they complete, not waiting for all
...
class KernelBuilderH118:
    """
    H118: Early store address computation - overlap store prep with final round.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel using 32 desks per tile with early store address computation.
        """
        ...
        # H105 CHANGE: Preload only tree nodes 0-6 (was 0-14)
        NUM_PRELOADED = 7
        v_tree = []
        ...
        # H118: Allocate more addr_tmp for early store address computation
        # We need 4 for load addresses + 4 for store addresses (computed early)
        addr_tmp = [self.alloc_scratch(f"addr_tmp_{i}") for i in range(4)]
        # Additional store address registers - precompute during round 15
        store_addr_idx = [self.alloc_scratch(f"store_addr_idx_{d}") for d in range(NUM_DESKS)]
        store_addr_val = [self.alloc_scratch(f"store_addr_val_{d}") for d in range(NUM_DESKS)]

        scratch_before = self.scratch_ptr
        print(f"Scratch usage: {self.scratch_ptr} / {SCRATCH_SIZE}")
        print(f"  - H118: Early store address computation")
        print(f"  - 32 desks with 4-desk temp sharing (16 shared temps)")
        print(f"  - Pre-allocated store addresses for all desks")
        ...
        def emit_hash_stages_all_desks():
            ...
            # Stage 1 (3 ops) - process groups sequentially
        ...
        def emit_hash_stages_with_early_store_addr():
            """
            H118: Emit hash stages for round 15 with interleaved store address computation.
            Store addresses only depend on offset_regs and inp_*_p which are constants.
            We can compute them during hash stages to overlap with VALU work.
            """
            # Stage 0 (FMA) - all desks can run in parallel
            for d in range(NUM_DESKS):
                self.emit("valu", ("multiply_add", desks[d]['val'], desks[d]['val'], v_fma_mult[0], v_hash_consts[0]))

            # Interleave store address computation with hash stage 1
            # Compute addresses for first batch of desks (0-15)
            for d in range(16):
                self.emit("alu", ("+", store_addr_idx[d], self.scratch["inp_indices_p"], offset_regs[d]))
            ...
            # Compute store addresses for second batch (16-31) and val addresses for first batch
            for d in range(16, 32):
                self.emit("alu", ("+", store_addr_idx[d], self.scratch["inp_indices_p"], offset_regs[d]))
            for d in range(16):
                self.emit("alu", ("+", store_addr_val[d], self.scratch["inp_values_p"], offset_regs[d]))

            # Stage 2 (FMA) - all desks
            for d in range(NUM_DESKS):
                self.emit("valu", ("multiply_add", desks[d]['val'], desks[d]['val'], v_fma_mult[2], v_hash_consts[2]))

            # Compute val addresses for second batch (16-31)
            for d in range(16, 32):
                self.emit("alu", ("+", store_addr_val[d], self.scratch["inp_values_p"], offset_regs[d]))
        ...
        def emit_tile():
            ...
            # H118: Compute ALL store addresses upfront during the load phase
            # This happens before the main computation so it's "free" latency-wise
            for d in range(NUM_DESKS):
                self.emit("alu", ("+", store_addr_idx[d], self.scratch["inp_indices_p"], offset_regs[d]))
                self.emit("alu", ("+", store_addr_val[d], self.scratch["inp_values_p"], offset_regs[d]))
            ...
            # Round 14: Gather without bounds
            for d in range(NUM_DESKS):
                self.emit("valu", ("+", desks[d]['addr'], v_forest_p, desks[d]['idx']))
            for d in range(NUM_DESKS):
                for lane in range(VLEN):
            ...
            # Round 15: Gather without bounds
            # H118: Store addresses were pre-computed at start of tile
            for d in range(NUM_DESKS):
                self.emit("valu", ("+", desks[d]['addr'], v_forest_p, desks[d]['idx']))
            for d in range(NUM_DESKS):
                for lane in range(VLEN):
            ...
            # H118: Store results using pre-computed addresses (computed at start of tile)
            # All addresses are ready, stores can be scheduled immediately after branches
            for d in range(NUM_DESKS):
                self.emit("store", ("vstore", store_addr_idx[d], desks[d]['idx']))
                self.emit("store", ("vstore", store_addr_val[d], desks[d]['val']))
