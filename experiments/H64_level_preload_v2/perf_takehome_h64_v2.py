...
class KernelBuilderH54:
    ...
    def build_kernel(
        ...
        # H64 OPTIMIZATION: Preload tree[0] for round 0
        # All 256 batch elements start at idx=0, so round 0 needs tree[0] for all
        tree0_scalar = self.alloc_scratch("tree0_scalar")
        tree0_addr = self.alloc_scratch("tree0_addr")
        self.add("alu", ("+", tree0_addr, self.scratch["forest_values_p"], zero_const))
        self.add("load", ("load", tree0_scalar, tree0_addr))
        v_tree0 = self.alloc_scratch("v_tree0", VLEN)
        self.add("valu", ("vbroadcast", v_tree0, tree0_scalar))
