"""
# Experiment H120: Fast Init Phase

**GOAL:** Reduce init phase cycles by eliminating unnecessary header loads.

**HYPOTHESIS:** The init phase loads 7 header values but only 4 are actually used:
- rounds (index 0) - NOT USED
- n_nodes (index 1) - USED for bounds checking
- batch_size (index 2) - NOT USED
- forest_height (index 3) - NOT USED
- forest_values_p (index 4) - USED for tree access
- inp_indices_p (index 5) - USED for input loading/storing
- inp_values_p (index 6) - USED for input loading/storing

By skipping the unused header loads, we save 6 cycles (2 ops per unused var x 3 vars).

Additionally, we can load v_n_nodes more efficiently by loading the n_nodes scalar
directly (index 1) without first allocating a scratch slot for it.
...
class KernelBuilderH120:
    """
    H120: Fast init phase - eliminate unused header loads.

    Only load 4 of 7 header values:
    - n_nodes (index 1) - for bounds checking
    - forest_values_p (index 4) - for tree access
    - inp_indices_p (index 5) - for input loading/storing
    - inp_values_p (index 6) - for input loading/storing
    ...
    def build_kernel(
        ...
        print(f"  - H120: Fast init - only loading 4 of 7 header values")
