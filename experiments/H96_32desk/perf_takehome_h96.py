"""
# Experiment H96: 32 Desks per Tile

**GOAL:** Use 32 desks x 1 tile instead of H87's 16 desks x 2 tiles to process 256 batch elements.

**HYPOTHESIS:** More desks = more instruction-level parallelism for the scheduler to exploit.
With 32 independent desks, the scheduler has more operations to choose from, potentially
filling VALU slots (limit: 6 per cycle) more efficiently.

**KEY INSIGHT:** H87 uses 16 desks per tile with 2 tiles processed sequentially.
By using 32 desks in a single tile, we:
1. Double the ILP available in each round
2. Eliminate the tile boundary overhead
3. Give the scheduler more independent operations to choose from
...
- Offset regs: 32
- Addr tmp: 64
- Total: ~1532/1536 (tight fit!)

**KEY CHANGE:** Share temps among 4 consecutive desks instead of 2 to fit in scratch.
Process all 32 desks in one tile with double-pump pattern.
