"""
# Experiment H109: 8-Way Arithmetic Selection for Round 3

**GOAL:** Extend arithmetic selection to round 3 which uses indices {7,8,9,10,11,12,13,14}.

**HYPOTHESIS:** We can use 8-way arithmetic selection with 3 bits:
- idx - 7 gives {0,1,2,3,4,5,6,7}
- bit0 = (idx-7) & 1
- bit1 = ((idx-7) >> 1) & 1
- bit2 = (idx-7) >> 2
- Select from pairs (4 pairs), then quads (2 quads), then final 8-way

This replaces 32*8=256 gather loads with arithmetic ops per round.
...
class KernelBuilderH109:
    """
    H109: 8-way arithmetic selection for round 3 (indices 7-14).
    Extends H105 by preloading nodes 0-14 and using arithmetic selection for round 3.
    Uses fewer shared temps (12 instead of 16) to make room for diff vectors.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel using 32 desks per tile with 8-way arithmetic selection for round 3.
        """
        ...
        v_seven = self.scratch_vconst(7, "v_seven")  # For 8-way selection (round 3)
        ...
        # H109 CHANGE: Preload tree nodes 0-14 (back to 15 nodes)
        # We need nodes 7-14 for 8-way arithmetic selection in round 3
        NUM_PRELOADED = 15
        v_tree = []
        ...
        # For 8-way selection (round 3, 14)
        # Pair differences: tree[8]-tree[7], tree[10]-tree[9], tree[12]-tree[11], tree[14]-tree[13]
        ...
        # SHARED TEMP REGISTERS - Use 12 vectors (96 slots)
        # Reduced from 16 to make room for diff vectors
        # Groups of 8 desks share 3 temps (4 groups * 3 temps = 12 temps)
        NUM_SHARED_TEMPS = 12
        ...
        def get_temps(desk_idx):
            ...
            8 desks share 3 temps to fit in scratch budget.
            """
            group = desk_idx // 8  # 4 groups total (0-7, 8-15, 16-23, 24-31)
            t1_idx = group * 3
            t2_idx = group * 3 + 1
            t3_idx = group * 3 + 2
            return shared_temps[t1_idx], shared_temps[t2_idx], shared_temps[t3_idx]
        ...
        print(f"  - H109: Preloading 15 nodes for 8-way arithmetic selection")
        print(f"  - 32 desks with 8-desk temp sharing ({NUM_SHARED_TEMPS} shared temps)")

        # Pause before main computation
        self.emit("flow", ("pause",))

        # Desk groupings for temp conflict avoidance
        # With 8-desk sharing, we need to process desks in groups that don't share temps
        ...
        def emit_hash_stages(desk_idx):
            ...
            d = desks[desk_idx]
            tmp1, tmp2, _ = get_temps(desk_idx)
            for hi in range(6):
                if hi in v_fma_mult:
        ...
        def emit_8way_selection(desk_idx):
            """Emit 8-way arithmetic selection for indices {7,8,9,10,11,12,13,14}.

            Uses 3 bits to select from 8 preloaded tree values:
            - bit0 = (idx-7) & 1
            - bit1 = ((idx-7) >> 1) & 1
            - bit2 = (idx-7) >> 2

            Selection tree:
            Level 1 (pairs): Select from (7,8), (9,10), (11,12), (13,14) using bit0
            Level 2 (quads): Select from pair results using bit1
            Level 3 (final): Select from quad results using bit2

            Uses tmp1, tmp2, tmp3 from shared temps.
            """
            d = desks[desk_idx]
            tmp1, tmp2, tmp3 = get_temps(desk_idx)

            # Extract bits from idx - 7
            # tmp1 = idx - 7 (0-7)
            self.emit("valu", ("-", tmp1, d['idx'], v_seven))
            # bit0 = tmp1 & 1 -> store in tmp2
            self.emit("valu", ("&", tmp2, tmp1, v_one))
            # tmp3 = tmp1 >> 1
            self.emit("valu", (">>", tmp3, tmp1, v_one))
            # bit1 = tmp3 & 1 -> store in d['addr']
            self.emit("valu", ("&", d['addr'], tmp3, v_one))
            # bit2 = tmp3 >> 1 -> store in tmp1
            self.emit("valu", (">>", tmp1, tmp3, v_one))

            # Now: bit0 in tmp2, bit1 in d['addr'], bit2 in tmp1

            # Level 1: Select from pairs using bit0 (in tmp2)
            # pair0 = tree[7] + bit0 * diff_7_8
            self.emit("valu", ("multiply_add", tmp3, tmp2, v_diff_7_8, v_tree[7]))  # pair0 in tmp3
            # pair1 = tree[9] + bit0 * diff_9_10
            self.emit("valu", ("multiply_add", d['node_val'], tmp2, v_diff_9_10, v_tree[9]))  # pair1 in node_val

            # Level 2: Select from first two pairs using bit1 (in d['addr'])
            # quad0 = pair0 + bit1 * (pair1 - pair0)
            self.emit("valu", ("-", tmp2, d['node_val'], tmp3))  # diff = pair1 - pair0
            self.emit("valu", ("multiply_add", tmp3, d['addr'], tmp2, tmp3))  # quad0 in tmp3

            # Now compute pair2 and pair3
            # Need bit0 again - recompute from idx
            self.emit("valu", ("-", tmp2, d['idx'], v_seven))  # tmp2 = idx - 7
            self.emit("valu", ("&", tmp2, tmp2, v_one))        # bit0 in tmp2

            # pair2 = tree[11] + bit0 * diff_11_12
            self.emit("valu", ("multiply_add", d['node_val'], tmp2, v_diff_11_12, v_tree[11]))  # pair2 in node_val
            # pair3 = tree[13] + bit0 * diff_13_14
            self.emit("valu", ("multiply_add", tmp2, tmp2, v_diff_13_14, v_tree[13]))  # pair3 in tmp2

            # Level 2: Select from pair2, pair3 using bit1 (still in d['addr'])
            # quad1 = pair2 + bit1 * (pair3 - pair2)
            self.emit("valu", ("-", tmp2, tmp2, d['node_val']))  # diff = pair3 - pair2
            self.emit("valu", ("multiply_add", d['node_val'], d['addr'], tmp2, d['node_val']))  # quad1 in node_val

            # Level 3: Select from quad0, quad1 using bit2 (in tmp1)
            # result = quad0 + bit2 * (quad1 - quad0)
            self.emit("valu", ("-", tmp2, d['node_val'], tmp3))  # diff = quad1 - quad0
            self.emit("valu", ("multiply_add", d['node_val'], tmp1, tmp2, tmp3))  # final result in node_val
        ...
        def emit_tile():
            ...
            # Round 3: Indices in {7,8,9,10,11,12,13,14}, use 8-way arithmetic selection
            for d in range(NUM_DESKS):
                emit_8way_selection(d)
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))
                emit_hash_stages(d)
                emit_branch(d)

            # Rounds 4-9: Gather without bounds
            for _round in range(4, 10):
            ...
            # Round 14: Indices in {7,8,...,14} after wrap, use 8-way arithmetic selection
            for d in range(NUM_DESKS):
                emit_8way_selection(d)
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))
                emit_hash_stages(d)
            ...
            # Round 15: Gather without bounds (max idx < 2047)
            for d in range(NUM_DESKS):
                self.emit("valu", ("+", desks[d]['addr'], v_forest_p, desks[d]['idx']))
            for d in range(NUM_DESKS):
                for lane in range(VLEN):
