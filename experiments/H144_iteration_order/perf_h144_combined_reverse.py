"""
# Experiment H144-G: Combined Reverse - Both Group and Desk Order

**GOAL:** Combine reverse group order AND reverse desk order within groups.
H144-A (reverse groups) gave 1,636 cycles.
H144-F (reverse desks in group) gave 1,644 cycles.

**HYPOTHESIS:** Combining both reversals may compound the improvement.

**BASE:** H140 (1,645 cycles)
**TARGET:** < 1,636 cycles (best so far)
"""
...
class KernelBuilderH144G:
    """
    H144-G: Combined reverse - both reverse group order AND reverse desk order within groups.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """Build kernel with COMBINED REVERSE ORDER."""
        ...
        def emit_tile_interleaved(tile_idx):
            """Emit all operations with COMBINED REVERSE ORDER."""
            ...
            # ===== COMBINED REVERSE: Reverse groups AND reverse desks within groups =====
            GROUP_SIZE = 4
            num_groups = NUM_DESKS // GROUP_SIZE  # 4 groups

            all_groups = []
            for g in range(num_groups - 1, -1, -1):  # Reverse group order: 3, 2, 1, 0
                group = list(range(g * GROUP_SIZE, (g + 1) * GROUP_SIZE))
                all_groups.append(list(reversed(group)))  # Also reverse within group
