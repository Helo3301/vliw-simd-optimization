"""
# Experiment H144-B: Interleaved Tiles

**GOAL:** Test if interleaving operations between tiles produces better scheduling.
Instead of: tile 0 completely, then tile 1
Try: tile 0 group 0, tile 1 group 0, tile 0 group 1, tile 1 group 1, ...

**HYPOTHESIS:** Interleaving tiles may allow better overlap of memory operations
between the two tiles, potentially hiding latency.
...
class KernelBuilderH144B:
    """
    H144-B: Interleaved tiles - process groups from both tiles alternately.
    Reuse same desk registers but store/load between tiles.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """Build kernel with INTERLEAVED TILE GROUPS.

        Process: tile0-group0, tile1-group0, tile0-group1, tile1-group1, ...
        This interleaves the work between tiles at the group level.
        """
        ...
        # Allocate per-desk vectors (16 desks - reused for both tiles)
        NUM_DESKS = 16
        desks = []
        ...
        def emit_group(tile_idx, group_idx, group_desks):
            """Emit processing for a single group in a tile."""
            ...
            # Compute load addresses for this group
            for d in group_desks:
        ...
        # Process: tile0-group0, tile1-group0, tile0-group1, tile1-group1, ...
        GROUP_SIZE = 4
        num_groups = NUM_DESKS // GROUP_SIZE  # 4 groups

        for group_idx in range(num_groups):
            group_desks = list(range(group_idx * GROUP_SIZE, (group_idx + 1) * GROUP_SIZE))
            # Process tile 0 group
            emit_group(0, group_idx, group_desks)
            # Process tile 1 group
            emit_group(1, group_idx, group_desks)
