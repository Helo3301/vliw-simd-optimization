"""
# Experiment H144-D: Smaller Group Size (2 instead of 4)

**GOAL:** Test if smaller group sizes produce better scheduling.
Instead of: 4 desks per group
Try: 2 desks per group (8 groups instead of 4)

**HYPOTHESIS:** Smaller groups may allow more overlap between groups,
but could also increase overhead from more context switches.

**BASE:** H140 (1,645 cycles with group size 4)
**TARGET:** < 1,645 cycles
"""
...
class KernelBuilderH144D:
    """
    H144-D: Smaller group size (2 instead of 4).
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """Build kernel with GROUP SIZE 2."""
        ...
        def emit_tile_interleaved(tile_idx):
            """Emit all operations with GROUP SIZE 2."""
            ...
            # ===== GROUP SIZE 2 =====
            GROUP_SIZE = 2  # Changed from 4
            num_groups = NUM_DESKS // GROUP_SIZE  # 8 groups
