"""
B2-2: Branch Reduction via vselect (FLOW engine)

Current: idx = 2*idx + 1 + (val & 1) = 3 VALU ops (AND, FMA, ADD)

New approach using vselect:
1. idx_base = idx * 2 + 1  (1 VALU - FMA)
2. idx_plus_one = idx_base + 1  (1 VALU - ADD)
3. cond = val & 1  (1 VALU - AND)
4. idx' = vselect(cond, idx_plus_one, idx_base)  (1 FLOW - free from VALU perspective)

This is still 3 VALU ops, but by moving selection to FLOW we might get better ILP
since FLOW and VALU can execute in parallel.

Alternative: Try to eliminate the AND by directly using val's LSB in vselect
If vselect checks the LSB of the condition vector, we might not need the AND!
...
class KernelBuilderB2_2:
    """B2-2: Branch using vselect on FLOW engine"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # B2-2: Branch using vselect
        # Formula: idx = 2*idx + 1 + (val & 1)
        # Rewrite as: vselect(val & 1, 2*idx + 2, 2*idx + 1)
        #
        # Operations:
        # 1. idx_odd = FMA(idx, 2, 2)  -- 1 VALU
        # 2. idx_even = idx_odd - 1   -- 1 VALU (cheaper than separate FMA)
        # 3. cond = val & 1           -- 1 VALU
        # 4. idx = vselect(cond, idx_odd, idx_even)  -- 1 FLOW (parallel with VALU)
        #
        # Still 3 VALU, but vselect is free (FLOW engine)
        def emit_branch_vselect(desk_idx):
            d = desks[desk_idx]
            # Compute both branches
            self.emit("valu", ("multiply_add", d['tmp1'], d['idx'], v_two, v_two))  # idx*2 + 2 (odd case)
            self.emit("valu", ("-", d['tmp2'], d['tmp1'], v_one))  # idx*2 + 1 (even case)
            # Extract condition
            self.emit("valu", ("&", d['addr'], d['val'], v_one))  # reuse addr as temp
            # Select
            self.emit("flow", ("vselect", d['idx'], d['addr'], d['tmp1'], d['tmp2']))
