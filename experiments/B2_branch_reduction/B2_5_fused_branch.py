"""
B2-5: Fused Branch Computation Investigation

After exhaustive analysis in B2-1 through B2-4, I've concluded:

The formula idx' = 2*idx + 1 + (val & 1) CANNOT be reduced to 2 VALU ops
with the available instruction set because:

1. We need to extract bit 0 of val -> requires AND operation (1 op)
2. We need to compute 2*idx + 1 -> requires FMA or shift+add (1 op with FMA)
3. We need to add the extracted bit -> requires ADD operation (1 op)

The dependencies are:
- FMA can compute 2*idx + 1 in one op
- AND can extract val & 1 in one op
- But combining these two results ALWAYS requires a third ADD op

Alternative formulations tried:
- vselect: Moves work to FLOW engine but still 3 VALU ops needed
- shift+OR: Actually 4 ops (worse)
- algebraic rewriting: No simplification found

HOWEVER, I can try a different optimization: REDUCE TOTAL BRANCH OPERATIONS.

Observation: Round 15 (final round) doesn't need branch computation!
We only need idx for rounds 0-14 (15 branch operations total).

Current: 32 desks x 15 branches x 3 ops = 1440 VALU ops for branch
If we skip round 15 branch: 32 desks x 14 branches x 3 ops = 1344 VALU ops
Savings: 96 VALU ops = 96/6 = 16 cycles

Let me also verify by checking the code: does round 15 actually need idx?

Looking at emit_round_15_final_interleaved:
- It computes addr = forest_p + idx (uses idx)
- Does gather loads
- XORs and hashes
- NO branch at the end!

So the current implementation already doesn't have branch in round 15!

Let me verify by counting branch calls in the code...
Actually, looking more carefully, the last branch call is in emit_gather_round_interleaved
which is called for round 14.

Let me now try a DIFFERENT optimization:
Can we reduce branch operations by computing them differently across rounds?

Observation: After round 10, there's a bounds check: idx = idx * (idx < n_nodes)
If idx becomes 0 after bounds, we're done - subsequent branches are wasted.

IDEA: Skip branch computation for elements that are out of bounds.
But this requires conditional execution, which we don't have efficiently.

NEW IDEA: Use multiply-based branch that can be combined with hash
idx' = 2*idx + 1 + (val & 1)

What if we incorporate the branch into the NEXT round's hash somehow?
hash_input = val ^ tree[idx']

If we precompute idx' during the hash stage...
No, that doesn't help - we still need 3 ops for idx'.

FINAL ATTEMPT: What if we use a lookup table?
Precompute: for each (idx, val_lsb) pair, store idx'
But idx can be 0-2047 and val_lsb is 0 or 1, so table would be huge.

OK let me try something practical:
Test if ordering branch ops differently helps ILP.

Current order:
  AND (depends on val)
  FMA (depends on idx)
  ADD (depends on AND, FMA)

Alternative order:
  FMA first (only needs idx)
  AND (only needs val)
  ADD (needs both)

This allows FMA and AND to execute in parallel if the scheduler supports it!
"""
...
class KernelBuilderB2_5:
    """B2-5: Fused branch - emit FMA before AND for better ILP"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # B2-5: Emit FMA first, then AND - allows parallel execution
        # FMA doesn't depend on val, AND doesn't depend on idx
        # Both can execute in parallel (if scheduler supports it)
        def emit_branch_reordered(desk_idx):
            d = desks[desk_idx]
            # FMA first - only depends on idx
            self.emit("valu", ("multiply_add", d['idx'], d['idx'], v_two, v_one))
            # AND - only depends on val (can run in parallel with FMA on different lanes)
            self.emit("valu", ("&", d['tmp1'], d['val'], v_one))
            # ADD - depends on both results
        ...
        # Interleave FMA and AND across desks for better ILP
        def emit_branch_interleaved_ilp(group_desks):
            # First pass: emit all FMAs (independent of val)
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("multiply_add", desk['idx'], desk['idx'], v_two, v_one))
            # Second pass: emit all ANDs (independent of idx)
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("&", desk['tmp1'], desk['val'], v_one))
            # Third pass: emit all ADDs
            for d in group_desks:
                desk = desks[d]
        ...
        def emit_round_0_interleaved(group_desks):
            ...
            for d in group_desks:
                emit_hash_stage_5(d)
            emit_branch_interleaved_ilp(group_desks)
