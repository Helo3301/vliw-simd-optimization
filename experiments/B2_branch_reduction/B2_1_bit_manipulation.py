"""
B2-1: Branch Reduction via Bit Manipulation

Current: idx = 2*idx + 1 + (val & 1) = 3 ops (AND, FMA, ADD)

Approach: (idx << 1) | ((val & 1) + 1)
But this doesn't directly help since we still need AND then ADD then OR.

Let me think more carefully:
- val & 1 gives 0 or 1
- 2*idx + 1 gives an odd number
- Adding (val & 1) gives idx*2+1 or idx*2+2

Alternative: idx*2 + 1 + (val & 1)
What if we use: (idx << 1) + ((val | -2) + 2)?
  When val is even: (val | -2) = even | 0xFFFFFFFE = 0xFFFFFFFE, +2 = 0
  When val is odd: (val | -2) = odd | 0xFFFFFFFE = 0xFFFFFFFF = -1, +2 = 1

That doesn't help either - still 3 ops.

Let me try: idx*2 + (val & 1) + 1
Using FMA: multiply_add(idx, 2, (val & 1) + 1)
This requires computing (val & 1) + 1 first, which is 2 ops.

What about using the low bit differently?
idx' = 2*idx + 1 + (val & 1)

If we compute: tmp = idx | (val & 1)  [OR preserves structure]
Then idx' = 2*tmp + 1 when val is even... no that's wrong.

Actually, let me try: idx' = idx*2 + ((val & 1) | 1)
When val is even: (val & 1) | 1 = 0 | 1 = 1 -> idx*2 + 1
When val is odd: (val & 1) | 1 = 1 | 1 = 1 -> idx*2 + 1
That loses information!

What about: idx' = idx*2 + ((val & 1) + 1)
This is: FMA(idx, 2, (val & 1) + 1)
But (val & 1) + 1 requires 2 ops to compute.

Let me try another angle:
idx' = 2*idx + 2 - (1 - (val & 1))
     = 2*idx + 2 - 1 + (val & 1)
     = 2*idx + 1 + (val & 1) [same thing]

What if: idx' = (2*idx + 2) - !(val & 1)
Need to compute !(val & 1), which is ~(val & 1) & 1 = 2 ops.

Let's try: FMA(idx, 2, 2) - ((val + 1) & 1)
When val even: (val+1) & 1 = 1 -> 2*idx + 2 - 1 = 2*idx + 1 ✓
When val odd: (val+1) & 1 = 0 -> 2*idx + 2 - 0 = 2*idx + 2 ✓

So: idx' = FMA(idx, 2, 2) - ((val + 1) & 1)
This is: 3 ops: add_imm/add, AND, subtract
Not better.

What about using vselect (FLOW engine)?
cond = val & 1
idx' = vselect(cond, 2*idx+2, 2*idx+1)

With precomputed: idx_odd = FMA(idx, 2, 2), idx_even = FMA(idx, 2, 1)
Then vselect(val & 1, idx_odd, idx_even)
But this is 3 ops: 2 FMAs + 1 vselect

Better: precompute v_two and v_one constants
odd_path = FMA(idx, 2, 2)  # when val is odd
even_path = idx*2 + 1      # when val is even = odd_path - 1

So: idx' = FMA(idx, 2, 2) - !(val & 1)
We need !(val & 1) efficiently.

Alternative approach:
val_lsb = val & 1  (1 op)
idx' = FMA(idx, 2, 1) + val_lsb  (but FMA doesn't allow this directly)

Actually, looking at it again:
Current: AND, FMA, ADD = 3 ops

What if we use:
tmp = FMA(idx, 2, val)  # idx*2 + val (we want idx*2 + 1 + (val&1))
# But val can be any number, not just 0 or 1

Hmm, let me reconsider the vselect approach since it runs on FLOW engine:
- val_lsb = val & 1  (VALU, 1 op)
- idx_plus_1 = FMA(idx, 2, 1)  (VALU, 1 op)
- idx_plus_2 = FMA(idx, 2, 2)  (VALU, 1 op)
- idx' = vselect(val_lsb, idx_plus_2, idx_plus_1)  (FLOW, 0 VALU ops)

Wait, that's still 3 VALU ops. But vselect is on FLOW engine.
If we can overlap vselect with other VALU work, effective VALU cost could be 2.

Actually, an even better approach - precompute both FMA results at once:
We could use a single FMA and add 1:
idx_even = FMA(idx, 2, 1)  # 1 VALU
idx_odd = idx_even + 1     # 1 VALU
idx' = vselect(val & 1, idx_odd, idx_even)  # 0 VALU (FLOW)

This is 2 VALU + 1 AND + 1 vselect
= 3 VALU + 1 FLOW

Still 3 VALU. The AND is on VALU.

Wait - can we eliminate the AND?
vselect uses the LSB of the condition? Let me check what vselect does.
If vselect(cond, a, b) returns a when cond != 0, b when cond == 0,
we can't directly use val since val could be any value.

But if vselect tests the LSB specifically, then:
idx' = vselect(val, 2*idx+2, 2*idx+1)  # if val's LSB is 1 -> idx*2+2, else idx*2+1
...
class KernelBuilderB2_1:
    """B2-1: Test bit manipulation branch approaches"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # B2-1: Attempt to reduce branch using bit manipulation
        # Current: idx = 2*idx + 1 + (val & 1) = 3 ops
        #
        # New approach: Use (idx << 1) | bit_computed
        # bit_computed = ((val & 1) + 1) requires 2 ops
        # So (idx << 1) | bit_computed = 3 ops total - same!
        #
        # Try: (idx << 1) + ((val & 1) | 1)
        # But (val & 1) | 1 = 1 always, so this doesn't work
        #
        # Keeping baseline for now to measure
