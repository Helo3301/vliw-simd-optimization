"""
B2-3: Branch Reduction via Algebraic Identity

Current: idx = 2*idx + 1 + (val & 1) = 3 VALU ops (AND, FMA, ADD)

Key insight: We can use FMA differently!

Formula: idx' = 2*idx + 1 + (val & 1)

What if we observe: (val & 1) is either 0 or 1
So we need: 2*idx + 1 when even, 2*idx + 2 when odd

Alternative formulation:
idx' = 2*idx + 1 + (val & 1)
     = 2*idx + (val & 1) + 1

Using FMA: multiply_add(idx, 2, X) where X = (val & 1) + 1
But computing (val & 1) + 1 requires 2 ops.

WAIT! What about: multiply_add(idx, 2, val) and then mask?
No, that doesn't give us what we want.

Let's try a completely different approach:
- The LSB of the new idx should be ~(val & 1) for the left child, (val & 1) for right
  Actually, left child = 2*idx + 1 (odd), right child = 2*idx + 2 (even)
  So idx' LSB = 1 when val is even, 0 when val is odd
  idx' LSB = ~(val & 1) & 1 = 1 - (val & 1)

Hmm, let me think differently:
idx' = (idx << 1) | 1    when val is even  (2*idx + 1)
idx' = (idx << 1) | 2    when val is odd   (2*idx + 2)
      = (idx << 1) + 1 + (val & 1)

Using shift: shift is same cost as multiply by constant power of 2.

What about using OR directly?
idx' = (idx << 1) | (1 + (val & 1))
The (1 + (val & 1)) part is 0b01 or 0b10

Could we use: idx' = (idx << 1) + 1 + (val & 1)?
  = shift + add_imm + and + add
  = 4 ops (worse!)

What about using multiply_add creatively?
multiply_add(a, b, c) = a * b + c

idx' = idx * 2 + (1 + (val & 1))
     = multiply_add(idx, 2, 1) + (val & 1)  -- but this is 3 ops

Key realization: We need either:
1. A 2-operand formula (impossible with current instruction set)
2. Better ILP through different organization

Let me try: can we precompute something that helps?

What if at the start of each round, we compute:
branch_offset = (val & 1)
Then: idx' = multiply_add(idx, 2, 1) + branch_offset

This is still 3 ops, but if we can overlap the AND with other work,
effective cost might be reduced!

NEW IDEA: Use the hash result more cleverly
After hash, val has been transformed. What if we extract the branch bit
BEFORE the final hash stage, allowing it to overlap with the last hash stage?

Current flow:
  hash_stage_5(val)
  branch_bit = val & 1
  idx = 2*idx + 1 + branch_bit

New flow:
  // Do hash stages 0-4
  // In stage 5, compute branch bit early from intermediate value
  tmp1 = val ^ hash_const_5
  shift_val = val >> shift_5
  val = tmp1 ^ shift_val
  // Now extract branch
  branch_bit = val & 1  -- this is dependent on val, so can't parallelize

Hmm, that doesn't help.

FINAL APPROACH for B2-3: What about using FMA with value directly?
idx' = idx * 2 + 1 + (val & 1)

What if: idx' = idx * 2 + val&1 + 1
We need val's LSB. If we mask val with 1 first, we get 0 or 1.
Then FMA can do: multiply_add(idx, 2, val_lsb + 1)?
No, FMA can't add two things to the product.

Let me try a mathematical transformation:
idx' = 2*idx + 1 + (val & 1)
     = 2*idx + 1 + val - (val & ~1)  [since val = (val & ~1) + (val & 1)]

That's getting more complex, not simpler.

OK let me try: maybe the AND can be avoided if we use subtraction cleverly
idx' = 2*(idx + 1) - 1 + (val & 1)
     = 2*idx + 2 - 1 + (val & 1)
     = 2*idx + 1 + (val & 1)  [same thing]

What about: idx' = 2*idx + 2 - (1 - (val & 1))
                 = 2*idx + 2 - 1 + (val & 1)
                 = 2*idx + 1 + (val & 1) [same]

To compute (1 - (val & 1)), we need:
  tmp = val & 1
  result = 1 - tmp
This is 2 ops for the subtraction part, then FMA, then add... worse.

ALTERNATIVE: Use comparison
If val is odd, val & 1 = 1, so val > 0 is true... no that doesn't help either.

I'll implement a version that reorders operations to try to get better ILP:
Put AND earlier so it can overlap with other work.
"""
...
class KernelBuilderB2_3:
    """B2-3: Branch with early AND for better ILP"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        for d in range(NUM_DESKS):
            desk = {
                ...
                'branch_bit': self.alloc_vec(f"v_branch_{d}"),  # Pre-store branch bit
            }
            desks.append(desk)
        ...
        # Modified stage 5: also extract branch bit
        def emit_hash_stage_5_with_branch_prep(desk_idx):
            d = desks[desk_idx]
            self.emit("valu", ("^", d['tmp1'], d['val'], v_hash_consts[5]))
            self.emit("valu", (">>", d['tmp2'], d['val'], v_hash_shifts[5]))
            self.emit("valu", ("^", d['val'], d['tmp1'], d['tmp2']))
            # Extract branch bit immediately after hash completes
            self.emit("valu", ("&", d['branch_bit'], d['val'], v_one))

        # Branch using pre-extracted bit
        def emit_branch_with_precomputed_bit(desk_idx):
            d = desks[desk_idx]
            # Just 2 ops: FMA + ADD
            self.emit("valu", ("multiply_add", d['idx'], d['idx'], v_two, v_one))
            self.emit("valu", ("+", d['idx'], d['idx'], d['branch_bit']))
        ...
        def emit_round_0_interleaved(group_desks):
            ...
            for d in group_desks:
                emit_hash_stage_4(d)
            # Stage 5 + branch bit extraction
            for d in group_desks:
                emit_hash_stage_5_with_branch_prep(d)
            # Branch using precomputed bit
            for d in group_desks:
                emit_branch_with_precomputed_bit(d)
        ...
        def emit_round_15_final_interleaved(group_desks):
            ...
            for d in group_desks:
                emit_hash_stage_4(d)
            # Final hash stage 5 (no branch needed)
            for d in group_desks:
                desk = desks[d]
