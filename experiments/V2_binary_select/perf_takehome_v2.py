"""
# Experiment V2: Hierarchical Binary Selection - Simulated vgather using binary tree selection

This experiment tests whether binary selection can replace gather operations efficiently.

**Analysis:**
For a level with N values, binary selection needs log2(N) stages.
Each stage selection: result = a + (b-a) * cond uses 3 VALU ops (sub, mul, add)

For 8 values (level 3):
- 3 bit extractions
- Step 1: 4 selections = 12 VALU ops
...
So binary selection is NOT faster for small levels. However, the advantage might be:
1. Better pipelining with other operations
2. Avoiding memory latency

For this experiment, let's focus on optimizing the C4 structure with better ALU/VALU overlap.
"""
...
class KernelBuilderV2:
    """
    V2: Optimized kernel with better instruction packing.

    Based on C4 but with experimental optimizations.
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def build_kernel(
        ...
        # Round 2 Desk3 gather 4-5 + desk0 branch prep with FMA + precompute next iteration's batch_offset
        self.instrs.append({
            "load": [
                ("load", desks[3]['node_val'] + 4, desks[3]['addr'] + 4),
                ("load", desks[3]['node_val'] + 5, desks[3]['addr'] + 5),
        ...
        # Round 2 Desk3 gather 6-7 + desk0 branch + desk1 hash4 + desk2 hash1 combine + next iteration addresses
        self.instrs.append({
            "load": [
                ("load", desks[3]['node_val'] + 6, desks[3]['addr'] + 6),
                ("load", desks[3]['node_val'] + 7, desks[3]['addr'] + 7),
...
def do_kernel_test(
    ...
    print(f"Comparison to C4 ({C4_CYCLES} cycles): ", C4_CYCLES / machine.cycle)
