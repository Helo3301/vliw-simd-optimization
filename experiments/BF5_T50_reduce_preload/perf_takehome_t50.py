"""
Theory 50: Reduce tree preload from 7 to 3 nodes

Hypothesis: Preloading 7 tree nodes uses scratch space but only nodes 0-2 are
used in the fused rounds 0-2 and 11-13. Reducing preload to 3 nodes saves
scratch and may allow better register allocation.

Baseline: 1,548 cycles
"""
...
class KernelBuilderT50:
    """T50: Reduce tree preload from 7 to 3 nodes"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # T50: Only preload 3 nodes instead of 7
        NUM_PRELOADED = 3
        v_tree = []
        ...
        # Only compute diff_1_2 since we only have 3 nodes
        ...
        # T50: Simplified rounds 0+1 fused, round 2 becomes a gather round
        ...
        def emit_rounds_11_12_fused(group_desks):
            # === Round 11 ===
            for d in group_desks:
                emit_xor_with_node(d, v_tree[0])
            emit_hash_interleaved(group_desks)
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                # T50: Fused rounds 0+1, then gather rounds 2-9
                emit_rounds_0_1_fused(group_desks)
                for _rnd in range(2, 10):
                    emit_gather_round_interleaved(group_desks)
                emit_round_10_optimized(group_desks)
                # Fused rounds 11+12, then gather rounds 13-14
                emit_rounds_11_12_fused(group_desks)
                emit_gather_round_interleaved(group_desks)  # Round 13
