"""
# Experiment H143d: Try to optimize hash stages by computing shift first

For hash stages 1, 3, 5 the pattern is:
  val = (val OP1 const) OP2 (val SHIFT n)
...
What if we reverse the order? Emit shift first:
  tmp2 = val SHIFT n
  tmp1 = val OP1 const
  val = tmp1 OP2 tmp2

This is the same number of ops, but might allow better scheduling
since the shift result isn't immediately needed.

Actually, let me try a different approach: emit ALL shifts first across
multiple stages, then do the rest. This might allow better ILP.

Wait, that doesn't work because stages are dependent.

Let me try yet another approach: using a 3-register form where we can
overlap operations across different desks.

Actually, looking more carefully at the scheduling, what if we try
emitting operations in a different order WITHIN a round?

Current order for hash:
- FMA (stage 0)
- 3 ops for stage 1 (XOR const, SHIFT, XOR)
- FMA (stage 2)
- 3 ops for stage 3 (ADD const, SHIFT, XOR)
- FMA (stage 4)
- 3 ops for stage 5 (XOR const, SHIFT, XOR)

Since each stage depends on the previous, we can't reorder stages.
But we might be able to interleave across desks better.

Let me try: Emit all stage 0 for all desks in a group first, then all stage 1, etc.
This way the scheduler can overlap FMAs from different desks.
"""
...
class KernelBuilderH143d:
    """H143d: vselect + stage-interleaved hash emission"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_branch(desk_idx):
            # Alternative branch: compute offset = 1 + branch_bit first, then single FMA
            # Original: tmp1 = val & 1; idx = idx * 2 + 1; idx = idx + tmp1 (3 ops)
            # Alternative: tmp1 = val & 1; tmp2 = tmp1 + 1; idx = idx * 2 + tmp2 (3 ops)
            # Both are 3 ops, but different dependency pattern
            d = desks[desk_idx]
