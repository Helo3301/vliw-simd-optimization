"""
B4-4: Smaller Groups (2 desks) for Better ILP

B4-3 showed that GROUP_SIZE=8 hurts performance (1643 cycles).
Let's try GROUP_SIZE=2 to see if smaller groups help the scheduler.

Hypothesis: With 2-desk groups, the scheduler has more freedom to
interleave operations from different groups, potentially filling
more slots per cycle.

With 6 valu slots per cycle:
- GROUP_SIZE=4: Hash stage 0 emits 4 ops, fits in one cycle
- GROUP_SIZE=2: Hash stage 0 emits 2 ops, leaves 4 slots free
  Maybe the scheduler can fill those with ops from the next group?
"""
...
class KernelBuilderB4_4:
    """B4-4: Smaller groups (2 desks) with fused early rounds"""
    def __init__(self):
