"""
B4-3: Larger Groups for Better ILP

From B4-2, we got 1558 cycles with fused early rounds.
Now let's try increasing the group size from 4 to 8 desks.

The hypothesis: Larger groups give the scheduler more independent operations
to interleave, potentially filling more slots per cycle.

Current: GROUP_SIZE = 4 desks
- Each hash stage (6 stages) emits 4 ops, needs ~1 cycle
- But we have 6 valu slots per cycle
- With GROUP_SIZE = 8, each hash stage emits 8 ops, needs ~2 cycles
  But maybe the scheduler can overlap stage X of desk 0 with stage X-1 of desk 7

The trade-off:
- Larger groups = more scratch usage (already at 1184/1536)
- But maybe better scheduling

Let's test GROUP_SIZE = 8.
"""
...
class KernelBuilderB4_3:
    """B4-3: Larger groups (8 desks) with fused early rounds"""
    def __init__(self):
