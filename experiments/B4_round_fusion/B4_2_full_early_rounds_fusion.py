"""
B4-2: Full Early Rounds Fusion

Extend B4-1 idea:
1. Fuse rounds 0+1 (saves 8 cycles in B4-1)
2. Apply same fusion to rounds 11+12 (after wrap, idx=0 again)
3. Try fusing 0+1+2 if possible

The key insight from B4-1:
- Round 0 starts with idx=0, so after hash: idx1 = 1 + (val0 & 1)
- Round 1 uses idx1 to select tree[1] or tree[2], but we can use bit = (val0 & 1) directly
- This saves the idx -> bit conversion in round 1

For round 11: After round 10's bounds check, idx wraps to 0.
So round 11 is identical to round 0: XOR with tree[0], hash, branch.
Then round 12 is identical to round 1: select tree[1] or tree[2] based on bit.

Can we fuse 0+1+2?
Round 2 uses idx2 which comes from round 1's branch.
idx2 = 2 * idx1 + 1 + bit1 = 2*(1 + bit0) + 1 + bit1 = 3 + 2*bit0 + bit1
So idx2 in {3, 4, 5, 6} maps to tree[3..6]

For 4-way selection:
- offset = idx2 - 3 = 2*bit0 + bit1 (2-bit number: bit0 is high bit, bit1 is low)
- We need to select tree[3 + offset]

Current approach computes this via vselect. But if we know bit0 and bit1:
- If bit0=0, bit1=0: offset=0, use tree[3]
- If bit0=0, bit1=1: offset=1, use tree[4]
- If bit0=1, bit1=0: offset=2, use tree[5]
- If bit0=1, bit1=1: offset=3, use tree[6]

The arithmetic selection:
node2 = tree[3] + bit1 * (tree[4] - tree[3]) when bit0=0
node2 = tree[5] + bit1 * (tree[6] - tree[5]) when bit0=1
Then: node2 = select(bit0, low_result, high_result)

But this requires a flow select. Actually the current code already does this!
The benefit of fusion is that we compute bit0 and bit1 directly instead of
extracting them from idx.
...
class KernelBuilderB4_2:
    """B4-2: Full early rounds fusion with bit tracking"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        for d in range(NUM_DESKS):
            desk = {
                ...
                'bit0': self.alloc_vec(f"v_bit0_{d}"),  # Track bit from round 0/11
            }
            desks.append(desk)
        ...
        # Fused rounds 0+1+2 with bit tracking
        def emit_rounds_0_1_2_fused(group_desks):
            ...
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("&", desk['bit0'], desk['val'], v_one))  # Save bit0 for round 2
                self.emit("valu", ("+", desk['idx'], v_one, desk['bit0']))   # idx1 = 1 + bit0

            # === Round 1 ===
            # Compute node1 = tree[1] + bit0 * diff_1_2
            for d in group_desks:
                desk = desks[d]
            ...
            # Hash
            emit_hash_interleaved(group_desks)
            # Extract bit1 and compute idx2 = 2*idx1 + 1 + bit1 = 3 + 2*bit0 + bit1
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("&", desk['tmp1'], desk['val'], v_one))  # bit1
                # idx2 = 3 + 2*bit0 + bit1
                self.emit("valu", ("multiply_add", desk['idx'], desk['bit0'], v_two, v_three))  # 3 + 2*bit0
                self.emit("valu", ("+", desk['idx'], desk['idx'], desk['tmp1']))  # + bit1

            # === Round 2 ===
            # We have offset = idx2 - 3 = 2*bit0 + bit1
            # bit0 is still in desk['bit0'], bit1 is in desk['tmp1']
            # Select node2:
            # low_pair = tree[3] + bit1 * diff_3_4  (for bit0=0)
            # high_pair = tree[5] + bit1 * diff_5_6 (for bit0=1)
            # node2 = select(bit0, high_pair, low_pair)
            for d in group_desks:
                desk = desks[d]
                # bit1 is in tmp1
        ...
        # Similar fusion for rounds 11+12+13 (after wrap)
        def emit_rounds_11_12_13_fused(group_desks):
            """Rounds 11+12+13 fused with direct bit tracking (same as 0+1+2 after wrap)"""
