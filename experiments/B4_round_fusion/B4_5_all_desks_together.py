"""
B4-5: Process All Desks Together

Previous experiments showed GROUP_SIZE=4 is optimal (B4-2: 1558 cycles).
GROUP_SIZE=8 gave 1643, GROUP_SIZE=2 gave 1594.

Let's try no grouping at all - emit all 16 desks together for each round.
The scheduler has full freedom to interleave operations.

Hypothesis: The greedy scheduler might find better packing when it sees
all 16 desks' operations at once.
"""
...
class KernelBuilderB4_5:
    """B4-5: All desks processed together (no grouping)"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Use ALL 16 desks as a single "group"
        ...
        def emit_tile(tile_idx):
            ...
            # Process ALL desks together (no grouping)
            emit_rounds_0_1_2_fused(all_desks)

            for _rnd in range(3, 10):
                emit_gather_round_interleaved(all_desks)

            emit_round_10_interleaved(all_desks)
            emit_rounds_11_12_13_fused(all_desks)
            emit_gather_round_interleaved(all_desks)
            emit_round_15_final_interleaved(all_desks)
