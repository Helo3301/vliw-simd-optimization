"""
# Experiment H110: Flat Hash Stage Processing (FAILED)

**GOAL:** Maximize VALU parallelism by flattening hash stage operations.

**HYPOTHESIS:** Hash stages 1, 3, 5 all have the pattern:
  tmp1 = OP(val,const); tmp2 = SHIFT(val); val = XOR(tmp1,tmp2).

Instead of processing by desk groups (to avoid temp conflicts), emit:
- ALL stage1_op1 for all 32 desks at once
- Then ALL stage1_op2 for all 32 desks at once
- Then ALL stage1_op3 for all 32 desks at once

**RESULT:** FAILED

**WHY IT FAILED:**
1. Flat emission with shared temps causes correctness bugs:
   - Desks 0-3 share temps [0,1]. When we emit all 32 op1s in order,
     desk 1's op1 overwrites tmp[0] before desk 0's op3 can read it.
   - The scheduler doesn't help because it sees these as intentional writes.

2. Dedicated temps per desk exceeds scratch space:
   - 32 desks * 5 vectors (idx, val, node_val, addr, tmp1) = 1280 slots
   - Plus shared tmp2 or another 256 slots for dedicated tmp2
   - Total > 1536 slot limit

3. Reducing to 30 desks to fit dedicated temps works but only processes 240
   of 256 batch elements, failing correctness checks.

**CONCLUSION:** The H105 grouped approach is already optimal for shared temps.
The hypothesis was theoretically sound but practically infeasible due to
scratch space constraints.

**BASELINE:** H105 = 1,843 cycles (unchanged, this experiment reverts to H105)
"""
...
class KernelBuilderH110:
    """
    H110: Flat hash stage processing (FAILED - reverted to H105 approach).
    See module docstring for details on why this hypothesis failed.
    """
    def __init__(self):
