"""
# Experiment H132: Init Order Optimization

**GOAL:** Reduce init phase cycles by reordering vector constant broadcasts.

**HYPOTHESIS:** In H120, the init phase emits scalar const loads followed by their
corresponding vbroadcast operations. By reordering operations to maximize parallelism
between the load engine and valu engine, we can overlap more operations.

**RESULT:** After testing multiple reordering strategies:
1. Batching all scalar consts before all vbroadcasts - No improvement
2. Interleaving tree loads with broadcasts - No improvement
3. Using multiple tmp registers for parallel tree loads - No improvement

The scheduler already does an excellent job of overlapping operations regardless
of emission order. The const_map and vconst_map already enable constant reuse.

The bottleneck appears to be elsewhere in the computation, not the init phase.

**BASELINE:** H120 = 1,840 cycles
**RESULT:** H132 = 1,840 cycles (no improvement from init reordering alone)
"""
...
class KernelBuilderH132:
    """
    H132: Init order optimization - reorder broadcasts to maximize load/valu parallelism.

    Key changes from H120:
    1. Emit all scalar consts first, then do all vbroadcasts
    2. Interleave header loads with const loads to keep load engine busy
    3. Batch vbroadcasts together to allow valu pipelining
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        H132: Reorder init operations for better load/valu parallelism.
        """
        tmp_scalar = self.alloc_scratch("tmp_scalar")
        tmp_addr = self.alloc_scratch("tmp_addr")

        # H132: Use H120 init exactly (to match baseline)
        ...
        print(f"  - H132: Init order optimization (matches H120 - scheduler handles order)")
