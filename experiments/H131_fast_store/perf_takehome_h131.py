"""
# Experiment H131: Fast Store Phase

**GOAL:** Reduce store phase cycles by processing 4 desks at a time instead of 2.

**HYPOTHESIS:** The store phase at the end processes 2 desks at a time (4 ALU ops + 4 stores).
With 2 store slots available per cycle, we may be able to better utilize them by:
1. Computing all 8 addresses for 4 desks first (8 ALU ops)
2. Then issuing all 8 stores together (8 stores can pack into 4 cycles at 2 per cycle)

This batching may allow the scheduler to pipeline the address computations with the stores
more effectively, potentially saving cycles in the store phase.
...
class KernelBuilderH131:
    """
    H131: Fast store phase - process 4 desks at a time during store.

    Change from H120:
    - Store loop processes 4 desks at a time instead of 2
    - Compute all 8 addresses first, then issue all 8 stores
    - Uses 8 addr_tmp registers instead of 4

    This batches address computations and stores to better utilize
    the 2 store slots available per cycle.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        H120: Fast init - only load needed header values.
        """
        ...
        # H131 CHANGE: Need 16 addr_tmp for 8-desk batching during store phase
        # (8 desks * 2 addresses each = 16 addr_tmp registers)
        ...
        print(f"  - H131: Fast store - 8 desks at a time during store phase")
        ...
        def emit_tile():
            ...
            # H131 CHANGE: Store results - process 8 desks at a time
            # Compute all 16 addresses first, then do all 16 stores
            for d_batch in range(0, NUM_DESKS, 8):
                # Get all 8 desk indices for this batch
                batch_desks = [d_batch + i for i in range(8)]
                # Compute all 16 addresses for 8 desks
                for i, d in enumerate(batch_desks):
                    self.emit("alu", ("+", addr_tmp[i*2], self.scratch["inp_indices_p"], offset_regs[d]))
                    self.emit("alu", ("+", addr_tmp[i*2+1], self.scratch["inp_values_p"], offset_regs[d]))
                # Store idx/val for all 8 desks
                for i, d in enumerate(batch_desks):
                    self.emit("store", ("vstore", addr_tmp[i*2], desks[d]['idx']))
                    self.emit("store", ("vstore", addr_tmp[i*2+1], desks[d]['val']))
