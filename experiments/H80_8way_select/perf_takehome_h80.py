"""
# Experiment H77: Automatic VLIW Scheduling
...
class KernelBuilderH80:
    ...
    def build_kernel(
        ...
        # Precompute for 8-way selection (round 3: indices 7-14)
        ...
        for d in range(NUM_DESKS):
            desk = {
                ...
                'tmp3': self.alloc_vec(f"v_tmp3_{d}"),  # Extra temp for 8-way selection
            }
            desks.append(desk)
        ...
        def emit_tile(tile_idx):
            ...
            # Round 3: 8-way selection for indices 7-14
            for d in range(NUM_DESKS):
                # Extract selection bits from offset = idx - 7
                self.emit("valu", ("-", desks[d]['tmp1'], desks[d]['idx'], v_seven))  # offset
                self.emit("valu", ("&", desks[d]['tmp2'], desks[d]['tmp1'], v_one))   # bit0
                self.emit("valu", (">>", desks[d]['tmp1'], desks[d]['tmp1'], v_one))  # tmp1 = offset >> 1
                self.emit("valu", ("&", desks[d]['tmp3'], desks[d]['tmp1'], v_one))   # bit1
                self.emit("valu", (">>", desks[d]['addr'], desks[d]['tmp1'], v_one))  # bit2
                # Now: tmp2=bit0, tmp3=bit1, addr=bit2, tmp1=free

                # Level 0: 4 pairs using bit0
                self.emit("valu", ("multiply_add", desks[d]['tmp1'], desks[d]['tmp2'], v_diff_7_8, v_tree[7]))     # p0
                self.emit("valu", ("multiply_add", desks[d]['node_val'], desks[d]['tmp2'], v_diff_9_10, v_tree[9]))  # p1

                # Level 1a: first quad using bit1
                self.emit("valu", ("-", desks[d]['tmp2'], desks[d]['node_val'], desks[d]['tmp1']))  # p1 - p0
                self.emit("valu", ("multiply_add", desks[d]['tmp1'], desks[d]['tmp3'], desks[d]['tmp2'], desks[d]['tmp1']))  # q0
                # Now tmp1=q0, tmp2=free, tmp3=bit1, addr=bit2, node_val=free

                # Level 0 continued: remaining pairs using bit0 (need to recompute bit0)
                self.emit("valu", ("-", desks[d]['tmp2'], desks[d]['idx'], v_seven))  # offset again
                self.emit("valu", ("&", desks[d]['node_val'], desks[d]['tmp2'], v_one))  # bit0 again in node_val

                self.emit("valu", ("multiply_add", desks[d]['tmp2'], desks[d]['node_val'], v_diff_11_12, v_tree[11]))  # p2
                self.emit("valu", ("multiply_add", desks[d]['node_val'], desks[d]['node_val'], v_diff_13_14, v_tree[13]))  # p3

                # Level 1b: second quad using bit1
                self.emit("valu", ("-", desks[d]['node_val'], desks[d]['node_val'], desks[d]['tmp2']))  # p3 - p2
                self.emit("valu", ("multiply_add", desks[d]['tmp2'], desks[d]['tmp3'], desks[d]['node_val'], desks[d]['tmp2']))  # q1
                # Now tmp1=q0, tmp2=q1, tmp3=free, addr=bit2, node_val=free

                # Level 2: final selection using bit2
                self.emit("valu", ("-", desks[d]['node_val'], desks[d]['tmp2'], desks[d]['tmp1']))  # q1 - q0
                self.emit("valu", ("multiply_add", desks[d]['node_val'], desks[d]['addr'], desks[d]['node_val'], desks[d]['tmp1']))  # result
