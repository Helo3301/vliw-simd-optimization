"""
C4-4: Gather Round Fusion with Speculative Loads

THEORY: In gather rounds (3-9, 14), we:
1. Compute addr = forest_p + idx
2. Load node = mem[addr]
3. XOR val ^= node
4. Hash
5. Branch: idx = 2*idx + 1 + bit

What if we SPECULATIVELY compute NEXT round's addresses (both children)
BEFORE hash completes, and LOAD both children speculatively?

This is expensive (doubles loads) but hides latency.

Actually, a better approach:
- Fuse rounds 3+4: After round 3's XOR, we know val (unhashed)
- Compute speculative addresses for round 4 DURING round 3's hash
- Start loading for round 4 before round 3's branch completes

But wait, we need idx from branch to compute addr. Let me think differently.

NEW APPROACH: Precompute idx for CURRENT round while PREVIOUS round is hashing.
- Round 3: After branch, idx3 is known
- During round 4's hash, compute addr4 = forest_p + idx3 (but idx3 depends on round 3 branch)

Actually the fundamental issue is that idx_next depends on hash result.

Let me try: reduce the number of branch operations by tracking 2 bits at once.
Similar to how B4-2 fuses 0+1+2 by tracking bit0 and bit1.

For rounds 3-4, can we:
- Round 3: load, XOR, hash -> bit3
- Round 4: need to load tree[idx4] where idx4 = 2*idx3 + 1 + bit3

Actually, idx3 is already known at start of round 3. So we can compute:
- left_addr3 = forest_p + 2*idx3 + 1
- right_addr3 = forest_p + 2*idx3 + 2

Load BOTH speculatively, then select with bit3.
...
class KernelBuilderC4_4:
    """C4-4: Speculative child loading in gather rounds"""
    def __init__(self):
