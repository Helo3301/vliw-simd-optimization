"""
C4-1: Arithmetic Selection Instead of vselect

THEORY: The vselect uses FLOW engine (1/cycle), which is a bottleneck.
Instead, compute both paths and use arithmetic selection:
  result = left * (1-mask) + right * mask
where mask is 0 or 1.

This uses VALU engine (6/cycle) instead of FLOW (1/cycle).
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        for d in range(NUM_DESKS):
            desk = {
                ...
                'tmp3': self.alloc_vec(f"v_tmp3_{d}"),  # Extra temp for arithmetic select
            }
            desks.append(desk)
        ...
        # MODIFIED: Rounds 0+1+2 with ARITHMETIC selection instead of vselect
        def emit_rounds_0_1_2_fused_arithmetic(group_desks):
            """Rounds 0+1+2 fused with arithmetic selection (no vselect)"""
            # === Round 0 ===
            for d in group_desks:
            ...
            # === Round 2 ===
            # ARITHMETIC SELECT instead of vselect:
            # low_pair = tree[3] + bit1 * diff_3_4
            # high_pair = tree[5] + bit1 * diff_5_6
            #
            # For arithmetic select: result = low*(1-bit0) + high*bit0
            # But multiplication distributes: result = low - low*bit0 + high*bit0
            # = low + bit0*(high - low)
            # So: node2 = low_pair + bit0 * (high_pair - low_pair)

            for d in group_desks:
                desk = desks[d]
                # bit1 is in tmp1, bit0 is in bit0
                # Compute low_pair in node_val
                self.emit("valu", ("multiply_add", desk['node_val'], desk['tmp1'], v_diff_3_4, v_tree[3]))
                # Compute high_pair in tmp2
                self.emit("valu", ("multiply_add", desk['tmp2'], desk['tmp1'], v_diff_5_6, v_tree[5]))

            for d in group_desks:
                desk = desks[d]
                # Compute diff = high_pair - low_pair in tmp3
                self.emit("valu", ("-", desk['tmp3'], desk['tmp2'], desk['node_val']))
                # result = low_pair + bit0 * diff
        ...
        # MODIFIED: Rounds 11+12+13 with ARITHMETIC selection
        def emit_rounds_11_12_13_fused_arithmetic(group_desks):
            """Rounds 11+12+13 fused with arithmetic selection (no vselect)"""
            # === Round 11 ===
            for d in group_desks:
                emit_xor_with_node(d, v_tree[0])
            emit_hash_interleaved(group_desks)
            ...
            # === Round 13 ===
            # Arithmetic select (same as round 2)
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("multiply_add", desk['node_val'], desk['tmp1'], v_diff_3_4, v_tree[3]))
                self.emit("valu", ("multiply_add", desk['tmp2'], desk['tmp1'], v_diff_5_6, v_tree[5]))
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                # Fused rounds 0+1+2 with arithmetic selection
                emit_rounds_0_1_2_fused_arithmetic(group_desks)
                ...
                # Fused rounds 11+12+13 with arithmetic selection
                emit_rounds_11_12_13_fused_arithmetic(group_desks)
