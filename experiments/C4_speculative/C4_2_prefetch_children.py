"""
C4-2: Prefetch Both Children Before Branch Decision

THEORY: For gather rounds, we load tree[idx] after computing idx from branch.
What if we load BOTH potential children before the branch decision?
- Load tree[2*idx+1] and tree[2*idx+2] speculatively
- After branch, use vselect to pick the right one

This hides memory latency since loads can start earlier.
...
class KernelBuilderC4_2:
    """C4-2: Prefetch both children before branch"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Original fused rounds from B4-2
