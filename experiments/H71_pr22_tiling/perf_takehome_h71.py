"""
# Experiment H71: PR #22 Tiling Strategy Implementation - v2

**GOAL:** Replicate PR #22's tiling strategy to achieve <2,000 cycles.

**KEY INSIGHTS FROM PR #22:**
- group_size = 17 (17 blocks/vectors per group)
- round_tile = 13 (13 rounds per tile)
- Flat slot emission with greedy scheduling
- CRITICAL: Emit ALL operations as individual slots, then pack greedily

**v2 STRATEGY:**
1. Use a flat slot emitter to collect all operations
2. Greedy pack them respecting dependencies
3. This should automatically find optimal schedules

**SCRATCH BUDGET:**
- 17 blocks x 6 vectors x 8 words = 816 words
- Constants: ~200 words
- Total: ~1,016 words (fits in 1,536)
"""
...
class FlatSlotEmitter:
    """
    Flat slot emission with greedy scheduling.

    Collects all operations and packs them into instructions respecting
    slot limits per engine type.
    """

    def __init__(self):
        self.pending = defaultdict(list)  # engine -> list of slots
        self.instrs = []

    def emit(self, engine, slot):
        """Add a slot to pending queue."""
        self.pending[engine].append(slot)

    def emit_many(self, engine, slots):
        """Add multiple slots to pending queue."""
        self.pending[engine].extend(slots)

    def barrier(self):
        """Flush all pending operations into instructions."""
        while any(self.pending.values()):
            instr = {}
            for engine in ["alu", "valu", "load", "store", "flow"]:
                limit = SLOT_LIMITS[engine]
                if self.pending[engine]:
                    take = min(limit, len(self.pending[engine]))
                    instr[engine] = self.pending[engine][:take]
                    self.pending[engine] = self.pending[engine][take:]
            if instr:
...
class KernelBuilderH71:
    """
    H71: PR #22 Tiling Strategy.

    Key changes from H68:
    1. Use 17-vector groups instead of 16 desks
    2. Process 13 rounds per tile instead of all 16 at once
    3. Greedy slot packing instead of manual scheduling
    """
    ...
    def build_kernel(
        ...
        """
        H71: PR #22 tiling kernel.

        The key insight is that instead of processing 16 desks through all 16 rounds,
        we process groups of vectors through a subset of rounds, allowing better
        instruction-level parallelism and reduced scratch pressure.
        """
        # Configuration
        GROUP_SIZE = 17  # 17 vectors per group (PR #22 insight)
        ROUND_TILE = 13  # 13 rounds per tile (PR #22 insight)

        # But we need to work within batch_size=256 elements
        # 256 / 8 = 32 vectors total
        # With 17+15 = 32, we can split into two groups

        NUM_VECTORS = batch_size // VLEN  # 32 vectors
        ...
        # Preload tree nodes 0-14 for vselect optimization
        NUM_PRELOADED = 15
        v_tree = []
        ...
        # Use a streamlined approach with 16 desks
        # This is similar to H68 but with better scheduling
        NUM_DESKS = 16
        ...
        tile_stride_const = self.scratch_const(NUM_DESKS * VLEN)
        num_tiles_const = self.scratch_const(batch_size // (NUM_DESKS * VLEN))
        ...
        # === MAIN STRATEGY ===
        # Process all 256 elements (32 vectors = 2 tiles of 16) through all 16 rounds
        # But use improved scheduling to maximize ILP
        ...
        def emit_gather_round_improved(skip_bounds=False):
            """
            Improved gather-based round for all 16 desks with better scheduling.
            Uses greedy slot packing to maximize throughput.
            """
            # Prepare gather addresses - broadcast forest_values_p to all desks
            for start in range(0, NUM_DESKS, 6):
                ops = []
                for d in range(start, min(start + 6, NUM_DESKS)):
                    ops.append(("vbroadcast", desks[d]['addr'], self.scratch["forest_values_p"]))
            ...
            for start in range(0, NUM_DESKS, 6):
                ops = []
                for d in range(start, min(start + 6, NUM_DESKS)):
                    ops.append(("+", desks[d]['addr'], desks[d]['addr'], desks[d]['idx']))
                self.instrs.append({"valu": ops})

            # Gather with deeply interleaved hash operations
            # This is the critical section - maximize parallelism

            # Stage 1: Start gathering desk 0-7, interleave hash ops
            ...
            # Generate remaining desks with consistent pattern
            for start_desk in range(4, 16):
                for lane_pair in range(4):
                    lane = lane_pair * 2
                    valu_ops = []

                    # Pipeline: desk N loads while desk N-4 does bounds, N-3 does branch, etc.
                    if lane_pair == 0:
                        if start_desk >= 4 and not skip_bounds:
                            valu_ops += emit_bounds_check(start_desk - 4)
                        if start_desk >= 3:
                            valu_ops += emit_hash_stage(start_desk - 3, 5)
                        if start_desk >= 2:
                            valu_ops += emit_hash_stage(start_desk - 2, 2)
                        if start_desk >= 1:
                            valu_ops += emit_xor_node(start_desk - 1)
                    elif lane_pair == 1:
                        if start_desk >= 4 and not skip_bounds:
                            valu_ops += emit_bounds_apply(start_desk - 4)
                        if start_desk >= 3:
                            valu_ops += emit_hash_combine(start_desk - 3, 5)
                        if start_desk >= 2:
                            valu_ops += emit_hash_stage(start_desk - 2, 3)
                        if start_desk >= 1:
                            valu_ops += emit_hash_stage(start_desk - 1, 0)
                    elif lane_pair == 2:
                        if start_desk >= 3:
                            valu_ops += emit_branch_ops(start_desk - 3)
                        if start_desk >= 2:
                            valu_ops += emit_hash_combine(start_desk - 2, 3)
                        if start_desk >= 1:
                            valu_ops += emit_hash_stage(start_desk - 1, 1)
                    elif lane_pair == 3:
                        if start_desk >= 3:
                            valu_ops += emit_branch_add(start_desk - 3)
                        if start_desk >= 2:
                            valu_ops += emit_hash_stage(start_desk - 2, 4)
                        if start_desk >= 1:
                            valu_ops += emit_hash_combine(start_desk - 1, 1)
            ...
            # Finish remaining operations for desks 12-15
            if not skip_bounds:
                self.instrs.append({
                ...
                self.instrs.append({
                    "valu": emit_hash_stage(13, 5) + emit_hash_stage(14, 2) + emit_xor_node(15),
                })
                self.instrs.append({
                    "valu": emit_hash_combine(13, 5) + emit_hash_stage(14, 3) + emit_hash_stage(15, 0),
                })
            ...
            if not skip_bounds:
                ...
                self.instrs.append({
                    "valu": emit_hash_stage(14, 5) + emit_hash_stage(15, 2),
                })
                self.instrs.append({
                    "valu": emit_hash_combine(14, 5) + emit_hash_stage(15, 3),
                })
            ...
            if not skip_bounds:
                ...
                self.instrs.append({
                    "valu": emit_hash_stage(15, 5),
                })
                self.instrs.append({
                    "valu": emit_hash_combine(15, 5),
                })
        ...
        def emit_preload_round_0():
            """Round 0: All indices = 0, use tree[0] broadcast."""
            # Broadcast tree[0] to all desk node_vals - 6 per cycle
            for d in range(0, NUM_DESKS, 6):
                ops = []
                for dd in range(d, min(d + 6, NUM_DESKS)):
                    ops.append(("+", desks[dd]['node_val'], v_tree[0], v_zero))
                self.instrs.append({"valu": ops})
            ...
            # Branch and no bounds check needed (max idx = 2)
            for d in range(0, NUM_DESKS, 3):
        ...
        def emit_preload_round_1():
            """Round 1: Indices are in {1, 2}, use arithmetic selection."""
            ...
            # Compute diff = tree[2] - tree[1] in addr
            for d in range(0, NUM_DESKS, 6):
                ops = []
                for dd in range(d, min(d + 6, NUM_DESKS)):
        ...
        # Round 0: All indices = 0 - no bounds check needed
        emit_preload_round_0()

        # Round 1: Indices in {1, 2} - no bounds check needed
        emit_preload_round_1()

        # Rounds 2-9: Use gather, skip bounds (indices guaranteed valid)
        for rnd in range(2, 10):
            emit_gather_round_improved(skip_bounds=True)

        # Round 10: Use gather WITH bounds check
        emit_gather_round_improved(skip_bounds=False)

        # Rounds 11-14: Need bounds check
        for rnd in range(11, 15):
            emit_gather_round_improved(skip_bounds=False)

        # Round 15: Final round with bounds check
        emit_gather_round_improved(skip_bounds=False)
