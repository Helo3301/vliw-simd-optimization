"""
# Experiment C3: Combined H12 (Round Fusion) + H14 (Parallel Address Pipeline)

**H12 Key Technique:** Process 2 consecutive rounds without intermediate memory
writeback - keeps tree indices in registers between rounds.

**H14 Key Technique:** Use ALU to pre-compute next iteration's base addresses
while VALU computes hash stages.

**Combination Strategy:**
- Use H12's round fusion structure (2 rounds per iteration, 8 double-rounds total)
- Add H14's address pre-computation during VALU-heavy hash phases
- Move vbroadcast operations earlier where possible (from H14)
- Overlap desk 2,3 gather address adds with first desk0 gather (from H14)
- Overlap store phase with loop control ALU ops (from H14)

**Expected:** Combine savings from both optimizations:
- H12: ~500 cycles saved from eliminated vload/vstore pairs
- H14: ~50-100 cycles saved from better ALU/VALU overlap
"""
...
class KernelBuilderC3:
    """
    C3: Combined H12 (Round Fusion) + H14 (Parallel Address Pipeline)

    H12 fusion: Process 2 rounds per iteration, keeping idx/val in registers
    H14 pipeline: Precompute next iteration addresses using ALU during VALU hash phases
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def build_kernel(
        ...
        """
        C3: Combined Round Fusion + Address Pipeline kernel.
        """
        ...
        # === MAIN LOOP with ROUND FUSION + ADDRESS PIPELINE ===
        ...
        # Load desk0 idx/val + compute addresses for desk1,2 + desk1 broadcast
        self.instrs.append({
            "load": [
...
BASELINE = 147734
H12_BASELINE = 5115  # H12's cycle count for comparison
...
def do_kernel_test(
    ...
    print(f"Improvement over H12 ({H12_BASELINE} cycles): {H12_BASELINE - machine.cycle} cycles ({100*(H12_BASELINE - machine.cycle)/H12_BASELINE:.2f}%)")
