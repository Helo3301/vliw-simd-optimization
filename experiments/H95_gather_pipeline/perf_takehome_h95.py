"""
# Experiment H95: Pipeline Gather Operations Across Rounds

**GOAL:** Overlap load latency with compute by starting gather for desk D while
still computing hash for desk D-1.

**HYPOTHESIS:** In gather rounds (3-9, 14-15), each desk performs 8 scalar loads.
By pipelining the gather operations - issuing loads for the next desk while
computing hash for the current desk - we can hide load latency.

**APPROACHES TESTED:**

1. **Lane-first load ordering** (`for lane: for d` instead of `for d: for lane`):
   - Result: 2,547 cycles (WORSE)
   - Analysis: Spreading loads by lane delays data availability for each desk

2. **Deep pipelining** (split desks into halves, load first half, compute, load second half):
   - Result: 1,911 cycles (WORSE)
   - Analysis: Added complexity without scheduler benefit

3. **Cross-round pipelining** (issue loads for N+1 after branch for N):
   - Result: 1,851 cycles (SAME as H87)
   - Analysis: Scheduler already overlaps this optimally

4. **Interleaved load+XOR** (issue loads for late desks interleaved with XOR for early desks):
   - Result: 1,851 cycles (SAME as H87)
   - Analysis: Scheduler already handles this

5. **Double-buffered node_val** (alternate between two buffers):
   - Result: 1,851 cycles (SAME as H87)
   - Analysis: Cannot help because idx is only known after branch

**BASELINE:** H87 = 1,851 cycles

**RESULTS:**
- H95 gather pipeline: 1,851 cycles (NO IMPROVEMENT)

**CONCLUSION:**
The automatic scheduler (`_schedule_slots`) already does an excellent job of
overlapping load latency with VALU computation. Various explicit pipelining
strategies either:
1. Match H87's performance (scheduler compensates)
2. Perform WORSE (explicit ordering constraints prevent scheduler optimization)

The key insight is that the scheduler sees ALL operations before scheduling and
can make globally optimal decisions. Explicit emit ordering only helps if it
exposes parallelism the scheduler couldn't find, or if it reduces register
pressure. In this case, neither benefit materializes.

**LESSON LEARNED:**
The note in the task spec was correct: "The automatic scheduler might already
do some of this." In fact, the scheduler does ALL of it optimally. Further
improvements would require:
1. Reducing total operation count (algorithmic changes)
2. Reducing dependencies (different data flow)
3. Better utilizing underused engines (ALU has 12 slots but mostly unused)
"""
...
class KernelBuilderH95:
    """
    H95: Pipeline gather operations across desks to overlap load latency with compute.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel with pipelined gather operations.
        """
        ...
        even_desks = list(range(0, NUM_DESKS, 2))
        odd_desks = list(range(1, NUM_DESKS, 2))
        ...
        def emit_gather_round_pipelined(with_bounds=False):
            """
            Pipeline gather operations: Use H87's desk-first load ordering but interleave
            XOR with early hash stages to create better overlap.

            The key insight is that after all loads are issued, we can start XOR and
            hash for early desks while later desks' loads are still completing.
            """
            # Phase 1: Compute all addresses in parallel
            for d in range(NUM_DESKS):
                self.emit("valu", ("+", desks[d]['addr'], v_forest_p, desks[d]['idx']))

            # Phase 2: Issue all loads (desk-first ordering like H87)
            for d in range(NUM_DESKS):
                for lane in range(VLEN):
                    self.emit("load", ("load", desks[d]['node_val'] + lane, desks[d]['addr'] + lane))

            # Phase 3: XOR for all desks
            for d in range(NUM_DESKS):
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))

            # Phase 4: Hash stages using double-pumped approach
            ...
            # Phase 6: Optional bounds check
            if with_bounds:
                for d in range(NUM_DESKS):
                    emit_bounds(d)

        def emit_gather_round_deeply_pipelined(with_bounds=False):
            """
            More aggressive pipelining: interleave address computation with loads
            from previous desks, and hash computation with loads from later desks.

            This creates a deeper pipeline:
            - Desk 0: compute addr -> issue loads
            - Desk 1: compute addr -> issue loads (while D0 loads in flight)
            - ...
            - Desk 0: XOR + hash stage 0 (D0 loads complete)
            - Desk 1: XOR + hash stage 0 (D1 loads complete)
            - ...

            The key is to spread operations so the scheduler can find parallelism.
            """
            # Batch 1: First half of desks - compute address and issue loads
            first_half = list(range(0, NUM_DESKS // 2))
            second_half = list(range(NUM_DESKS // 2, NUM_DESKS))

            # Compute addresses for first half
            for d in first_half:
                self.emit("valu", ("+", desks[d]['addr'], v_forest_p, desks[d]['idx']))

            # Issue loads for first half (interleaved by lane)
            for lane in range(VLEN):
                for d in first_half:
                    self.emit("load", ("load", desks[d]['node_val'] + lane, desks[d]['addr'] + lane))

            # Compute addresses for second half (while first half loads are in flight)
            for d in second_half:
                self.emit("valu", ("+", desks[d]['addr'], v_forest_p, desks[d]['idx']))

            # Issue loads for second half
            for lane in range(VLEN):
                for d in second_half:
                    self.emit("load", ("load", desks[d]['node_val'] + lane, desks[d]['addr'] + lane))

            # XOR and hash for first half (first half loads should be complete or nearly complete)
            for d in first_half:
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))

            # Start hash for first half while second half XORs happen
            # Stage 0 (FMA) for first half
            for d in first_half:
                self.emit("valu", ("multiply_add", desks[d]['val'], desks[d]['val'], v_fma_mult[0], v_hash_consts[0]))
            ...
            # Stage 0 (FMA) for second half
            for d in second_half:
                self.emit("valu", ("multiply_add", desks[d]['val'], desks[d]['val'], v_fma_mult[0], v_hash_consts[0]))

            # Remaining hash stages - continue double-pumped pattern
            # Stage 1 for all
            for d in even_desks:
                tmp1, tmp2 = get_temps(d)
                self.emit("valu", ("^", tmp1, desks[d]['val'], v_hash_consts[1]))
        ...
        def emit_tile(tile_idx):
            ...
            # Standard H87 approach - scheduler handles overlap automatically
            ...
            # Rounds 14-15: Gather without bounds (standard H87 approach)
