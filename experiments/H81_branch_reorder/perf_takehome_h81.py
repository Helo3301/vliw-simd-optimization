...
class KernelBuilderH81:
    ...
    def build_kernel(
        ...
        def emit_branch(desk_idx):
            d = desks[desk_idx]
            # Reorder: multiply_add only depends on idx, not val/hash result
            # This allows scheduler to move it earlier, hiding it in load latency
            self.emit("valu", ("multiply_add", d['tmp2'], d['idx'], v_two, v_one))  # tmp2 = 2*idx+1
            self.emit("valu", ("&", d['tmp1'], d['val'], v_one))                    # tmp1 = val & 1
            self.emit("valu", ("+", d['idx'], d['tmp2'], d['tmp1']))                # idx = tmp2 + tmp1
