...
Current (per desk iteration):
  Round N:   vload idx,val -> compute -> vstore idx,val
  Round N+1: vload idx,val -> compute -> vstore idx,val

Optimized (round fusion):
  Round N:   vload idx,val -> compute -> [keep in registers]
  Round N+1: [use registers] -> compute -> vstore idx,val

This eliminates 50% of vload/vstore pairs for each desk.

**Challenge:**
- Now we have 8 double-rounds instead of 16 single-rounds
- Each "desk" needs to hold intermediate results between rounds
- Register pressure increases (need vectors for both rounds' computation)

**Expected:** Save ~500 cycles from eliminated vload/vstore pairs
"""
...
class KernelBuilderH12:
    """
    H12: Round Fusion - Keep Values in Registers Between Consecutive Rounds

    Instead of:
      Round N:   vload -> compute -> vstore
      Round N+1: vload -> compute -> vstore

    We do:
      Rounds N,N+1: vload -> compute N -> compute N+1 -> vstore

    This eliminates half of the vload/vstore operations per desk.
    The loop now processes 2 rounds per iteration.
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def build_kernel(
        ...
        """
        H12: Round Fusion kernel - process 2 rounds per memory access cycle.
        """
