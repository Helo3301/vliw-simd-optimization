"""
BF3 Theory 26: Double-pump - 2 hash values per desk

Try to compute 2 hash values at once using the same desk resources.
This could improve VALU utilization by keeping more ops in flight.
Expected: Likely performance improvement if scheduling works well.
"""
...
class KernelBuilderT26:
    """T26: Double-pump - 2 hash values interleaved per round"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # DOUBLE-PUMP: Interleave two desks' hash stages more aggressively
        # The idea is to emit stage N of desk A, then stage N of desk B, alternating
        def emit_hash_double_pump(desk_a, desk_b):
            da = desks[desk_a]
            db = desks[desk_b]

            # Stage 0 FMA for both - interleaved
            self.emit("valu", ("multiply_add", da['val'], da['val'], v_fma_mult[0], v_hash_consts[0]))
            self.emit("valu", ("multiply_add", db['val'], db['val'], v_fma_mult[0], v_hash_consts[0]))

            # Stage 1 for both - interleaved
            self.emit("valu", ("^", da['tmp1'], da['val'], v_hash_consts[1]))
            self.emit("valu", ("^", db['tmp1'], db['val'], v_hash_consts[1]))
            self.emit("valu", (">>", da['tmp2'], da['val'], v_hash_shifts[1]))
            self.emit("valu", (">>", db['tmp2'], db['val'], v_hash_shifts[1]))
            self.emit("valu", ("^", da['val'], da['tmp1'], da['tmp2']))
            self.emit("valu", ("^", db['val'], db['tmp1'], db['tmp2']))

            # Stage 2 FMA for both
            self.emit("valu", ("multiply_add", da['val'], da['val'], v_fma_mult[2], v_hash_consts[2]))
            self.emit("valu", ("multiply_add", db['val'], db['val'], v_fma_mult[2], v_hash_consts[2]))

            # Stage 3 for both - interleaved
            self.emit("valu", ("+", da['tmp1'], da['val'], v_hash_consts[3]))
            self.emit("valu", ("+", db['tmp1'], db['val'], v_hash_consts[3]))
            self.emit("valu", ("<<", da['tmp2'], da['val'], v_hash_shifts[3]))
            self.emit("valu", ("<<", db['tmp2'], db['val'], v_hash_shifts[3]))
            self.emit("valu", ("^", da['val'], da['tmp1'], da['tmp2']))
            self.emit("valu", ("^", db['val'], db['tmp1'], db['tmp2']))

            # Stage 4 FMA for both
            self.emit("valu", ("multiply_add", da['val'], da['val'], v_fma_mult[4], v_hash_consts[4]))
            self.emit("valu", ("multiply_add", db['val'], db['val'], v_fma_mult[4], v_hash_consts[4]))

            # Stage 5 for both - interleaved
            self.emit("valu", ("^", da['tmp1'], da['val'], v_hash_consts[5]))
            self.emit("valu", ("^", db['tmp1'], db['val'], v_hash_consts[5]))
            self.emit("valu", (">>", da['tmp2'], da['val'], v_hash_shifts[5]))
            self.emit("valu", (">>", db['tmp2'], db['val'], v_hash_shifts[5]))
            self.emit("valu", ("^", da['val'], da['tmp1'], da['tmp2']))
            self.emit("valu", ("^", db['val'], db['tmp1'], db['tmp2']))
        ...
        # DOUBLE-PUMP approach for gather rounds
        def emit_gather_round_double_pump(desk_a, desk_b):
            da = desks[desk_a]
            db = desks[desk_b]

            # Addr compute interleaved
            self.emit("valu", ("+", da['addr'], v_forest_p, da['idx']))
            self.emit("valu", ("+", db['addr'], v_forest_p, db['idx']))

            # Gather interleaved
            for lane in range(VLEN):
                self.emit("load", ("load", da['node_val'] + lane, da['addr'] + lane))
            for lane in range(VLEN):
                self.emit("load", ("load", db['node_val'] + lane, db['addr'] + lane))

            # XOR interleaved
            self.emit("valu", ("^", da['val'], da['val'], da['node_val']))
            self.emit("valu", ("^", db['val'], db['val'], db['node_val']))

            # Hash double-pumped
            emit_hash_double_pump(desk_a, desk_b)
...
def do_kernel_test(forest_height: int, rounds: int, batch_size: int, seed: int = 123,
    ...
    for i, ref_mem in enumerate(reference_kernel2(mem, value_trace)):
        ...
        if check:
            if (machine.mem[inp_values_p : inp_values_p + len(inp.values)]
                    != ref_mem[inp_values_p : inp_values_p + len(inp.values)]):
                correct = False
                print(f"INCORRECT result on round {i}")
...
if __name__ == "__main__":
    ...
    cycles, correct = do_kernel_test(10, 16, 256, check=True)
    print(f"T26 Double-pump: Cycles={cycles}, Correct={correct}")
