...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            # THEORY 42: Process all desks at once across all groups
            # Use all 16 desks as a single group. This means the scheduler sees all
            # 16 desks' operations at similar dependency depths, maximizing ILP.
            all_desks = list(range(NUM_DESKS))
            
            emit_rounds_0_1_2_fused(all_desks)
            
            for _rnd in range(3, 10):
                emit_gather_round_addr_tracking(all_desks)
            
            emit_round_10_optimized(all_desks)
