...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # v_zero and v_n_nodes REMOVED: unused (no bounds check, R10 idx=0 removed)
        ...
        def emit_round_10_optimized(group_desks):
            """Round 10: addr already ready from R9 addr-tracking. Skip branch.
            R10's idx=0 XOR is DEAD CODE: R11 overwrites idx with (1+bit0)
            without reading old idx. Removing saves 1 VALU per desk."""
            ...
            # Hash
            emit_hash_interleaved(group_desks)
            # idx=0 XOR REMOVED: R11 sets idx = 1 + bit0, never reads old idx
