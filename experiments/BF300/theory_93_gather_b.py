...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_gather_round_addr_tracking(group_desks):
            # Pattern B: Pipeline loads and XOR/hash
            n = len(group_desks)
            # Stage 0: load desk 0
            d0 = group_desks[0]
            desk0 = desks[d0]
            ...
            # Stage 1+: load desk N, XOR desk N-1
            for i in range(1, n):
                d = group_desks[i]
                desk = desks[d]
                for lane in range(VLEN):
                    self.emit("load", ("load", desk['node_val'] + lane, desk['addr'] + lane))
                prev_d = group_desks[i-1]
                prev = desks[prev_d]
                self.emit("valu", ("^", prev['val'], prev['val'], prev['node_val']))
            # XOR last desk
            last_d = group_desks[-1]
            last = desks[last_d]
            self.emit("valu", ("^", last['val'], last['val'], last['node_val']))
            emit_hash_interleaved(group_desks)
            for d in group_desks:
