...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            # THEORY 82: Round-major with all groups
            # For each round phase, process ALL groups before moving to next phase
            
            # Fused R0+R1+R2 for all groups
            for group_desks in all_groups:
                emit_rounds_0_1_2_fused(group_desks)
            
            # R3 preloaded for all groups
            for group_desks in all_groups:
                emit_round_level3_preloaded_v2(group_desks)
            
            # R4-R9 gather rounds - round-major across all groups
            ...
            # Fused R11+R12+R13 for all groups
            for group_desks in all_groups:
                emit_rounds_11_12_13_fused(group_desks)
            
            # R14 preloaded for all groups
            for group_desks in all_groups:
                emit_round_level3_preloaded_v2(group_desks)
            
            # R15 final for all groups
            for group_desks in all_groups:
