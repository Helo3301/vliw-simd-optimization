...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def _emit_hash_single(d_idx):
            """Emit hash for a single desk"""
            hd = desks[d_idx]
            self.emit("valu", ("multiply_add", hd['val'], hd['val'], v_fma_mult[0], v_hash_consts[0]))
            self.emit("valu", ("^", hd['tmp1'], hd['val'], v_hash_consts[1]))
            self.emit("valu", (">>", hd['tmp2'], hd['val'], v_hash_shifts[1]))
            self.emit("valu", ("^", hd['val'], hd['tmp1'], hd['tmp2']))
            self.emit("valu", ("multiply_add", hd['val'], hd['val'], v_fma_mult[2], v_hash_consts[2]))
            self.emit("valu", ("+", hd['tmp1'], hd['val'], v_hash_consts[3]))
            self.emit("valu", ("<<", hd['tmp2'], hd['val'], v_hash_shifts[3]))
            self.emit("valu", ("^", hd['val'], hd['tmp1'], hd['tmp2']))
            self.emit("valu", ("multiply_add", hd['val'], hd['val'], v_fma_mult[4], v_hash_consts[4]))
            self.emit("valu", ("^", hd['tmp1'], hd['val'], v_hash_consts[5]))
            self.emit("valu", (">>", hd['tmp2'], hd['val'], v_hash_shifts[5]))
            self.emit("valu", ("^", hd['val'], hd['tmp1'], hd['tmp2']))

        def emit_gather_round_addr_tracking(group_desks):
            """Gather round with pipelined load/hash overlap.
            Pipeline: load[0], load[1]+XOR[0], load[2]+XOR[1]+hash[0], load[3]+XOR[2]+hash[1],
                      XOR[3]+hash[2], hash[3], branch all"""
            n = len(group_desks)
            ...
            # Stages 1 to n-1: Load desk[stage], XOR desk[stage-1], hash desk[stage-2]
            for stage in range(1, n):
                ...
                # XOR previous desk
                prev_d = group_desks[stage - 1]
                self.emit("valu", ("^", desks[prev_d]['val'], desks[prev_d]['val'], desks[prev_d]['node_val']))
                
                # Hash desk[stage-2] if valid
                if stage >= 2:
                    _emit_hash_single(group_desks[stage - 2])
            
            # XOR last desk
            last_d = group_desks[n - 1]
            self.emit("valu", ("^", desks[last_d]['val'], desks[last_d]['val'], desks[last_d]['node_val']))
            
            # Hash remaining desks: stage n-2 and n-1
            for i in range(max(0, n - 2), n):
                _emit_hash_single(group_desks[i])
