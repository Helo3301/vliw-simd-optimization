...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_branch_addr_tracking_batched(desk_indices):
            """THEORY 7: Batch branch - emit all ANDs, then FMAs, then ADDs across desks"""
        ...
        def emit_gather_round_addr_tracking(group_desks):
            ...
            # Hash
            emit_hash_interleaved(group_desks)
            # Branch (updates addr, not idx) - THEORY 7: batched across desks
            emit_branch_addr_tracking_batched(group_desks)
