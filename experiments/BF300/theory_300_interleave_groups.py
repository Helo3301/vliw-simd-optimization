"""
Theory 300: Interleave groups within each tile.

Instead of: all-16-rounds(group0), all-16-rounds(group1), ...
Try: for each round, process across groups to allow load/compute overlap.

Focus on R4-R9 which are the gather rounds with 32 loads each.
"""
import random
import argparse
import sys
...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        ...
        def emit_gather_loads(group_desks):
            """Just the loads part of a gather round"""
            for d in group_desks:
                desk = desks[d]
                for lane in range(VLEN):
                    self.emit("load", ("load", desk['node_val'] + lane, desk['addr'] + lane))

        def emit_gather_compute(group_desks):
            """XOR + hash + branch for a gather round"""
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            emit_hash_interleaved(group_desks)
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            # THEORY 300: Interleave groups
            # Process fused R0-R3 per group (can't interleave, uses vselects)
            for group_desks in all_groups:
                emit_rounds_0_1_2_3_fused(group_desks)

            # R4-R9: Interleave loads across groups, then compute
            for rnd in range(4, 10):
                # Emit ALL groups' loads
                for group_desks in all_groups:
                    emit_gather_loads(group_desks)
                # Emit ALL groups' compute
                for group_desks in all_groups:
                    emit_gather_compute(group_desks)

            # R10 per group
            for group_desks in all_groups:
                emit_round_10_optimized(group_desks)

            # Fused R11-R14 per group
            for group_desks in all_groups:
...
def do_kernel_test(forest_height=10, rounds=16, batch_size=256, seed=123,
                   trace=False, prints=False, check=False):
...
if __name__ == "__main__":
    ...
    if args.check:
        ...
        do_kernel_test(trace=args.trace)
