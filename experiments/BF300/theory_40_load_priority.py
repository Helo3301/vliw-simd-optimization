...
def _schedule_slots(slots: list[tuple[str, tuple]]) -> list[dict[str, list[tuple]]]:
    """Priority-based list scheduler that processes ready operations by priority.
    Loads get highest priority since they are the throughput bottleneck."""
    
    n = len(slots)
    # Precompute reads/writes for all slots
    all_rw = [_slot_rw(slots[i][0], slots[i][1]) for i in range(n)]
    ...
    # Engine priority: load > valu > store > flow > alu
    engine_priority = {'load': 0, 'valu': 1, 'store': 2, 'flow': 3, 'alu': 4}
    
    # Process in order but with lookahead for loads
    current_cycle = 0
    next_unscheduled = 0
    
    while next_unscheduled < n:
        # Find the next unscheduled operation
        while next_unscheduled < n and scheduled[next_unscheduled]:
            next_unscheduled += 1
        if next_unscheduled >= n:
            break
        
        # Compute earliest cycle for this operation
        engine, slot = slots[next_unscheduled]
        earliest = compute_earliest(next_unscheduled)
        
        # Find a cycle with available slot
        cycle = earliest
        ...
        place_op(next_unscheduled, cycle)
        next_unscheduled += 1
