...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # THEORY 3: Interleaved tile processing
        # Emit loads for both tiles first, then compute, then stores
        for tile_idx in range(2):
            tile_offset = tile_idx * NUM_DESKS * VLEN
            off_base = tile_idx * NUM_DESKS  # We'll reuse offset_regs since tile 0 is done when tile 1 starts... 
            # Actually we can't easily do this without more scratch. Let's try alternating groups.
            pass
        
        # Simpler approach: alternate groups from tile 0 and tile 1
        # This gives the scheduler operations from both tiles at similar dependency depths
        def emit_tile_loads(tile_idx):
            tile_offset = tile_idx * NUM_DESKS * VLEN
            for d in range(NUM_DESKS):
                self.emit("load", ("const", offset_regs[d], tile_offset + d * VLEN))
            for d in range(NUM_DESKS):
        ...
        def emit_tile_stores():
            for d in range(NUM_DESKS):
                self.emit("store", ("vstore", addr_tmp[d*2], desks[d]['idx']))
                self.emit("store", ("vstore", addr_tmp[d*2+1], desks[d]['val']))

        # Process: load tile0, compute tile0, store tile0, load tile1, compute tile1, store tile1
        # But emit stores of tile0 AFTER loads of tile1 start (interleave)
        # Actually can't easily interleave with greedy scheduler since desk regs are shared.
        # Just do standard sequential for now and rely on per-group stores to help.
        emit_tile_loads(0)
        emit_tile_compute()
        emit_tile_stores()
        emit_tile_loads(1)
