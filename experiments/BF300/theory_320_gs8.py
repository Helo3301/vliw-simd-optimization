"""
Theory 320: GROUP_SIZE=8 with Theory 222 optimizations.

8 desks per group, 2 groups per tile.
More VALU per gather round (96 hash + 24 branch = 120 VALU)
More loads per gather round (64 loads = 32 cycles at 2/cycle)
Better VALU-to-load ratio: 120/6 = 20 VALU cycles vs 64/2 = 32 load cycles
This should overlap better.

Same flow budget: 11 vselects * 8 desks = 88 per fused block per group.
704 total flow ops: 11*8*2*2*2 = 704 (same as GS=4!)
"""
...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        ...
        def emit_hash_interleaved(group_desks):
            # For GS=8, interleave differently
            if len(group_desks) == 8:
                gd = [group_desks[0], group_desks[7], group_desks[4], group_desks[3],
                      group_desks[2], group_desks[5], group_desks[6], group_desks[1]]
            else:
        ...
        def emit_gather_round_addr_tracking(group_desks):
            ...
            emit_hash_interleaved(group_desks)
            for d in group_desks: emit_branch_addr_tracking(d)
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for gd in all_groups:
                ...
                for _ in range(4, 10): emit_gather_round_addr_tracking(gd)
