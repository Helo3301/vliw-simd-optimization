...
def _schedule_slots(slots: list[tuple[str, tuple]]) -> list[dict[str, list[tuple]]]:
    """Greedy scheduler with post-scheduling compaction pass."""
    ...
    ops_list = [s[1] for s in slots]
    all_rw = [_slot_rw(engines[i], ops_list[i]) for i in range(n)]
    
    # Phase 1: Standard greedy scheduling (same as original)
    cycles = []
    usage = []
    ...
    scheduled_cycle = [0] * n  # which cycle each op is scheduled at
    op_to_slot_idx = {}  # (cycle, engine, slot_index_in_engine) -> op_index
    ...
    for i in range(n):
        ...
        ensure_cycle(cycle)
        slot_idx = len(cycles[cycle].get(engine, []))
        cycles[cycle].setdefault(engine, []).append(slot)
        usage[cycle][engine] += 1
        scheduled_cycle[i] = cycle
        op_to_slot_idx[(cycle, engine, slot_idx)] = i
    ...
    # Phase 2: Build dependency graph for compaction
    reg_writer_map = {}
    reg_readers_map = defaultdict(list)
    predecessors = [set() for _ in range(n)]
    successors_set = [set() for _ in range(n)]
    
    for i in range(n):
        reads, writes = all_rw[i]
        preds = set()
        for r in reads:
            if r in reg_writer_map:
                preds.add(reg_writer_map[r])
        for w in writes:
            if w in reg_writer_map:
                preds.add(reg_writer_map[w])
            for reader in reg_readers_map.get(w, []):
                preds.add(reader)
        predecessors[i] = preds
        for p in preds:
            successors_set[p].add(i)
        for r in reads:
            reg_readers_map[r].append(i)
        for w in writes:
            reg_writer_map[w] = i
            reg_readers_map[w] = []
    
    # Phase 3: Try to move operations to earlier cycles
    # Process in reverse scheduled order (latest ops first)
    # For each op, compute its earliest possible cycle based on predecessor completion
    improved = True
    passes = 0
    while improved and passes < 3:
        ...
        # Sort ops by current scheduled cycle (latest first)
        order = sorted(range(n), key=lambda i: -scheduled_cycle[i])
        
        for i in order:
            cur_cycle = scheduled_cycle[i]
            engine = engines[i]
            reads, writes = all_rw[i]
            
            # Compute earliest cycle from predecessors
            earliest = 0
            for p in predecessors[i]:
                earliest = max(earliest, scheduled_cycle[p] + 1)
            
            if earliest >= cur_cycle:
                continue  # Can't improve
            
            # Find earliest cycle with available slot
            best_cycle = None
            for c in range(earliest, cur_cycle):
                ensure_cycle(c)
                if usage[c][engine] < SLOT_LIMITS[engine]:
                    # Check that moving here doesn't violate successor constraints
                    # Successors need this op to complete before they start
                    valid = True
                    for s in successors_set[i]:
                        if scheduled_cycle[s] <= c:
                            valid = False
                            break
                    if valid:
                        best_cycle = c
            ...
            if best_cycle is not None and best_cycle < cur_cycle:
                # Move op from cur_cycle to best_cycle
                # Remove from current cycle
                engine_slots = cycles[cur_cycle].get(engine, [])
                slot = ops_list[i]
                if slot in engine_slots:
                    engine_slots.remove(slot)
                    usage[cur_cycle][engine] -= 1
                    
                    # Add to new cycle
                    cycles[best_cycle].setdefault(engine, []).append(slot)
                    usage[best_cycle][engine] += 1
                    scheduled_cycle[i] = best_cycle
                    improved = True
    
    # Remove empty cycles at the end
    while cycles and not any(cycles[-1].values()):
        cycles.pop()
        usage.pop()
