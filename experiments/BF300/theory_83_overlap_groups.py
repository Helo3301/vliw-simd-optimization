...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            # THEORY 83: Overlap group transitions
            # Process groups with overlapping ends/beginnings
            for gi, group_desks in enumerate(all_groups):
                if gi == 0:
                    ...
                    # Start this group's R0-R2 while previous group still computing R14-R15
                    # The previous group's R14-R15 was already emitted above
                    emit_rounds_0_1_2_fused(group_desks)
                    emit_round_level3_preloaded_v2(group_desks)
                    for _rnd in range(4, 10):
                        emit_gather_round_addr_tracking(group_desks)
                ...
                if gi < len(all_groups) - 1:
                    # Emit R14 and R15 for current group, THEN start next group
                    # The scheduler will overlap because next group's R0 is independent
                    emit_round_level3_preloaded_v2(group_desks)
                    next_group = all_groups[gi + 1]
                    # Interleave: current group R15 with next group R0 init
                    emit_round_15_final_interleaved(group_desks)
                else:
                    # Last group: just finish
                    emit_round_level3_preloaded_v2(group_desks)
                    emit_round_15_final_interleaved(group_desks)
