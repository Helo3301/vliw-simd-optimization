...
def _schedule_slots(slots: list[tuple[str, tuple]]) -> list[dict[str, list[tuple]]]:
    """Critical-path list scheduler: schedule ready ops prioritized by urgency.
    Uses reverse topological ordering to compute critical path lengths,
    then schedules highest-urgency ops first at each cycle."""
    ...
    # Build dependency graph: for each register, track last writer and readers
    # Deps: i depends on j if j writes a reg that i reads, or j reads/writes a reg that i writes
    
    # Build successor lists for the dependency graph
    # We track: for each register, who last wrote it and who has read since
    reg_last_writer = {}  # reg -> op_idx
    reg_readers_since_write = defaultdict(list)  # reg -> [op_idx, ...]
    
    predecessors = [set() for _ in range(n)]  # ops that must complete before i
    successors = [set() for _ in range(n)]  # ops that depend on i
    
    for i in range(n):
        reads, writes = all_rw[i]
        deps = set()
        
        # RAW: i reads reg that was written by j
        for r in reads:
            if r in reg_last_writer:
                deps.add(reg_last_writer[r])
        
        # WAW: i writes reg that was written by j
        for w in writes:
            if w in reg_last_writer:
                deps.add(reg_last_writer[w])
            # WAR: i writes reg that was read by j (after last write)
            for reader in reg_readers_since_write[w]:
                deps.add(reader)
        ...
        # Update tracking
        for r in reads:
            reg_readers_since_write[r].append(i)
        for w in writes:
            reg_last_writer[w] = i
            reg_readers_since_write[w] = []
    
    # Compute in-degree for topological processing
    in_degree = [len(predecessors[i]) for i in range(n)]
    
    # Compute critical path length (longest path to any sink) via reverse topo order
    # This gives priority: higher critical path = more urgent to schedule early
    critical_path = [1] * n  # minimum 1 cycle for each op
    
    # Process in reverse topological order (sinks first)
    # Use Kahn's algorithm in reverse
    rev_in_degree = [len(successors[i]) for i in range(n)]
    rev_queue = [i for i in range(n) if rev_in_degree[i] == 0]
    rev_order = []
    while rev_queue:
        i = rev_queue.pop()
        rev_order.append(i)
        for j in predecessors[i]:
            rev_in_degree[j] -= 1
            if rev_in_degree[j] == 0:
                rev_queue.append(j)
    
    for i in rev_order:
        max_succ = 0
        for j in successors[i]:
            max_succ = max(max_succ, critical_path[j])
        critical_path[i] = 1 + max_succ
    
    # Schedule using priority queue
    # Priority: (-critical_path, engine_priority, emission_order)
    # Engine priority: load=0, flow=1, valu=2, store=3, alu=4
    engine_prio = {'load': 0, 'flow': 1, 'valu': 2, 'store': 3, 'alu': 4}
    
    cycles_list: list[dict[str, list[tuple]]] = []
    usage_list: list[dict[str, int]] = []
    
    def ensure_cycle(cycle: int) -> None:
        while len(cycles_list) <= cycle:
            cycles_list.append({})
            usage_list.append(defaultdict(int))
    
    # Track when each op becomes ready
    ready_time_op = [0] * n  # earliest cycle each op can execute
    remaining_deps = [len(predecessors[i]) for i in range(n)]
    
    # Initialize ready queue with ops that have no dependencies
    ready_queue = []  # heap of (-critical_path, engine_prio, emission_order, op_idx)
    for i in range(n):
        if remaining_deps[i] == 0:
            heapq.heappush(ready_queue, (-critical_path[i], engine_prio.get(engines[i], 5), i, i))
    
    scheduled_cycle = [0] * n
    current_cycle = 0
    
    # Track register state for WAR/WAW timing
    reg_ready = defaultdict(int)  # reg -> cycle when available for writing
    reg_last_read_cycle = defaultdict(lambda: -1)  # reg -> last cycle read
    reg_last_write_cycle = defaultdict(lambda: -1)  # reg -> last cycle written
    
    ops_scheduled = 0
    deferred = []  # ops ready but couldn't fit in current cycle
    
    while ops_scheduled < n:
        if not ready_queue and not deferred:
            current_cycle += 1
            continue
        
        ensure_cycle(current_cycle)
        
        # Try to schedule as many ready ops as possible in current_cycle
        still_deferred = []
        
        # Merge deferred ops back into ready queue
        ...
        while ready_queue:
            neg_cp, ep, emission_idx, i = heapq.heappop(ready_queue)
            engine = engines[i]
            reads, writes = all_rw[i]
            
            # Check if op is actually ready at current_cycle
            earliest = ready_time_op[i]
            # Also check register timing
            for r in reads:
                earliest = max(earliest, reg_ready.get(r, 0))
            for w in writes:
                earliest = max(earliest, reg_last_write_cycle.get(w, -1) + 1)
                earliest = max(earliest, reg_last_read_cycle.get(w, -1))
            
            if earliest > current_cycle:
                # Not ready yet, defer
                next_cycle_ops.append((neg_cp, ep, emission_idx, i))
                continue
            
            # Check slot availability
            if usage_list[current_cycle][engine] < SLOT_LIMITS[engine]:
                # Schedule it!
                cycles_list[current_cycle].setdefault(engine, []).append(ops[i])
                usage_list[current_cycle][engine] += 1
                scheduled_cycle[i] = current_cycle
                ops_scheduled += 1
                
                for r in reads:
                    reg_last_read_cycle[r] = max(reg_last_read_cycle.get(r, -1), current_cycle)
                for w in writes:
                    reg_last_write_cycle[w] = current_cycle
                    reg_ready[w] = current_cycle + 1
                
                # Notify successors
                for j in successors[i]:
                    remaining_deps[j] -= 1
                    if remaining_deps[j] == 0:
                        ready_time_op[j] = current_cycle + 1
                        heapq.heappush(ready_queue, (-critical_path[j], engine_prio.get(engines[j], 5), j, j))
            else:
                # Slot full, defer to next cycle
                next_cycle_ops.append((neg_cp, ep, emission_idx, i))
        
        deferred = next_cycle_ops
        current_cycle += 1
    
    return [c for c in cycles_list if c]
