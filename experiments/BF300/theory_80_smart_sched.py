...
def _schedule_slots(slots: list[tuple[str, tuple]]) -> list[dict[str, list[tuple]]]:
    """Smart scheduler: maintains a ready set and packs cycles optimally.
    Uses topological order to respect dependencies while maximizing throughput."""
    ...
    # Build dependency graph using register tracking
    # For each op, compute set of predecessor ops
    pred_count = [0] * n  # number of unscheduled predecessors
    successors = [[] for _ in range(n)]
    
    # Track register state for dependency computation
    reg_writer = {}  # reg -> last op that wrote it
    reg_readers = defaultdict(list)  # reg -> list of ops that read it since last write
    
    for i in range(n):
        reads_i, writes_i = all_rw[i]
        preds = set()
        
        # RAW: i reads a register written by some earlier op
        for r in reads_i:
            if r in reg_writer:
                preds.add(reg_writer[r])
        
        # WAW: i writes a register written by some earlier op
        for w in writes_i:
            if w in reg_writer:
                preds.add(reg_writer[w])
            # WAR: i writes a register read by some earlier op (after last write)
            for reader in reg_readers.get(w, []):
                preds.add(reader)
        
        pred_count[i] = len(preds)
        for p in preds:
    ...
    # Initialize ready set with ops that have no predecessors
    # Use a list sorted by (engine_priority, emission_order) for determinism
    ready = []  # list of (priority, op_idx)
    scheduled_at = [-1] * n
    
    # Engine priority for scheduling: scarce resources first
    # flow (1/cycle) > load (2/cycle) > store (2/cycle) > valu (6/cycle) > alu (12/cycle)
    engine_pri = {'flow': 0, 'load': 1, 'store': 2, 'valu': 3, 'alu': 4}
    
    for i in range(n):
        if pred_count[i] == 0:
            ready.append((engine_pri.get(engines[i], 5), i))
    ready.sort()
    ...
    # Track when ops complete (for successor notification)
    # All ops take 1 cycle: result available at current_cycle + 1
    
    # For each op, track earliest cycle it can be scheduled
    earliest = [0] * n
    ...
    while ops_done < n:
        ...
        for pri, idx in ready:
            if earliest[idx] > current_cycle:
                still_ready.append((pri, idx))
                continue
            ...
            if usage_out[current_cycle][eng] < SLOT_LIMITS[eng]:
                ...
                scheduled_at[idx] = current_cycle
                ops_done += 1
                just_scheduled.append(idx)
            else:
                still_ready.append((pri, idx))
        
        # Notify successors of just-scheduled ops
        new_ready = []
        for idx in just_scheduled:
            for s in successors[idx]:
                pred_count[s] -= 1
                if pred_count[s] == 0:
                    earliest[s] = current_cycle + 1
                    new_ready.append((engine_pri.get(engines[s], 5), s))
        
        ready = still_ready + sorted(new_ready)
        ready.sort()
        current_cycle += 1
