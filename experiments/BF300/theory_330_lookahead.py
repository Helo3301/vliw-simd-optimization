"""
Theory 330: Lookahead scheduler.

The greedy scheduler places each op at the earliest cycle where its engine has room.
This can create suboptimal packing when a load op gets placed at a cycle where VALU
also has room, "wasting" the potential to overlap.

Modified scheduler: When placing a load op, if the earliest cycle has VALU < 6,
try to delay the load by 1 cycle to see if a VALU op can fill that slot instead.

Actually, the better approach: emit operations in an order that gives the greedy
scheduler the best material. The key insight is that the greedy scheduler processes
ops IN EMISSION ORDER. So if we emit loads first, they get placed early, and VALU
ops fill in later. But if we emit VALU first, they pack tightly, and loads have to
find whatever cycles remain.

The optimal strategy: emit loads and VALU in alternating fashion so they naturally
interleave. Specifically, for a gather round:
  - Emit 2 loads, then 6 VALU, then 2 loads, then 6 VALU, etc.
  This gives the scheduler a perfect mix to fill all slots.

Let me try: for each gather round, interleave loads with hash ops.
"""
...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        ...
        def emit_gather_round_interleaved_loads_hash(group_desks):
            """Interleave loads with previous desk's hash.
            Emit: desk0 loads, desk0 XOR, desk0 hash stage0, 
                  desk3 loads, desk3 XOR, desk0 hash stages1-5, desk3 hash,
                  desk2 loads, ...
            The idea: while hash of desk0 is happening (12 VALU), 
            desk1's loads can be placed by the scheduler in the same cycles."""
            
            gd = group_desks
            ho = [gd[0], gd[3], gd[2], gd[1]]  # hash order
            
            # Emit loads for first desk, then interleave
            for i, d in enumerate(ho):
                ...
                # Branch
                self.emit("valu", ("&", desk['tmp1'], desk['val'], v_one))
                self.emit("valu", ("multiply_add", desk['addr'], desk['addr'], v_two, v_1_minus_fp))
                self.emit("valu", ("+", desk['addr'], desk['addr'], desk['tmp1']))
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for gd in all_groups:
                ...
                for _ in range(4, 10): emit_gather_round_interleaved_loads_hash(gd)
