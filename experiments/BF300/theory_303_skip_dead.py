"""
Theory 303: Same as 222 but skip dead VALU emissions.
Keep scratch layout identical to 222 (allocate but don't broadcast dead vars).
"""
...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        ...
        # Keep same allocation order as 222 to preserve layout
        v_zero = self.scratch_vconst(0, "v_zero")  # allocate but we'll skip broadcast below
        v_one = self.scratch_vconst(1, "v_one")
        v_two = self.scratch_vconst(2, "v_two")
        # v_n_nodes - allocate space but skip broadcast
        v_n_nodes = self.alloc_vec("v_n_nodes")
        # DON'T emit broadcast: self.emit("valu", ("vbroadcast", v_n_nodes, self.scratch["n_nodes"]))
        ...
        # v_fp_plus_1 - allocate but skip
        v_fp_plus_1 = self.alloc_vec("v_fp_plus_1")
        # DON'T emit: self.emit("valu", ("+", v_fp_plus_1, v_forest_p, v_one))
