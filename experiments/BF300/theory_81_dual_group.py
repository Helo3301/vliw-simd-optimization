...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            # THEORY 81: Dual-group interleaving
            # Process pairs of groups with interleaved phases
            for pair_idx in range(0, len(all_groups), 2):
                ga = all_groups[pair_idx]
                gb = all_groups[pair_idx + 1]
                
                # Phase 1: Both groups do fused R0-R2
                emit_rounds_0_1_2_fused(ga)
                emit_rounds_0_1_2_fused(gb)
                
                # Phase 2: Group A does R3 (preloaded, VALU+flow heavy)
                #          Interleaved with Group B R3 (preloaded)
                emit_round_level3_preloaded_v2(ga)
                emit_round_level3_preloaded_v2(gb)
                
                # Phase 3: Gather rounds R4-R9 - interleave A and B
                for _rnd in range(4, 10):
                    emit_gather_round_addr_tracking(ga)
                    emit_gather_round_addr_tracking(gb)
                
                # Phase 4: R10
                emit_round_10_optimized(ga)
                emit_round_10_optimized(gb)
                
                # Phase 5: Fused R11-R13
                emit_rounds_11_12_13_fused(ga)
                emit_rounds_11_12_13_fused(gb)
                
                # Phase 6: R14 preloaded
                emit_round_level3_preloaded_v2(ga)
                emit_round_level3_preloaded_v2(gb)
                
                # Phase 7: R15 final
                emit_round_15_final_interleaved(ga)
                emit_round_15_final_interleaved(gb)
