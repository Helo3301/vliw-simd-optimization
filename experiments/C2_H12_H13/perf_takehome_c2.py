"""
# Experiment C2: Combined H12 (Round Fusion) + H13 (Store Coalescing)

**H12 Key technique:** Process 2 consecutive rounds without intermediate memory writeback
- Keeps tree indices in registers between rounds
- Eliminates 50% of vload/vstore pairs for each desk
- Loop now processes 2 rounds per iteration (8 double-rounds instead of 16 single-rounds)

**H13 Key technique:** Batch all stores and overlap them with next iteration's loads
- Overlap stores with loop control ALU operations
- Store engine (2 slots) and ALU engine (12 slots) are independent

**Combined approach:**
- Use H12's round fusion structure (process 2 rounds per iteration)
- Apply H13's store coalescing at the end of each double-round iteration
- This should give us both benefits: fewer total stores AND better overlap

**Expected:** Better than either H12 (5,115 cycles) or H13 (5,435 cycles) alone
"""
...
class KernelBuilderC2:
    """
    C2: Combined H12 (Round Fusion) + H13 (Store Coalescing)

    Combines:
    - H12: Process 2 consecutive rounds without intermediate memory writeback
    - H13: Overlap stores with ALU operations for better throughput

    The key insight is that H12 already reduces the number of store operations
    by 50%, and H13's store coalescing can then be applied to those remaining
    stores to overlap them with other work.
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def build_kernel(
        ...
        """
        C2: Combined Round Fusion + Store Coalescing kernel.
        """
        ...
        # H12: Now we process 2 rounds per iteration
        ...
        # === MAIN LOOP with ROUND FUSION + STORE COALESCING ===
        ...
        # ============================================================
        # H13 STORE COALESCING: Overlap stores with remaining computation
        # and loop control operations
        # ============================================================

        # Cycle: Desk3 bounds check + compute desk 0,1 store addresses
        self.instrs.append({
            "valu": [
                ("<", desks[3]['tmp1'], desks[3]['idx'], v_n_nodes),
            ],
        ...
        # Cycle: Desk3 vselect bypass + desk2,3 store addresses + store desk0
        self.instrs.append({
            "valu": [
                ("*", desks[3]['idx'], desks[3]['idx'], desks[3]['tmp1']),
            ],
        ...
        # Cycle: Store desk1 + loop control (update batch_offset)
        self.instrs.append({
            "store": [
        ...
        # Cycle: Store desk2 + loop control (compare batch_offset)
        self.instrs.append({
            "store": [
        ...
        # Cycle: Store desk3 + select for batch_offset wrap
        self.instrs.append({
            "store": [
