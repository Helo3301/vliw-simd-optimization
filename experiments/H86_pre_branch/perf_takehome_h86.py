"""
# Experiment H86: Pre-Branch Index Computation

**GOAL:** Move the "2*idx + 1" computation BEFORE hash stages so it can overlap
with hash computation, potentially hiding latency.

**KEY CHANGE:** The branch computation is: idx = 2*idx + 1 + (val & 1)
The "2*idx + 1" part doesn't depend on the hash result. By computing this BEFORE
the hash stages, the scheduler can potentially overlap it with hash operations.

**ALGORITHM:** Same as H79 but with pre-branch optimization
**SCHEDULING:** Automatic via _schedule_slots()

**EXPECTED:** Better ILP by allowing 2*idx+1 to be computed during hash stages
"""
...
class KernelBuilderH86:
    """
    H86: Pre-branch index computation to improve ILP.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        # IMPORTANT: We need an extra temp register for pre-branch computation
        NUM_DESKS = 16
        desks = []
        for d in range(NUM_DESKS):
            desk = {
                ...
                'pre_idx': self.alloc_vec(f"v_pre_idx_{d}"),  # NEW: for pre-branch computation
            }
            desks.append(desk)
        ...
        # Helper: emit pre-branch computation (2*idx + 1) BEFORE hash
        def emit_pre_branch(desk_idx):
            """Compute 2*idx + 1 and store in pre_idx, to be used after hash."""
            d = desks[desk_idx]
            self.emit("valu", ("multiply_add", d['pre_idx'], d['idx'], v_two, v_one))
        ...
        # Helper: emit post-branch computation (uses pre-computed 2*idx+1)
        def emit_post_branch(desk_idx):
            """Complete branch: idx = pre_idx + (val & 1)"""
            d = desks[desk_idx]
            self.emit("valu", ("&", d['tmp1'], d['val'], v_one))
            self.emit("valu", ("+", d['idx'], d['pre_idx'], d['tmp1']))
        ...
        def emit_tile(tile_idx):
            ...
            for d in range(NUM_DESKS):
                ...
                emit_pre_branch(d)  # NEW: pre-compute 2*idx+1 BEFORE hash
                emit_hash_stages(d)
                emit_post_branch(d)  # NEW: complete branch AFTER hash
