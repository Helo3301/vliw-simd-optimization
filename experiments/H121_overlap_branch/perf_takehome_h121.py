"""
# Experiment H121: Overlap Branch with Hash

**GOAL:** Overlap branch computation with hash stages for non-arithmetic rounds.

**HYPOTHESIS:** Branch computation (3 ops per desk) depends only on the final val after hash.
What if we start branch for early-finishing desks while other desks are still hashing?

Currently: All hash stages for all desks -> All branch for all desks
Try: Hash for half the desks -> Branch for first half while second half hashes -> Branch for second half

For non-arithmetic rounds (3-10, 14-15), we can interleave:
- Emit hash for first half of desks
- Emit branch for first half while emitting hash for second half
- Emit branch for second half

This requires careful temp management to avoid conflicts.
...
class KernelBuilderH121:
    """
    H121: Overlap branch computation with hash stages for non-arithmetic rounds.

    For rounds 3-10 and 14-15, instead of:
      hash_all_desks -> branch_all_desks
    We try:
      hash_first_half -> branch_first_half INTERLEAVED with hash_second_half -> branch_second_half
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel using 32 desks per tile with overlapped branch/hash.
        """
        ...
        print(f"  - H121: Overlapping branch with hash for non-arithmetic rounds")
        ...
        # Split desks into halves for overlapping
        first_half = list(range(0, 16))
        ...
        def emit_hash_stages_for_desks(desk_list):
            """Emit hash stages for a specific subset of desks."""
            # Get groups within this desk list
            groups = [
                [d for d in desk_list if d % 4 == 0],
                [d for d in desk_list if d % 4 == 1],
                [d for d in desk_list if d % 4 == 2],
                [d for d in desk_list if d % 4 == 3],
            ]
        ...
        def emit_branch_for_desks(desk_list):
            """Emit branch for a list of desks, grouped by temp sharing."""
            # Get groups within this desk list
            groups = [
                [d for d in desk_list if d % 4 == 0],
                [d for d in desk_list if d % 4 == 1],
            ...
            # Process groups sequentially to avoid temp conflicts
            for group in groups:
                for d in group:
                    tmp1, _ = get_temps(d)
                    self.emit("valu", ("&", tmp1, desks[d]['val'], v_one))
                for d in group:
        ...
        def emit_overlapped_gather_round():
            """
            Emit a standard gather round (no overlap attempt).
            The scheduler already does a good job - we revert to the standard approach.
            """
        ...
        def emit_tile():
            ...
            # Rounds 3-9: Gather without bounds - USE OVERLAPPED VERSION
            for _round in range(3, 10):
                emit_overlapped_gather_round()
            ...
            # Rounds 14-15: Gather without bounds - USE OVERLAPPED VERSION
