"""
# Experiment H138: H82 + Fast Init (Skip Unused Headers)

**GOAL:** Reduce init overhead by only loading the 4 header values that are
actually used, instead of all 7.

**HYPOTHESIS:** The init section loads all 7 header values (rounds, n_nodes,
batch_size, forest_height, forest_values_p, inp_indices_p, inp_values_p) but
only 4 are actually used in the computation (n_nodes, forest_values_p,
inp_indices_p, inp_values_p). By loading only the used values with correct
offsets, we can save cycles in the init phase.

**BASELINE:** H82 at 1,656 cycles

**ALGORITHM:** Same as H82 (interleaved round processing with groups of 4)
...
class KernelBuilderH138:
    """
    H138: H82 with fast init - only load the 4 header values that are actually used.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        Interleaved round processing: process desk groups through multiple rounds.
        Fast init: only load 4 used header values instead of all 7.
        """
        tmp_scalar = self.alloc_scratch("tmp_scalar")
        tmp_addr = self.alloc_scratch("tmp_addr")

        # Fast init: Only load the 4 header values that are actually used
        # Memory layout: [rounds, n_nodes, batch_size, forest_height, forest_values_p, inp_indices_p, inp_values_p]
        # Indices:       [0,      1,       2,          3,             4,               5,             6            ]
        # Used:                   n_nodes(1),                         forest_values_p(4), inp_indices_p(5), inp_values_p(6)
        init_vars = ["n_nodes", "forest_values_p", "inp_indices_p", "inp_values_p"]
        init_indices = [1, 4, 5, 6]  # Memory indices for each
        for v in init_vars:
            self.alloc_scratch(v)
        # Load only the used headers with correct offsets
        for v, idx in zip(init_vars, init_indices):
        ...
        def emit_gather_round(desk_idx):
            """Rounds 3-9, 14-15: Gather without bounds"""
            d = desks[desk_idx]
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            # Process 3 desks through all rounds, then next 3, etc.

            GROUP_SIZE = 4
            num_full_groups = NUM_DESKS // GROUP_SIZE  # 5 groups of 3, 1 leftover
            remainder = NUM_DESKS % GROUP_SIZE  # 1
