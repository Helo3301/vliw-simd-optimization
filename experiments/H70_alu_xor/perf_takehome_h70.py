"""
# Experiment H70: ALU XOR Exploration

**GOAL:** Explore using ALU for XOR operations instead of VALU.

**RESULT:** Same 2,775 cycles as H68 - ALU XOR does not help.

**ANALYSIS:**
1. Moving XOR from VALU to ALU in gather rounds doesn't reduce cycles
   because the bottleneck is loads (2/cycle), not VALU (6/cycle).
2. VALU XOR takes 1 op per vector vs ALU XOR taking 8 ops per vector.
3. Even though ALU has 12 slots vs VALU's 6, VALU is more efficient for
   vector operations when not bottlenecked.
4. The current H68 interleaved schedule already uses VALU efficiently.

**CONCLUSION:**
To get below 2,775 cycles, need fundamentally different approach:
- Reduce total loads (e.g., skip more rounds, use cached values)
- Increase parallelism (more desks, but limited by scratch space)
- Different algorithm (e.g., speculative traversal)
