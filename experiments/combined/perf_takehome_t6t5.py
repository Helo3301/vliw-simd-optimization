...
This combines:
- T5 V4's 6-VALU slot packing strategy (processes hash stages across multiple desks)
- T6's FMA (multiply_add) optimization for hash stages 0, 2, 4

Hash Stage Analysis:
- Stage 0: val = (val + C0) + (val << 12) = val * 4097 + C0 --> FMA
- Stage 1: val = (val ^ C1) ^ (val >> 13) --> Standard (XOR-based, cannot FMA)
- Stage 2: val = (val + C2) + (val << 5) = val * 33 + C2 --> FMA
- Stage 3: val = (val ^ C3) ^ (val >> 6) --> Standard (XOR-based, cannot FMA)
- Stage 4: val = (val + C4) + (val << 3) = val * 9 + C4 --> FMA
- Stage 5: val = (val ^ C5) ^ (val >> 16) --> Standard (XOR-based, cannot FMA)

Expected gains:
- T5 V4: 8,758 cycles (10.6% improvement from VALU packing)
- T6: 8,514 cycles (13.1% improvement from FMA on stages 0, 2, 4)
- Combined: ~7,500-8,000 cycles if optimizations are additive
...
class KernelBuilderT6T5:
    """
    Combined T6+T5: FMA hash optimization + 6-VALU slot maximization.

    Strategy:
    - Use 4 desks (32 elements per iteration)
    - For FMA stages (0, 2, 4): Single multiply_add per desk, can pack all 4
    - For non-FMA stages (1, 3, 5): Use T5's 6-slot packing (prep for 3 desks per cycle)

    FMA stages reduce from 3 cycles (prep+prep+combine) to 1 cycle per desk group.
    Non-FMA stages use efficient 6-slot packing across desks.
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def build_kernel(
        ...
        """
        T6+T5: Combined FMA + 6-VALU slot packing.

        Hash phase strategy:
        - FMA stages (0, 2, 4): Pack 4 multiply_add ops (one per desk)
        - Non-FMA stages (1, 3, 5): Use T5's 6-slot packing pattern
          - Cycle 1: prep for desks 0,1,2 (6 ops)
          - Cycle 2: prep for desk 3 + combine for desks 0,1,2 (5 ops)
          - Cycle 3: combine for desk 3 (1 op)
        """
        ...
        # FMA multipliers for stages 0, 2, 4:
        ...
        v_hash_consts = []
        v_hash_shifts = []
        v_fma_mult = {}  # FMA multipliers for stages 0, 2, 4
        ...
        for hi, (op1, val1, op2, op3, val3) in enumerate(HASH_STAGES):
            ...
            # For FMA stages, also broadcast the multiplier
        ...
        # PHASE 2: HASH - COMBINED FMA + 6-VALU PACKING
        ...
        # Hash stages with combined optimization
        for hi in range(6):
            if hi in FMA_MULTIPLIERS:
                # FMA stages (0, 2, 4): Single multiply_add per desk, pack all 4
                # This replaces 3 cycles with 1 cycle!
                self.instrs.append({
                    "valu": [
                ...
                # Prep for desk 3 + combine for desks 0,1,2 (5 VALU ops)
                self.instrs.append({
                    "valu": [
