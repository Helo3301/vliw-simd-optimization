"""
Sweep structural changes that could reduce total operations or improve scheduling.
Building on the theory_15 winner (per_desk hash + interleave desk order = 1536).
"""
import sys
import os
import random
...
class KernelBuilder:
    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        ...
        def emit_hash_per_desk_interleaved(group_desks):
        ...
        emit_hash = emit_hash_per_desk_interleaved
        ...
        def emit_tile(tile_idx):
            ...
            all_groups = [list(range(g * GROUP_SIZE, (g + 1) * GROUP_SIZE)) for g in range(num_groups)]
            ...
            for group_desks in all_groups:
                if variant == 'baseline':
                    ...
                    for _ in range(3, 10):
                        emit_gather_round(group_desks)
                    emit_round_10_optimized(group_desks)
                    emit_rounds_11_12_13_fused(group_desks)
                    emit_gather_round(group_desks)
                    emit_round_15_final(group_desks)

                elif variant == 'overlap_branch_addr':
                    # Emit addr for current round RIGHT AFTER branch for previous
                    # Within each gather round: interleave branch(d) + addr(d) for next
                    emit_rounds_0_1_2_fused(group_desks)
                    for rnd in range(3, 10):
                        ...
                        emit_hash(group_desks)
                        if rnd < 9:
                            # Emit branch per desk, interleaved with addr for next round
                            for d in group_desks:
                                emit_branch(d)
                                desk = desks[d]
                                # addr for next round (overlaps with branch of next desk)
                        else:
                            for d in group_desks:
                                emit_branch(d)
                    emit_round_10_optimized(group_desks)
                ...
                elif variant == 'interleave_xor_branch':
                    # Emit XOR ops interleaved with branch ops (XOR doesn't depend on branch)
                    emit_rounds_0_1_2_fused(group_desks)
                    for rnd in range(3, 10):
                        for d in group_desks:
                            desk = desks[d]
                        ...
                        for d in group_desks:
                            ...
                            for lane in range(VLEN):
                                self.emit("load", ("load", desk['node_val'] + lane, desk['addr'] + lane))
                        # Interleave XOR and hash ops per desk
                        for d in group_desks:
                            desk = desks[d]
                            self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
                        emit_hash(group_desks)
                        # Emit branch for each desk, then immediately next round's XOR
                        for d in group_desks:
                            emit_branch(d)
                    emit_round_10_optimized(group_desks)
                    emit_rounds_11_12_13_fused(group_desks)
                ...
                elif variant == 'branch_per_desk_immediate':
                    # Emit branch immediately after hash for each desk (not batched)
                    emit_rounds_0_1_2_fused(group_desks)
                    for rnd in range(3, 10):
                        for d in group_desks:
                            desk = desks[d]
                        ...
                        for d in group_desks:
                            desk = desks[d]
                            self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
                        # Hash + branch per desk (not batched across desks)
                        gd = [group_desks[i] for i in range(0, len(group_desks), 2)] + [group_desks[i] for i in range(1, len(group_desks), 2)]
                        for d in gd:
                            desk = desks[d]
                            # Hash for this desk
                ...
                elif variant == 'addr_during_hash':
                    # Emit addr computation DURING hash (before it finishes, since addr only needs idx)
                    emit_rounds_0_1_2_fused(group_desks)
                    for rnd in range(3, 10):
                        # Normal gather round but emit addr in a different position
                        for d in group_desks:
                            desk = desks[d]
                            self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
                        for d in group_desks:
                ...
                elif variant == 'loads_between_hash_stages':
                    # Emit 2 loads between each hash stage to fill load slots during VALU-heavy periods
                    emit_rounds_0_1_2_fused(group_desks)
                    for rnd in range(3, 9):
                        # Current round normal
                        emit_gather_round(group_desks)
                    # R9: interleave loads of next round with hash
                    for d in group_desks:
                        desk = desks[d]
                        self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
                    for d in group_desks:
...
def run_test(variant, check=True):
    random.seed(123)
    forest = Tree.generate(10)
    inp = Input.generate(forest, 256, 16)
    mem = build_mem_image(forest, inp)

    kb = KernelBuilder(variant=variant)
...
if __name__ == "__main__":
    variants = [
        'baseline',
        'overlap_branch_addr',
        'interleave_xor_branch',
        'branch_per_desk_immediate',
        'addr_during_hash',
        'loads_between_hash_stages',
    ]

    print(f"{'Variant':<35} | {'Cycles':>6} | {'vs A1':>5} | OK?")
    print("-" * 60)
    for v in variants:
        try:
            cycles, correct = run_test(v, check=True)
            delta = 1548 - cycles
            status = "YES" if correct else "NO"
            marker = " **WIN**" if delta > 0 and correct else ""
            print(f"{v:<35} | {cycles:>6} | {delta:>+5} | {status:>3}{marker}")
        except Exception as e:
            print(f"{v:<35} | ERROR  | N/A   | ERR: {str(e)[:30]}")
