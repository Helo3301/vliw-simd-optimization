"""
Sweep many hash emission patterns to find the best scheduling.
"""
import sys
import os
import random
...
class KernelBuilder:
    def __init__(self, hash_pattern='per_desk', desk_order='interleave', branch_order='forward'):
        self.slots = []
        self.scratch = {}
        self.scratch_debug = {}
        self.scratch_ptr = 0
    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        ...
        def reorder_desks(group_desks):
            if self.desk_order == 'interleave':
                return [group_desks[i] for i in range(0, len(group_desks), 2)] + [group_desks[i] for i in range(1, len(group_desks), 2)]
            elif self.desk_order == 'reverse':
                return list(reversed(group_desks))
            elif self.desk_order == 'forward':
                return list(group_desks)
            elif self.desk_order == 'stride3':
                # 0,3,1,2 for group of 4
                n = len(group_desks)
                return [group_desks[i % n] for i in range(0, n * 3, 3)][:n] if n > 1 else list(group_desks)
            return list(group_desks)

        def emit_hash_for_desk(d):
            desk = desks[d]
        ...
        def emit_hash_interleaved(group_desks):
            gd = reorder_desks(group_desks)
            hp = self.hash_pattern

            if hp == 'per_desk':
                for d in gd:
                    emit_hash_for_desk(d)
            elif hp == 'stage_interleaved':
                # Original pattern
                for d in gd:
                    desk = desks[d]
                    self.emit("valu", ("multiply_add", desk['val'], desk['val'], v_fma_mult[0], v_hash_consts[0]))
                ...
                # Pairs of desks, stage-interleaved within pair
                ...
                # First half stages 0-2 for all, then stages 3-5 for all
                for d in gd:
                    desk = desks[d]
                    self.emit("valu", ("multiply_add", desk['val'], desk['val'], v_fma_mult[0], v_hash_consts[0]))
                    self.emit("valu", ("^", desk['tmp1'], desk['val'], v_hash_consts[1]))
                ...
                # Alternate between desks: d0-s0, d1-s0, d0-s1, d1-s1, ...
                if len(gd) >= 2:
                    d0, d1 = gd[0], gd[1]
                    rest = gd[2:]
                    desk0 = desks[d0]
        ...
        def emit_rounds_0_1_2_fused(group_desks):
            ...
            for d in group_desks:
                ...
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            emit_hash_interleaved(group_desks)
            br_order = list(group_desks) if self.branch_order == 'forward' else list(reversed(group_desks))
            for d in br_order:
                emit_branch(d)
...
def run_test(hash_pattern='per_desk', desk_order='interleave', branch_order='forward', check=True):
    random.seed(123)
    forest = Tree.generate(10)
    inp = Input.generate(forest, 256, 16)
    mem = build_mem_image(forest, inp)

    kb = KernelBuilder(hash_pattern=hash_pattern, desk_order=desk_order, branch_order=branch_order)
...
if __name__ == "__main__":
    hash_patterns = ['per_desk', 'stage_interleaved', 'pairs_stage', 'half_stage', 'alternate']
    desk_orders = ['forward', 'interleave', 'reverse']
    branch_orders = ['forward', 'reverse']

    print(f"{'Hash Pattern':<20} | {'Desk Order':<12} | {'Branch':<8} | {'Cycles':>6} | {'vs A1':>5} | OK?")
    print("-" * 75)

    best = (None, None, None, 99999)
    wins = []

    for hp in hash_patterns:
        for do in desk_orders:
            for bo in branch_orders:
                try:
                    cycles, correct = run_test(hp, do, bo, check=True)
                    delta = 1548 - cycles
                    status = "YES" if correct else "NO"
                    marker = " **WIN**" if delta > 0 and correct else ""
                    if correct and cycles < best[3]:
                        best = (hp, do, bo, cycles)
                    if delta > 0 and correct:
                        wins.append((hp, do, bo, cycles, delta))
                    print(f"{hp:<20} | {do:<12} | {bo:<8} | {cycles:>6} | {delta:>+5} | {status:>3}{marker}")
                except Exception as e:
                    print(f"{hp:<20} | {do:<12} | {bo:<8} | {'ERROR':>6} | {'N/A':>5} | ERR: {str(e)[:30]}")

    print(f"\nBest: {best}")
    print(f"\n=== WINS ({len(wins)}) ===")
    for hp, do, bo, cycles, delta in sorted(wins, key=lambda x: x[3]):
        print(f"  {hp}/{do}/{bo} -> {cycles} (improvement: {delta})")
