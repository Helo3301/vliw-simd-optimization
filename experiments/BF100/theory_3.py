...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_gather_round_interleaved(group_desks):
            # Theory 3: Emit XOR first (before addr for next desk in group),
            # then hash, then branch - reversed from addr/load/XOR/hash/branch
            # Actually: emit addr+loads together, then XOR+hash+branch reversed across desks
            for d in group_desks:
                desk = desks[d]
            ...
            # Emit XOR for all desks, hash, branch in reverse desk order
