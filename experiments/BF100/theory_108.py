...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            # Theory 108: Process pairs of groups, interleaving their rounds
            for gi in range(0, len(all_groups), 2):
                g0 = all_groups[gi]
                g1 = all_groups[gi + 1] if gi + 1 < len(all_groups) else None

                # Fused R0+1+2 for both groups
                emit_rounds_0_1_2_fused(g0)
                if g1:
                    emit_rounds_0_1_2_fused(g1)

                # Gather rounds 3-9: interleave between groups
                for _rnd in range(3, 10):
                    emit_gather_round_interleaved(g0)
                    if g1:
                        emit_gather_round_interleaved(g1)

                # R10
                emit_round_10_optimized(g0)
                if g1:
                    emit_round_10_optimized(g1)

                # R11+12+13
                emit_rounds_11_12_13_fused(g0)
                if g1:
                    emit_rounds_11_12_13_fused(g1)
                ...
                # R15
                emit_round_15_final_interleaved(g0)
                if g1:
                    emit_round_15_final_interleaved(g1)
