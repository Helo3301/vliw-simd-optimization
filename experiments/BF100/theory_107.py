...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_rounds_0_1_2_fused(group_desks):
            ...
            # === Round 2 === (vselect replaced with arithmetic)
            for d in group_desks:
                desk = desks[d]
                ...
                # Replace vselect with: node_val = node_val + bit0 * (tmp2 - node_val)
                self.emit("valu", ("-", desk['tmp2'], desk['tmp2'], desk['node_val']))  # tmp2 = tmp2 - node_val
                self.emit("valu", ("multiply_add", desk['node_val'], desk['bit0'], desk['tmp2'], desk['node_val']))  # node = bit0*diff + node
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            emit_hash_interleaved(group_desks)
        ...
        def emit_rounds_11_12_13_fused(group_desks):
            ...
            # === Round 13 === (vselect replaced with arithmetic)
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("multiply_add", desk['node_val'], desk['tmp1'], v_diff_3_4, v_tree[3]))
                self.emit("valu", ("multiply_add", desk['tmp2'], desk['tmp1'], v_diff_5_6, v_tree[5]))
                self.emit("valu", ("-", desk['tmp2'], desk['tmp2'], desk['node_val']))
                self.emit("valu", ("multiply_add", desk['node_val'], desk['bit0'], desk['tmp2'], desk['node_val']))
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            emit_hash_interleaved(group_desks)
