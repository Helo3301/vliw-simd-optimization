"""
ADV2-1: Bit-History Tracking in Gather Rounds

This experiment tests the optimization identified in Adversarial Agent 2's analysis:

Instead of computing idx = 2*idx + 1 + bit (3 ops: AND + FMA + ADD),
use bit_history = (bit_history << 1) | bit (2 ops: SHIFT + OR)
and compute addr = addr_base_L + bit_history (1 op: ADD)

This saves 1 op per gather round per desk.

Expected savings: 9 gather rounds * 32 desks * 1 op = 288 ops = ~48 cycles

Starting from B4-2 baseline (1,558 cycles), target: ~1,510 cycles
"""
...
class KernelBuilderADV2:
    """ADV2: Bit-history tracking optimization"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Precompute addr_base for each level (forest_p + (2^L - 1))
        # Level 3: forest_p + 7
        # Level 4: forest_p + 15
        # etc.
        v_addr_bases = []
        for level in range(3, 12):  # Levels 3-11 (rounds 3-10, 14-15)
            offset = (1 << level) - 1
            v_base = self.alloc_vec(f"v_addr_base_{level}")
            self.emit("valu", ("+", v_base, v_forest_p, self.scratch_vconst(offset, f"v_offset_{level}")))
            v_addr_bases.append(v_base)
        ...
        for d in range(NUM_DESKS):
            desk = {
                ...
                'bit_history': self.alloc_vec(f"v_bithist_{d}"),  # NEW: track bit history
            }
            desks.append(desk)
        ...
        def emit_branch_bithist(desk_idx):
            """Update bit_history = (bit_history << 1) | bit"""
            d = desks[desk_idx]
            self.emit("valu", ("&", d['tmp1'], d['val'], v_one))  # bit
            self.emit("valu", ("<<", d['bit_history'], d['bit_history'], v_one))  # shift
            self.emit("valu", ("|", d['bit_history'], d['bit_history'], d['tmp1']))  # or

        def emit_bounds_from_bithist(desk_idx, level):
            """Compute idx from bit_history and check bounds"""
            d = desks[desk_idx]
            idx_offset = (1 << level) - 1
            # idx = idx_offset + bit_history
            self.emit("valu", ("+", d['idx'], d['bit_history'], self.scratch_vconst(idx_offset)))
            # bounds check
        ...
        def emit_rounds_0_1_2_fused(group_desks):
            """Rounds 0+1+2 fused with bit tracking"""
            # === Round 0 ===
            for d in group_desks:
            ...
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("&", desk['tmp1'], desk['val'], v_one))  # bit1
            ...
            # Initialize bit_history from bits and extract bit2
            # bit_history = 4*bit0 + 2*bit1 + bit2
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("&", desk['tmp2'], desk['val'], v_one))  # bit2
                # bit_history = 2*bit0 + bit1 (stored in tmp1)
                self.emit("valu", ("multiply_add", desk['bit_history'], desk['bit0'], v_two, desk['tmp1']))
                # bit_history = 2*bit_history + bit2 = 4*bit0 + 2*bit1 + bit2
                self.emit("valu", ("multiply_add", desk['bit_history'], desk['bit_history'], v_two, desk['tmp2']))

        def emit_gather_round_bithist(group_desks, level):
            """Gather round using bit_history for addressing

            At level L, the tree indices are in range [2^L - 1, 2^(L+1) - 2].
            bit_history encodes the path from root, which maps directly to the offset
            within this level's range.

            addr = forest_p + (2^L - 1) + bit_history
            """
            # For this implementation, we compute idx then addr to stay compatible
            # with bounds checking. The optimization is in using shift+OR for branch.
            for d in group_desks:
                desk = desks[d]
                idx_offset = (1 << level) - 1
                # idx = idx_offset + bit_history
                self.emit("valu", ("+", desk['idx'], desk['bit_history'], self.scratch_vconst(idx_offset)))
                # addr = forest_p + idx
            ...
            emit_hash_interleaved(group_desks)
            for d in group_desks:
                emit_branch_bithist(d)

        def emit_round_10_bithist(group_desks):
            """Round 10: gather + bounds using bit_history"""
            # At round 10, we're at level 10 (gathering from nodes at that level)
            for d in group_desks:
                desk = desks[d]
                idx_offset = (1 << 10) - 1  # 1023
                self.emit("valu", ("+", desk['idx'], desk['bit_history'], self.scratch_vconst(idx_offset)))
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
            for d in group_desks:
                desk = desks[d]
            ...
            for d in group_desks:
                emit_branch_bithist(d)
            # Bounds check: after the branch, compute idx and check bounds
            for d in group_desks:
                desk = desks[d]
                # idx = (2^11 - 1) + bit_history
                idx_offset = (1 << 11) - 1  # 2047
                self.emit("valu", ("+", desk['idx'], desk['bit_history'], self.scratch_vconst(idx_offset)))
                # bounds check
                self.emit("valu", ("<", desk['tmp1'], desk['idx'], v_n_nodes))
                self.emit("valu", ("*", desk['idx'], desk['idx'], desk['tmp1']))

        def emit_rounds_11_12_13_fused(group_desks):
            """Rounds 11+12+13 fused (same as 0+1+2 after wrap)"""
            # === Round 11 ===
            for d in group_desks:
                emit_xor_with_node(d, v_tree[0])
            emit_hash_interleaved(group_desks)
            ...
            # Initialize bit_history from bits for rounds 14+
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("&", desk['tmp2'], desk['val'], v_one))  # bit13
                self.emit("valu", ("multiply_add", desk['bit_history'], desk['bit0'], v_two, desk['tmp1']))
                self.emit("valu", ("multiply_add", desk['bit_history'], desk['bit_history'], v_two, desk['tmp2']))

        def emit_round_14_bithist(group_desks):
            """Round 14: gather using bit_history"""
            # After rounds 11-13 (reset to 0 then 3 branches), we're at level 3
            # Gathering from level 3 nodes
            for d in group_desks:
                desk = desks[d]
                idx_offset = (1 << 3) - 1  # 7
                self.emit("valu", ("+", desk['idx'], desk['bit_history'], self.scratch_vconst(idx_offset)))
        ...
        def emit_round_15_final_bithist(group_desks):
            """Round 15: final gather (no branch stored, but we need idx for output)"""
            # After round 14 branch, we're at level 4
            for d in group_desks:
                desk = desks[d]
                idx_offset = (1 << 4) - 1  # 15
                self.emit("valu", ("+", desk['idx'], desk['bit_history'], self.scratch_vconst(idx_offset)))
            ...
            for d in group_desks:
                ...
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            emit_hash_interleaved(group_desks)
            # No branch needed for final round

            # Need to compute final idx for storage
            for d in group_desks:
                desk = desks[d]
                # idx = (1 << 6) - 1 + bit_history after round 15 branch would give
                # But we skip the branch. So idx = current state.
                # Actually, the final idx stored is AFTER round 15's branch would be computed
                # But H133 says we skip this. Let me check what idx should be stored.
                #
                # Looking at reference_kernel: idx is updated AFTER the branch
                # So after round 15, idx = 2*idx + 1 + bit
                # We need to compute this for correctness, but not use it for gather
                self.emit("valu", ("&", desk['tmp1'], desk['val'], v_one))  # bit
                # Compute idx from bit_history: current level is 5
                self.emit("valu", ("+", desk['idx'], desk['bit_history'], self.scratch_vconst(31)))  # idx_base_5 = 31
                # Now compute next idx
                self.emit("valu", ("multiply_add", desk['idx'], desk['idx'], v_two, v_one))  # 2*idx + 1
                self.emit("valu", ("+", desk['idx'], desk['idx'], desk['tmp1']))  # + bit
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                ...
                # Rounds 3-9: gather with bit_history
                for level in range(3, 10):  # Levels 3-9
                    emit_gather_round_bithist(group_desks, level)

                # Round 10: bounds check
                emit_round_10_bithist(group_desks)

                # Fused rounds 11+12+13
                emit_rounds_11_12_13_fused(group_desks)

                # Round 14
                emit_round_14_bithist(group_desks)

                # Round 15 final
                emit_round_15_final_bithist(group_desks)
