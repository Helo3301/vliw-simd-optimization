"""
BF8 Theory 74: Process 2 desks in lockstep
Hypothesis: Tight lockstep of 2 desks may reduce scheduling overhead.
"""
...
class KernelBuilderBF8_74:
    """BF8 Theory 74: 2 desks in lockstep"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # THEORY 74: Use 2 desks in lockstep
        NUM_DESKS = 16
        GROUP_SIZE = 2  # 8 groups of 2
        ...
        # LOCKSTEP: emit operations for both desks in pair simultaneously
        def emit_hash_lockstep(d0, d1):
            desk0, desk1 = desks[d0], desks[d1]
            # Stage 0 - FMA
            self.emit("valu", ("multiply_add", desk0['val'], desk0['val'], v_fma_mult[0], v_hash_consts[0]))
            self.emit("valu", ("multiply_add", desk1['val'], desk1['val'], v_fma_mult[0], v_hash_consts[0]))
            # Stage 1 - XOR/shift
            self.emit("valu", ("^", desk0['tmp1'], desk0['val'], v_hash_consts[1]))
            self.emit("valu", ("^", desk1['tmp1'], desk1['val'], v_hash_consts[1]))
            self.emit("valu", (">>", desk0['tmp2'], desk0['val'], v_hash_shifts[1]))
            self.emit("valu", (">>", desk1['tmp2'], desk1['val'], v_hash_shifts[1]))
            self.emit("valu", ("^", desk0['val'], desk0['tmp1'], desk0['tmp2']))
            self.emit("valu", ("^", desk1['val'], desk1['tmp1'], desk1['tmp2']))
            # Stage 2 - FMA
            self.emit("valu", ("multiply_add", desk0['val'], desk0['val'], v_fma_mult[2], v_hash_consts[2]))
            self.emit("valu", ("multiply_add", desk1['val'], desk1['val'], v_fma_mult[2], v_hash_consts[2]))
            # Stage 3 - ADD/shift
            self.emit("valu", ("+", desk0['tmp1'], desk0['val'], v_hash_consts[3]))
            self.emit("valu", ("+", desk1['tmp1'], desk1['val'], v_hash_consts[3]))
            self.emit("valu", ("<<", desk0['tmp2'], desk0['val'], v_hash_shifts[3]))
            self.emit("valu", ("<<", desk1['tmp2'], desk1['val'], v_hash_shifts[3]))
            self.emit("valu", ("^", desk0['val'], desk0['tmp1'], desk0['tmp2']))
            self.emit("valu", ("^", desk1['val'], desk1['tmp1'], desk1['tmp2']))
            # Stage 4 - FMA
            self.emit("valu", ("multiply_add", desk0['val'], desk0['val'], v_fma_mult[4], v_hash_consts[4]))
            self.emit("valu", ("multiply_add", desk1['val'], desk1['val'], v_fma_mult[4], v_hash_consts[4]))
            # Stage 5 - XOR/shift
            self.emit("valu", ("^", desk0['tmp1'], desk0['val'], v_hash_consts[5]))
            self.emit("valu", ("^", desk1['tmp1'], desk1['val'], v_hash_consts[5]))
            self.emit("valu", (">>", desk0['tmp2'], desk0['val'], v_hash_shifts[5]))
            self.emit("valu", (">>", desk1['tmp2'], desk1['val'], v_hash_shifts[5]))
            self.emit("valu", ("^", desk0['val'], desk0['tmp1'], desk0['tmp2']))
            self.emit("valu", ("^", desk1['val'], desk1['tmp1'], desk1['tmp2']))

        def emit_branch_lockstep(d0, d1):
            desk0, desk1 = desks[d0], desks[d1]
            self.emit("valu", ("&", desk0['tmp1'], desk0['val'], v_one))
            self.emit("valu", ("&", desk1['tmp1'], desk1['val'], v_one))
            self.emit("valu", ("multiply_add", desk0['idx'], desk0['idx'], v_two, v_one))
            self.emit("valu", ("multiply_add", desk1['idx'], desk1['idx'], v_two, v_one))
            self.emit("valu", ("+", desk0['idx'], desk0['idx'], desk0['tmp1']))
            self.emit("valu", ("+", desk1['idx'], desk1['idx'], desk1['tmp1']))

        def emit_rounds_0_1_2_lockstep(d0, d1):
            desk0, desk1 = desks[d0], desks[d1]
            # Round 0 XOR
            self.emit("valu", ("^", desk0['val'], desk0['val'], v_tree[0]))
            self.emit("valu", ("^", desk1['val'], desk1['val'], v_tree[0]))
            emit_hash_lockstep(d0, d1)
            self.emit("valu", ("&", desk0['bit0'], desk0['val'], v_one))
            self.emit("valu", ("&", desk1['bit0'], desk1['val'], v_one))
            self.emit("valu", ("+", desk0['idx'], v_one, desk0['bit0']))
            self.emit("valu", ("+", desk1['idx'], v_one, desk1['bit0']))

            # Round 1
            self.emit("valu", ("multiply_add", desk0['node_val'], desk0['bit0'], v_diff_1_2, v_tree[1]))
            self.emit("valu", ("multiply_add", desk1['node_val'], desk1['bit0'], v_diff_1_2, v_tree[1]))
            self.emit("valu", ("^", desk0['val'], desk0['val'], desk0['node_val']))
            self.emit("valu", ("^", desk1['val'], desk1['val'], desk1['node_val']))
            emit_hash_lockstep(d0, d1)
            self.emit("valu", ("&", desk0['tmp1'], desk0['val'], v_one))
            self.emit("valu", ("&", desk1['tmp1'], desk1['val'], v_one))
            self.emit("valu", ("multiply_add", desk0['idx'], desk0['bit0'], v_two, v_three))
            self.emit("valu", ("multiply_add", desk1['idx'], desk1['bit0'], v_two, v_three))
            self.emit("valu", ("+", desk0['idx'], desk0['idx'], desk0['tmp1']))
            self.emit("valu", ("+", desk1['idx'], desk1['idx'], desk1['tmp1']))

            # Round 2
            self.emit("valu", ("multiply_add", desk0['node_val'], desk0['tmp1'], v_diff_3_4, v_tree[3]))
            self.emit("valu", ("multiply_add", desk1['node_val'], desk1['tmp1'], v_diff_3_4, v_tree[3]))
            self.emit("valu", ("multiply_add", desk0['tmp2'], desk0['tmp1'], v_diff_5_6, v_tree[5]))
            self.emit("valu", ("multiply_add", desk1['tmp2'], desk1['tmp1'], v_diff_5_6, v_tree[5]))
            self.emit("flow", ("vselect", desk0['node_val'], desk0['bit0'], desk0['tmp2'], desk0['node_val']))
            self.emit("flow", ("vselect", desk1['node_val'], desk1['bit0'], desk1['tmp2'], desk1['node_val']))
            self.emit("valu", ("^", desk0['val'], desk0['val'], desk0['node_val']))
            self.emit("valu", ("^", desk1['val'], desk1['val'], desk1['node_val']))
            emit_hash_lockstep(d0, d1)
            emit_branch_lockstep(d0, d1)

        def emit_rounds_11_12_13_lockstep(d0, d1):
            desk0, desk1 = desks[d0], desks[d1]
            # Round 11
            self.emit("valu", ("^", desk0['val'], desk0['val'], v_tree[0]))
            self.emit("valu", ("^", desk1['val'], desk1['val'], v_tree[0]))
        ...
        def emit_gather_round_lockstep(d0, d1):
            desk0, desk1 = desks[d0], desks[d1]
            self.emit("valu", ("+", desk0['addr'], v_forest_p, desk0['idx']))
            self.emit("valu", ("+", desk1['addr'], v_forest_p, desk1['idx']))
            for lane in range(VLEN):
                self.emit("load", ("load", desk0['node_val'] + lane, desk0['addr'] + lane))
                self.emit("load", ("load", desk1['node_val'] + lane, desk1['addr'] + lane))
            self.emit("valu", ("^", desk0['val'], desk0['val'], desk0['node_val']))
            self.emit("valu", ("^", desk1['val'], desk1['val'], desk1['node_val']))
            emit_hash_lockstep(d0, d1)
            emit_branch_lockstep(d0, d1)

        def emit_round_10_lockstep(d0, d1):
            desk0, desk1 = desks[d0], desks[d1]
            self.emit("valu", ("+", desk0['addr'], v_forest_p, desk0['idx']))
            self.emit("valu", ("+", desk1['addr'], v_forest_p, desk1['idx']))
            for lane in range(VLEN):
            ...
            self.emit("valu", ("^", desk1['val'], desk1['val'], desk1['node_val']))
            emit_hash_lockstep(d0, d1)
            self.emit("valu", ("^", desk0['idx'], desk0['idx'], desk0['idx']))
            self.emit("valu", ("^", desk1['idx'], desk1['idx'], desk1['idx']))

        def emit_round_15_lockstep(d0, d1):
            desk0, desk1 = desks[d0], desks[d1]
            self.emit("valu", ("+", desk0['addr'], v_forest_p, desk0['idx']))
            self.emit("valu", ("+", desk1['addr'], v_forest_p, desk1['idx']))
            for lane in range(VLEN):
        ...
        def emit_tile_lockstep(tile_idx):
            ...
            for pair_idx in range(NUM_DESKS // GROUP_SIZE):
                d0 = pair_idx * 2
                d1 = pair_idx * 2 + 1

                emit_rounds_0_1_2_lockstep(d0, d1)
                for _rnd in range(3, 10):
                    emit_gather_round_lockstep(d0, d1)
                emit_round_10_lockstep(d0, d1)
                emit_rounds_11_12_13_lockstep(d0, d1)
                emit_gather_round_lockstep(d0, d1)
                emit_round_15_lockstep(d0, d1)

            for d in range(NUM_DESKS):
                self.emit("store", ("vstore", addr_tmp[d*2], desks[d]['idx']))
                self.emit("store", ("vstore", addr_tmp[d*2+1], desks[d]['val']))

        emit_tile_lockstep(0)
        emit_tile_lockstep(1)
