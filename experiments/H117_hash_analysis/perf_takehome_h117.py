"""
# Experiment H117: Hash Constants Analysis

**GOAL:** Analyze HASH_STAGES constants to find potential savings through deduplication.

**HYPOTHESIS:** We allocate 6 v_hash_consts and 3 v_hash_shifts (9 vectors = 72 slots).
Check if any constants repeat or can be derived from others.

**ANALYSIS RESULTS:**
  HASH_STAGES constants:
    Stage 0: val1=0x7ED55D16, val3=12 (FMA stage, mult=4097)
    Stage 1: val1=0xC761C23C, val3=19
    Stage 2: val1=0x165667B1, val3=5  (FMA stage, mult=33)
    Stage 3: val1=0xD3A2646C, val3=9
    Stage 4: val1=0xFD7046C5, val3=3  (FMA stage, mult=9)
    Stage 5: val1=0xB55A4F09, val3=16

  Key finding:
    - v_hash_shift_3 = 9 (stage 3 shift amount)
    - v_fma_mult_4 = 9 (stage 4 FMA multiplier)
    - THESE ARE THE SAME VALUE!

  ALREADY OPTIMIZED: The existing scratch_vconst() method handles deduplication:
    - When stage 3 calls scratch_vconst(9), it creates the vector
    - When stage 4 calls scratch_vconst(9), it reuses the same vector
    - No additional savings possible

  Total unique vector constants actually allocated:
    - 6 v_hash_consts (unique large values)
    - 2 v_hash_shifts (19, 16 - value 9 is shared with fma_mult_4)
    - 2 v_fma_mult (4097, 33 - value 9 is shared with hash_shift_3)
    = 10 vectors instead of 12 (already saving 16 slots)

**BASELINE:** H105 = 1,843 cycles
**CONCLUSION:** No additional savings possible - deduplication already working.
"""
...
class KernelBuilderH117:
    """
    H117: Hash constants analysis - verifying deduplication is already working.
    Based on H105 with added analysis print statements.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        # H117: Hash constants analysis
        print("=" * 60)
        print("H117: HASH_STAGES Constants Analysis")
        print("=" * 60)
        print("\nHASH_STAGES values:")
        for hi, (op1, val1, op2, op3, val3) in enumerate(HASH_STAGES):
            print(f"  Stage {hi}: op1={op1}, val1=0x{val1:08X}, op2={op2}, op3={op3}, val3={val3}")
        ...
        print("\nVector constant allocation:")
        vconst_count_before = len(self.vconst_map)

        for hi, (op1, val1, op2, op3, val3) in enumerate(HASH_STAGES):
            v_const = self.scratch_vconst(val1, f"v_hash_const_{hi}")
            v_hash_consts.append(v_const)
            is_new = len(self.vconst_map) > vconst_count_before + hi
            print(f"  v_hash_const_{hi} = 0x{val1:08X} -> addr {v_const}")

            if hi in FMA_MULTIPLIERS:
                mult = FMA_MULTIPLIERS[hi]
                before = len(self.vconst_map)
                v_fma_mult[hi] = self.scratch_vconst(mult, f"v_fma_mult_{hi}")
                is_reused = len(self.vconst_map) == before
                reuse_note = " (REUSED from v_hash_shift_3!)" if is_reused and mult == 9 else ""
                print(f"  v_fma_mult_{hi} = {mult} -> addr {v_fma_mult[hi]}{reuse_note}")
                ...
                print(f"  v_hash_shift_{hi} = {val3} -> addr {v_shift}{reuse_note}")
                v_hash_shifts.append(v_shift)

        print(f"\nTotal unique vector constants in vconst_map: {len(self.vconst_map)}")
        print("  (Expected 10 if deduplication working: 6 hash consts + 2 fma mults + 2 shifts)")
        print("  (Would be 12 without deduplication: 6 hash consts + 3 fma mults + 3 shifts)")
        print("=" * 60)
        ...
        NUM_PRELOADED = 7  # Changed from 15
        v_tree = []
        ...
        print(f"  - H105: Reduced preloading from 15 to 7 nodes (saved 64 slots)")
