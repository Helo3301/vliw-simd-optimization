"""
# Experiment H87: Double-Pumped Hash Stages

**GOAL:** Overlap hash computations between pairs of desks by interleaving FMA and multi-op stages.

**HYPOTHESIS:** Hash stages 0, 2, and 4 use FMA (single op), but stages 1, 3, 5 use 3 ops each.
By batching operations of the same type across all desks, we can maximize VALU slot utilization.

**KEY INSIGHT:** The scheduler already overlaps operations between desks when there are no
dependencies. The key is to emit operations in an order that avoids temp register conflicts
while exposing maximum parallelism.

**KEY CHANGE:** Modified emit pattern to batch operations by type across all desks:
1. Emit all XOR operations for all desks
2. Emit FMA stage 0 for all desks
3. For 3-op stages, process even desks (0,2,4,...) then odd desks (1,3,5,...) to avoid
   temp register conflicts (consecutive pairs share temps)
4. Emit all branch operations for all desks

This "even-then-odd" pattern allows 8 independent operations to be emitted together,
maximizing the scheduler's ability to fill VALU slots (limit: 6 per cycle).
...
- H87 double-pump: 1,851 cycles
- Improvement: 47 cycles (2.5% faster)

**ANALYSIS:** The improvement comes from better instruction packing. By batching operations
of the same type, the scheduler can fill more VALU slots per cycle. The even-then-odd
pattern for temp-using operations avoids conflicts while maintaining high parallelism.
"""
