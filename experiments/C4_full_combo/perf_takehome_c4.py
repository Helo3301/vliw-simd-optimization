"""
# Experiment C4: Full Combo - H12 + H11 + H13 + H14

This is the ultimate combination of all winning optimizations from round 3:

**H12 (Round Fusion)**: Process 2 rounds without intermediate memory writeback
- Eliminates 50% of vload/vstore pairs per desk
- Base: 5,115 cycles

**H11 (Branch FMA)**: Use multiply_add for branch computation
- multiply_add(idx, v_two, v_one) replaces idx*2 followed by idx+1
- Saves 1 cycle per desk per iteration
- Base: 5,819 cycles

**H13 (Store Coalescing)**: Overlap stores with ALU operations
- Batch stores with loop control and address computation
- Base: 5,435 cycles

**H14 (Address Pipeline)**: Pre-compute addresses with ALU while VALU computes hashes
- Use idle ALU slots during VALU-heavy phases
- Base: 5,307 cycles

**Combined Strategy:**
1. Start with H12's round fusion structure (2 rounds per iteration)
2. Layer in H11's branch FMA optimization for all desks
3. Add H13's store coalescing at the end of double-rounds
4. Integrate H14's address pipelining throughout

**Expected:** Maximum cycle reduction while maintaining correctness.
"""
...
class KernelBuilderC4:
    """
    C4: Full Combination of H12 + H11 + H13 + H14
