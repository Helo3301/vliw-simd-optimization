"""
# Experiment H119: 16 Desks x 2 Tiles with Reduced Tree Preloading

**GOAL:** Test whether H105's improvement came from reduced preload (7 nodes) or 32 desks.

**HYPOTHESIS:** 32 desks in one tile might not be optimal. Try 16 desks x 2 tiles
(like H87) but WITH reduced preload (7 nodes from H105). This tests if H105's
improvement was from reduced preload rather than the desk count.

**KEY CHANGES FROM H105:**
- NUM_DESKS = 16 (was 32)
- Process 2 tiles, each handling 128 elements (was 1 tile, 256 elements)
- 2-desk temp sharing (was 4-desk sharing due to scratch constraints)
- Keep reduced tree preload: 7 nodes (from H105)
...
class KernelBuilderH119:
    """
    H119: 16 desks x 2 tiles with reduced tree preloading (7 nodes).
    Tests hypothesis that H105's improvement was from reduced preload, not desk count.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel using 16 desks per tile with reduced tree preloading (7 nodes).
        Process 2 tiles to cover all 256 batch elements.
        """
        ...
        # H119: Reduced tree preload (7 nodes from H105)
        ...
        NUM_PRELOADED = 7  # Keep reduced preload from H105
        v_tree = []
        ...
        # H119: 16 desks (like H87) with 2-desk temp sharing
        NUM_DESKS = 16
        desks = []
        ...
        print(f"  - H119: 16 desks x 2 tiles with reduced preloading (7 nodes)")
        print(f"  - 2-desk temp sharing (16 shared temps)")

        # Pause before main computation
        self.emit("flow", ("pause",))

        # Even/odd desk groupings for temp conflict avoidance (2-desk sharing)
        ...
        def emit_hash_stages_all_desks():
            """Emit hash stages for all 16 desks in a double-pumped fashion."""
        ...
        def emit_tile(tile_idx):
            """Emit all operations for one tile (128 elements = 16 desks * 8 lanes)."""
        ...
        # H119: Emit both tiles (2 tiles x 16 desks x 8 lanes = 256 elements)
        emit_tile(0)
        emit_tile(1)
