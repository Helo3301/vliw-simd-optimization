"""
Theory 43: Use load_offset instead of load

Hypothesis: The load_offset instruction may have different scheduling properties
that could improve performance when gathering tree values.
...
def _slot_rw(engine: str, slot: tuple) -> tuple[list[int], list[int]]:
    ...
    if engine == "alu":
        ...
        if op == "load":
            ...
            reads = [addr + lane]  # Read from addr+lane
            writes = [dest + lane]  # Write to dest+lane
        else:
...
class KernelBuilderT43:
    """T43: Use load_offset instead of load"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # T43: Use load_offset instead of load
        def emit_gather_round_load_offset(group_desks):
            for d in group_desks:
                desk = desks[d]
            ...
            for d in group_desks:
                ...
                for lane in range(VLEN):
                    # Use load_offset: loads from scratch[addr+lane] into dest+lane
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                ...
                for _rnd in range(3, 10):
                    emit_gather_round_load_offset(group_desks)
                emit_round_10_optimized(group_desks)
                emit_rounds_11_12_13_fused(group_desks)
                emit_gather_round_load_offset(group_desks)  # Round 14
