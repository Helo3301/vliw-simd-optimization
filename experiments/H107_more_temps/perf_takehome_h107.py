"""
# Experiment H107: More Shared Temps

**GOAL:** Increase NUM_SHARED_TEMPS from 16 to 24 (4 temps per 4-desk group instead of 2).

**HYPOTHESIS:** H105 freed up 64 scratch slots by reducing tree preload from 15 to 7 nodes.
Now at 1,431/1,536 slots used. With 105 free slots, we can afford 24 shared temps (192 slots)
instead of 16 (128 slots), using an extra 64 slots.

With 4 temps per group instead of 2, we can reduce WAW hazards in hash stages by using
different temp registers for different operations, allowing more parallelism.
...
class KernelBuilderH107:
    """
    H107: More shared temps - 24 instead of 16 (4 temps per 4-desk group instead of 2).
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel using 32 desks per tile with more shared temps.
        """
        ...
        # H107 CHANGE: SHARED TEMP REGISTERS - Use 24 vectors (192 slots) instead of 16 (128 slots)
        # Every 4 consecutive desks share 3 temps (tmp1, tmp2, tmp3)
        # This allows more parallelism by having independent temps for different hash stage ops
        NUM_SHARED_TEMPS = 24  # Changed from 16: 8 groups * 3 temps per group
        ...
        def get_temps(desk_idx):
            ...
            H107: 4 desks share 3 temps to reduce WAW hazards.
            """
        ...
        print(f"  - H107: 32 desks with 4-desk temp sharing (24 shared temps = 3 per group)")
        ...
        def emit_hash_stages_all_desks():
            ...
            H107: Use 3 temps per group to reduce WAW hazards in hash stages.
            ...
            # Stage 1 (3 ops) - use tmp1 and tmp2, all groups can run in parallel
            # With 3 temps per group, each group is independent
            for group in [group_0, group_1, group_2, group_3]:
                for d in group:
                    tmp1, tmp2, tmp3 = get_temps(d)
            ...
            # Stage 3 (3 ops) - use tmp1 and tmp2
            for group in [group_0, group_1, group_2, group_3]:
                for d in group:
                    tmp1, tmp2, tmp3 = get_temps(d)
            ...
            # Stage 5 (3 ops) - use tmp1 and tmp2
            for group in [group_0, group_1, group_2, group_3]:
                for d in group:
                    tmp1, tmp2, tmp3 = get_temps(d)
        ...
        def emit_tile():
            ...
            # H107: Use tmp3 as additional temp to reduce dependencies
            for d in range(NUM_DESKS):
                tmp1, tmp2, tmp3 = get_temps(d)
                ...
                self.emit("valu", (">>", tmp3, tmp1, v_one))  # bit1 = tmp1 >> 1 (use tmp3 instead of addr)
                ...
                self.emit("valu", ("multiply_add", desks[d]['node_val'], tmp3, tmp2, desks[d]['node_val']))  # result
                # XOR and hash
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))
                emit_hash_stages(d)
                emit_branch(d)
            ...
            # H107: Use tmp3 as additional temp
            for d in range(NUM_DESKS):
                tmp1, tmp2, tmp3 = get_temps(d)
                # Extract selection bits: tmp = idx - 3 gives {0, 1, 2, 3}
                self.emit("valu", ("-", tmp1, desks[d]['idx'], v_three))  # tmp1 = idx - 3
                self.emit("valu", ("&", tmp2, tmp1, v_one))   # bit0 = tmp1 & 1
                self.emit("valu", (">>", tmp3, tmp1, v_one))  # bit1 = tmp1 >> 1 (use tmp3)
                # Select from low pair (tree[3] or tree[4]) using precomputed diff
                self.emit("valu", ("multiply_add", desks[d]['node_val'], tmp2, v_diff_3_4, v_tree[3]))  # low_pair
                # Select from high pair (tree[5] or tree[6]) using precomputed diff
                self.emit("valu", ("multiply_add", tmp1, tmp2, v_diff_5_6, v_tree[5]))  # high_pair in tmp1
