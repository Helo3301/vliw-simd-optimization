"""
BF3 Theory 24: Combine stages 1+3 into single XOR chain

Stages 1 and 3 both use XOR/shift patterns. Try to combine them.
Stage 1: val = (val ^ const1) ^ (val >> 19)
Stage 3: val = (val + const3) ^ (val << 9)

This tests whether we can combine XOR stages.
Expected: INCORRECT - stage 2 FMA is in between and must be preserved.
"""
...
class KernelBuilderT24:
    """T24: Combine stages 1+3 XOR chain"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # MODIFIED HASH: Combine stages 1+3 (skipping stage 2)
        # NOTE: This is WRONG as it skips the FMA in stage 2
        def emit_hash_interleaved_combined_13(group_desks):
            # Stage 0 with FMA
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("multiply_add", desk['val'], desk['val'], v_fma_mult[0], v_hash_consts[0]))

            # Combined stage 1+3: apply both XOR patterns
            # NOTE: This skips stage 2 FMA which is WRONG
            v_combined_const = self.scratch_vconst((0xC761C23C ^ 0xD3A2646C) % (2**32), "v_combined_13")
            for d in group_desks:
                desk = desks[d]
                # Try combined XOR
                self.emit("valu", ("^", desk['tmp1'], desk['val'], v_combined_const))
                self.emit("valu", (">>", desk['tmp2'], desk['val'], v_hash_shifts[1]))
                self.emit("valu", ("<<", desk['addr'], desk['val'], v_hash_shifts[3]))  # reuse addr
                self.emit("valu", ("^", desk['val'], desk['tmp1'], desk['tmp2']))
                self.emit("valu", ("^", desk['val'], desk['val'], desk['addr']))
        ...
        def emit_rounds_0_1_2_fused(group_desks):
            ...
            emit_hash_interleaved_combined_13(group_desks)
            for d in group_desks:
                desk = desks[d]
...
if __name__ == "__main__":
    ...
    print(f"T24 Combined 1+3 XOR: Cycles={cycles}, Correct={correct}")
