"""
# Experiment H22: Level-Aware Gathering (Simplified)

**CRITICAL DISCOVERY:** All batch elements are at the SAME tree level during each round!
...
**Simplified Approach:**
Start with C4 baseline but optimize just the first double-round (rounds 0-1)
where we can avoid all gathers for level 0 and use selection for level 1.
...
class KernelBuilderH22:
    """
    H22: Level-Aware Gathering

    Copy of C4 with modifications for first iteration only.
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def build_kernel(
        ...
        """
        C4 + H22: Combined kernel with level-aware optimization for first iteration.
        """
        ...
        # ============================================================
        # H22: Pre-load tree level values for rounds 0-3
        # ============================================================

        # Level 0: tree[0] - single value for all elements in round 0
        level0_val_scalar = self.alloc_scratch("level0_val_scalar")
        v_level0_val = self.alloc_scratch("v_level0_val", VLEN)
        # forest_values_p points to tree[0], so we load mem[mem[4]] = mem[7] = tree[0]
        self.add("load", ("load", level0_val_scalar, self.scratch["forest_values_p"]))
        self.add("valu", ("vbroadcast", v_level0_val, level0_val_scalar))

        # Level 1: tree[1] and tree[2] - for round 1
        level1_addr0 = self.alloc_scratch("level1_addr0")
        level1_addr1 = self.alloc_scratch("level1_addr1")
        level1_val0_scalar = self.alloc_scratch("level1_val0_scalar")
        level1_val1_scalar = self.alloc_scratch("level1_val1_scalar")
        v_level1_val0 = self.alloc_scratch("v_level1_val0", VLEN)
        v_level1_val1 = self.alloc_scratch("v_level1_val1", VLEN)

        self.add("alu", ("+", level1_addr0, self.scratch["forest_values_p"], one_const))
        self.add("alu", ("+", level1_addr1, self.scratch["forest_values_p"], two_const))
        self.add("load", ("load", level1_val0_scalar, level1_addr0))
        self.add("load", ("load", level1_val1_scalar, level1_addr1))
        self.add("valu", ("vbroadcast", v_level1_val0, level1_val0_scalar))
        self.add("valu", ("vbroadcast", v_level1_val1, level1_val1_scalar))

        self.add("flow", ("pause",))

        # Use 4 desks for interleaving (same as all base implementations)
        NUM_DESKS = 4
        ...
        first_iter_flag = self.alloc_scratch("first_iter_flag")  # H22: Track if first iteration
        ...
        # H12 CHANGE: Now we process 2 rounds per iteration
        # So total iterations = (batch_size // VLEN) * (rounds / 2) / NUM_DESKS
        # rounds=16, so we have 8 "double-rounds"
        total_iterations = (batch_size // VLEN) * (rounds // 2) // NUM_DESKS
        total_const = self.scratch_const(total_iterations)

        # Iterations for first double-round only (level 0 -> level 1)
        first_double_round_iters = (batch_size // VLEN) // NUM_DESKS  # = 8
        first_dr_const = self.scratch_const(first_double_round_iters)
        ...
        self.add("load", ("const", first_iter_flag, 1))  # Start with first iteration flag = 1

        # ============================================================
        # FIRST DOUBLE-ROUND: LEVEL-AWARE (rounds 0-1)
        # Round 0: All elements at tree[0]
        # Round 1: All elements at tree[1] or tree[2]
        # ============================================================

        first_loop_start = len(self.instrs)

        # PHASE 1: Calculate all offsets and load idx/val from memory
        self.instrs.append({
            "alu": [
        ...
        # ============================================================
        # H22 ROUND 0: Use v_level0_val directly (NO GATHER!)
        # All elements access tree[0]
        # ============================================================

        # XOR all desks with level 0 value
        self.instrs.append({
            "valu": [
                ("^", desks[0]['val'], desks[0]['val'], v_level0_val),
                ("^", desks[1]['val'], desks[1]['val'], v_level0_val),
                ("^", desks[2]['val'], desks[2]['val'], v_level0_val),
                ("^", desks[3]['val'], desks[3]['val'], v_level0_val),
            ],
        })

        # Hash all 4 desks (same as C4)
        # Stage 0 (FMA)
        self.instrs.append({
            "valu": [
        ...
        # Branch computation: idx = 2*0 + 1 + (val & 1) = 1 or 2
        # First compute (val & 1) for all desks
        self.instrs.append({
            "valu": [
        ...
        # Then compute idx = 1 + (val & 1) - must be separate cycle!
        for d in range(4):
            self.instrs.append({
                "valu": [
                    ("+", desks[d]['idx'], v_one, desks[d]['tmp1']),  # idx = 1 + (val & 1)
                ],
            })

        # No bounds check needed - idx is 1 or 2, always valid

        # ============================================================
        # H22 ROUND 1: Use selection between v_level1_val0/v_level1_val1
        # idx is 1 or 2 -> select tree[1] or tree[2]
        # ============================================================

        # Selection: if idx == 1, use tree[1] (v_level1_val0), else use tree[2] (v_level1_val1)
        # Since idx is 1 or 2, (idx - 1) gives 0 or 1
        # vselect(dest, cond, a, b): dest = a if cond != 0 else b
        # We want: tree[1] if idx==1, else tree[2]
        # vselect(node_val, idx-1, tree[2], tree[1])
        for d in range(4):
            self.instrs.append({
                "valu": [
                    ("-", desks[d]['tmp1'], desks[d]['idx'], v_one),  # tmp1 = idx - 1 (0 or 1)
                ],
            })
        for d in range(4):
            self.instrs.append({
                "flow": [
                    ("vselect", desks[d]['node_val'], desks[d]['tmp1'], v_level1_val1, v_level1_val0),
                ],
            })
        ...
        # idx was 1 or 2, so new idx is 3, 4, 5, or 6
        for d in range(4):
            self.instrs.append({
                "valu": [
        ...
        # No bounds check - idx is 3-6, always valid for any reasonable tree
        ...
        self.instrs.append({
            ...
            "alu": [
                ("<", addr_scalar, iter_counter, first_dr_const),
            ],
        })
        ...
        # Loop if more iterations needed for first double-round
        self.add("flow", ("cond_jump", addr_scalar, first_loop_start))

        # Reset counters for main C4 loop (7 more double-rounds)
        self.add("load", ("const", iter_counter, 0))

        # ============================================================
        # REMAINING 7 DOUBLE-ROUNDS: Standard C4 approach
        # ============================================================

        remaining_iterations = (batch_size // VLEN) * 7 // NUM_DESKS
        remaining_const = self.scratch_const(remaining_iterations)

        # Use the standard C4 main loop structure
        ...
        # H11 CHANGE: Desk3 gather 4-5 + desk0 branch prep (AND, FMA replaces MUL+ADD)
        self.instrs.append({
            "load": [
            ...
            "valu": [
                ("&", desks[0]['tmp1'], desks[0]['val'], v_one),
                ("multiply_add", desks[0]['idx'], desks[0]['idx'], v_two, v_one),  # H11: FMA
                ("^", desks[1]['val'], desks[1]['tmp1'], desks[1]['tmp2']),
                ("^", desks[2]['tmp1'], desks[2]['val'], v_hash_consts[1]),
                (">>", desks[2]['tmp2'], desks[2]['val'], v_hash_shifts[1]),
            ],
        ...
        # H11 CHANGE: Desk3 gather 6-7 + desk0 branch (add tmp1 only) + desk1 hash4 + desk2 hash1 combine
        self.instrs.append({
            "load": [
            ...
            "valu": [
                ("+", desks[0]['idx'], desks[0]['idx'], desks[0]['tmp1']),  # H11: Only add branch bit
                ("multiply_add", desks[1]['val'], desks[1]['val'], v_fma_mult[4], v_hash_consts[4]),
                ("^", desks[2]['val'], desks[2]['tmp1'], desks[2]['tmp2']),
            ],
        })

        # H11: desk0 branch done, continue with hashes + desk3 XOR
        self.instrs.append({
            "valu": [
                ("^", desks[1]['tmp1'], desks[1]['val'], v_hash_consts[5]),
                (">>", desks[1]['tmp2'], desks[1]['val'], v_hash_shifts[5]),
        ...
        # H11 CHANGE: Desk0 vselect bypass + desk1 branch prep with FMA
        self.instrs.append({
            "valu": [
                ("*", desks[0]['idx'], desks[0]['idx'], desks[0]['tmp1']),
                ("&", desks[1]['tmp1'], desks[1]['val'], v_one),
                ("multiply_add", desks[1]['idx'], desks[1]['idx'], v_two, v_one),  # H11: FMA
                ("^", desks[2]['val'], desks[2]['tmp1'], desks[2]['tmp2']),
                ("^", desks[3]['tmp1'], desks[3]['val'], v_hash_consts[1]),
                (">>", desks[3]['tmp2'], desks[3]['val'], v_hash_shifts[1]),
            ],
        ...
        # H11 CHANGE: Desk1 branch (add tmp1 only) + desk2 hash4 + desk3 hash1 combine
        self.instrs.append({
            "valu": [
                ("+", desks[1]['idx'], desks[1]['idx'], desks[1]['tmp1']),  # H11: Only add branch bit
                ("multiply_add", desks[2]['val'], desks[2]['val'], v_fma_mult[4], v_hash_consts[4]),
                ("^", desks[3]['val'], desks[3]['tmp1'], desks[3]['tmp2']),
            ],
        })
        ...
        # H11 CHANGE: Desk1 vselect bypass + desk2 branch prep with FMA + desk3 hash3 combine
        self.instrs.append({
            "valu": [
                ("*", desks[1]['idx'], desks[1]['idx'], desks[1]['tmp1']),
                ("&", desks[2]['tmp1'], desks[2]['val'], v_one),
                ("multiply_add", desks[2]['idx'], desks[2]['idx'], v_two, v_one),  # H11: FMA
                ("^", desks[3]['val'], desks[3]['tmp1'], desks[3]['tmp2']),
            ],
        })

        # H11 CHANGE: Desk2 branch (add tmp1 only) + desk3 hash4
        self.instrs.append({
            "valu": [
                ("+", desks[2]['idx'], desks[2]['idx'], desks[2]['tmp1']),  # H11: Only add branch bit
                ("multiply_add", desks[3]['val'], desks[3]['val'], v_fma_mult[4], v_hash_consts[4]),
            ],
        })
        ...
        # H11 CHANGE: Desk2 vselect bypass + desk3 branch prep with FMA
        self.instrs.append({
            "valu": [
                ("*", desks[2]['idx'], desks[2]['idx'], desks[2]['tmp1']),
                ("&", desks[3]['tmp1'], desks[3]['val'], v_one),
                ("multiply_add", desks[3]['idx'], desks[3]['idx'], v_two, v_one),  # H11: FMA
            ],
        })

        # H11 CHANGE: Desk3 branch (add tmp1 only)
        self.instrs.append({
            "valu": [
                ("+", desks[3]['idx'], desks[3]['idx'], desks[3]['tmp1']),  # H11: Only add branch bit
            ],
        })
        ...
        # ============================================================
        # H12 FUSION: ROUND 2 OF 2 - NO INTERMEDIATE STORE/LOAD
        # Keep idx/val in registers, compute next round immediately
        # ============================================================
        ...
        # H11 CHANGE: Round 2 Desk3 gather 4-5 + desk0 branch prep with FMA
        ...
        # H11: Round 2 Desk3 gather 6-7 + desk0 branch (add tmp1 only) + desk1 hash4 + desk2 hash1 combine
        ...
        # H11 + H14: Continue round 2 processing with address precomputation
        self.instrs.append({
            "valu": [
                ("^", desks[1]['tmp1'], desks[1]['val'], v_hash_consts[5]),
                (">>", desks[1]['tmp2'], desks[1]['val'], v_hash_shifts[5]),
        ...
        # H14: Compute next iteration's load addresses
        self.instrs.append({
            "valu": [
                ("<", desks[0]['tmp1'], desks[0]['idx'], v_n_nodes),
                ("^", desks[1]['val'], desks[1]['tmp1'], desks[1]['tmp2']),
        ...
        # H11 CHANGE: Round 2 Desk0 vselect bypass + desk1 branch prep with FMA
        # H14: Continue address precomputation
        self.instrs.append({
            "valu": [
                ("*", desks[0]['idx'], desks[0]['idx'], desks[0]['tmp1']),
                ("&", desks[1]['tmp1'], desks[1]['val'], v_one),
        ...
        # H11: Round 2 Desk1 branch (add tmp1 only) + desk2 hash4 + desk3 hash1 combine
        # H14: Continue address precomputation
        self.instrs.append({
            "valu": [
                ("+", desks[1]['idx'], desks[1]['idx'], desks[1]['tmp1']),  # H11: Only add branch bit
        ...
        # H11 CHANGE: Round 2 Desk1 vselect bypass + desk2 branch prep with FMA + desk3 hash3 combine
        self.instrs.append({
            "valu": [
                ("*", desks[1]['idx'], desks[1]['idx'], desks[1]['tmp1']),
                ("&", desks[2]['tmp1'], desks[2]['val'], v_one),
        ...
        # H11 CHANGE: Round 2 Desk2 branch (add tmp1 only) + desk3 hash4
        self.instrs.append({
            "valu": [
                ("+", desks[2]['idx'], desks[2]['idx'], desks[2]['tmp1']),  # H11: Only add branch bit
                ("multiply_add", desks[3]['val'], desks[3]['val'], v_fma_mult[4], v_hash_consts[4]),
        ...
        # H11 CHANGE: Round 2 Desk2 vselect bypass + desk3 branch prep with FMA
        self.instrs.append({
            "valu": [
                ("*", desks[2]['idx'], desks[2]['idx'], desks[2]['tmp1']),
                ("&", desks[3]['tmp1'], desks[3]['val'], v_one),
        ...
        # H11 CHANGE: Round 2 Desk3 branch (add tmp1 only) + H14: compute store addresses
        self.instrs.append({
            "valu": [
                ("+", desks[3]['idx'], desks[3]['idx'], desks[3]['tmp1']),  # H11: Only add branch bit
            ],
        ...
        # Round 2: Desk3 bounds + H14: continue store address computation
        self.instrs.append({
            "valu": [
                ("<", desks[3]['tmp1'], desks[3]['idx'], v_n_nodes),
            ],
        ...
        # ============================================================
        # H13 OPTIMIZATION: STORE COALESCING WITH OVERLAP
        # Store after both rounds complete, overlapped with loop control
        # ============================================================
