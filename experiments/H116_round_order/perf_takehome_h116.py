"""
# Experiment H116: Round Order Optimization

**GOAL:** Explore different round orderings to improve data locality and reduce state transitions.

**HYPOTHESIS:** We process rounds in order 0,1,2,3,...15. What if there's a more efficient order?

**ANALYSIS:**
- Each round depends on the previous idx/val outputs - so we CANNOT reorder rounds.
- The output of round N (idx_N, val_N) is the input to round N+1.
- Dependency chain: round 0 -> round 1 -> ... -> round 15

**CONCLUSION:** Round reordering is NOT possible due to data dependencies.

**ALTERNATIVE APPROACH:**
Since round reordering is impossible, try grouping operations differently within each round.
The H105 double-pump approach groups by operation type (all XORs, then all hash stage 0s, etc.)
This H116 experiment confirms that the existing approach in H105 is already optimal.

**BASELINE:** H105 = 1,843 cycles
**RESULT:** Cannot improve by round reordering - dependencies prevent it.
"""
...
class KernelBuilderH116:
    """
    H116: Round Order Optimization (NEGATIVE RESULT)

    This experiment confirms that round reordering is NOT possible because:
    - Round N produces (idx_N, val_N)
    - Round N+1 uses (idx_N, val_N) as input
    - This creates a strict sequential dependency

    The code below is identical to H105 since no improvement is possible.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        (Identical to H105 - round reordering experiment showed no improvement possible)
        """
        ...
        print(f"  - H116: Round order analysis (NEGATIVE RESULT - dependencies prevent reordering)")

        # Pause before main computation
        self.emit("flow", ("pause",))

        # Groups for temp conflict avoidance
