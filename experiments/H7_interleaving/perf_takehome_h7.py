...
Based on T3's constraint solver proof that 34 cycles per loop body is optimal,
vs the current ~62 cycles. The gap exists because we process desks sequentially.

**Key insight from T3:** The optimal schedule has operations from different desks
at different pipeline stages running simultaneously:
- Cycle N: desk0_hash_stage3 + desk1_hash_stage2 + desk2_hash_stage1 + desk3_gather

This experiment implements aggressive interleaving where every cycle tries to pack:
- 6 VALU ops (from desks at different pipeline stages)
- 2 Load ops (gathering for one desk while hashing others)
- Up to 12 ALU ops for address computation

**Strategy:**
- Use a software pipeline approach where desks are at different stages
- Desk 0 leads, others follow in staggered fashion
- Combine with T6's FMA optimization and T5's 6-VALU packing

**Expected:** ~5,000-6,000 cycles (vs 7,995 current best)
"""
...
class KernelBuilderH7:
    """
    H7: Aggressive Cross-Desk Interleaving

    Pipeline structure (conceptual):
    - Stage 0: Load indices/values
    - Stage 1: Compute gather addresses
    - Stage 2: Perform gather (load node values)
    - Stage 3: XOR with node value
    - Stage 4-9: Hash stages 0-5
    - Stage 10: Branch computation
    - Stage 11: Bounds check & vselect
    - Stage 12: Store results

    With 4 desks staggered, we have 4 concurrent activities at different stages.
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def build_kernel(
        ...
        """
        H7: Aggressive interleaving kernel.

        The key optimization is a software pipeline where we overlap:
        - Loading for desk N+1 while hashing desk N
        - Early stages of desk N+1 while late stages of desk N
        """
        ...
        # === MAIN LOOP with AGGRESSIVE INTERLEAVING ===
        ...
        # ============================================================
        # The key is to start gathering for the next desk while the
        # previous desk is computing hash stages.
        #
        # Gather takes 4 cycles per desk (8 lanes, 2 loads/cycle)
        # Hash stages: 0,2,4 use FMA (1 cycle), 1,3,5 use 2-step (2 cycles)
        # Total hash: 6 cycles if we overlap prep/combine properly
        #
        # Timeline (conceptual):
        #   Cycle 0-3:  Gather desk0
        #   Cycle 4:    Desk0 XOR + gather desk1 starts
        #   Cycle 5:    Desk0 hash0 + desk1 gather
        #   ...
        ...
        # Compute all store addresses first
        self.instrs.append({
            "alu": [
