"""
# Experiment H129: Batch Loads Across Desks

**GOAL:** Improve gather operation load scheduling by interleaving loads across desks.

**HYPOTHESIS:** In gather operations we emit loads desk-by-desk:
  desk0_lane0, desk0_lane1, ..., desk1_lane0, ...

This may cause load slot underutilization. Instead, emit:
  desk0_lane0, desk1_lane0, desk2_lane0, ..., desk0_lane1, desk1_lane1, ...

This interleaves loads across desks to maximize load slot utilization (2 loads/cycle).
...
class KernelBuilderH129:
    """
    H129: Batch loads across desks in gather operations.

    Instead of emitting loads desk-by-desk:
      desk0_lane0, desk0_lane1, ..., desk1_lane0, ...

    Emit loads lane-by-lane across all desks:
      desk0_lane0, desk1_lane0, desk2_lane0, ..., desk0_lane1, desk1_lane1, ...

    This interleaves loads to maximize 2 loads/cycle slot utilization.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        print(f"  - H129: Batch loads across desks in gather operations")
        ...
        def emit_gather_loads_batched(desks_list):
            """H129 CHANGE: Emit gather loads lane-by-lane across all desks.

            Instead of: desk0_lane0, desk0_lane1, ..., desk1_lane0, ...
            Emit:       desk0_lane0, desk1_lane0, ..., desk0_lane1, desk1_lane1, ...

            This interleaves loads across desks to maximize 2 loads/cycle utilization.
            """
        ...
        def emit_tile():
            ...
            # H129 CHANGE: Use batched gather loads (lane-by-lane across desks)
            for _round in range(3, 10):
                # Compute addresses for all desks
                for d in range(NUM_DESKS):
                    self.emit("valu", ("+", desks[d]['addr'], v_forest_p, desks[d]['idx']))
                # Gather for all desks - H129: batched lane-by-lane
                emit_gather_loads_batched(range(NUM_DESKS))
                # XOR for all desks
                for d in range(NUM_DESKS):
                    self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))
            ...
            # H129 CHANGE: Use batched gather loads
            for d in range(NUM_DESKS):
                self.emit("valu", ("+", desks[d]['addr'], v_forest_p, desks[d]['idx']))
            emit_gather_loads_batched(range(NUM_DESKS))
            for d in range(NUM_DESKS):
