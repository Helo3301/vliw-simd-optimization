"""
# Experiment H16: Cross-Iteration Pipelining (Double Buffering)

Building on C4 (4,667 cycles), this experiment tests cross-iteration pipelining.

**Hypothesis:** Currently we finish storing iteration N before loading iteration N+1.
With double-buffered registers, we can overlap:
- While storing set A's results, load set B's data
- Swap register sets each iteration
- Hide store latency behind load operations

**Key insight:** The store engine (2 slots) and load engine (2 slots) are independent,
so we can store from one register set while loading into another.

**Implementation Notes:**
This is a copy of C4 with attempted store/load overlap. The key change is:
- At the end of processing, we store results AND load next batch simultaneously
- This requires pre-computing the next batch addresses with wrap-around
...
class KernelBuilderH16:
    """
    H16: Cross-Iteration Pipelining with Double Buffering

    Based on C4 with store/load overlap optimization.
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def build_kernel(
        ...
        """
        H16: Double-buffered kernel - copy of C4 for comparison.

        Note: Full double-buffering requires significant restructuring.
        This implementation matches C4 to verify correctness.
        """
