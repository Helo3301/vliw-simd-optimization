"""
# Experiment H17: 4-Round Fusion (Extended Round Fusion)

**Key Insight:**
H12's 2-round fusion saved 14% by eliminating 50% of vload/vstore.
4-round fusion would:
- Eliminate 75% of vload/vstore (vs 50%)
- Only 32 iterations instead of 64
- Process 4 consecutive rounds with values in registers

Current (2-round fusion per desk iteration):
  Rounds N,N+1:   vload idx,val -> compute x2 -> vstore idx,val
  Rounds N+2,N+3: vload idx,val -> compute x2 -> vstore idx,val

Optimized (4-round fusion):
  Rounds N,N+1,N+2,N+3: vload idx,val -> compute x4 -> vstore idx,val

**Memory operations analysis:**
- 2-round: Load once, hash twice, store once per 2 rounds
- 4-round: Load once, hash 4 times, store once per 4 rounds
- Expected savings: ~500 additional cycles

**Risk:** Register pressure - need to track values across 4 rounds.
The architecture has enough scratch space but scheduling becomes complex.

**Expected:** Save ~500 cycles from further reduced vload/vstore pairs
"""
...
class KernelBuilderH17:
    """
    H17: 4-Round Fusion - Keep Values in Registers for 4 Consecutive Rounds

    Instead of:
      Rounds N,N+1:   vload -> compute x2 -> vstore
      Rounds N+2,N+3: vload -> compute x2 -> vstore

    We do:
      Rounds N,N+1,N+2,N+3: vload -> compute x4 -> vstore

    This eliminates 75% of the vload/vstore operations per desk.
    The loop now processes 4 rounds per iteration.
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def emit_single_round_for_desk(self, desk, v_hash_consts, v_hash_shifts, v_fma_mult, v_one, v_two, v_n_nodes):
        """
        Emit hash computation and branching for a single round for one desk.
        This processes the gathered node_val with val, computes new idx.
        """
        # XOR with gathered value
        self.instrs.append({
            "valu": [
                ("^", desk['val'], desk['val'], desk['node_val']),
            ],
        })
        ...
        self.instrs.append({
            "valu": [
                ("multiply_add", desk['val'], desk['val'], v_fma_mult[0], v_hash_consts[0]),
            ],
        })
        ...
        self.instrs.append({
            "valu": [
                ("^", desk['tmp1'], desk['val'], v_hash_consts[1]),
                (">>", desk['tmp2'], desk['val'], v_hash_shifts[1]),
            ],
        })
        ...
        self.instrs.append({
            "valu": [
                ("multiply_add", desk['val'], desk['val'], v_fma_mult[2], v_hash_consts[2]),
            ],
        })
        ...
        self.instrs.append({
            "valu": [
                ("+", desk['tmp1'], desk['val'], v_hash_consts[3]),
                ("<<", desk['tmp2'], desk['val'], v_hash_shifts[3]),
            ],
        })
        ...
        self.instrs.append({
            "valu": [
                ("multiply_add", desk['val'], desk['val'], v_fma_mult[4], v_hash_consts[4]),
            ],
        })
        ...
        self.instrs.append({
            "valu": [
                ("^", desk['tmp1'], desk['val'], v_hash_consts[5]),
                (">>", desk['tmp2'], desk['val'], v_hash_shifts[5]),
            ],
        })
        ...
        # Branch computation: idx = idx * 2 + 1 + (val & 1)
        # With bounds check
        self.instrs.append({
            "valu": [
                ("&", desk['tmp1'], desk['val'], v_one),
                ("*", desk['idx'], desk['idx'], v_two),
            ],
        })

        self.instrs.append({
            "valu": [
                ("+", desk['idx'], desk['idx'], v_one),
            ],
        })

        self.instrs.append({
            "valu": [
                ("+", desk['idx'], desk['idx'], desk['tmp1']),
            ],
        })

        # Bounds check and vselect bypass
        self.instrs.append({
            "valu": [
                ("<", desk['tmp1'], desk['idx'], v_n_nodes),
            ],
        })
    ...
    def build_kernel(
        ...
        """
        H17: 4-Round Fusion kernel - process 4 rounds per memory access cycle.
        """
        ...
        # Use 4 desks for interleaving (same as H12)
        NUM_DESKS = 4
        ...
        # H17 CHANGE: Now we process 4 rounds per iteration
        # So total iterations = (batch_size // VLEN) * (rounds / 4) / NUM_DESKS
        # rounds=16, so we have 4 "quad-rounds"
        # (256 / 8) * (16 / 4) / 4 = 32 * 4 / 4 = 32
        total_iterations = (batch_size // VLEN) * (rounds // 4) // NUM_DESKS
        ...
        # ============================================================
        # ROUND 1 OF 4: INTERLEAVED GATHER + HASH PIPELINE
        # ============================================================
        ...
        # ============================================================
        # ROUND 2 OF 4: NO INTERMEDIATE STORE/LOAD
        # ============================================================
        self._emit_round_n(desks, v_hash_consts, v_hash_shifts, v_fma_mult, v_one, v_two, v_n_nodes)

        # ============================================================
        # ROUND 3 OF 4: NO INTERMEDIATE STORE/LOAD
        # ============================================================
        self._emit_round_n(desks, v_hash_consts, v_hash_shifts, v_fma_mult, v_one, v_two, v_n_nodes)

        # ============================================================
        # ROUND 4 OF 4: FINAL ROUND WITH STORE
        # ============================================================
        self._emit_final_round(desks, v_hash_consts, v_hash_shifts, v_fma_mult, v_one, v_two, v_n_nodes,
                               offset_regs, addr_tmp, zero_const)

        # STORE PHASE - after all 4 rounds complete, write back to memory
    ...
    def _emit_round_n(self, desks, v_hash_consts, v_hash_shifts, v_fma_mult, v_one, v_two, v_n_nodes):
        """
        Emit a full round (gather + hash + branch) for all 4 desks.
        This is used for rounds 2 and 3 (intermediate rounds without store).
        """
        # Compute gather addresses for this round (all desks)
        self.instrs.append({
            "valu": [
                ("vbroadcast", desks[0]['addr'], self.scratch["forest_values_p"]),
                ("vbroadcast", desks[1]['addr'], self.scratch["forest_values_p"]),
        ...
        # Desk3 gather 6-7 + desk0 branch (add 1) + desk1 hash4 + desk2 hash1 combine
        self.instrs.append({
            "load": [
                ("load", desks[3]['node_val'] + 6, desks[3]['addr'] + 6),
                ("load", desks[3]['node_val'] + 7, desks[3]['addr'] + 7),
    ...
    def _emit_final_round(self, desks, v_hash_consts, v_hash_shifts, v_fma_mult, v_one, v_two, v_n_nodes,
                          offset_regs, addr_tmp, zero_const):
        """
        Emit the final round (round 4 of 4) with store address computation at the end.
        """
        # Compute gather addresses for this round (all desks)
        self.instrs.append({
            "valu": [
        ...
        # Desk3 bounds + start computing store addresses for desks 0,1,2
        self.instrs.append({
            "valu": [
                ("<", desks[3]['tmp1'], desks[3]['idx'], v_n_nodes),
            ],
        ...
        # Desk3 vselect bypass + compute desk3 store addresses
        self.instrs.append({
            "valu": [
                ("*", desks[3]['idx'], desks[3]['idx'], desks[3]['tmp1']),
            ],
