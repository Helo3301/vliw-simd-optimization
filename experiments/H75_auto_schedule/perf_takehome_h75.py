"""
# Experiment H75: Automatic VLIW Scheduling
...
class KernelBuilderH75:
    """
    H75: Automatic VLIW scheduling with wrap exploitation.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        def emit_tile(tile_idx):
            ...
            for d in range(NUM_DESKS):
                ...
                self.emit("valu", ("-", desks[d]['addr'], v_tree[2], v_tree[1]))  # diff
                self.emit("valu", ("multiply_add", desks[d]['node_val'], desks[d]['tmp1'], desks[d]['addr'], v_tree[1]))
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))
                emit_hash_stages(d)
                emit_branch(d)

            # Rounds 2-9: Gather without bounds
            for _round in range(2, 10):
                for d in range(NUM_DESKS):
            ...
            for d in range(NUM_DESKS):
                ...
                self.emit("valu", ("-", desks[d]['addr'], v_tree[2], v_tree[1]))
                self.emit("valu", ("multiply_add", desks[d]['node_val'], desks[d]['tmp1'], desks[d]['addr'], v_tree[1]))
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))
                emit_hash_stages(d)
                emit_branch(d)

            # Rounds 13-15: Gather without bounds (max idx < 2047)
