"""
# Experiment H65v2: Loop Tiling with Preload and Arithmetic Selection

**GOAL:** Process small batches through ALL 16 rounds before moving to next batch,
using tree preloading for rounds 0-7 to eliminate gathers.

**KEY INSIGHT FROM H63:**
- All 256 elements start at index 0 (tree root)
- At round N, at most 2^N unique indices (all in level N of tree)
- Rounds 0-7: indices are in levels 0-7 (nodes 0-254)
- By preloading these 255 nodes, we can use arithmetic to select values
  instead of doing 8 scalar loads per vector (gather)

**DESIGN:**
- 4 tiles of 64 elements (8 vectors) each
- Per tile:
  1. Load input idx/val
  2. Preload tree levels 0-7 (255 nodes)
  3. Rounds 0-7: Use preloaded values via lookup (still need load, but from scratch)
  4. Rounds 8-15: Standard gather from main memory
  5. Store results

**WHY THIS HELPS:**
- Preload: 255 nodes = 32 vloads = 16 cycles per tile
- Without preload (current H65): 8 rounds x 8 vectors x 4 cycles = 256 cycles
- With preload: 16 + (8 rounds x 8 vectors x ~1 cycle lookup) = ~80 cycles
- Savings per tile: ~176 cycles
- Total savings: 176 x 4 tiles = 704 cycles

**EXPECTED:** ~2,800 cycles (vs current 3,494)
"""
...
class KernelBuilderH65v2:
    """
    H65v2: Loop Tiling with Preloaded Tree Levels

    Process 64 elements (8 vectors) through all 16 rounds per tile.
    Preload tree levels 0-7 for fast lookup in early rounds.
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def build_kernel(
        ...
        """
        H65v2: Loop Tiling kernel with tree preloading.
        """
        ...
        eight_const = self.scratch_const(8)  # For checking round < 8
