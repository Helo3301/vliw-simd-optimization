"""
C1-4: Optimized idx math in fused rounds

Hypothesis: In B4-2's fused rounds, we can simplify the idx computation.

Analysis of current B4-2 approach for rounds 0-2:
- Round 0: idx=0, hash, bit0 = val & 1, idx1 = 1 + bit0 (2 ops)
- Round 1: node1 = tree[1] + bit0 * diff, hash, bit1 = val & 1,
           idx2 = 3 + 2*bit0 + bit1 (4 ops: AND, FMA, ADD)
- Round 2: 4-way select (2 FMA + 1 vselect), hash,
           idx3 = 2*idx2 + 1 + bit2 (3 ops: AND, FMA, ADD)

For rounds 11-13 (same pattern after wrap).

Potential optimization: In round 2's branch, instead of:
  idx3 = 2*idx2 + 1 + bit2
  where idx2 = 3 + 2*bit0 + bit1

We have:
  idx3 = 2*(3 + 2*bit0 + bit1) + 1 + bit2
       = 7 + 4*bit0 + 2*bit1 + bit2

We're already computing 2*bit0 + bit1 for idx2. Let's track this as 'bits01'.
Then idx3 = 7 + 2*bits01 + bit2, which is: FMA(bits01, 2, 7) + bit2 = 3 ops
But we need bits01 = 2*bit0 + bit1 anyway.

Actually the current approach already uses:
- idx2 = 3 + 2*bit0 + bit1 via FMA(bit0, 2, 3) + bit1

Let me try a different optimization: delay the idx computation until needed.
In gather rounds, we compute addr = forest_p + idx. If we track bits separately,
we might be able to compute addr more directly.

Actually, looking more carefully: the gather rounds (3-9, 14) MUST compute idx
because the tree structure requires data-dependent addresses. The fusion only
helps when we can preload tree values.

Let me try: simplify the bit extraction by overlapping it with other operations.
"""
...
class KernelBuilderC1_4:
    """C1-4: Optimized idx computation using multiply_add more aggressively"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Optimized fused rounds 0+1+2 - try to use multiply_add more
        def emit_rounds_0_1_2_fused_opt(group_desks):
            """Rounds 0+1+2 fused with optimized bit tracking"""
            # === Round 0 ===
            # XOR with tree[0], hash, extract bit0
            for d in group_desks:
                emit_xor_with_node(d, v_tree[0])
            emit_hash_interleaved(group_desks)
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("&", desk['bit0'], desk['val'], v_one))  # bit0
                # idx1 = 1 + bit0 -- but we don't need idx1, we need bit0 for node selection
                self.emit("valu", ("+", desk['idx'], v_one, desk['bit0']))

            # === Round 1 ===
            # node1 = tree[1] + bit0 * (tree[2] - tree[1])
            for d in group_desks:
                desk = desks[d]
            ...
            for d in group_desks:
                ...
                # idx2 = 3 + 2*bit0 + bit1 = multiply_add(bit0, 2, 3) + bit1
                self.emit("valu", ("multiply_add", desk['idx'], desk['bit0'], v_two, v_three))
                self.emit("valu", ("+", desk['idx'], desk['idx'], desk['tmp1']))

            # === Round 2 ===
            # 4-way select based on (bit0, bit1)
            # offset = 2*bit0 + bit1
            # node2 = tree[3 + offset]
            for d in group_desks:
                desk = desks[d]
                # bit1 is in tmp1
                self.emit("valu", ("multiply_add", desk['node_val'], desk['tmp1'], v_diff_3_4, v_tree[3]))  # low
                self.emit("valu", ("multiply_add", desk['tmp2'], desk['tmp1'], v_diff_5_6, v_tree[5]))  # high
                self.emit("flow", ("vselect", desk['node_val'], desk['bit0'], desk['tmp2'], desk['node_val']))
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            emit_hash_interleaved(group_desks)
            # Branch: idx3 = 2*idx2 + 1 + bit2
            for d in group_desks:
                emit_branch(d)

        # Same for rounds 11+12+13
        def emit_rounds_11_12_13_fused_opt(group_desks):
            """Rounds 11+12+13 fused with optimized bit tracking"""
            # === Round 11 ===
            for d in group_desks:
                emit_xor_with_node(d, v_tree[0])
            emit_hash_interleaved(group_desks)
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                # Fused rounds 0+1+2
                emit_rounds_0_1_2_fused_opt(group_desks)
                ...
                emit_rounds_11_12_13_fused_opt(group_desks)
