"""
C1-2: Fuse rounds 9-10 (before the 11-13 block)

Hypothesis: Round 10 is the last before wrap. We can track the bit from round 9
and use it in round 10's branch. Since round 10 ends with bounds check that wraps
to 0, fusing 9-10 might save operations.

Key insight: The bounds check after round 10 uses idx < n_nodes.
If idx >= n_nodes, idx becomes 0 (wrap).
Rounds 11-13 are already fused in B4-2 and start from idx=0.

The potential savings are limited because rounds 9-10 both use gather loads.
But we might save the AND operation in round 10's branch by tracking bit from 9.
"""
...
class KernelBuilderC1_2:
    """C1-2: Extended fusion with rounds 9-10 bit tracking"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_gather_round_with_early_bit(group_desks):
            """Gather round that extracts bit early for next round"""
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
            for d in group_desks:
            ...
            for d in group_desks:
                ...
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            emit_hash_interleaved(group_desks)
            # Extract bit early and save for next round
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("&", desk['bit0'], desk['val'], v_one))
                self.emit("valu", ("multiply_add", desk['idx'], desk['idx'], v_two, v_one))
                self.emit("valu", ("+", desk['idx'], desk['idx'], desk['bit0']))

        def emit_round_10_with_saved_bit(group_desks):
            """Round 10 using bit saved from round 9, with bounds check"""
            # bit0 already has bit from round 9
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
            for d in group_desks:
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                ...
                # Rounds 3-8: gather rounds
                for _rnd in range(3, 9):
                    emit_gather_round_interleaved(group_desks)

                # Round 9: gather with early bit extraction
                emit_gather_round_with_early_bit(group_desks)

                # Round 10: uses saved bit, with bounds check
                emit_round_10_with_saved_bit(group_desks)
