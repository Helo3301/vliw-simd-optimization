"""
C1-3: Larger fusion blocks (0-3 with preloaded tree[0..14])

Hypothesis: Extend B4-2's fusion from rounds 0-2 to rounds 0-3 by preloading
tree[7..14] (level 3 nodes).

Analysis of round 0-3 pattern:
- Round 0: idx=0, hash, bit0 = val & 1, idx1 = 1 + bit0
- Round 1: node1 = tree[1+bit0], hash, bit1 = val & 1, idx2 = 3 + 2*bit0 + bit1
- Round 2: node2 = tree[3+2*bit0+bit1], hash, bit2 = val & 1, idx3 = 7 + 4*bit0 + 2*bit1 + bit2
- Round 3: node3 = tree[7+4*bit0+2*bit1+bit2], hash, bit3, idx4 = ...

For 8-way selection in round 3:
offset3 = 4*bit0 + 2*bit1 + bit2 (0..7)
We need tree[7..14] preloaded.

This requires 8 additional vector registers = 64 scalars = not too bad.
And we can use 8-way select with multiple vselects.
"""
...
class KernelBuilderC1_3:
    """C1-3: Extended fusion with rounds 0-3 and 11-14 using preloaded tree"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Preload first 15 tree nodes (levels 0-3)
        NUM_PRELOADED = 15
        v_tree = []
        ...
        # Precompute differences for 2-way selects
        ...
        # Precompute differences for level 3 (8-way select broken into pairs)
        ...
        # Fused rounds 0+1+2+3 with bit tracking
        def emit_rounds_0_1_2_3_fused(group_desks):
            """Rounds 0+1+2+3 fused with direct bit tracking"""
            # === Round 0 ===
            for d in group_desks:
            ...
            # === Round 2 ===
            # 4-way select: offset = 2*bit0 + bit1
            for d in group_desks:
                desk = desks[d]
            ...
            for d in group_desks:
                ...
                self.emit("valu", ("multiply_add", desk['tmp2'], desk['bit0'], v_two, desk['bit1']))  # 2*bit0 + bit1
                self.emit("valu", ("multiply_add", desk['idx'], desk['tmp2'], v_two, v_seven))  # 7 + 2*(2*bit0 + bit1)
                self.emit("valu", ("+", desk['idx'], desk['idx'], desk['tmp1']))  # + bit2

            # === Round 3 ===
            # 8-way select: offset = 4*bit0 + 2*bit1 + bit2
            # bit0 in desk['bit0'], bit1 in desk['bit1'], bit2 in desk['tmp1']
            # Use hierarchical vselect:
            # First level: select pairs based on bit2
            #   pair_0 = tree[7] + bit2 * diff_7_8   (for bit0=0, bit1=0)
            #   pair_1 = tree[9] + bit2 * diff_9_10  (for bit0=0, bit1=1)
            #   pair_2 = tree[11] + bit2 * diff_11_12 (for bit0=1, bit1=0)
            #   pair_3 = tree[13] + bit2 * diff_13_14 (for bit0=1, bit1=1)
            # Second level: select based on bit1
            #   low = select(bit1, pair_1, pair_0)   (for bit0=0)
            #   high = select(bit1, pair_3, pair_2)  (for bit0=1)
            # Third level: select based on bit0
            #   node3 = select(bit0, high, low)
            for d in group_desks:
                desk = desks[d]
                # bit2 is in tmp1
                # Compute all 4 pairs
                self.emit("valu", ("multiply_add", desk['node_val'], desk['tmp1'], v_diff_7_8, v_tree[7]))  # pair_0
                self.emit("valu", ("multiply_add", desk['tmp2'], desk['tmp1'], v_diff_9_10, v_tree[9]))  # pair_1
                # Select between pair_0 and pair_1 based on bit1
                self.emit("flow", ("vselect", desk['node_val'], desk['bit1'], desk['tmp2'], desk['node_val']))  # low
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("multiply_add", desk['addr'], desk['tmp1'], v_diff_11_12, v_tree[11]))  # pair_2
                self.emit("valu", ("multiply_add", desk['tmp2'], desk['tmp1'], v_diff_13_14, v_tree[13]))  # pair_3
                # Select between pair_2 and pair_3 based on bit1
                self.emit("flow", ("vselect", desk['addr'], desk['bit1'], desk['tmp2'], desk['addr']))  # high
            for d in group_desks:
                desk = desks[d]
                # Final select between low and high based on bit0
        ...
        # Similar fusion for rounds 11+12+13+14 (after wrap)
        def emit_rounds_11_12_13_14_fused(group_desks):
            """Rounds 11+12+13+14 fused with direct bit tracking (same as 0+1+2+3 after wrap)"""
            # === Round 11 ===
            for d in group_desks:
                emit_xor_with_node(d, v_tree[0])
            emit_hash_interleaved(group_desks)
            ...
            for d in group_desks:
                ...
                self.emit("valu", ("multiply_add", desk['tmp2'], desk['bit0'], v_two, desk['bit1']))
                self.emit("valu", ("multiply_add", desk['idx'], desk['tmp2'], v_two, v_seven))
            ...
            for d in group_desks:
                ...
                self.emit("valu", ("multiply_add", desk['tmp2'], desk['tmp1'], v_diff_9_10, v_tree[9]))
                self.emit("flow", ("vselect", desk['node_val'], desk['bit1'], desk['tmp2'], desk['node_val']))
            for d in group_desks:
                desk = desks[d]
