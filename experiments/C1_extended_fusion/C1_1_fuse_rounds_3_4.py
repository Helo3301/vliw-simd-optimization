"""
C1-1: Fuse rounds 3-4 (similar bit-tracking pattern)

Hypothesis: After B4-2's fusion of rounds 0-2 and 11-13, we can apply similar
bit-tracking to rounds 3-4.

Analysis of round 3-4 pattern:
- After round 2: idx is in {3,4,5,6} mapping to tree level 2
- Round 3: gathers tree[idx3], hashes, branches to idx4 = 2*idx3 + 1 + bit3
  - idx4 is in {7..14} (level 3)
- Round 4: gathers tree[idx4], hashes, branches to idx5 = 2*idx4 + 1 + bit4
  - idx5 is in {15..30} (level 4)

The key insight is that we can't eliminate the gather loads (they're the bottleneck),
but we CAN track bit3 from round 3's hash and use it to precompute idx4 offset,
potentially saving an idx extraction operation in round 4.

However, unlike rounds 0-2 where we preload all tree values, rounds 3-4 require
gather loads that depend on the data-dependent idx values. The fusion opportunity
is limited to:
1. Computing bit3 earlier (during round 3's hash)
2. Starting idx4 address computation sooner
3. Overlapping round 3's branch with round 4's gather address prep

Let me test if tracking bit3 and computing partial idx4 saves cycles.
"""
...
class KernelBuilderC1_1:
    """C1-1: Extended fusion with rounds 3-4 bit tracking"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        for d in range(NUM_DESKS):
            desk = {
                ...
                'bit_prev': self.alloc_vec(f"v_bit_prev_{d}"),  # Track bit for round fusion
            }
            desks.append(desk)
        ...
        def emit_hash_with_early_bit(group_desks):
            """Emit hash stages with early bit extraction after stage 5"""
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("multiply_add", desk['val'], desk['val'], v_fma_mult[0], v_hash_consts[0]))
            for d in group_desks:
            ...
            for d in group_desks:
                ...
                self.emit("valu", (">>", desk['tmp2'], desk['val'], v_hash_shifts[5]))
                self.emit("valu", ("^", desk['val'], desk['tmp1'], desk['tmp2']))
                # Extract bit early - save for next round
                self.emit("valu", ("&", desk['bit_prev'], desk['val'], v_one))
        ...
        def emit_branch_with_saved_bit(desk_idx):
            """Branch using previously extracted bit"""
            d = desks[desk_idx]
            # bit_prev already has the bit
            self.emit("valu", ("multiply_add", d['idx'], d['idx'], v_two, v_one))
            self.emit("valu", ("+", d['idx'], d['idx'], d['bit_prev']))
        ...
        # Fused rounds 0+1+2 with bit tracking (same as B4-2)
        def emit_rounds_0_1_2_fused(group_desks):
            ...
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            # Use hash with early bit extraction for round 3
            emit_hash_with_early_bit(group_desks)
            for d in group_desks:
                emit_branch(d)

        # Fused rounds 3+4 with bit tracking
        def emit_rounds_3_4_fused(group_desks):
            """Rounds 3+4 fused by using early bit extraction"""
            # === Round 3 ===
            # bit_prev was extracted during round 2's hash
            for d in group_desks:
                desk = desks[d]
            ...
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            # Hash with early bit extraction
            emit_hash_with_early_bit(group_desks)
            # Branch using saved bit from round 2
            for d in group_desks:
                emit_branch_with_saved_bit(d)
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                ...
                # Fused rounds 3+4
                emit_rounds_3_4_fused(group_desks)

                # Rounds 5-9: gather rounds
