"""
GA Evolution Variant V1: Deep Round Interleaving
=================================================

Instead of processing desk groups through all rounds, interleave at the
OPERATION level - emit multiple desk operations simultaneously for better
VALU utilization.

Hypothesis: The 6 VALU slots are underutilized because operations within
a single desk have data dependencies. By interleaving operations from
different desks at a finer granularity, we can fill more slots per cycle.
"""
...
class KernelBuilder:
    """Deep interleaving kernel - emit operations in round-robin across desks."""
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # DEEP INTERLEAVING: Instead of processing desks through rounds,
        # emit operations in interleaved order to maximize VALU utilization

        def emit_hash_stage_interleaved(desk_indices, stage):
            """Emit one hash stage for multiple desks in interleaved fashion."""
            if stage in v_fma_mult:
                for d in desk_indices:
                    desk = desks[d]
                    self.emit("valu", ("multiply_add", desk['val'], desk['val'],
                                       v_fma_mult[stage], v_hash_consts[stage]))
            elif stage == 1:
                for d in desk_indices:
                    desk = desks[d]
                    self.emit("valu", ("^", desk['tmp1'], desk['val'], v_hash_consts[stage]))
                for d in desk_indices:
                    desk = desks[d]
                    self.emit("valu", (">>", desk['tmp2'], desk['val'], v_hash_shifts[stage]))
                for d in desk_indices:
                    desk = desks[d]
        ...
        def emit_hash_all_interleaved(desk_indices):
            """Emit all hash stages for multiple desks, interleaving stages."""
            for stage in range(6):
                emit_hash_stage_interleaved(desk_indices, stage)

        def emit_branch_interleaved(desk_indices):
            for d in desk_indices:
                desk = desks[d]
        ...
        def emit_bounds_interleaved(desk_indices):
            for d in desk_indices:
                desk = desks[d]
        ...
        # Process in groups of 6 (match VALU slot count for maximum utilization)
        INTERLEAVE_WIDTH = 6
        all_desks = list(range(NUM_DESKS))

        for group_start in range(0, NUM_DESKS, INTERLEAVE_WIDTH):
            group_desks = all_desks[group_start:group_start + INTERLEAVE_WIDTH]

            # Round 0
            for d in group_desks:
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], v_tree[0]))
            emit_hash_all_interleaved(group_desks)
            emit_branch_interleaved(group_desks)
            ...
            emit_hash_all_interleaved(group_desks)
            emit_branch_interleaved(group_desks)
            emit_bounds_interleaved(group_desks)
