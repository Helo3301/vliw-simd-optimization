"""
GA Evolution Variant V2: Reduced Operations Kernel
====================================================

This variant attempts to REDUCE the total number of operations through:
1. Combining operations where possible
2. Exploiting symmetries in the hash function
3. Eliminating redundant computations

Key insight from theoretical analysis:
- Current: 9,083 VALU ops -> 1,514 cycle minimum
- Target 1,363: 8,178 VALU ops maximum -> need to save 905 ops

Strategy:
1. Skip idx store for final round (we already skip branch, but still store idx)
   - Can skip 32 vstore = saves ~16 cycles overhead
2. Share temp registers between phases (already done)
3. Try to merge XOR into FMA stages

Actually, let me try a different approach: What if we can fuse the
XOR operation with the first hash stage?

val' = hash(val XOR node_val)
     = hash_stage0(val XOR node_val) ... etc

stage0: (val XOR node_val) * 4097 + C
      = val * 4097 + node_val * 4097 + C  (if + was XOR this would work)

No, XOR doesn't distribute over multiplication.

Alternative: Can we precompute anything across rounds?
No, each round depends on the previous.

Let me try: reduce the selection operations by using lookup tables.
No, we don't have enough scratch for lookup tables.

New idea: What if we process ALL 32 desks together instead of in groups?
This maximizes ILP for the scheduler.
"""
...
class KernelBuilder:
    """
    Reduced Operations Kernel - tries to minimize total VALU operations.

    Key optimizations:
    1. GROUP_SIZE = 1 (best from sweep)
    2. NUM_PRELOADED = 7 (best from sweep)
    3. Skip final idx store (save 32 vstores)
    4. Fuse as many operations as possible
    """
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Preload tree (only 7 nodes needed)
        NUM_PRELOADED = 7
        v_tree = []
        ...
        # Allocate desks (16 per tile, 2 tiles)
        NUM_DESKS = 16
        desks = []
        ...
        def emit_round_15_final(desk_idx):
            ...
            self.emit("valu", ("^", d['val'], d['val'], d['node_val']))
            emit_hash_stages(desk_idx)
            # Skip branch - not needed for final round
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            # Best configuration: GROUP_SIZE = 1
            GROUP_SIZE = 1
            num_groups = (NUM_DESKS + GROUP_SIZE - 1) // GROUP_SIZE

            for g in range(num_groups):
                group_start = g * GROUP_SIZE
                group_end = min(group_start + GROUP_SIZE, NUM_DESKS)
            ...
            # Store both idx and val (idx needed for correctness)
            for d in range(NUM_DESKS):
