"""
# Experiment V5: Duplicate Detection & Sharing - ANALYSIS

Investigate whether detecting duplicate indices across SIMD lanes can save loads.

**The Concept:**
Within 8 SIMD lanes, multiple lanes often need the SAME tree value (same idx).
Example: 8 lanes with indices [7, 9, 7, 12, 9, 7, 14, 9]
- Unique indices: {7, 9, 12, 14} - only 4!
- Current: 8 loads (wasteful - loading tree[7] three times!)
- V5: 4 loads + distribute to lanes that need each value

**Statistical insight:**
- Round 0: ALL lanes have idx=0 (100% duplicates!) -> 1 load instead of 8
- Round 1: At most 2 unique indices -> at most 2 loads
- Round 2: At most 4 unique indices -> at most 4 loads
- Later rounds: Diminishing duplicate probability

**Key Question:** Is the duplicate detection overhead worth the load savings?

**ANSWER: NO - Duplicate detection does NOT help for this workload.**

**Why:**
1. Detection overhead: No horizontal reduction instruction, must compare pairwise
   - Estimated cost: 4-6 cycles to detect all-same condition
   - Potential savings: 2 cycles (reduce 4 -> 2 for single load + broadcast)
   - Net: LOSS of 2-4 cycles per gather

2. Pipeline disruption: The gather operations are overlapped with VALU hash
   computations. Changing the gather pattern disrupts this careful scheduling.
   Even small structural changes caused 65+ cycle regressions.

3. Loads are NOT on critical path: The load engine work is "free" - it runs
   in parallel with VALU operations which are the actual bottleneck.

4. Conditional overhead for round 0: Even knowing all idx=0, the conditional
   check + branch adds ~2 cycles * 64 iterations = 128 cycles overhead, while
   savings would be 8 cycles * 8 iterations = 64 cycles. Net: LOSS.

**Result:** This implementation matches C4 exactly (4,667 cycles).
Duplicate detection was analyzed and determined to be counterproductive.
...
class KernelBuilderV5:
    """
    V5: Duplicate Detection Analysis (Copy of C4)

    After analysis, duplicate detection was determined to NOT be beneficial:

    1. Detection overhead exceeds savings (no horizontal reduce instruction)
    2. Pipeline disruption from changed gather pattern
    3. Loads are NOT on critical path - overlapped with VALU operations
    4. Conditional branching overhead for round 0 exceeds potential savings

    This implementation matches C4 exactly to serve as baseline for comparison.

    Combined C4 optimizations (unchanged):
    ...
    def build_kernel(
        ...
        """
        V5: Kernel with duplicate detection for early rounds.

        Key insight: In early rounds, all lanes start with idx=0.
        Round 0: 8 lanes, all idx=0 -> 1 unique load
        Round 1: 8 lanes, idx in {1,2} -> 2 unique loads
        Round 2: 8 lanes, idx in {1,2,3,4,5,6} -> at most 6 unique

        The duplicate detection overhead for general case might be too high.
        But for round 0 specifically, we KNOW there's only 1 unique value.
        """
        ...
        # V5: Scalar temp for single-load optimization
