"""
# Experiment H90: Cross-tile Round Pipelining (Software Pipelining Style)

**GOAL:** Improve pipelining by interleaving rounds between tiles instead of processing tile 0 completely then tile 1.

**KEY CHANGE:** Software pipelining approach for gather rounds:
- Phase 1: Issue ALL loads for all desks across BOTH tiles
- Phase 2: Compute hash/branch for all desks across BOTH tiles

This gives memory subsystem more time to fetch data while compute operations execute.

**RATIONALE:** By batching all loads together before compute, we maximize overlap between
memory latency and ALU operations. The scheduler can pack more useful work into cycles
while waiting for memory.

**RISK:** Higher register pressure since both tiles need their data loaded simultaneously.
Scratch usage increases from ~1002 to ~1530 slots.

**ALGORITHM:** Same as H85 (wrap exploitation, no bounds on R11-15)
**SCHEDULING:** Automatic via _schedule_slots()

**RESULTS:**
- H85 baseline: 1,898 cycles
- H90 cross-tile: 1,867 cycles
- Improvement: 31 cycles (1.6% faster)
"""
...
class KernelBuilderH90:
    """
    H90: Cross-tile round pipelining - interleave rounds between tiles.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel using cross-tile round pipelining.
        Instead of tile0 complete then tile1, we interleave rounds:
        tile0 round0, tile1 round0, tile0 round1, tile1 round1, etc.
        """
        NUM_TILES = 2
        NUM_DESKS = 16
        ...
        # Allocate per-desk vectors for BOTH tiles (2 tiles * 16 desks)
        all_desks = []  # [tile][desk]
        ...
        # Shared temp registers - share across tiles since rounds don't overlap
        # With interleaving, tile 0 desk 0 completes before tile 1 desk 0 starts
        # So we can share temps across tiles within the same desk group
        NUM_SHARED_TEMPS = 16
        shared_temps = [self.alloc_vec(f"v_shared_tmp_{i}") for i in range(NUM_SHARED_TEMPS)]

        def get_temps(tile_idx, desk_idx):
            """Get temp registers for a desk (shared across tiles)."""
            pair = desk_idx // 2
            t1_idx = pair * 2
            t2_idx = pair * 2 + 1
        ...
        # Share addr_tmp across tiles - compute addresses just before use
        ...
        # Helper: emit hash stages for a specific tile/desk
        def emit_hash_stages(tile_idx, desk_idx):
            d = all_desks[tile_idx][desk_idx]
            tmp1, tmp2 = get_temps(tile_idx, desk_idx)
            for hi in range(6):
                if hi in v_fma_mult:
        ...
        def emit_branch(tile_idx, desk_idx):
            d = all_desks[tile_idx][desk_idx]
            tmp1, _ = get_temps(tile_idx, desk_idx)
        ...
        for t in range(NUM_TILES):
            ...
            # Load idx/val for all desks in this tile
            for d in range(NUM_DESKS):
                self.emit("load", ("vload", all_desks[t][d]['idx'], addr_tmp[d*2]))
                self.emit("load", ("vload", all_desks[t][d]['val'], addr_tmp[d*2+1]))

        # ===== CROSS-TILE ROUND PIPELINING - Software Pipelining Style =====
        # For gather rounds, issue loads for tile 0 desk, then tile 1 desk,
        # then compute for tile 0, then compute for tile 1
        # This maximizes overlap between memory latency and compute
        ...
        # Rounds 3-9: Gather without bounds - software pipelining style
        # Issue ALL loads first, then compute
        for _round in range(3, 10):
            # Phase 1: Compute addresses and issue loads for ALL desks/tiles
            for d in range(NUM_DESKS):
                for t in range(NUM_TILES):
                    desk = all_desks[t][d]
            ...
            # Phase 2: Compute hash/branch for ALL desks/tiles (loads have time to complete)
            for d in range(NUM_DESKS):
                for t in range(NUM_TILES):
                    desk = all_desks[t][d]
                    self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
        ...
        # Round 10: Gather WITH bounds - software pipelining style
        ...
        # Rounds 14-15: Gather without bounds - software pipelining style
        ...
        for t in range(NUM_TILES):
            # Recompute store addresses for this tile
            for d in range(NUM_DESKS):
                self.emit("alu", ("+", addr_tmp[d*2], self.scratch["inp_indices_p"], offset_regs[t][d]))
                self.emit("alu", ("+", addr_tmp[d*2+1], self.scratch["inp_values_p"], offset_regs[t][d]))
            for d in range(NUM_DESKS):
