...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            # Theory 202: Round-level interleaving across groups
            # Emit each round for ALL groups before moving to next round.
            # This gives the scheduler maximum freedom to overlap loads with VALU.

            # Fused rounds 0+1+2 for all groups
            for group_desks in all_groups:
                emit_rounds_0_1_2_fused(group_desks)

            # Gather rounds 3-9 for all groups
            for _rnd in range(3, 10):
                for group_desks in all_groups:
                    emit_gather_round_addr_tracking(group_desks)

            # Round 10 for all groups
            for group_desks in all_groups:
                emit_round_10_optimized(group_desks)

            # Fused rounds 11+12+13 for all groups
            for group_desks in all_groups:
                emit_rounds_11_12_13_fused(group_desks)

            # Round 14 for all groups
            for group_desks in all_groups:
                emit_gather_round_addr_tracking(group_desks)

            # Round 15 for all groups
            for group_desks in all_groups:
