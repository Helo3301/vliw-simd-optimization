"""
BF200 Theory 234: Pipelined tile transitions

Based on Theory 229 (best: 1,398 cycles).
Key optimization: Start loading next tile pair DURING last group's drain.

Instead of:
  Pair 0: [load] [groups] [store]
  Pair 1: [load] [groups] [store]

Do:
  [load pair 0] [groups pair 0 except last] [last group pair 0 + store pair 0 + load pair 1] [groups pair 1] [store pair 1]

The scheduler already overlaps tiles naturally, but by emitting stores and next-tile
loads BEFORE the last group's emission finishes, we can reduce drain.

Key trick: We use 3 bank address sets:
- Bank 0 addr_tmp for current stores
- Bank 1 addr_tmp for current stores
- A third set for next tile's loads
This avoids WAR hazards between stores and loads.

Actually, simpler: Just interleave the store emissions with next tile's const/load emissions
so the scheduler can overlap them with the last group's VALU drain.
"""
...
class KernelBuilderA1:
    """Theory 234: Pipelined tile transitions"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        DESKS_PER_BANK = 8
        NUM_BANKS = 2
        NUM_DESKS = DESKS_PER_BANK * NUM_BANKS  # 16 total
        ...
        # === PIPELINED EMISSION: Overlap stores + next-tile loads with last group ===
        NUM_TILES = batch_size // (DESKS_PER_BANK * VLEN)  # = 4
        GROUP_SIZE = 4
        num_groups = DESKS_PER_BANK // GROUP_SIZE  # = 2

        # First tile pair: normal load, then groups, then store overlapped with next load
        tile_a = 0
        tile_b = 1
        emit_tile_load(tile_a, 0)
        emit_tile_load(tile_b, 1)

        # All groups for first pair
        ...
        # Now: interleave stores for pair 0 with loads for pair 1
        # Emit stores for bank A (pair 0), then loads for bank A (pair 1)
        # Emit stores for bank B (pair 0), then loads for bank B (pair 1)
        # The scheduler will overlap these since stores read from desks that are done,
        # while loads write to desks that haven't started yet.

        # Store pair 0 bank A
        emit_tile_store(0)
        # Load pair 1 bank A (tile 2 on bank 0)
        emit_tile_load(2, 0)
        # Store pair 0 bank B
        emit_tile_store(1)
        # Load pair 1 bank B (tile 3 on bank 1)
        emit_tile_load(3, 1)

        # All groups for second pair
        for g in range(num_groups):
            group_desks_a = list(range(0 + g * GROUP_SIZE, 0 + (g + 1) * GROUP_SIZE))
            emit_all_rounds_for_group(group_desks_a)
            group_desks_b = list(range(DESKS_PER_BANK + g * GROUP_SIZE,
