...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Fused rounds 11+12+13 (R11 XOR already done in R10)
        def emit_rounds_11_12_13_fused(group_desks):
            # === Round 11 === (XOR with tree[0] already done in R10 optimized)
            emit_hash_interleaved(group_desks)
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("&", desk['bit0'], desk['val'], v_one))
        ...
        # *** R10 with branch skip + addr-tracking + R11 XOR pre-merged ***
        def emit_round_10_optimized(group_desks):
            """Round 10: addr ready from R9. Skip branch, set idx=0, pre-XOR for R11."""
            # Gather
            for d in group_desks:
                desk = desks[d]
                for lane in range(VLEN):
            ...
            # Hash
            emit_hash_interleaved(group_desks)
            # Set idx = 0 for R11 fused rounds
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['idx'], desk['idx'], desk['idx']))
            # Pre-XOR val with tree[0] for R11 (moved from fused R11 start)
            for d in group_desks:
                emit_xor_with_node(d, v_tree[0])
