"""
BF200 Theory 236: GROUP_SIZE=8 to eliminate hash bubbles

Based on Theory 229 (best: 1,398 cycles).
Key insight: With GROUP_SIZE=4, hash throughput (8 cycles) < latency (9 cycles),
causing 1 cycle bubble per round per group.
Total: 16 rounds * 4 groups * 2 tiles = 128 wasted VALU slots ≈ 21 cycles.

With GROUP_SIZE=8: hash throughput = 8*12/6 = 16 cycles > 9 latency → NO bubbles.
Trade-off: Larger groups = longer drain for last group.
But fewer groups = fewer group transitions.

Configuration: 16 desks, GROUP_SIZE=8, 2 groups/tile, 2 tiles, single bank.
"""
...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # === SINGLE BANK, 16 desks, GROUP_SIZE=8 ===
