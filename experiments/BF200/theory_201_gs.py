...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int, group_size_param: int = 4):
...
def do_kernel_test(forest_height: int, rounds: int, batch_size: int, seed: int = 123,
                   ...
                   group_size: int = 4):
    print(f"{forest_height=}, {rounds=}, {batch_size=}, {group_size=}")
    random.seed(seed)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gs", type=int, default=0, help="GROUP_SIZE (0=sweep)")
    args = parser.parse_args()

    if args.gs > 0:
        cycles = do_kernel_test(10, 16, 256, check=args.check, group_size=args.gs,
                                trace=args.trace)
        if args.check:
            print(f"Correctness check PASSED! Cycles: {cycles}")
    else:
        # Sweep all valid group sizes
        results = {}
        for gs in [1, 2, 4, 8, 16]:
            ...
            print(f"{'='*60}")
            try:
                cycles = do_kernel_test(10, 16, 256, check=args.check, group_size=gs)
                results[gs] = cycles
        ...
        print(f"\n{'='*60}")
        print("SUMMARY:")
        for gs, cycles in sorted(results.items()):
            status = f"{cycles} cycles" if cycles else "FAILED"
            print(f"  GS={gs}: {status}")
        if results:
            best_gs = min((c for c in results.values() if c), default=None)
            if best_gs:
                best = [gs for gs, c in results.items() if c == best_gs]
                print(f"BEST: GS={best} at {best_gs} cycles")
