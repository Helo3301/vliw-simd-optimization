"""
Theory 232: Early partial addr computation + cascade without addr temp

Key optimizations over theory_226:
1. Compute partial addr FMA after R1 and R2 (runs in parallel with vselects)
2. Modify R3/R14 vselect cascade to use tmp1 instead of addr as temp
3. At R3/R14 end, only need 3 VALU (not 5) for final addr: AND + FMA + ADD

Same total VALU count (5 ops spread across R1/R2/R3 vs all 5 at R3)
but shorter critical path at R3 end, potentially saving ~2 cycles per group.

With 8 groups (4 per tile * 2 tiles), potential savings: ~16 cycles.
Target: 1,402 - 16 = ~1,386 (vs target 1,363)
"""
...
class KernelBuilder:
    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        ...
        def emit_fused_0123(gd):
            """Fused rounds 0-3 with EARLY partial addr computation"""
            # === R0 ===
            ...
            # ** EARLY: Compute partial addr = FMA(bit0, 2, bit1) **
            for d in gd:
                dk = desks[d]
            ...
            # ** EARLY: Extend partial addr = FMA(addr, 2, idx) **
            for d in gd:
                dk = desks[d]
                self.emit("valu", ("multiply_add", dk['addr'], dk['addr'], v_two, dk['idx']))

            # === R3 === (7-vselect cascade using tmp1 NOT addr as temp)
            for d in gd:
                dk = desks[d]
                ...
                # ** CHANGED: Use tmp1 instead of addr as temp **
                self.emit("flow", ("vselect", dk['tmp1'],     dk['idx'], v_tree[14], v_tree[13]))
                self.emit("flow", ("vselect", dk['node_val'], dk['bit1'], dk['tmp1'], dk['node_val']))
                self.emit("flow", ("vselect", dk['node_val'], dk['bit0'], dk['node_val'], dk['tmp2']))
            for d in gd: self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))
            emit_hash(gd)
            # ** SHORTENED: Only 3 VALU for final addr (was 5) **
            for d in gd:
                dk = desks[d]
                self.emit("valu", ("&", dk['tmp1'], dk['val'], v_one))  # bit3
                self.emit("valu", ("multiply_add", dk['addr'], dk['addr'], v_two, dk['tmp1']))  # s = 2*partial + bit3
                self.emit("valu", ("+", dk['addr'], dk['addr'], v_fp_plus_15))  # addr = s + fp + 15

        def emit_fused_11_14(gd):
            """Fused rounds 11-14 with EARLY partial addr computation"""
            # === R11 ===
            for d in gd: self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], v_tree[0]))
            emit_hash(gd)
            for d in gd: self.emit("valu", ("&", desks[d]['bit0'], desks[d]['val'], v_one))
            ...
            for d in gd:
                ...
                self.emit("flow", ("vselect", dk['tmp2'],     dk['bit1'], dk['node_val'], dk['tmp2']))
                self.emit("flow", ("vselect", dk['node_val'], dk['idx'], v_tree[12], v_tree[11]))
                # ** CHANGED: tmp1 instead of addr **
                self.emit("flow", ("vselect", dk['tmp1'],     dk['idx'], v_tree[14], v_tree[13]))
                self.emit("flow", ("vselect", dk['node_val'], dk['bit1'], dk['tmp1'], dk['node_val']))
                self.emit("flow", ("vselect", dk['node_val'], dk['bit0'], dk['node_val'], dk['tmp2']))
            for d in gd: self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))
            emit_hash(gd)
            # ** SHORTENED final addr **
            for d in gd:
                dk = desks[d]
                self.emit("valu", ("&", dk['tmp1'], dk['val'], v_one))
        ...
        def emit_tile(tile_idx):
            ...
            GROUP_SIZE = 4
            groups = [list(range(g*GROUP_SIZE, (g+1)*GROUP_SIZE)) for g in range(NUM_DESKS // GROUP_SIZE)]

            for gd in groups:
                emit_fused_0123(gd)
                for _ in range(4, 10): emit_gather_at(gd)
                emit_r10(gd)
                emit_fused_11_14(gd)
                emit_r15(gd)
