"""
BF200 Theory 235: Parallel init with separate temporaries

Based on Theory 229 (best: 1,398 cycles).
Key optimization: Use separate temp scalar locations for each tree preload
to break the serial dependency chain in the init phase.

Currently init does:
  for i in range(15):
    ALU: tmp_addr = forest_values_p + const(i)   # WAW on tmp_addr!
    LOAD: tmp_scalar = mem[tmp_addr]              # WAW on tmp_scalar!
    VALU: v_tree[i] = vbroadcast(tmp_scalar)      # depends on tmp_scalar

This creates serial deps: each load must wait for previous load to finish
because they all write to tmp_scalar.

With separate temps:
    ALU: tree_addr[i] = forest_values_p + const(i)  # independent!
    LOAD: tree_scalar[i] = mem[tree_addr[i]]          # independent!
    VALU: v_tree[i] = vbroadcast(tree_scalar[i])      # depends only on its own load

Now all 15 loads can be issued in parallel (limited by 2 loads/cycle = 8 cycles).

Also: batch all ALU ops first, then all loads, then all vbroadcasts.
This lets the scheduler overlap the load phase with VALU work.

Cost: 15 extra scalar scratch locations for tree_addr + 15 for tree_scalar = 30.
We have 152 free. Plenty!
"""
...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # === PARALLEL TREE PRELOAD ===
        # Pre-allocate all const indices first
        NUM_PRELOADED = 15
        tree_const_addrs = []
        for i in range(NUM_PRELOADED):
            tree_const_addrs.append(self.scratch_const(i))  # These may already be cached

        # Allocate separate address and scalar temps for each tree node
        tree_addr_temps = [self.alloc_scratch(f"tree_addr_{i}") for i in range(NUM_PRELOADED)]
        tree_scalar_temps = [self.alloc_scratch(f"tree_scalar_{i}") for i in range(NUM_PRELOADED)]

        v_tree = []
        for i in range(NUM_PRELOADED):
            v_tree.append(self.alloc_vec(f"v_tree_{i}"))

        # Phase 1: All ALU ops (compute all addresses in parallel)
        for i in range(NUM_PRELOADED):
            self.emit("alu", ("+", tree_addr_temps[i], self.scratch["forest_values_p"], tree_const_addrs[i]))

        # Phase 2: All loads (independent, limited only by 2/cycle)
        for i in range(NUM_PRELOADED):
            self.emit("load", ("load", tree_scalar_temps[i], tree_addr_temps[i]))

        # Phase 3: All vbroadcasts (each depends only on its own scalar)
        for i in range(NUM_PRELOADED):
            self.emit("valu", ("vbroadcast", v_tree[i], tree_scalar_temps[i]))

        # === DUAL BANK: 2 banks of 8 desks each ===
        DESKS_PER_BANK = 8
        NUM_BANKS = 2
        ...
        # --- All round emission functions (same as theory_229) ---
