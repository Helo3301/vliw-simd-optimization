"""
BF200 Theory 200: Combine all proven optimizations + R0/R11 vselect for idx

Building on Theory 1b (best: 7,994 VALU, 1,477 cycles):
- Theory 1: Address-tracking branch fusion (-255 VALU)
- Theory 10c: 3 vselect for R2/R13 node selection (removes diff vectors)
- Theory 1b: Combined branch+conversion (-64 VALU)
- NEW Theory 200a: Replace idx = 1 + bit0 with vselect(bit0, v_two, v_one) in R0/R11
  Saves: 1 VALU per desk * 16 desks * 2 tiles * 2 rounds = 64 VALU
  Cost: 1 flow per desk * 16 * 2 * 2 = 64 flow ops
- NEW Theory 200b: Remove unused v_zero and v_n_nodes constants
  Saves: 2 VALU (vbroadcast)

Expected: 7,994 - 64 - 2 = 7,928 VALU
"""
...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Theory 200b: Removed unused v_zero and v_n_nodes (-2 VALU)
        ...
        def emit_rounds_0_1_2_fused(group_desks):
            ...
            for d in group_desks:
                ...
                # Theory 200a: vselect instead of ADD (0 VALU, 1 flow)
                # bit0=0 -> idx=1, bit0=1 -> idx=2
                self.emit("flow", ("vselect", desk['idx'], desk['bit0'], v_two, v_one))
