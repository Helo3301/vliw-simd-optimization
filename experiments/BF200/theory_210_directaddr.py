...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # THEORY 210: Direct addr from bits: addr = fp + 7 + 4*bit0 + 2*bit1 + bit2
        v_four = self.scratch_vconst(4, "v_four")
        v_seven = self.scratch_vconst(7, "v_seven")
        v_fp_plus_7 = self.alloc_vec("v_fp_plus_7")
        self.emit("valu", ("+", v_fp_plus_7, v_forest_p, v_seven))
        ...
        def emit_rounds_0_1_2_fused(group_desks):
            ...
            for d in group_desks:
                ...
                # R1's FMA overwrites idx using bit0 directly, so R0's idx is never read.
            ...
            emit_hash_interleaved(group_desks)
            # Theory 210: R1 only extracts bit1, no idx computation
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("&", desk['tmp1'], desk['val'], v_one))  # bit1 in tmp1

            # === Round 2 === (Theory 10c: 3 vselect, 0 VALU for node selection)
            # Note: tmp1 = bit1 is READ by vselect (not written), so it survives
            for d in group_desks:
                desk = desks[d]
            ...
            # Theory 210: Save bit1 into idx before hash clobbers tmp1
            for d in group_desks:
                desk = desks[d]
                # Use vselect to copy tmp1 to idx (0 VALU, 1 flow)
                # vselect(idx, tmp1, tmp1, tmp1) always returns tmp1 regardless of condition
                self.emit("flow", ("vselect", desk['idx'], desk['tmp1'], desk['tmp1'], desk['tmp1']))
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            emit_hash_interleaved(group_desks)
            # Theory 210: Direct addr from bits: addr = fp+7 + 4*bit0 + 2*bit1 + bit2
            # bit0 in desk['bit0'], bit1 in desk['idx'], bit2 extracted from val
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("&", desk['tmp1'], desk['val'], v_one))        # bit2 in tmp1
                self.emit("valu", ("+", desk['tmp2'], desk['idx'], desk['idx']))   # 2*bit1 in tmp2
                self.emit("valu", ("+", desk['tmp2'], desk['tmp2'], desk['tmp1'])) # 2*bit1 + bit2
                self.emit("valu", ("multiply_add", desk['addr'], desk['bit0'], v_four, desk['tmp2']))  # 4*bit0 + 2*bit1 + bit2
                self.emit("valu", ("+", desk['addr'], desk['addr'], v_fp_plus_7)) # + fp + 7
        ...
        def emit_rounds_11_12_13_fused(group_desks):
            ...
            for d in group_desks:
                ...
                self.emit("valu", ("&", desk['bit0'], desk['val'], v_one))
                # Theory 209: REMOVED dead code: idx = 1 + bit0
                # R12's FMA overwrites idx using bit0 directly.
