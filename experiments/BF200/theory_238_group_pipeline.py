"""
BF200 Theory 238: Software pipelining across groups

Based on Theory 229 (best: 1,398 cycles).
Key idea: Instead of completing all rounds for group 0 before starting group 1,
interleave rounds of different groups so their dependency chains overlap.

Emission pattern for 4 groups (A0, A1, B0, B1):
  A0-R0_1_2_3, A0-R4, A1-R0_1_2_3, A0-R5, A1-R4, A0-R6, A1-R5, ...

The idea is that while A0's hash is stalling on latency, A1's independent
operations fill the VALU gaps.

This is different from theory_233 which interleaved banks at round level.
This interleaves groups within the SAME processing, using the same desk
registers (which are independent between groups by design).
"""
...
class KernelBuilderA1:
    """Theory 238: Group-pipelined emission"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # --- Round emission building blocks ---
        ...
        # Build round emitters as callable pieces
        def emit_fused_0123(group_desks):
        ...
        def emit_gather_round(group_desks):
            for d in group_desks:
                desk = desks[d]
        ...
        def emit_fused_11121314(group_desks):
            for d in group_desks:
                emit_xor_with_node(d, v_tree[0])
            emit_hash_interleaved(group_desks)
            for d in group_desks:
        ...
        # Instead of: A0_all, B0_all, A1_all, B1_all
        # Do: A0_fused0123, B0_fused0123, A0_R4, B0_R4, A1_fused0123, A0_R5, B0_R5, B1_fused0123, ...
        # The key insight: emit round N+1 for group K BEFORE round N+1 for group K,
        # so the scheduler can overlap them

        NUM_TILES = batch_size // (DESKS_PER_BANK * VLEN)
        GROUP_SIZE = 4
        num_groups = DESKS_PER_BANK // GROUP_SIZE  # = 2

        # Build all 4 group desk lists
        for pair in range(NUM_TILES // 2):
            ...
            for g in range(num_groups):
                groups.append(list(range(0 + g * GROUP_SIZE, 0 + (g + 1) * GROUP_SIZE)))  # Bank A
            for g in range(num_groups):
                groups.append(list(range(DESKS_PER_BANK + g * GROUP_SIZE,
                                         DESKS_PER_BANK + (g + 1) * GROUP_SIZE)))  # Bank B
            # groups = [A0, A1, B0, B1]
            # Reorder to interleave banks: A0, B0, A1, B1
            groups = [groups[0], groups[2], groups[1], groups[3]]

            # Build round sequence for each group
            # Each group has these phases:
            # Phase 0: fused_0123 (4 rounds)
            # Phase 1-6: gather rounds R4-R9
            # Phase 7: R10
            # Phase 8: fused_11121314
            # Phase 9: R15
            # Total: 10 phases per group

            round_emitters = []
            for grp in groups:
                emitters = [
                    lambda g=grp: emit_fused_0123(g),
                    lambda g=grp: emit_gather_round(g),
                    lambda g=grp: emit_gather_round(g),
                    lambda g=grp: emit_gather_round(g),
                    lambda g=grp: emit_gather_round(g),
                    lambda g=grp: emit_gather_round(g),
                    lambda g=grp: emit_gather_round(g),
                    lambda g=grp: emit_round10(g),
                    lambda g=grp: emit_fused_11121314(g),
                    lambda g=grp: emit_round15(g),
                ]
                round_emitters.append(emitters)

            # Software pipeline: emit phases with stride
            # Depth-2 pipeline: start group K+1's phase 0 after group K's phase 0
            # This means at any point, 2 groups are in-flight

            # Simple approach: just interleave phases from all 4 groups
            # Phase N: emit group 0 phase N, group 1 phase N, group 2 phase N, group 3 phase N
            # This maximizes overlap between groups
            num_phases = 10
            for phase in range(num_phases):
                for gi in range(len(groups)):
                    round_emitters[gi][phase]()
