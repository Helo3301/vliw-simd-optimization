...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # v_diff_1_2 removed: R1/R12 now use vselect instead of FMA
        ...
        def emit_rounds_0_1_2_fused(group_desks):
            ...
            # === Round 2 === (Theory 10: 2 vselect + 1 FMA instead of 2 FMA + 1 vselect)
            for d in group_desks:
                desk = desks[d]
                # Select diff and base using bit0 (2 vselect = 0 VALU, 2 flow)
                self.emit("flow", ("vselect", desk['tmp2'], desk['bit0'], v_diff_5_6, v_diff_3_4))
                self.emit("flow", ("vselect", desk['node_val'], desk['bit0'], v_tree[5], v_tree[3]))
                # FMA: node_val = bit1 * diff_selected + base_selected (1 VALU)
                self.emit("valu", ("multiply_add", desk['node_val'], desk['tmp1'], desk['tmp2'], desk['node_val']))
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            emit_hash_interleaved(group_desks)
        ...
        def emit_rounds_11_12_13_fused(group_desks):
            ...
            # === Round 13 === (Theory 10: 2 vselect + 1 FMA)
            for d in group_desks:
                desk = desks[d]
                self.emit("flow", ("vselect", desk['tmp2'], desk['bit0'], v_diff_5_6, v_diff_3_4))
                self.emit("flow", ("vselect", desk['node_val'], desk['bit0'], v_tree[5], v_tree[3]))
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                ...
                # Convert idx -> addr for addr-tracking (1 VALU per desk)
                for d in group_desks:
                    desk = desks[d]
                    self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
                ...
                # Fused rounds 11+12+13 (uses idx, not addr)
