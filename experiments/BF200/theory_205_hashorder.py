...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_hash_interleaved(group_desks):
            # Theory 205: Stage-by-stage emission across all desks
            # This creates maximum ILP since desks are independent
            gd = group_desks  # straight order
            # Stage 0: FMA for all desks
            for d in gd:
                desk = desks[d]
                self.emit("valu", ("multiply_add", desk['val'], desk['val'], v_fma_mult[0], v_hash_consts[0]))
            # Stage 1: XOR + SHIFT + XOR for all desks
            for d in gd:
                desk = desks[d]
            ...
            # Stage 3: ADD + SHIFT + XOR for all desks
            for d in gd:
                desk = desks[d]
            ...
            # Stage 5: XOR + SHIFT + XOR for all desks
            for d in gd:
                desk = desks[d]
