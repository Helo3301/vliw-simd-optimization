"""
BF200 Theory 237: Stage-by-stage hash interleaving

Based on Theory 229 (best: 1,398 cycles).
Key optimization: Instead of emitting all 12 hash ops per desk sequentially,
interleave at the hash-stage level:

Current (desk-sequential):
  D0: stage0_fma, stage1_xor, stage1_shift, stage1_xor2, stage2_fma, ...
  D1: stage0_fma, stage1_xor, stage1_shift, stage1_xor2, stage2_fma, ...

New (stage-interleaved):
  D0: stage0_fma | D1: stage0_fma | D2: stage0_fma | D3: stage0_fma
  D0: stage1_xor | D1: stage1_xor | D2: stage1_xor | D3: stage1_xor
  D0: stage1_shift | D1: stage1_shift | ...
  D0: stage1_xor2 | D1: stage1_xor2 | ...
  ...

This might reduce bubbles in the dependency chain by allowing the scheduler
to interleave the stages more efficiently.
"""
...
class KernelBuilderA1:
    """Theory 237: Stage-interleaved hash"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # --- STAGE-INTERLEAVED HASH ---
        def emit_hash_stage_interleaved(group_desks):
            """Emit hash with stage-level interleaving across desks"""
            gd = group_desks[:]

            # Stage 0: FMA (per desk)
            for d in gd:
                desk = desks[d]
                self.emit("valu", ("multiply_add", desk['val'], desk['val'], v_fma_mult[0], v_hash_consts[0]))

            # Stage 1: XOR + SHIFT + XOR (per desk)
            for d in gd:
                desk = desks[d]
        ...
        def emit_rounds_0_1_2_3_fused(group_desks):
            ...
            emit_hash_stage_interleaved(group_desks)
            for d in group_desks:
                desk = desks[d]
