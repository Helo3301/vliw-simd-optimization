"""
BF200 Theory 222: Deferred idx computation
...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Theory 222: v_three and v_seven removed (no longer needed with deferred idx)
        ...
        # Theory 227: Separate offset/addr_tmp registers per tile for better overlap
        offset_regs = [[self.alloc_scratch(f"off_t{t}_{d}") for d in range(NUM_DESKS)] for t in range(2)]
        addr_tmp = [[self.alloc_scratch(f"addr_tmp_t{t}_{i}") for i in range(32)] for t in range(2)]
        ...
        def emit_hash_interleaved(group_desks):
            # Theory 226: Pair-based interleaving (0,1,2,3 -> 0,2,1,3 or pairs)
            # Try alternating: D0 all, D2 all, D1 all, D3 all (same as before but ensure pair overlap)
            gd = group_desks[:]  # Try natural order: 0,1,2,3
            for d in gd:
                desk = desks[d]
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            t_off = offset_regs[tile_idx]
            t_addr = addr_tmp[tile_idx]

            for d in range(NUM_DESKS):
                self.emit("load", ("const", t_off[d], tile_offset + d * VLEN))

            for d in range(NUM_DESKS):
                self.emit("alu", ("+", t_addr[d*2], self.scratch["inp_indices_p"], t_off[d]))
                self.emit("alu", ("+", t_addr[d*2+1], self.scratch["inp_values_p"], t_off[d]))

            for d in range(NUM_DESKS):
                self.emit("load", ("vload", desks[d]['idx'], t_addr[d*2]))
                self.emit("load", ("vload", desks[d]['val'], t_addr[d*2+1]))
            ...
            for d in range(NUM_DESKS):
                self.emit("store", ("vstore", t_addr[d*2], desks[d]['idx']))
                self.emit("store", ("vstore", t_addr[d*2+1], desks[d]['val']))
