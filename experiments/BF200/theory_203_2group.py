...
class KernelBuilderA1:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            # Theory 203: Paired-group interleaving
            # Process 2 groups together, interleaving at the gather-round level.
            # Group A does hash while Group B does loads, and vice versa.
            for pair_idx in range(0, len(all_groups), 2):
                gA = all_groups[pair_idx]
                gB = all_groups[pair_idx + 1] if pair_idx + 1 < len(all_groups) else None

                # Fused R0+1+2 for both groups (no gathers, just preloaded nodes)
                emit_rounds_0_1_2_fused(gA)
                if gB:
                    emit_rounds_0_1_2_fused(gB)

                # Gather rounds 3-9: interleave A and B
