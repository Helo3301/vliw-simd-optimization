...
def _schedule_slots(slots: list[tuple[str, tuple]]) -> list[dict[str, list[tuple]]]:
    ...
    Theory 206: Instead of processing instructions in emission order,
    build a dependency graph and prioritize instructions with longer
    critical paths (more successors in the dependency chain).
    """
    n = len(slots)
    if n == 0:
        return []
    ...
    # For each instruction, compute reads/writes
    rw = []
    for engine, slot in slots:
        reads, writes = _slot_rw(engine, slot)
        rw.append((reads, writes))

    # Track last writer and last readers for each address
    last_writer: dict[int, int] = {}  # addr -> instruction index
    last_readers: dict[int, list[int]] = defaultdict(list)  # addr -> [instruction indices]

    # predecessors[i] = set of instructions that must complete before i
        ...
        for addr in writes:
            if addr in last_writer:
                ...
                if w != i:
                    predecessors[i].add(w)
                    successors[w].add(i)
        # WAR: write depends on last reads
        for addr in writes:
            for r in last_readers.get(addr, []):
                if r != i:
                    predecessors[i].add(r)
                    successors[r].add(i)
        # Update tracking
        for addr in writes:
            last_writer[addr] = i
    ...
    # Phase 2: Compute critical path length (longest path to any exit)
    # Using reverse topological order
    crit_path = [0] * n
    # Process in reverse order (approximate reverse topological sort since
    # instructions were emitted in roughly topological order)
    for i in range(n - 1, -1, -1):
        max_succ = 0
        for s in successors[i]:
            max_succ = max(max_succ, crit_path[s] + 1)
        crit_path[i] = max_succ

    # Phase 3: Schedule using priority
    cycles: list[dict[str, list[tuple]]] = []
    usage: list[dict[str, int]] = []
    scheduled = [False] * n
    pred_count = [len(p) for p in predecessors]  # remaining predecessor count
    ...
    # Ready queue: instructions with all predecessors scheduled
    # Track earliest_ready for each instruction
    earliest_ready = [0] * n

    # Initialize ready set
    import heapq
    ready = []  # min-heap of (-priority, index)
    for i in range(n):
        if pred_count[i] == 0:
            heapq.heappush(ready, (-crit_path[i], i))

    scheduled_cycle = [0] * n  # cycle when instruction was scheduled

    while ready:
        # Pop highest priority (most negative = longest critical path)
        neg_prio, idx = heapq.heappop(ready)

        engine, slot = slots[idx]
        earliest = earliest_ready[idx]
        ...
        for s in successors[idx]:
            pred_count[s] -= 1
            # Update earliest ready time: predecessor completes at cycle+1
            earliest_ready[s] = max(earliest_ready[s], cycle + 1)
            if pred_count[s] == 0:
                heapq.heappush(ready, (-crit_path[s], s))
