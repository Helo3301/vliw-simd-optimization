"""
BF200 Theory 229: Dual-bank tile overlap

Based on Theory 226 (best: 1,402 cycles).
Key optimization: Split 16 desks into 2 banks of 8, run 4 tiles (instead of 2).
Tiles 0+1 use banks A+B simultaneously (overlapping compute).
Tiles 2+3 use banks A+B simultaneously.

Each bank's desks use different scratch addresses, so the scheduler
can overlap loads/stores from one bank with VALU from the other.
Interleave group emissions between banks to maximize overlap.

Expected: Same VALU floor (1,297) but fewer wasted cycles due to
tile overlap eliminating the drain penalty.
"""
...
class KernelBuilderA1:
    """Theory 229: Dual-bank tile overlap"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Per-bank scaffolding (separate so no cross-bank dependencies)
        ...
        def emit_all_rounds_for_group(group_desks):
            """Emit all 16 rounds for a group of desks"""
        ...
        def emit_tile_load(tile_idx, bank_idx):
            """Load data for a tile into the given bank's desks"""
        ...
        def emit_tile_store(bank_idx):
            """Store results from the given bank's desks"""
            bank_desk_start = bank_idx * DESKS_PER_BANK
            for d in range(DESKS_PER_BANK):
                global_desk = bank_desk_start + d
        ...
        def emit_tile_compute(bank_idx):
            """Run all rounds for all groups in the bank"""
            bank_desk_start = bank_idx * DESKS_PER_BANK
            GROUP_SIZE = 4
            num_groups = DESKS_PER_BANK // GROUP_SIZE

            for g in range(num_groups):
                group_desks = list(range(bank_desk_start + g * GROUP_SIZE,
                                        bank_desk_start + (g + 1) * GROUP_SIZE))
                emit_all_rounds_for_group(group_desks)

        # === MAIN EMISSION: Interleave bank A and bank B ===
        # 4 tiles: Tile 0 (bank A), Tile 1 (bank B), Tile 2 (bank A), Tile 3 (bank B)
        # Process in pairs: (Tile 0 + Tile 1), then (Tile 2 + Tile 3)
        # Within each pair, interleave: load A, load B, compute A group by group interleaved with B

        NUM_TILES = batch_size // (DESKS_PER_BANK * VLEN)  # = 4

        for pair in range(NUM_TILES // 2):
            tile_a = pair * 2      # Tiles 0, 2
            tile_b = pair * 2 + 1  # Tiles 1, 3

            # Load both banks
            emit_tile_load(tile_a, 0)
            emit_tile_load(tile_b, 1)

            # Interleave computation: alternate groups between banks
            GROUP_SIZE = 4
