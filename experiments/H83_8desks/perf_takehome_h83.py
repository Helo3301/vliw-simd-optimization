"""
# Experiment H83: Reduced Desk Count (8 desks, 4 tiles)

**GOAL:** Reduce register pressure by using 8 desks instead of 16, while
processing 4 tiles instead of 2.

**HYPOTHESIS:** Using 8 desks reduces register pressure and might allow shorter
dependency chains within each desk. Trade-off is 4 tiles instead of 2,
increasing setup overhead.

**KEY CHANGE:** NUM_DESKS=8, process 4 tiles instead of 2
- 8 desks * 8 VLEN * 4 tiles = 256 batch_size

**BASED ON:** H79 (1,904 cycles, 77.6x speedup)
"""
...
class KernelBuilderH83:
    """
    H83: 8 desks with 4 tiles for reduced register pressure.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        # Allocate per-desk vectors (8 desks instead of 16)
        NUM_DESKS = 8
        desks = []
        ...
        # Offset addresses (8 desks now)
        offset_regs = [self.alloc_scratch(f"off_{d}") for d in range(NUM_DESKS)]
        addr_tmp = [self.alloc_scratch(f"addr_tmp_{i}") for i in range(16)]  # 8 desks * 2 = 16
        ...
        def emit_tile(tile_idx):
            """Emit all operations for one tile (8 desks)."""
        ...
        # Emit 4 tiles (8 desks * 8 VLEN * 4 tiles = 256 batch_size)
        for tile in range(4):
            emit_tile(tile)
