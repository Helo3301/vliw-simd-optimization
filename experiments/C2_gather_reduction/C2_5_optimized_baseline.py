"""
C2-5: Optimized Baseline - Apply small improvements to B4-2

Let's go back to B4-2 and try smaller optimizations:
1. Verify we're not doing unnecessary work
2. Try to reduce VALU ops in the branch computation
3. Look for redundant operations

Key observation: The B4-2 code is already very optimized. Let me profile
the actual breakdown of cycles to understand the bottleneck better.

From B4-2 analysis:
- 1558 cycles total
- Theoretical minimum for gathers: 1280 cycles (2560 loads / 2 per cycle)
- Gap: 278 cycles for VALU, flow, etc.

Let's count the operations:
- 16 rounds total
- Rounds 0,1,2,11,12,13 are fused (no gathers, use preloaded nodes)
- Rounds 3-9 (7 rounds), 10, 14, 15 need gathers = 10 gather rounds

Per gather round per group (4 desks):
- 4 addr computations (VALU)
- 32 loads (4 desks x 8 lanes)
- 4 XORs (VALU)
- 72 hash ops (18 per desk x 4 desks, VALU)
- 12 branch ops (3 per desk x 4 desks, VALU)

Wait, round 15 doesn't need a branch (it's the last round)!
The current code might be skipping the branch for round 15 already.

Let me check if there's any optimization possible in how we compute addresses.
Currently: addr[lane] = forest_p + idx[lane]
This is done as valu ("+", addr, v_forest_p, idx)

What if we precompute idx + forest_p and store it directly?
Actually, that would require extra storage.

Let me try a different approach: what if we emit the loads in a different
order to improve scheduling?

Current: emit all addr for group, then all loads
Alternative: interleave addr and loads more finely

Actually, let me just reproduce B4-2 exactly and verify the baseline.
"""
...
class KernelBuilderC2_5:
    """C2-5: Exact copy of B4-2 for baseline verification"""
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        for engine, slot in self.slots:
            if engine == "valu":
                ...
                load_count += 1
            elif engine == "flow" and slot[0] == "vselect":
                flow_count += 1

        print(f"Total slots: {len(self.slots)}, Cycles: {len(self.instrs)}")
        print(f"  VALU ops: {valu_count}")
        print(f"  Load ops: {load_count}")
        print(f"  vselect ops: {flow_count}")
