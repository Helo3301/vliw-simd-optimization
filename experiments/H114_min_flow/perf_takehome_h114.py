"""
# Experiment H114: Minimize Flow Engine Operations

**GOAL:** Identify and minimize flow engine operations that bottleneck at 1 slot/cycle.

**HYPOTHESIS:** The flow engine has only 1 slot per cycle. We use flow for:
- select, vselect (if any)
- add_imm (rare)
- pause

This experiment instruments flow op counting to see if there are any vselect or
other flow ops bottlenecking the kernel.
...
class KernelBuilderH114:
    """
    H114: Minimize flow engine operations.
    """
    def __init__(self):
        ...
        self.const_map = {}
        self.vconst_map = {}
        # H114: Flow operation counter
        self.flow_op_counts = defaultdict(int)
    ...
    def emit(self, engine: str, slot: tuple):
        ...
        # H114: Count flow operations
        if engine == "flow":
            op_name = slot[0] if slot else "unknown"
            self.flow_op_counts[op_name] += 1
    ...
    def print_flow_stats(self):
        """Print flow operation statistics."""
        print("\n===== FLOW ENGINE OPERATION COUNTS =====")
        total = 0
        for op, count in sorted(self.flow_op_counts.items()):
            print(f"  {op}: {count}")
            total += count
        print(f"  TOTAL FLOW OPS: {total}")
        print("=========================================\n")
    ...
    def build_kernel(
        ...
        # H114: Print flow operation statistics before scheduling
        ...
        # H114: Count flow ops in scheduled instructions
        scheduled_flow_ops = defaultdict(int)
        for instr in self.instrs:
            if "flow" in instr:
                for flow_op in instr["flow"]:
                    op_name = flow_op[0] if flow_op else "unknown"
                    scheduled_flow_ops[op_name] += 1

        print("\n===== SCHEDULED FLOW OPS PER CYCLE =====")
        for op, count in sorted(scheduled_flow_ops.items()):
            print(f"  {op}: {count}")
        print("=========================================\n")
