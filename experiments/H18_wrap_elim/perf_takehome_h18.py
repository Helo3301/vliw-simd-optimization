"""
# Experiment H18: Wrap-Around Elimination via Bitwise AND

**Base implementation:** C4 (4,667 cycles)

**Hypothesis:** The batch wrap-around logic uses a flow `select` operation which:
- Takes a flow slot (limited to 1 per cycle)
- Could be replaced with ALU-based modulo using bitwise AND

**Key insight:** Since batch_size=256 is a power of 2:
- batch_offset = (batch_offset + 32) % 256 = (batch_offset + 32) & 255
- This can use ALU instead of flow, freeing the flow slot

**Implementation approach:**
1. Replace select-based wrap with AND-based modulo
2. Use ALU slots that are already available during store phase
3. This frees the flow slot entirely

**Expected savings:**
- Potential latency reduction if flow was the critical path
- Better utilization of ALU slots
"""
...
class KernelBuilderH18:
    """
    H18: Wrap-Around Elimination via Bitwise AND

    Key optimization: Replace select-based wrap with AND-based modulo.
    Since batch_size=256 is a power of 2:
    - batch_offset = (batch_offset + 32) & 255 gives automatic wrap

    This uses ALU instead of flow, potentially improving throughput.
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def build_kernel(
        ...
        """
        H18: Single loop kernel with AND-based batch wrap.

        Same structure as C4 but replaces the select-based wrap with:
        batch_offset = (batch_offset + 32) & (batch_size - 1)
        """
        ...
        # H18: Constant for AND-based modulo (batch_size - 1 = 255)
        batch_mask_const = self.scratch_const(batch_size - 1)
        ...
        # H18: Use AND-based wrap instead of select
        self.instrs.append({
            "load": [
                ("load", desks[3]['node_val'] + 4, desks[3]['addr'] + 4),
                ("load", desks[3]['node_val'] + 5, desks[3]['addr'] + 5),
            ...
            "alu": [
                # H14 + H18: Precompute next batch_offset with AND-based wrap
        ...
        # H14 + H18: Apply AND mask for wrap-around
        self.instrs.append({
            "load": [
                ("load", desks[3]['node_val'] + 6, desks[3]['addr'] + 6),
                ("load", desks[3]['node_val'] + 7, desks[3]['addr'] + 7),
            ...
            "alu": [
                # H18: Apply AND mask for wrap-around (batch_offset & 255)
                ("&", next_batch_offset, next_batch_offset, batch_mask_const),
        ...
        # H18: No select needed - batch_offset already wrapped via AND
        # ============================================================

        # Store desk0 + update batch_offset and iter_counter
        # H18: batch_offset is updated from next_batch_offset (already wrapped)
        self.instrs.append({
            "store": [
            ...
            "alu": [
                # H18: Copy pre-wrapped batch_offset
                ("+", batch_offset, next_batch_offset, zero_const),
        ...
        # Store desk3 - H18: no select needed!
        self.instrs.append({
            "store": [
