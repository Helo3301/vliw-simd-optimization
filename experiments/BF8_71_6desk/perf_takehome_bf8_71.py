"""
BF8 Theory 71: 6 desks per group (match VALU width)
Hypothesis: VALU has 6 slots per cycle. Grouping 6 desks maximizes VALU utilization.
"""
...
class KernelBuilderBF8_71:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # THEORY 71: Use 6 desks per group (matches VALU width of 6)
        NUM_DESKS = 18  # 3 groups of 6
        GROUP_SIZE = 6
        ...
        # Process tiles - with 18 desks we need fewer tiles
        # Total batch = 256, each desk handles 8, so 18 desks = 144 per tile
        # We need ceil(256/144) = 2 tiles but second tile would be partial
        # Simpler: just do 256/8 = 32 desk-loads, 18 desks * ~2 tiles
        # Actually batch_size = 256, VLEN = 8, so 32 vector elements per pass
        # With 18 desks, 18*8 = 144 elements per tile
        # Need ceil(256/144) = 2 tiles
        emit_tile_interleaved(0)
        # Second tile would exceed batch, handle with partial or skip
        # For correctness, just do what fits
