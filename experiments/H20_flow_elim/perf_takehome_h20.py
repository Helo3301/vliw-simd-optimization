"""
# Experiment H20: Flow Unit Complete Elimination

This experiment tests whether eliminating Flow unit operations (except cond_jump)
can improve performance by avoiding the Flow unit's serialization bottleneck.

**Background:**
- H10 showed 4% gain by replacing vselect (flow) with VALU multiply
- The flow unit is limited to 1 op/cycle - a serialization bottleneck

**Base Implementation:** C4 (4,667 cycles) still uses Flow for:
1. `cond_jump` - loop control (cannot be eliminated)
2. `select` - wrap-around handling (CAN be eliminated)
3. `pause` - sync operations (required)

**Optimization:**
Replace scalar `select(dest, cond, a, b)` with arithmetic:
- `dest = a * cond + b * (1 - cond)`

For the specific case in C4:
- `select(batch_offset, tmp_scalar, batch_offset, zero_const)`
- When tmp_scalar=1: keep batch_offset
- When tmp_scalar=0: use zero_const (0)
- Simplifies to: `batch_offset = batch_offset * tmp_scalar`

**Expected savings:** Modest - the select is only 1 op per iteration, but it
removes a flow unit dependency and can potentially be overlapped with other work.
"""
...
class KernelBuilderH20:
    """
    H20: Flow Unit Complete Elimination

    Based on C4 but replaces all flow operations (except cond_jump/pause) with
    arithmetic equivalents using ALU.

    Key change:
    - Replace: flow.select(batch_offset, tmp_scalar, batch_offset, zero_const)
    - With: alu.multiply(batch_offset, batch_offset, tmp_scalar)

    This moves work from the flow unit (1 op/cycle) to ALU (12 slots available).
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def build_kernel(
        ...
        """
        H20: Combined kernel with flow elimination optimization.
        """
        ...
        # ============================================================
        # H20 CHANGE: Replace flow.select with alu.multiply
        # Original: flow.select(batch_offset, tmp_scalar, batch_offset, zero_const)
        # New: alu.multiply(batch_offset, batch_offset, tmp_scalar)
        #
        # This works because:
        # - When tmp_scalar=1 (batch_offset < batch_size): batch_offset * 1 = batch_offset
        # - When tmp_scalar=0 (batch_offset >= batch_size): batch_offset * 0 = 0
        # ============================================================
        self.instrs.append({
            "store": [
            ...
            "alu": [
                # H20: Replace flow.select with alu.multiply
