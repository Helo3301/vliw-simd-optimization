"""
# Experiment H104: Alternative FMA Usage

**GOAL:** Explore alternative ways to use FMA and reduce operations in hash stages.

**HYPOTHESIS:** Currently we use FMA for hash stages 0, 2, 4 where the operation is
val = val * mult + const. What if we could combine operations differently using FMA
in unexpected places?

**ANALYSIS of Hash Stages:**
- Stage 0: a' = (a + c0) + (a << 12) = a * 4097 + c0  [FMA]
- Stage 1: a' = (a ^ c1) ^ (a >> 19)  [3 ops: XOR, SHIFT, XOR]
- Stage 2: a' = (a + c2) + (a << 5) = a * 33 + c2  [FMA]
- Stage 3: a' = (a + c3) ^ (a << 9)  [3 ops: ADD, SHIFT, XOR]
- Stage 4: a' = (a + c4) + (a << 3) = a * 9 + c4  [FMA]
- Stage 5: a' = (a ^ c5) ^ (a >> 16)  [3 ops: XOR, SHIFT, XOR]

**KEY INSIGHT:** For stages 1, 3, 5, the first two operations are independent (both read a).
We can emit them for all desks simultaneously, then emit the combine ops, maximizing VALU
utilization. This is "wave scheduling" - emit all stage1_op1 for all desks, then all
stage1_op2 for all desks, then all stage1_combine for all desks.

**ALTERNATIVE OPTIMIZATION ATTEMPTS:**
1. Stage 3: (a + c) ^ (a * 512) - could use FMA for a*512+0 but no savings
2. Reorder to overlap stage boundaries across desks
3. Use FMA creatively: a*1+c = a+c (but XOR doesn't distribute)
...
class KernelBuilderH104:
    """
    H104: Alternative FMA usage - explore different operation orderings.

    Key changes from H96:
    1. Wave scheduling: emit all first ops of each stage across all desks,
       then all second ops, then all combines - maximizing VALU slot fill
    2. Interleave independent operations more aggressively
    """
    def __init__(self):
