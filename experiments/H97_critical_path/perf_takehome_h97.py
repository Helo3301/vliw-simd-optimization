"""
# Experiment H97: Critical Path Scheduler

**GOAL:** Reduce cycle count by using a critical-path-first scheduling algorithm instead of greedy ASAP.

**HYPOTHESIS:** The greedy list scheduler may not be optimal. A critical-path-first scheduler that
prioritizes operations on the longest dependency chain could reduce total cycles by ensuring
bottleneck operations get scheduled as early as possible.

**KEY INSIGHT:** The greedy ASAP scheduler processes operations in emission order. This can lead to
suboptimal scheduling when operations with long dependency chains are emitted late. By computing
the critical path length for each operation (max distance to any sink) and scheduling operations
with longer critical paths first, we ensure bottleneck operations are not delayed.

**ALGORITHM:**
1. Build dependency graph from operations
2. Compute critical path length for each operation (max distance to any sink)
3. Sort operations by critical path length (longest first)
4. Schedule using the same resource constraints but prioritizing critical path operations

**BASELINE:** H96 = 1,850 cycles
**TARGET:** 1,790 cycles (60 cycle reduction)
"""
...
def _schedule_slots(slots: list[tuple[str, tuple]]) -> list[dict[str, list[tuple]]]:
    """Automatically schedule operations into VLIW bundles using a two-pass approach.

    Pass 1: Greedy ASAP to establish cycle assignments
    Pass 2: Within each cycle, reorder operations to minimize register pressure
            and maximize overlap with next cycle's operations.

    This approach combines the efficiency of greedy scheduling with critical-path
    awareness for tiebreaking.
    """
    if not slots:
        return []
    ...
    # Compute reads/writes for all operations
    rw_info = []
    engines = []
    for engine, slot in slots:
        reads, writes = _slot_rw(engine, slot)
        rw_info.append((reads, writes))
        engines.append(engine)

    # Build dependency graph for critical path calculation
    last_writer: dict[int, int] = {}
    last_readers: dict[int, set] = defaultdict(set)
    dependencies: list[set] = [set() for _ in range(n)]
    dependents: list[set] = [set() for _ in range(n)]

    for i, (reads, writes) in enumerate(rw_info):
        for addr in reads:
            if addr in last_writer:
                dep = last_writer[addr]
    ...
    while changed:
        ...
        for i in range(n - 1, -1, -1):
            if dependents[i]:
                max_dep_path = max(critical_path[j] for j in dependents[i])
                new_path = 1 + max_dep_path
                if new_path > critical_path[i]:
                    critical_path[i] = new_path
                    changed = True

    # Pass 1: Standard greedy ASAP scheduling
    ...
    op_cycle: list[int] = []  # Which cycle each op is assigned to
    ...
    for idx, (engine, slot) in enumerate(slots):
        ...
        ensure_cycle(cycle)
        cycles[cycle].setdefault(engine, []).append((slot, idx, critical_path[idx]))
        usage[cycle][engine] += 1
        op_cycle.append(cycle)
    ...
    # Pass 2: Within each cycle, sort operations by critical path (descending)
    # This ensures critical operations execute first within a cycle
    result_cycles = []
    for cycle_data in cycles:
        if not cycle_data:
            continue
        new_cycle = {}
        for engine, ops_with_info in cycle_data.items():
            # Sort by critical path descending, then by original index for stability
            ops_with_info.sort(key=lambda x: (-x[2], x[1]))
            # Extract just the slot
            new_cycle[engine] = [op[0] for op in ops_with_info]
        result_cycles.append(new_cycle)

    return result_cycles


class KernelBuilderH97:
    """
    H97: Critical-path-first scheduler - process all 256 batch elements with one tile.
    Uses critical path priority instead of greedy ASAP scheduling.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel using 32 desks per tile for maximum ILP.
        """
