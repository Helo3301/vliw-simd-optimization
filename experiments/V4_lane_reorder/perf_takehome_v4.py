"""
# Experiment V4: Lane Reordering / Broadcast Optimization

**Concept:**
The killer insight: for early rounds, all batch elements have the same (or few) indices.

**Key Observations:**
1. Round 0: ALL 256 batch elements have idx=0
   - Load tree[0] ONCE, broadcast to all lanes
   - This eliminates 8 scalar loads per vector (4 cycles of gather per desk)!

2. Round 1: Elements have idx in {1, 2} (only 2 possibilities)
3. Round 2: Elements have idx in {3, 4, 5, 6} (4 possibilities)
4. Round 3+: idx spreads across the tree

**This experiment:**
- Modifies the C4 loop to detect when we're in round 0
- Uses broadcast instead of gather for round 0
- Falls back to standard gather for all other rounds

**Expected benefit:**
- Round 0: Save ~32 cycles per iteration (8 scalar loads * 4 desks)
- With 8 batch chunks (256/32), that's ~256 cycles saved for round 0
- Total: ~256 cycles over 16 rounds = minor improvement

**Reality check:**
The savings are marginal because:
- Only round 0 benefits significantly
- The overhead of detecting "is this round 0?" might negate savings
- C4's interleaved pipeline already hides much of the gather latency
"""
...
class KernelBuilderV4:
    """
    V4: Broadcast Optimization for Round 0

    Key insight: For round 0, ALL batch elements have idx=0.
    We can load tree[0] once and broadcast to all vectors,
    eliminating the expensive gather operations.

    Implementation:
    - Uses separate loops for round 0-1 (first double-round) and rounds 2-15
    - Round 0: broadcast tree[0] instead of gather
    - Round 1+: standard gather (similar to C4)
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def emit_hash_pipeline(self, desk, v_hash_consts, v_hash_shifts, v_fma_mult):
        """Emit hash computation for a single desk."""
        # Stage 0 (FMA)
        self.instrs.append({
            "valu": [
    ...
    def build_kernel(
        ...
        """
        V4: Build kernel with broadcast optimization for round 0.

        Strategy: Use C4's structure but with a specialized first double-round
        that uses broadcast instead of gather for round 0.
        """
        ...
        # V4: Pre-load tree[0] and tree[1] and tree[2] for broadcast optimization
        ...
        # Load tree[0], tree[1], tree[2]
        self.add("load", ("load", tree_0_scalar, self.scratch["forest_values_p"]))
        self.instrs.append({
            "alu": [
                ("+", tmp_scalar, self.scratch["forest_values_p"], one_const),
            ],
        })
        self.add("load", ("load", tree_1_scalar, tmp_scalar))
        self.instrs.append({
            "alu": [
                ("+", tmp_scalar, self.scratch["forest_values_p"], two_const),
            ],
        })
        self.add("load", ("load", tree_2_scalar, tmp_scalar))

        # Process rounds in pairs (8 double-round iterations total for 16 rounds)
        ...
        # ============================================================
        # ROUND 2 OF 2: FUSED (no intermediate store/load)
        # ============================================================
        ...
        # Round 2 Desk3 gather 4-5 + desk0 branch prep with FMA + precompute next_batch_offset
        self.instrs.append({
            "load": [
                ("load", desks[3]['node_val'] + 4, desks[3]['addr'] + 4),
                ("load", desks[3]['node_val'] + 5, desks[3]['addr'] + 5),
        ...
        # Round 2 Desk3 gather 6-7 + desk0 branch + desk1 hash4 + desk2 hash1 combine + next offsets
        self.instrs.append({
            "load": [
                ("load", desks[3]['node_val'] + 6, desks[3]['addr'] + 6),
                ("load", desks[3]['node_val'] + 7, desks[3]['addr'] + 7),
        ...
        # Continue round 2 + address precomputation
        self.instrs.append({
            "valu": [
                ("^", desks[1]['tmp1'], desks[1]['val'], v_hash_consts[5]),
                (">>", desks[1]['tmp2'], desks[1]['val'], v_hash_shifts[5]),
