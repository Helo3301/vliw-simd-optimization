"""
# Experiment H28: Pre-computed Branch Masks

This hypothesis explores whether we can reduce the critical path by pre-computing
the idx*2+1 base value earlier, allowing better overlap with other operations.

**Key Insight:** The branch computation requires:
1. idx_base = idx * 2 + 1
2. branch_bit = val & 1 (after hash completes)
3. new_idx = idx_base + branch_bit

In the baseline C4, steps 1 and 2 happen in the same cycle (using different VALU slots).
Then step 3 happens in the next cycle.

**H28 Optimization Strategy:**
Pre-compute idx*2+1 EARLIER (before hash stage 5 completes), allowing us to
potentially use VALU slots that would otherwise be idle during the hash computation.

The key constraint: In VLIW, all reads happen before all writes in the same cycle.
So if we compute `tmp1 = val & 1` and `idx = base + tmp1` in the same cycle,
the ADD reads the OLD value of tmp1.

**Correct sequence:**
- Cycle N-1: branch_base = idx * 2 + 1 (can overlap with hash stage 4)
- Cycle N: tmp1 = val & 1 (after hash stage 5 completes)
- Cycle N+1: idx = branch_base + tmp1

This is the same number of cycles as C4, but potentially allows better packing
of VALU slots by moving the idx*2+1 computation earlier.
"""
...
class KernelBuilderH28:
    """
    H28: Pre-computed Branch Masks

    Key optimization: Pre-compute idx*2+1 during hash stage 4 or earlier,
    before the hash completes. This moves work out of the critical path
    and into cycles where VALU slots might otherwise be idle.
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def build_kernel(
        ...
        """
        H28: Kernel with pre-computed branch base (idx*2+1).
        """
        ...
        for d in range(NUM_DESKS):
            desk = {
                ...
                # H28: Pre-computed branch base (idx*2+1)
                'idx_base': self.alloc_scratch(f"v_idx_base_{d}", VLEN),
            }
            desks.append(desk)
        ...
        # Process 2 rounds per iteration (round fusion from H12)
        ...
        # Desk1 lanes 4-5 + desk0 hash1 prep
        self.instrs.append({
            "load": [
        ...
        # Desk1 lanes 6-7 + desk0 hash1 combine
        self.instrs.append({
            "load": [
        ...
        # Desk1 complete: XOR + desk0 hash2 + start desk2
        self.instrs.append({
            "load": [
        ...
        # Desk2 lanes 2-3 + desk0 hash3 prep + desk1 hash0
        self.instrs.append({
            "load": [
        ...
        # Desk2 lanes 4-5 + desk0 hash3 combine + desk1 hash1 prep
        self.instrs.append({
            "load": [
        ...
        # Desk2 lanes 6-7 + desk0 hash4 + desk1 hash1 combine
        # H28: Pre-compute desk0's idx*2+1 here, during hash stage 4
        self.instrs.append({
            "load": [
            ...
            "valu": [
                ...
                # H28: Pre-compute idx*2+1 for desk0 during hash4
                ("multiply_add", desks[0]['idx_base'], desks[0]['idx'], v_two, v_one),
            ],
        })

        # Desk2 complete: XOR + desk0 hash5 prep + desk1 hash2 + start desk3
        self.instrs.append({
            "load": [
        ...
        # Desk3 lanes 2-3 + desk0 hash5 combine + desk1 hash3 prep + desk2 hash0
        self.instrs.append({
            "load": [
        ...
        # Desk3 lanes 4-5 + desk0 branch bit extraction + desk1 hash3 combine + desk2 hash1
        # H28: Now that desk0's val has stage5 result, extract the branch bit
        self.instrs.append({
            "load": [
            ...
            "valu": [
                ("&", desks[0]['tmp1'], desks[0]['val'], v_one),  # Extract LSB
                ("^", desks[1]['val'], desks[1]['tmp1'], desks[1]['tmp2']),
                ("^", desks[2]['tmp1'], desks[2]['val'], v_hash_consts[1]),
                (">>", desks[2]['tmp2'], desks[2]['val'], v_hash_shifts[1]),
                # H28: Pre-compute idx*2+1 for desk1 during desk1's hash3/4 phase
                ("multiply_add", desks[1]['idx_base'], desks[1]['idx'], v_two, v_one),
            ],
        })

        # Desk3 lanes 6-7 + desk0 branch add + desk1 hash4 + desk2 hash1 combine
        # H28: Add branch bit to pre-computed idx_base
        self.instrs.append({
            "load": [
            ...
            "valu": [
                ("+", desks[0]['idx'], desks[0]['idx_base'], desks[0]['tmp1']),  # H28: idx = idx_base + bit
        ...
        # Desk0 vselect bypass + desk1 hash5 combine + desk2 hash3 prep + desk3 hash0
        self.instrs.append({
            "valu": [
        ...
        # Desk1 branch bit + desk2 hash3 combine + desk3 hash1
        # H28: Pre-compute idx*2+1 for desk2
        self.instrs.append({
            "valu": [
                ...
                ("multiply_add", desks[2]['idx_base'], desks[2]['idx'], v_two, v_one),
            ],
        })
        ...
        # Desk1 vselect bypass + desk2 hash5 combine + desk3 hash3 prep
        # H28: Pre-compute idx*2+1 for desk3
        self.instrs.append({
            "valu": [
                ...
                ("multiply_add", desks[3]['idx_base'], desks[3]['idx'], v_two, v_one),
            ],
        })

        # Desk2 branch bit + desk3 hash3 combine
        self.instrs.append({
            "valu": [
        ...
        self.instrs.append({
            "valu": [
                ("+", desks[2]['idx'], desks[2]['idx_base'], desks[2]['tmp1']),  # H28
                ("multiply_add", desks[3]['val'], desks[3]['val'], v_fma_mult[4], v_hash_consts[4]),
            ],
        })

        # Desk2 bounds + desk3 hash5 prep
        self.instrs.append({
            "valu": [
        ...
        # Desk2 vselect bypass + desk3 hash5 combine
        self.instrs.append({
            "valu": [
        ...
        self.instrs.append({
            "valu": [
                ("+", desks[3]['idx'], desks[3]['idx_base'], desks[3]['tmp1']),  # H28
            ],
        })
        ...
        # Round 2: After desk0 gather: XOR + start desk1
        self.instrs.append({
            "load": [
                ("load", desks[1]['node_val'], desks[1]['addr']),
                ("load", desks[1]['node_val'] + 1, desks[1]['addr'] + 1),
        ...
        # Desk2 lanes 6-7 + desk0 hash4 + desk1 hash1 combine
        # H28: Pre-compute desk0's idx*2+1 here
        self.instrs.append({
            "load": [
                ("load", desks[2]['node_val'] + 6, desks[2]['addr'] + 6),
                ("load", desks[2]['node_val'] + 7, desks[2]['addr'] + 7),
        ...
        # Desk2 complete + desk0 hash5 prep + desk1 hash2 + start desk3
        self.instrs.append({
            "load": [
                ("load", desks[3]['node_val'], desks[3]['addr']),
                ("load", desks[3]['node_val'] + 1, desks[3]['addr'] + 1),
        ...
        # Desk3 lanes 2-3 + desk0 hash5 combine + desk1 hash3 prep + desk2 hash0
        # Precompute next iteration's batch_offset
        self.instrs.append({
            "load": [
                ("load", desks[3]['node_val'] + 2, desks[3]['addr'] + 2),
                ("load", desks[3]['node_val'] + 3, desks[3]['addr'] + 3),
        ...
        # Desk3 lanes 4-5 + desk0 branch bit + desk1 hash3 combine + desk2 hash1
        # H28: Pre-compute desk1's idx_base
        self.instrs.append({
            "load": [
                ("load", desks[3]['node_val'] + 4, desks[3]['addr'] + 4),
                ("load", desks[3]['node_val'] + 5, desks[3]['addr'] + 5),
        ...
        self.instrs.append({
            ...
            "valu": [
                ("+", desks[0]['idx'], desks[0]['idx_base'], desks[0]['tmp1']),
                ("multiply_add", desks[1]['val'], desks[1]['val'], v_fma_mult[4], v_hash_consts[4]),
                ("^", desks[2]['val'], desks[2]['tmp1'], desks[2]['tmp2']),
            ],
            "alu": [
        ...
        # Desk0 bounds + desk1 hash5 prep + desk2 hash2 + desk3 XOR
        self.instrs.append({
            "valu": [
                ("<", desks[0]['tmp1'], desks[0]['idx'], v_n_nodes),
                ("^", desks[1]['tmp1'], desks[1]['val'], v_hash_consts[5]),
        ...
        # Desk1 branch bit + desk2 hash3 combine + desk3 hash1
        # H28: Pre-compute desk2's idx_base
        self.instrs.append({
            "valu": [
                ("&", desks[1]['tmp1'], desks[1]['val'], v_one),
                ("^", desks[2]['val'], desks[2]['tmp1'], desks[2]['tmp2']),
        ...
        self.instrs.append({
            "valu": [
                ("+", desks[1]['idx'], desks[1]['idx_base'], desks[1]['tmp1']),
                ("multiply_add", desks[2]['val'], desks[2]['val'], v_fma_mult[4], v_hash_consts[4]),
                ("^", desks[3]['val'], desks[3]['tmp1'], desks[3]['tmp2']),
            ],
        })
        ...
        # Desk1 vselect bypass + desk2 hash5 combine + desk3 hash3 prep
        # H28: Pre-compute desk3's idx_base
        self.instrs.append({
            "valu": [
                ("*", desks[1]['idx'], desks[1]['idx'], desks[1]['tmp1']),
                ("^", desks[2]['val'], desks[2]['tmp1'], desks[2]['tmp2']),
        ...
        # Desk2 branch bit + desk3 hash3 combine + compute store addresses
        self.instrs.append({
            "valu": [
                ("&", desks[2]['tmp1'], desks[2]['val'], v_one),
                ("^", desks[3]['val'], desks[3]['tmp1'], desks[3]['tmp2']),
        ...
        # Desk2 branch add + desk3 hash4 + more store addresses
        self.instrs.append({
            "valu": [
                ("+", desks[2]['idx'], desks[2]['idx_base'], desks[2]['tmp1']),
                ("multiply_add", desks[3]['val'], desks[3]['val'], v_fma_mult[4], v_hash_consts[4]),
            ],
            "alu": [
                ("+", addr_tmp[4], self.scratch["inp_indices_p"], offset_regs[2]),
        ...
        self.instrs.append({
            "valu": [
                ("+", desks[3]['idx'], desks[3]['idx_base'], desks[3]['tmp1']),
            ],
        })
