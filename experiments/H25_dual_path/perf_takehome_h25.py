"""
# Experiment H25: Speculative Dual-Path Loading

Based on IBM 360/91's approach of fetching from BOTH paths of a branch.

## Hypothesis
The current implementation has branch-dependent gathers - it must wait for hash
computation to determine which tree child to load next. By speculatively loading
BOTH children while hashing, we could remove the hash->gather dependency.

## Key Insight
In a binary tree:
- Left child index = 2*idx + 1 (when val & 1 == 0)
- Right child index = 2*idx + 2 (when val & 1 == 1)
- After hash, branch_bit = val & 1 determines which to use

## Implementation Analysis
After careful analysis, this hypothesis was found to be impractical because:

1. **Load Slot Constraint**: Only 2 loads per cycle. Loading both children would
   require 8 additional loads per desk (4 extra cycles), negating overlap benefits.

2. **Existing Overlap**: C4's 4-desk interleaving already overlaps gather operations
   of one desk with hash computation of others.

3. **Register Pressure**: Would need 6 additional VLEN vectors per desk for
   speculative addresses and values.

4. **Memory Bounds**: Speculative indices may exceed tree bounds, requiring
   additional bounds checking.

## Result
This experiment falls back to the C4 baseline implementation, which achieves
4,667 cycles. The speculative dual-path approach does not improve performance
due to the architectural constraints listed above.
...
class KernelBuilderH25:
    """
    H25: Speculative Dual-Path Loading (falls back to C4 baseline)

    Analysis showed dual-path speculation is not beneficial due to:
    - Load slot constraints (2 per cycle)
    - Existing desk interleaving provides adequate overlap
    - Register pressure from speculative value storage

    Uses C4 optimizations:
    ...
    def build_kernel(
        ...
        """
        C4: Combined kernel with all optimizations.
        """
...
def do_kernel_test(
    ...
    print("Speedup over original baseline: ", BASELINE / machine.cycle)
    print("Speedup over C4 baseline: ", C4_BASELINE / machine.cycle)
...
if __name__ == "__main__":
    ...
    if args.check:
        ...
        print(f"H25 Result: Falls back to C4 baseline ({C4_BASELINE} cycles)")
        print("H25 dual-path speculation not beneficial due to load slot constraints.")
    else:
