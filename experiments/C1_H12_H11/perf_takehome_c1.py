"""
# Experiment C1: Combined H12 (Round Fusion) + H11 (Branch FMA)

This kernel combines two winning optimizations:

**H12 Key Technique (Base):** Round Fusion
...
- Achieved: 5,115 cycles

**H11 Key Technique (Added):** Branch FMA
- Use `multiply_add(idx, idx, 2, 1)` for branch computation instead of 3 separate ops
- Before: idx = idx * 2; idx = idx + 1; idx = idx + branch
- After:  idx = multiply_add(idx, v_two, v_one); idx = idx + branch
- Saves 1 VALU operation per desk per round

**Expected Combined Savings:**
- H12 already saves ~500 cycles from eliminated vload/vstore
- H11 saves ~128 cycles from FMA optimization (but H12 has 2 rounds per iteration)
- Since H12 processes double-rounds, we have 8 iterations * 4 desks = 32 branch sequences
  But each sequence has 2 rounds, so 64 total branch FMAs
- Expected: ~5,000 cycles or better
"""
...
class KernelBuilderC1:
    """
    C1: Combined H12 (Round Fusion) + H11 (Branch FMA)

    Base: H12 Round Fusion - process 2 rounds per memory access cycle
    Added: H11 Branch FMA - use multiply_add for idx*2+1 computation

    Branch computation change:
    Before (H12):
    ...
    def build_kernel(
        ...
        """
        C1: Combined Round Fusion + Branch FMA kernel.
        """
        ...
        # Use 4 desks for interleaving (same as H7H10)
        NUM_DESKS = 4
        ...
        # === MAIN LOOP with ROUND FUSION + BRANCH FMA ===
        ...
        # C1 CHANGE: Desk3 gather 4-5 + desk0 branch (AND + FMA) + desk1 hash3 combine + desk2 hash1 prep
        # H11 optimization: Use FMA for idx*2+1 instead of separate MUL and ADD
        self.instrs.append({
            "load": [
            ...
            "valu": [
                ...
                ("multiply_add", desks[0]['idx'], desks[0]['idx'], v_two, v_one),  # C1: FMA replaces MUL+ADD
                ("^", desks[1]['val'], desks[1]['tmp1'], desks[1]['tmp2']),
        ...
        # C1 CHANGE: Desk3 gather 6-7 + desk0 branch (add tmp1 only) + desk1 hash4 + desk2 hash1 combine
        # The +1 is now part of the FMA above
        self.instrs.append({
            "load": [
            ...
            "valu": [
                ("+", desks[0]['idx'], desks[0]['idx'], desks[0]['tmp1']),  # C1: Only add branch bit
                ("multiply_add", desks[1]['val'], desks[1]['val'], v_fma_mult[4], v_hash_consts[4]),
                ("^", desks[2]['val'], desks[2]['tmp1'], desks[2]['tmp2']),
            ],
        })

        # Continue desk pipeline - hash ops for desks 1,2,3
        self.instrs.append({
            "valu": [
        ...
        # C1 CHANGE: Desk0 vselect bypass + desk1 branch (AND + FMA) + desk2 hash3 combine + desk3 hash1 prep
        self.instrs.append({
            "valu": [
                ("*", desks[0]['idx'], desks[0]['idx'], desks[0]['tmp1']),
                ("&", desks[1]['tmp1'], desks[1]['val'], v_one),
                ("multiply_add", desks[1]['idx'], desks[1]['idx'], v_two, v_one),  # C1: FMA replaces MUL+ADD
                ("^", desks[2]['val'], desks[2]['tmp1'], desks[2]['tmp2']),
        ...
        # C1 CHANGE: Desk1 branch (add tmp1 only) + desk2 hash4 + desk3 hash1 combine
        self.instrs.append({
            "valu": [
                ("+", desks[1]['idx'], desks[1]['idx'], desks[1]['tmp1']),  # C1: Only add branch bit
        ...
        # C1 CHANGE: Desk1 vselect bypass + desk2 branch (AND + FMA) + desk3 hash3 combine
        self.instrs.append({
            "valu": [
                ("*", desks[1]['idx'], desks[1]['idx'], desks[1]['tmp1']),
                ("&", desks[2]['tmp1'], desks[2]['val'], v_one),
                ("multiply_add", desks[2]['idx'], desks[2]['idx'], v_two, v_one),  # C1: FMA replaces MUL+ADD
                ("^", desks[3]['val'], desks[3]['tmp1'], desks[3]['tmp2']),
            ],
        })

        # C1 CHANGE: Desk2 branch (add tmp1 only) + desk3 hash4
        self.instrs.append({
            "valu": [
                ("+", desks[2]['idx'], desks[2]['idx'], desks[2]['tmp1']),  # C1: Only add branch bit
        ...
        # C1 CHANGE: Desk2 vselect bypass + desk3 branch (AND + FMA)
        self.instrs.append({
            "valu": [
                ("*", desks[2]['idx'], desks[2]['idx'], desks[2]['tmp1']),
                ("&", desks[3]['tmp1'], desks[3]['val'], v_one),
                ("multiply_add", desks[3]['idx'], desks[3]['idx'], v_two, v_one),  # C1: FMA replaces MUL+ADD
            ],
        })

        # C1 CHANGE: Desk3 branch (add tmp1 only)
        self.instrs.append({
            "valu": [
                ("+", desks[3]['idx'], desks[3]['idx'], desks[3]['tmp1']),  # C1: Only add branch bit
            ],
        })
        ...
        # STORE PHASE - after both rounds complete, write back to memory
