"""
# Experiment H145e: H140 + Aggressive Batching

**GOAL:** Build on H145b's double-pump success with more aggressive batching
of operations across desks within each round.

**HYPOTHESIS:** H145b only double-pumps the hash stages. We can extend this
to batch all XOR operations, all address computations, all gathers, etc.
This might further improve VALU/load slot utilization.

**BASE:** H145b = 1,643 cycles
"""
...
class KernelBuilderH145e:
    """
    H145e: H140 + aggressive batching of all operations, not just hash.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel with aggressive batching of operations.
        """
        ...
        # Per-desk vectors with per-desk temps
        NUM_DESKS = 16
        desks = []
        ...
        # Batched branch for a group
        def emit_branch_batched(group_desks):
            even_in_group = [d for i, d in enumerate(group_desks) if i % 2 == 0]
            odd_in_group = [d for i, d in enumerate(group_desks) if i % 2 == 1]

            # Batch the AND operation
            for d in even_in_group:
                self.emit("valu", ("&", desks[d]['tmp1'], desks[d]['val'], v_one))
            for d in odd_in_group:
                self.emit("valu", ("&", desks[d]['tmp1'], desks[d]['val'], v_one))
            # Batch the FMA
            for d in group_desks:
            ...
            # Batch the ADD
            for d in even_in_group:
                self.emit("valu", ("+", desks[d]['idx'], desks[d]['idx'], desks[d]['tmp1']))
            for d in odd_in_group:
                self.emit("valu", ("+", desks[d]['idx'], desks[d]['idx'], desks[d]['tmp1']))

        def emit_bounds_batched(group_desks):
            even_in_group = [d for i, d in enumerate(group_desks) if i % 2 == 0]
            odd_in_group = [d for i, d in enumerate(group_desks) if i % 2 == 1]

            for d in even_in_group:
                self.emit("valu", ("<", desks[d]['tmp1'], desks[d]['idx'], v_n_nodes))
            for d in odd_in_group:
                self.emit("valu", ("<", desks[d]['tmp1'], desks[d]['idx'], v_n_nodes))
            for d in even_in_group:
                self.emit("valu", ("*", desks[d]['idx'], desks[d]['idx'], desks[d]['tmp1']))
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                # Round 0 - batch XOR, double-pump hash, batch branch
                for d in group_desks:
                    self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], v_tree[0]))
                emit_hash_stages_double_pump(group_desks)
                emit_branch_batched(group_desks)

                # Round 1 - batch selection ops
                even_in_group = [d for i, d in enumerate(group_desks) if i % 2 == 0]
                odd_in_group = [d for i, d in enumerate(group_desks) if i % 2 == 1]
                for d in even_in_group:
                ...
                # Round 2 - 4-way selection (kept sequential for correctness)
                for d in group_desks:
                ...
                # Rounds 3-9: Gather - batch addr, batch loads, batch XOR
                ...
                emit_hash_stages_double_pump(group_desks)
                emit_branch_batched(group_desks)
                emit_bounds_batched(group_desks)
