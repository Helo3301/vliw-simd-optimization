"""
# Experiment H145c: H140 + Inline Diff (from H134)

**GOAL:** Combine H140's interleaved round processing with H134's inline diff computation.

**HYPOTHESIS:** H134 computes diffs inline just before the rounds that need them
instead of precomputing during init. This might enable better scheduling overlap
with the preceding round's operations.
...
class KernelBuilderH145c:
    """
    H145c: H140 + inline diff computation from H134.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel combining H140's interleaved rounds with H134's inline diffs.
        """
        ...
        print(f"  - H134: Inline diff computation")
        ...
        def emit_tile_interleaved(tile_idx):
            """H82-style interleaved rounds with H134-style inline diffs."""
            ...
            for gi, group_desks in enumerate(all_groups):
                # Round 0
                for d in group_desks:
                    emit_round_0(d)

                # H134 CHANGE: Compute v_diff_1_2 inline just before Round 1
                # Only for the first group in each tile (avoid redundant computation)
                if gi == 0:
                ...
                # H134 CHANGE: Compute v_diff_3_4, v_diff_5_6 inline just before Round 2
