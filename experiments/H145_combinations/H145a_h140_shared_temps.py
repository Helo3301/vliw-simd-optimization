"""
# Experiment H145a: H140 + Shared Temps (from H85)

**GOAL:** Combine H140's interleaved round processing with H85's shared temporary registers.

**HYPOTHESIS:** H140 uses per-desk tmp1/tmp2 (16*2=32 vectors = 256 slots).
H85 uses shared temps (16 vectors = 128 slots), saving 128 slots.
This might improve cache efficiency or allow other optimizations.

**BASE:** H140 = 1,645 cycles
**OPTIMIZATIONS:**
1. H82: Interleaved round processing (groups of 4)
2. H105: Reduced preload (7 nodes)
3. H120: Fast init (4 header values)
4. H133: Skip final branch
5. NEW H85: Shared temp registers (8 pairs for 16 desks)

**NOTE:** H140 already has interleaved rounds. Adding shared temps reduces register pressure
but may cause more register conflicts within the scheduler.
"""
...
class KernelBuilderH145a:
    """
    H145a: H140 + shared temp registers from H85.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel combining H140's interleaved rounds with H85's shared temps.
        """
        tmp_scalar = self.alloc_scratch("tmp_scalar")
        tmp_addr = self.alloc_scratch("tmp_addr")

        # H120: Fast init - only load 4 of 7 header values
        fast_init_vars = [
            ("n_nodes", 1),
            ("forest_values_p", 4),
            ("inp_indices_p", 5),
        ...
        # H105: Reduced preload - only 7 nodes
        NUM_PRELOADED = 7
        v_tree = []
        ...
        # Allocate per-desk vectors (16 desks) - WITHOUT tmp1/tmp2
        NUM_DESKS = 16
        desks = []
        for d in range(NUM_DESKS):
            desk = {
                ...
                # NO tmp1/tmp2 - using shared temps
            }
            desks.append(desk)

        # H85: SHARED TEMP REGISTERS - 8 pairs for 16 desks
        # Each pair of desks (0-1, 2-3, etc.) shares temps
        NUM_SHARED_TEMPS = 16  # 8 pairs * 2 temps
        shared_temps = [self.alloc_vec(f"v_shared_tmp_{i}") for i in range(NUM_SHARED_TEMPS)]

        def get_temps(desk_idx):
            """Get temp registers for a desk. Each pair shares temps."""
            pair = desk_idx // 2
            return shared_temps[pair * 2], shared_temps[pair * 2 + 1]
        ...
        # Helper functions using shared temps
        ...
        def emit_round_1(desk_idx):
            d = desks[desk_idx]
            tmp1, _ = get_temps(desk_idx)
            self.emit("valu", ("-", tmp1, d['idx'], v_one))
            self.emit("valu", ("multiply_add", d['node_val'], tmp1, v_diff_1_2, v_tree[1]))
        ...
        def emit_round_2(desk_idx):
            d = desks[desk_idx]
            tmp1, tmp2 = get_temps(desk_idx)
            self.emit("valu", ("-", tmp1, d['idx'], v_three))
            self.emit("valu", ("&", tmp2, tmp1, v_one))
            self.emit("valu", (">>", d['addr'], tmp1, v_one))
            self.emit("valu", ("multiply_add", d['node_val'], tmp2, v_diff_3_4, v_tree[3]))
            self.emit("valu", ("multiply_add", tmp1, tmp2, v_diff_5_6, v_tree[5]))
            self.emit("valu", ("-", tmp2, tmp1, d['node_val']))
            self.emit("valu", ("multiply_add", d['node_val'], d['addr'], tmp2, d['node_val']))
            self.emit("valu", ("^", d['val'], d['val'], d['node_val']))
            emit_hash_stages(desk_idx)
            emit_branch(desk_idx)
        ...
        def emit_tile_interleaved(tile_idx):
            """H82-style interleaved rounds with groups of 4."""
            ...
            # Process in groups of 4 (H82 interleaving)
            GROUP_SIZE = 4
