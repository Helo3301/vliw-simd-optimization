"""
# Experiment H102: Hash Stage Operation Reordering

**GOAL:** Reduce critical path by reordering operations within and across 3-op hash stages.

**HYPOTHESIS:** The 3-op hash stages (1, 3, 5) have this pattern:
- tmp1 = val OP1 const
- tmp2 = val OP2 shift
- val = tmp1 ^ tmp2

Maybe reordering operations within and across stages can reduce critical path:
- Emit all first ops of stage 1/3/5 for all desks
- Emit all second ops of stage 1/3/5 for all desks
- Emit all third ops of stage 1/3/5 for all desks

**KEY CHANGE:** Instead of completing stage 1 before stage 2, interleave:
- All stage1_op1, all stage1_op2, stage2_FMA, all stage1_op3, all stage3_op1...
...
class KernelBuilderH102:
    """
    H102: Hash stage operation reordering for reduced critical path.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        def emit_hash_stages_all_desks():
            """Emit hash stages for all 32 desks with interleaved operations.

            NEW APPROACH (H102): Interleave operations across stages to reduce critical path:
            - Emit all stage1_op1 for all groups
            - Emit all stage1_op2 for all groups
            - Emit stage2_FMA for all desks (can start while stage1 ops complete)
            - Emit all stage1_op3 for all groups (final XOR)
            - Emit all stage3_op1 for all groups
            - Emit all stage3_op2 for all groups
            - Emit stage4_FMA for all desks
            - Emit all stage3_op3 for all groups
            - Emit all stage5_op1 for all groups
            - Emit all stage5_op2 for all groups
            - Emit all stage5_op3 for all groups
            """
            ...
            # INTERLEAVED STAGES 1, 2, 3 ================================

            # Stage 1, op1 (XOR with const) - all groups
            for group in [group_0, group_1, group_2, group_3]:
                for d in group:
                    tmp1, tmp2 = get_temps(d)
                    self.emit("valu", ("^", tmp1, desks[d]['val'], v_hash_consts[1]))

            # Stage 1, op2 (right shift) - all groups
            for group in [group_0, group_1, group_2, group_3]:
                for d in group:
                    tmp1, tmp2 = get_temps(d)
                    self.emit("valu", (">>", tmp2, desks[d]['val'], v_hash_shifts[1]))

            # Stage 2 (FMA) - emit early to overlap with stage1 completion
            # This can start as soon as stage1_op2 reads val (doesn't need stage1 complete)
            for d in range(NUM_DESKS):
                self.emit("valu", ("multiply_add", desks[d]['val'], desks[d]['val'], v_fma_mult[2], v_hash_consts[2]))

            # Stage 1, op3 (final XOR) - this completes stage 1 but val already consumed by stage 2
            # Wait, this writes to val which was already read by stage 2 FMA - this is correct
            # because the scheduler will order these properly based on dependencies
            for group in [group_0, group_1, group_2, group_3]:
                for d in group:
                    tmp1, tmp2 = get_temps(d)
                    self.emit("valu", ("^", desks[d]['val'], tmp1, tmp2))

            # Stage 3, op1 (ADD with const) - all groups
            for group in [group_0, group_1, group_2, group_3]:
                for d in group:
                    tmp1, tmp2 = get_temps(d)
                    self.emit("valu", ("+", tmp1, desks[d]['val'], v_hash_consts[3]))

            # Stage 3, op2 (left shift) - all groups
            for group in [group_0, group_1, group_2, group_3]:
                for d in group:
                    tmp1, tmp2 = get_temps(d)
                    self.emit("valu", ("<<", tmp2, desks[d]['val'], v_hash_shifts[3]))

            # Stage 4 (FMA) - emit early to overlap
            for d in range(NUM_DESKS):
                self.emit("valu", ("multiply_add", desks[d]['val'], desks[d]['val'], v_fma_mult[4], v_hash_consts[4]))

            # Stage 3, op3 (final XOR)
            for group in [group_0, group_1, group_2, group_3]:
                for d in group:
                    tmp1, tmp2 = get_temps(d)
                    self.emit("valu", ("^", desks[d]['val'], tmp1, tmp2))
