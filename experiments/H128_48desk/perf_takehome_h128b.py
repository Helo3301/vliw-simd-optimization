"""
# Experiment H128b: 2-Tile Strategy with 24 Desks Each

**GOAL:** Test if processing batch in 2 tiles of 24 desks provides better ILP than 1 tile of 32 desks.

**HYPOTHESIS:**
- 24 desks with 4-desk sharing = 24*32 + 12*16 + 24 + fixed = 768 + 192 + 24 + ~250 = 1234 slots
- Each tile processes 192 elements (24*8)
- 2 tiles needed for 256 elements, but we'll process 384 total (192 unused)

Actually, batch_size = 256 = 32 * 8, so:
- 1 tile of 32 desks processes exactly 256 elements
- 2 tiles of 16 desks each would also work (128 per tile)

Let me try 2 tiles of 16 desks to see if tile overhead hurts or helps.
...
class KernelBuilderH128b:
    """
    H128b: 2 tiles of 16 desks each (vs 1 tile of 32 desks in H120).

    Test if smaller tiles with less register pressure provide better scheduling.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel using 2 tiles of 16 desks each.
        """
        ...
        # 16 desks with 4-desk sharing (4 groups)
        NUM_DESKS = 16
        desks = []
        ...
        # Shared temps: 4 groups of 4 desks, 2 temps each = 8 temps
        NUM_SHARED_TEMPS = 8
        shared_temps = [self.alloc_vec(f"v_shared_tmp_{i}") for i in range(NUM_SHARED_TEMPS)]

        def get_temps(desk_idx):
            group = desk_idx // 4
            return shared_temps[group * 2], shared_temps[group * 2 + 1]
        ...
        print(f"  - H128b: 2 tiles of 16 desks each")
        print(f"  - 4-desk temp sharing (8 shared temps)")
        ...
        def emit_tile(tile_base_offset):
            """Emit operations for one tile."""
            # Set tile offset
            self.emit("load", ("const", tile_offset, tile_base_offset))

            # Load offsets for all desks in this tile
            for d in range(NUM_DESKS):
                self.emit("load", ("const", offset_regs[d], tile_base_offset + d * VLEN))
        ...
        # Emit both tiles
        emit_tile(0)           # Elements 0-127 (desks 0-15, 8 elements each)
        emit_tile(16 * VLEN)   # Elements 128-255 (desks 16-31 conceptually)
