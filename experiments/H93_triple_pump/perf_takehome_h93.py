"""
# Experiment H93: Triple-Pumped Hash Stages

**GOAL:** Extend H87's double-pumping to triple-pumping by interleaving 3 desks at a time.

**HYPOTHESIS:** If double-pumping (2 desks interleaved) helped improve from H85's 1,898 to H87's 1,851,
then triple-pumping (3 desks interleaved) might help even more by exposing more parallelism.

**BASELINE:** H87 achieved 1,851 cycles

**APPROACHES TESTED:**

1. **Mod-3 grouping (3 waves of 5-6 desks):**
   - Groups desks by d % 3: [0,3,6,9,12,15], [1,4,7,10,13], [2,5,8,11,14]
   - Result: 1,861 cycles (WORSE - smaller waves reduce parallelism)

2. **Per-desk temps with deep interleaving:**
   - Give each desk its own temps (32 vectors total)
   - Allows interleaving op1/op2 across all desks
   - Result: 1,908 cycles (WORSE - memory pressure from extra temps)

3. **Batched branch operations:**
   - Emit branch ops by type across all desks
   - Result: 1,875 cycles (WORSE - adds scheduling overhead)

4. **H87-style even-then-odd (final):**
   - Same as H87: process even desks, then odd desks
   - 8 desks per wave, maximum parallelism
   - Result: 1,851 cycles (MATCHES H87)

**CONCLUSION:**
Triple-pumping does NOT improve over H87's double-pumping because:
- H87's even/odd split (8 desks per wave) maximizes parallelism
- Smaller waves (5-6 desks) reduce opportunities for the scheduler
- Temp register sharing constraints force even-then-odd ordering
- The scheduler already efficiently packs independent operations

The optimal approach remains H87's even-then-odd pattern with 2 waves of 8 desks each.

**ALGORITHM:** Same as H87 (shared temps, wrap exploitation, no bounds on R11-15)
**SCHEDULING:** Automatic via _schedule_slots()

**FINAL RESULT:** 1,851 cycles (no improvement over H87)
"""
...
class KernelBuilderH93:
    """
    H93: Triple-pumped hash stages - interleave FMA and multi-op stages between desk triplets.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        Uses shared temp registers with TRIPLE-PUMPED interleaving.
        """
        ...
        # SHARED TEMP REGISTERS - Use 16 vectors like H87 (128 slots)
        ...
        def get_temps(desk_idx):
            """Get temp registers for a desk (pairs: 0-1, 2-3, etc. share temps)."""
        ...
        print(f"  - Triple-pump with 3 waves (mod-3 grouping)")

        # Pause before main computation
        self.emit("flow", ("pause",))

        # Triple-pump approach: Process all 6 hash stages together with optimized emit order
        #
        # Key insight: Instead of processing stages sequentially, we can batch operations
        # of the same type across consecutive stages when they don't share temps.
        #
        # H87 emits: Stage0, Stage1, Stage2, Stage3, Stage4, Stage5
        #
        # Triple-pump batches 3 FMA stages (0, 2, 4) and 3 multi-op stages (1, 3, 5)
        # For multi-op stages, we process in larger batches:
        # - All op1 for stages 1,3,5 for even desks
        # - All op1 for stages 1,3,5 for odd desks
        # - All op2 for stages 1,3,5 for even desks
        # - etc.
        #
        # This creates more independent operations per batch.
        even_desks = list(range(0, NUM_DESKS, 2))  # [0, 2, 4, 6, 8, 10, 12, 14]
        odd_desks = list(range(1, NUM_DESKS, 2))   # [1, 3, 5, 7, 9, 11, 13, 15]

        def emit_hash_stages_triple_pumped():
            """Emit hash stages batched by operation type across all stages.

            Instead of sequential stages, batch all FMAs together and all 3-op
            stages together. For 3-op stages, use even-then-odd pattern like H87.
            """
            # ALL FMA stages (0, 2, 4) - batch together for maximum parallelism
            # Stage 0 FMA
            for d in range(NUM_DESKS):
        ...
        def emit_all_branches():
            """Emit branch operations for all desks (sequential, same as H87)."""
            for d in range(NUM_DESKS):
                emit_branch(d)
        ...
        def emit_tile(tile_idx):
            ...
            # Triple-pump approach: emit operations by type to maximize VALU utilization
            for d in range(NUM_DESKS):
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], v_tree[0]))
            emit_hash_stages_triple_pumped()
            emit_all_branches()

            # Round 1: Indices in {1, 2}, use arithmetic selection
            # Even-then-odd pattern (same as H87)
            for d in even_desks:
                tmp1, _ = get_temps(d)
            ...
            # Triple-pump: batch addr calc, gather, xor, then triple-pump hash
            for _round in range(3, 10):
                ...
                for d in range(NUM_DESKS):
                    self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))
                # Triple-pumped hash stages
                emit_hash_stages_triple_pumped()
                # Branch for all desks (batched)
                emit_all_branches()

            # Round 10: Gather WITH bounds
            # Full triple-pump for everything except bounds which must be after branch
            for d in range(NUM_DESKS):
                self.emit("valu", ("+", desks[d]['addr'], v_forest_p, desks[d]['idx']))
            for d in range(NUM_DESKS):
                for lane in range(VLEN):
