"""
C3-3: Stage 3 Optimization

Looking at stage 3: val = (val + C) ^ (val << 9)
...
The FMA optimization doesn't work here because of the XOR at the end.
But we could potentially use: val = val*512 + C somehow?

No wait, (val + C) ^ (val << 9) is fundamentally different from val*k + C.

Let me verify this stage is correctly implemented and see if there's any slack.
"""
...
class KernelBuilderC3_3:
    """C3-3: Try alternative stage implementations"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Precompute stage 3 multiplier: 512 (for << 9)
        v_512 = self.scratch_vconst(512, "v_512")
        ...
        def emit_hash_interleaved(group_desks):
            """Emit hash stages interleaved across desks - try using multiply for << 9"""
            # Stage 0: FMA
            ...
            # Stage 3: val = (val + C) ^ (val << 9) = (val + C) ^ (val * 512)
            # Try using multiply instead of shift - maybe allows better pipelining?
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("+", desk['tmp1'], desk['val'], v_hash_consts[3]))
                self.emit("valu", ("*", desk['tmp2'], desk['val'], v_512))  # val * 512 instead of val << 9
                self.emit("valu", ("^", desk['val'], desk['tmp1'], desk['tmp2']))
