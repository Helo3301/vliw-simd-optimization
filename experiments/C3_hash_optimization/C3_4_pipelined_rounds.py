"""
C3-4: Pipelined Round Processing

Hypothesis: Instead of processing each round completely before starting the next,
overlap the operations more aggressively.

The gather round structure is:
1. addr = forest_p + idx (VALU)
2. node = load(addr) (LOAD - slow)
3. val = val XOR node (VALU)
4. hash (VALU - 12 ops)
5. branch (VALU - 3 ops)

We could potentially:
- Start addr computation for round N+1 while still doing branch for round N
- The loads for round N+1 can start while the branch for round N completes

But the key insight is: the load is the bottleneck. Let me try overlapping groups.
"""
...
class KernelBuilderC3_4:
    """C3-4: Pipelined round processing - emit all addr/load first, then hash"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_gather_rounds_pipelined(group_desks, round_start, round_end):
            """
            Emit rounds with better pipelining:
            - All addr computations first
            - All loads (interleaved per round)
            - All XOR + hash + branch
            """
            num_rounds = round_end - round_start

            # Store round-specific node values
            round_nodes = {}
            for rnd in range(round_start, round_end):
                round_nodes[rnd] = {}
                for d in group_desks:
                    # Reuse node_val since we process sequentially
                    round_nodes[rnd][d] = desks[d]['node_val']

            # Process each round sequentially (we can't fully parallelize due to idx dependencies)
            for rnd in range(round_start, round_end):
                # Addr computation
                for d in group_desks:
                    desk = desks[d]
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                ...
                # Rounds 3-9 pipelined
                emit_gather_rounds_pipelined(group_desks, 3, 10)

                emit_round_10_interleaved(group_desks)
                emit_rounds_11_12_13_fused(group_desks)

                # Round 14
                emit_gather_rounds_pipelined(group_desks, 14, 15)
