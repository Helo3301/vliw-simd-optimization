"""
C3-5: Branch Simplification

Looking at the branch computation:
  bit = val & 1
  idx = 2*idx + 1 + bit = idx*2 + 1 + bit

What if we can simplify by using idx*2 as a single multiply?
  idx = (idx << 1) + 1 + bit

But wait - 2*idx is the same as idx << 1, and we're using FMA for 2*idx+1.

Actually, let me try: idx = (idx << 1) | 1 | bit
Since bit is 0 or 1, and we're adding 1...
  2*idx + 1 + bit = 2*idx + 1 when bit=0, 2*idx + 2 when bit=1

OR approach: (idx << 1) | 1 | bit
  = (idx << 1) | 1 when bit=0 = 2*idx + 1 (correct)
  = (idx << 1) | 1 | 1 when bit=1 = 2*idx + 1 (WRONG - should be 2*idx + 2)

So OR doesn't work. Stick with the current approach.

Let me try: val & 1 can be extracted early and used for multiple rounds.
But val changes after each hash, so we can't cache the bit.

Actually, let me look at reducing operations in a different way.
The current branch is:
  bit = val & 1  (1 op)
  idx = idx*2 + 1 (FMA: 1 op)
  idx = idx + bit (1 op)

Could we combine the last two?
  idx = idx*2 + 1 + bit

If we had a "multiply_add_add" instruction that does a*b + c + d, we could do it in 1 op.
...
But (1 + bit) is either 1 or 2, which we'd need to compute separately.

Wait - can we use vselect to pick between 2*idx+1 and 2*idx+2?
  idx_plus_1 = 2*idx + 1  (FMA)
  idx_plus_2 = 2*idx + 2  (FMA)
  bit = val & 1
  idx = vselect(bit, idx_plus_2, idx_plus_1)

This is 4 ops instead of 3. Worse.

What about: (2*idx + 1) + bit using precomputed (2*idx + 1)?

Actually the current approach is already optimal. Let me focus elsewhere.

Let me try a completely different approach: What if we process two groups in parallel?
The current setup processes 4 desks per group sequentially.
What if we emit operations from two groups interleaved?
"""
...
class KernelBuilderC3_5:
    """C3-5: Try processing two groups in parallel for better ILP"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_tile_two_groups_parallel(tile_idx):
            """Process two groups in parallel instead of one at a time"""
            ...
            # Process 2 groups of 4 desks at a time (8 desks total per round)
            GROUP_SIZE = 4
            SUPER_GROUP_SIZE = 8  # 2 groups processed together

            for super_g in range(NUM_DESKS // SUPER_GROUP_SIZE):
                # Two groups processed together
                group1 = list(range(super_g * SUPER_GROUP_SIZE, super_g * SUPER_GROUP_SIZE + GROUP_SIZE))
                group2 = list(range(super_g * SUPER_GROUP_SIZE + GROUP_SIZE, (super_g + 1) * SUPER_GROUP_SIZE))
                combined = group1 + group2

                emit_rounds_0_1_2_fused(combined)

                for _rnd in range(3, 10):
                    emit_gather_round_interleaved(combined)

                emit_round_10_interleaved(combined)
                emit_rounds_11_12_13_fused(combined)
                emit_gather_round_interleaved(combined)
                emit_round_15_final_interleaved(combined)

            for d in range(NUM_DESKS):
                self.emit("store", ("vstore", addr_tmp[d*2], desks[d]['idx']))
                self.emit("store", ("vstore", addr_tmp[d*2+1], desks[d]['val']))

        emit_tile_two_groups_parallel(0)
        emit_tile_two_groups_parallel(1)
...
def do_kernel_test(forest_height: int, rounds: int, batch_size: int, seed: int = 123,
    ...
    for i, ref_mem in enumerate(reference_kernel2(mem, value_trace)):
        ...
        if check:
            our_vals = machine.mem[inp_values_p : inp_values_p + len(inp.values)]
            ref_vals = ref_mem[inp_values_p : inp_values_p + len(inp.values)]
            if our_vals != ref_vals:
                print(f"FAILED correctness on round {i}")
                return None
