"""
C3-2: Finer Hash Interleaving

Hypothesis: The current interleaving emits all of stage N for all desks, then stage N+1.
What if we interleave at a finer granularity - emit stage N for desk 0, stage N for desk 1,
then stage N+1 for desk 0, etc.

This might allow better pipeline overlap between dependent operations.
"""
...
class KernelBuilderC3_2:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        def emit_hash_interleaved_fine(group_desks):
            """Emit hash stages with FINER interleaving - 2 stages at a time"""
            # Stages 0+1 interleaved
        ...
        def emit_rounds_0_1_2_fused(group_desks):
            ...
            emit_hash_interleaved_fine(group_desks)
            for d in group_desks:
                desk = desks[d]
