"""
C3-1: Reduced Hash Stages

Hypothesis: What if fewer than 6 hash stages can produce correct results?

The hash function is used to determine branching direction (val & 1).
If the hash only needs to produce random-looking bits (not cryptographic),
maybe 4 stages are sufficient.

This will likely FAIL correctness - but we need to verify.
"""
...
class KernelBuilderC3_1:
    """C3-1: Reduced hash stages experiment"""
    def __init__(self, num_hash_stages=4):
        ...
        self.const_map = {}
        self.vconst_map = {}
        self.num_hash_stages = num_hash_stages
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Only use first num_hash_stages stages
        FMA_MULTIPLIERS = {0: 4097, 2: 33, 4: 9}
        v_hash_consts = []
        v_hash_shifts = []
        v_fma_mult = {}

        for hi in range(self.num_hash_stages):
        ...
        print(f"Using {self.num_hash_stages} hash stages (instead of 6)")

        self.emit("flow", ("pause",))

        def emit_hash_interleaved(group_desks):
            """Emit hash stages interleaved across desks - REDUCED STAGES"""
            # Only emit stages 0 to num_hash_stages-1
            for stage_idx in range(self.num_hash_stages):
                for d in group_desks:
                    desk = desks[d]
                    if stage_idx in v_fma_mult:
                        # FMA stage (0, 2, 4)
                        self.emit("valu", ("multiply_add", desk['val'], desk['val'], v_fma_mult[stage_idx], v_hash_consts[stage_idx]))
                    else:
                        # XOR stage (1, 3, 5)
                        op1, val1, op2, op3, val3 = HASH_STAGES[stage_idx]
                        if op1 == "^":
                            self.emit("valu", ("^", desk['tmp1'], desk['val'], v_hash_consts[stage_idx]))
                        else:
                            self.emit("valu", ("+", desk['tmp1'], desk['val'], v_hash_consts[stage_idx]))
                        if op3 == ">>":
                            self.emit("valu", (">>", desk['tmp2'], desk['val'], v_hash_shifts[stage_idx]))
                        else:
                            self.emit("valu", ("<<", desk['tmp2'], desk['val'], v_hash_shifts[stage_idx]))
                        self.emit("valu", (op2, desk['val'], desk['tmp1'], desk['tmp2']))
...
def do_kernel_test(forest_height: int, rounds: int, batch_size: int, seed: int = 123,
                   ...
                   num_hash_stages: int = 4):
    print(f"{forest_height=}, {rounds=}, {batch_size=}, {num_hash_stages=}")
    random.seed(seed)
    forest = Tree.generate(forest_height)
    inp = Input.generate(forest, batch_size, rounds)
    mem = build_mem_image(forest, inp)

    kb = KernelBuilderC3_1(num_hash_stages=num_hash_stages)
    ...
    for i, ref_mem in enumerate(reference_kernel2(mem, value_trace)):
        ...
        if check:
            ...
            if our_vals != ref_vals:
                ...
                print(f"First mismatch: our={our_vals[:10]} vs ref={ref_vals[:10]}")
                return None
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stages", type=int, default=4, help="Number of hash stages to use")
    args = parser.parse_args()

    if args.check:
        cycles = do_kernel_test(10, 16, 256, check=True, num_hash_stages=args.stages)
        if cycles is not None:
            print(f"Correctness check PASSED! Cycles: {cycles}")
        else:
            print(f"Correctness check FAILED with {args.stages} stages")
    else:
        do_kernel_test(10, 16, 256, trace=args.trace, num_hash_stages=args.stages)
