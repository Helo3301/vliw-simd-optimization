"""
# Experiment H103: Early Address Computation (Prefetch Pattern)

**GOAL:** Hide address computation latency by computing addresses earlier in the pipeline.

**HYPOTHESIS:** Address computation (addr = forest_p + idx) happens after branch, creating
a dependency chain. If we compute addresses earlier (before hash or during hash), we can
hide some latency.

**KEY INSIGHT:** For gather rounds (3-9, 14-15), the pattern is currently:
  addr computation -> gather -> xor -> hash -> branch

After branch, the NEW idx is available. If we compute addresses for round N right after
round N-1's branch (using the just-computed idx), then by the time we get to round N's
gather, the addresses are already ready.

**NEW PATTERN for rounds 3-9, 14-15:**
  - Round N-1 branch produces idx
  - Immediately compute addr = forest_p + idx (for round N)
  - Round N: gather (addresses ready!) -> xor -> hash -> branch -> compute addr for N+1

**SCRATCH BUDGET:** Same as H96 (1532/1536)
...
class KernelBuilderH103:
    """
    H103: Early address computation to hide latency.

    For gather rounds (3-9, 14-15), compute addresses immediately after the
    previous round's branch, so addresses are ready when gather starts.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel using 32 desks per tile with early address computation.
        """
        ...
        print(f"  - H103: Early address computation for gather rounds")
        ...
        def emit_addr_computation_all_desks():
            """Emit address computation for all desks (addr = forest_p + idx)."""
            for d in range(NUM_DESKS):
                self.emit("valu", ("+", desks[d]['addr'], v_forest_p, desks[d]['idx']))

        def emit_gather_all_desks():
            """Emit gather (load from precomputed addresses) for all desks."""
            for d in range(NUM_DESKS):
        ...
        def emit_tile():
            ...
            # === H103 OPTIMIZATION: Early address computation for gather rounds ===
            # After round 2's branch, we have idx values ready for round 3
            # Compute addresses BEFORE the hash stages of round 2 finish
            # Actually, compute them right after branch since idx is now available

            # Compute addresses for round 3 (first gather round)
            emit_addr_computation_all_desks()

            # Rounds 3-9: Gather without bounds, with early address computation
            # Pattern: gather (addrs ready!) -> xor -> hash -> branch -> compute addrs for next round
            for _round in range(3, 10):
                # Gather for all desks (addresses already computed!)
                emit_gather_all_desks()
                # XOR for all desks
                for d in range(NUM_DESKS):
                    self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))
                ...
                for d in range(NUM_DESKS):
                    emit_branch(d)
                # Compute addresses for NEXT round (if not last gather round before bounds)
                if _round < 9:
                    emit_addr_computation_all_desks()

            # After round 9, compute addresses for round 10 (needs bounds after)
            emit_addr_computation_all_desks()

            # Round 10: Gather WITH bounds
            # Addresses already computed
            emit_gather_all_desks()
            for d in range(NUM_DESKS):
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))
            emit_hash_stages_all_desks()
            ...
            # After round 13's branch, compute addresses for round 14
            emit_addr_computation_all_desks()
            ...
            for _round in range(14, 16):
                ...
                for d in range(NUM_DESKS):
                    emit_branch(d)
                # Compute addresses for next round (only if not last round)
                if _round < 15:
                    emit_addr_computation_all_desks()
