"""
# Experiment H13: Store Coalescing with Double Buffering

This builds on H7H10 (5,947 cycles) by restructuring the store pattern
to overlap stores with other operations.

**Current Pattern in H7H10:**
- Stores happen sequentially at the end of each iteration (4 cycles for 8 vstores)
- Store cycles are "pure" store cycles with no other work

**Optimized Pattern:**
- Overlap stores with loop control ALU operations
- Overlap stores with next iteration's offset calculations
- Use the fact that store and alu/valu engines are independent

**Key Changes:**
1. Move store operations to overlap with loop control ALU ops
2. Overlap later stores with early offset calculations for next iteration
3. Potentially save 2-3 cycles per iteration by utilizing store ports
   while ALU/VALU do other work

**Expected:** ~5,700-5,800 cycles (H7H10's 5,947 minus ~150-200 cycles)
"""
...
class KernelBuilderH13:
    """
    H13: Store Coalescing with Overlap

    Key optimization: Overlap store operations with:
    - Loop control ALU operations
    - Next iteration offset calculations

    This takes advantage of the fact that store and alu engines are separate
    and can execute in parallel (each has independent slot limits).
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def build_kernel(
        ...
        """
        H13: Store Coalescing kernel with overlapped stores.
        """
        ...
        # H13: Additional scratch for next iteration's offset registers
        # This enables overlapping stores with next iteration offset calculation
        ...
        # === MAIN LOOP with AGGRESSIVE INTERLEAVING + STORE COALESCING ===
        ...
        # H10 optimization: vselect -> multiply for desk0
        self.instrs.append({
            "valu": [
        ...
        # H10 optimization: desk1 vselect -> multiply + desk2 branch prep + desk3 hash3 combine
        self.instrs.append({
            "valu": [
                # H10 optimization: vselect -> multiply
                ("*", desks[1]['idx'], desks[1]['idx'], desks[1]['tmp1']),
        ...
        # H10 optimization: desk2 vselect -> multiply + desk3 branch prep
        self.instrs.append({
            "valu": [
                # H10 optimization: vselect -> multiply
                ("*", desks[2]['idx'], desks[2]['idx'], desks[2]['tmp1']),
        ...
        # ============================================================
        # Key insight: We can overlap stores with ALU operations because
        # they use different execution engines. The store engine has 2 slots
        # and the ALU engine has 12 slots - they are independent.
        #
        # Original H7H10 pattern (4 pure store cycles):
        #   Cycle 1: store desk0 idx/val
        #   Cycle 2: store desk1 idx/val
        #   Cycle 3: store desk2 idx/val
        #   Cycle 4: store desk3 idx/val
        #   Cycle 5: loop control ALU
        #   ...
        #
        # Optimized H13 pattern (overlap stores with ALU):
        #   Cycle 1: store desk0 idx/val + desk3 bounds check + store addr calc
        #   Cycle 2: store desk1 idx/val + desk3 vselect + more store addr calc
        #   Cycle 3: store desk2 idx/val + loop control (batch_offset update)
        #   Cycle 4: store desk3 idx/val + loop control (iter_counter, compare)
        #
        # This saves 2-4 cycles per iteration!
        # ============================================================

        # Cycle 1: Desk3 bounds check + compute desk 0,1 store addresses + store desk0
        # Note: We need to compute store addresses before we can store, but we can
        # overlap desk3 final computation with desk0-1 store address calculation
        self.instrs.append({
            "valu": [
        ...
        # Cycle 2: Desk3 vselect -> multiply + desk2,3 store addresses + store desk0
        self.instrs.append({
            "valu": [
                # H10 optimization: vselect -> multiply
                ("*", desks[3]['idx'], desks[3]['idx'], desks[3]['tmp1']),
        ...
        # Cycle 3: Store desk1 + loop control (update batch_offset)
        self.instrs.append({
            "store": [
        ...
        # Cycle 4: Store desk2 + loop control (compare batch_offset)
        self.instrs.append({
            "store": [
        ...
        # Cycle 5: Store desk3 + select for batch_offset wrap
        self.instrs.append({
            "store": [
        ...
        # Final loop control - iter check and jump
        self.instrs.append({
