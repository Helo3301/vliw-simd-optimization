"""
# Experiment H136 v3: Fused Branch + Address with Precomputed Base

**GOAL:** Reduce cycles by fusing branch and address computation more tightly.

**HYPOTHESIS:** The branch computation and address computation for the next round
...
Fused approach:
  tmp = val & 1
  idx_plus_one = idx + 1  (can overlap with hash)
  new_idx = 2*idx + 1 + tmp = idx + idx_plus_one + tmp
  addr = forest_p + new_idx

Or even better - precompute `idx_plus_1 = idx + 1` DURING hash,
then the branch becomes:
  tmp = val & 1
  new_idx = idx + idx_plus_1 + tmp
  addr = forest_p + new_idx

This might allow better overlap since idx_plus_1 doesn't depend on hash.
...
class KernelBuilderH136v3:
    """
    H136 v3: Precompute idx+1 during hash to speed up branch.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        print(f"  - H136 v3: Precompute idx+1 during hash")
        ...
        def emit_hash_stages_with_precompute():
            """
            H136 v3: Emit hash stages with idx+1 precomputation interleaved.

            During hash, we precompute idx_plus_one = idx + 1 and store it
            in the 'addr' register (which we don't need during hash).

            Then the branch can use: new_idx = idx + idx_plus_one + bit
            Which is equivalent to: new_idx = idx + (idx+1) + bit = 2*idx + 1 + bit
            """
            # Stage 0 (FMA)
            for d in range(NUM_DESKS):
                self.emit("valu", ("multiply_add", desks[d]['val'], desks[d]['val'], v_fma_mult[0], v_hash_consts[0]))

            # H136 v3: Precompute idx + 1 (use addr as storage since it's free)
            for d in range(NUM_DESKS):
                self.emit("valu", ("+", desks[d]['addr'], desks[d]['idx'], v_one))
        ...
        def emit_branch_with_precomputed(desk_idx):
            """
            H136 v3: Branch using precomputed idx+1 (stored in addr).

            At this point, addr = old_idx + 1
            new_idx = old_idx + (old_idx + 1) + bit = 2*old_idx + 1 + bit

            So: new_idx = idx + addr + bit
            """
            d = desks[desk_idx]
            tmp1, _ = get_temps(desk_idx)
            # Get branch bit
            self.emit("valu", ("&", tmp1, d['val'], v_one))
            # new_idx = idx + addr (where addr = idx+1) = 2*idx + 1
        ...
        def emit_tile():
            ...
            # Rounds 3-9: Gather with precomputed idx+1
            for _round in range(3, 10):
                # Gather (addr already computed)
                for d in range(NUM_DESKS):
                ...
                for d in range(NUM_DESKS):
                    self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))
                # Hash with idx+1 precomputation
                emit_hash_stages_with_precompute()
                # Branch using precomputed value
                for d in range(NUM_DESKS):
                    emit_branch_with_precomputed(d)
            ...
            for d in range(NUM_DESKS):
                self.emit("valu", ("^", desks[d]['val'], desks[d]['val'], desks[d]['node_val']))
            # Use regular hash (no precompute needed - next round is arithmetic)
            emit_hash_stages_all_desks()
            for d in range(NUM_DESKS):
                emit_branch(d)
            for d in range(NUM_DESKS):
            ...
            # Round 15 (last round - no precompute needed)
            for d in range(NUM_DESKS):
                for lane in range(VLEN):
                    self.emit("load", ("load", desks[d]['node_val'] + lane, desks[d]['addr'] + lane))
            for d in range(NUM_DESKS):
