"""
Theory 19: Combine R10 skip with R11 skip
After R10's branch, all indices wrap to 0. Since we know this, and R11 always
XORs with tree[0], we can potentially fuse R10+R11 into a single operation.

Actually we can't skip R11's computation - we still need to XOR, hash, and branch.
But we can avoid the gather for R11 since we know idx=0.

This is similar to what the baseline already does with fused R11+R12+R13.
Let's try being more aggressive: skip R10's XOR entirely? No - we still need it.

Alternative: Use precomputed hash constants for the known tree[0] XOR pattern.
"""
...
class KernelBuilderTheory19:
    """Theory 19: Combine R10+R11 skip optimizations"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # R10 optimized: skip branch, set idx=0
        def emit_round_10_optimized(group_desks):
            ...
            for d in group_desks:
                ...
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            emit_hash_interleaved(group_desks)
            # Skip branch computation, just set idx=0
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['idx'], desk['idx'], desk['idx']))

        # Fused R11+R12+R13 (same as baseline - already optimal given R10 skip)
