"""
Theory 15: Fuse rounds 0+1+2+3 with 8-way selection
Preload all 15 nodes of levels 0-3, then use 8-way selection to pick the node
for round 3 based on the 3 bits from rounds 0,1,2.
"""
...
class KernelBuilderTheory15:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Preload all 15 nodes of levels 0-3
        NUM_PRELOADED = 15
        v_tree = []
        ...
        # Pre-compute differences for 2-way selections
        ...
        # Pre-compute differences for level 3 (8 nodes: 7-14)
        ...
        for d in range(NUM_DESKS):
            desk = {
                ...
                'bit2': self.alloc_vec(f"v_bit2_{d}"),
            }
            desks.append(desk)
        ...
        # Fused rounds 0+1+2+3 with 8-way selection for round 3
        def emit_rounds_0_1_2_3_fused(group_desks):
            ...
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("&", desk['bit2'], desk['val'], v_one))
            for d in group_desks:
                emit_branch(d)

            # === Round 3 (8-way selection from preloaded level 3 nodes) ===
            # idx is now 7-14, node_val = tree[idx]
            # We use 8-way selection: tree[7 + bit0*4 + bit1*2 + bit2]
            for d in group_desks:
                desk = desks[d]
                # First level: select pairs based on bit2
                self.emit("valu", ("multiply_add", desk['node_val'], desk['bit2'], v_diff_7_8, v_tree[7]))
                self.emit("valu", ("multiply_add", desk['tmp1'], desk['bit2'], v_diff_9_10, v_tree[9]))
                self.emit("valu", ("multiply_add", desk['tmp2'], desk['bit2'], v_diff_11_12, v_tree[11]))
                self.emit("valu", ("multiply_add", desk['addr'], desk['bit2'], v_diff_13_14, v_tree[13]))
                # Second level: select quads based on bit1
                self.emit("flow", ("vselect", desk['node_val'], desk['bit1'], desk['tmp1'], desk['node_val']))
                self.emit("flow", ("vselect", desk['tmp2'], desk['bit1'], desk['addr'], desk['tmp2']))
                # Third level: select octets based on bit0
                self.emit("flow", ("vselect", desk['node_val'], desk['bit0'], desk['tmp2'], desk['node_val']))
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                # Fused rounds 0+1+2+3 with 8-way selection
