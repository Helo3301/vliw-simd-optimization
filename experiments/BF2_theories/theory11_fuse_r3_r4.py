"""
Theory 11: Fuse rounds 3+4 with 4-way preload
Preload tree nodes for rounds 3 and 4 together and compute them in one fused block.
"""
...
class KernelBuilderTheory11:
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Preload 15 tree nodes for levels 0-3 (1+2+4+8=15 nodes)
        NUM_PRELOADED = 15
        v_tree = []
        ...
        # Pre-compute differences for select operations
        ...
        # Pre-compute diffs for level 3 nodes (indices 7-14)
        ...
        # NEW: Fused rounds 3+4 using preloaded level 3 nodes
        def emit_rounds_3_4_fused(group_desks):
            """Fuse R3+R4: Use 4-way selection from preloaded nodes"""
            # Round 3: gather from level 3 (nodes 7-14)
            for d in group_desks:
                desk = desks[d]
            ...
            # Round 4: still gather (nodes 15-30)
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("+", desk['addr'], v_forest_p, desk['idx']))
            for d in group_desks:
