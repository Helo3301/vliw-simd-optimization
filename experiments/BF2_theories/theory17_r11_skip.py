"""
Theory 17: Skip round 11 computation (like R10 skip)
After R10, idx=0 for all. Round 11 XORs with tree[0], which is a broadcast constant.
We can skip the XOR and hash tree[0] directly into the value.
"""
...
class KernelBuilderTheory17:
    """Theory 17: Skip R11 computation"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Modified: R11 knows idx=0, so we just XOR with tree[0] which is already preloaded
        # Still need to do the full XOR/hash/branch, can't actually skip anything
        # But we can avoid the addr calculation and gather
        def emit_round_11_optimized(group_desks):
            """R11: idx=0 from R10, so we know we XOR with tree[0]"""
            # XOR with tree[0] (preloaded constant)
            for d in group_desks:
                emit_xor_with_node(d, v_tree[0])
            # Hash
            emit_hash_interleaved(group_desks)
            # Branch: idx = 2*0 + 1 + (val&1) = 1 + bit
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("&", desk['bit0'], desk['val'], v_one))
                self.emit("valu", ("+", desk['idx'], v_one, desk['bit0']))

        # Round 12: idx is either 1 or 2 based on R11
        def emit_round_12_optimized(group_desks):
            """R12: idx is 1 or 2 (children of 0), use preloaded tree[1], tree[2]"""
            for d in group_desks:
                desk = desks[d]
                # node_val = bit0 ? tree[2] : tree[1] = tree[1] + bit0 * (tree[2]-tree[1])
                self.emit("valu", ("multiply_add", desk['node_val'], desk['bit0'], v_diff_1_2, v_tree[1]))
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("^", desk['val'], desk['val'], desk['node_val']))
            ...
            for d in group_desks:
                desk = desks[d]
                self.emit("valu", ("&", desk['tmp1'], desk['val'], v_one))
                # idx = 2*idx + 1 + bit = 2*(1+bit0) + 1 + bit = 3 + 2*bit0 + bit
                self.emit("valu", ("multiply_add", desk['idx'], desk['bit0'], v_two, v_three))
                self.emit("valu", ("+", desk['idx'], desk['idx'], desk['tmp1']))

        # Round 13: idx is 3,4,5,6, use preloaded tree nodes
        def emit_round_13_optimized(group_desks):
            """R13: idx is 3,4,5,6 (level 2), use 4-way selection from preloaded"""
            for d in group_desks:
                desk = desks[d]
                # 4-way select based on bit0 and tmp1 (current bit)
                self.emit("valu", ("multiply_add", desk['node_val'], desk['tmp1'], v_diff_3_4, v_tree[3]))
                self.emit("valu", ("multiply_add", desk['tmp2'], desk['tmp1'], v_diff_5_6, v_tree[5]))
                self.emit("flow", ("vselect", desk['node_val'], desk['bit0'], desk['tmp2'], desk['node_val']))
            for d in group_desks:
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                ...
                # Optimized R11, R12, R13 (skip gather, use preloaded)
                emit_round_11_optimized(group_desks)
                emit_round_12_optimized(group_desks)
                emit_round_13_optimized(group_desks)
