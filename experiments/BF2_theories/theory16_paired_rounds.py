"""
Theory 16: Process rounds in pairs (0-1, 2-3, 4-5...)
Instead of processing all rounds serially, process them in overlapping pairs
to improve ILP by starting next round's addr calc while current is hashing.
"""
...
class KernelBuilderTheory16:
    """Theory 16: Process rounds in pairs for better ILP"""
    def __init__(self):
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # Use 2 desks per group: one processes even rounds, one odd rounds
        NUM_DESKS = 16
        desks = []
        ...
        # Paired gather rounds: Emit addr calc for both, then gather for both
        def emit_gather_rounds_paired(group_desks, round_a, round_b):
            """Process two rounds with interleaved addr/gather operations"""
            # R-a: addr calc
            for d in group_desks:
                desk = desks[d]
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            for group_desks in all_groups:
                ...
                # Rounds 3-9: process in pairs (3-4, 5-6, 7-8) then 9 alone
                emit_gather_rounds_paired(group_desks, 3, 4)
                emit_gather_rounds_paired(group_desks, 5, 6)
                emit_gather_rounds_paired(group_desks, 7, 8)
                emit_gather_round_interleaved(group_desks)  # Round 9
