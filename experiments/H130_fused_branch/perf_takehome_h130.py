"""
# Experiment H130: Fused Branch Optimization

**GOAL:** Reduce branch computation cycles by fusing operations.

**HYPOTHESIS:** The current branch computation does:
1. tmp = val & 1
2. idx = FMA(idx, 2, 1)  // idx*2 + 1
3. idx = idx + tmp

Can we combine this into fewer operations? Analysis:
- idx_new = idx*2 + 1 + (val & 1)
- If we compute bit = val & 1 first, then bit + 1 is either 1 or 2
- Then: idx_new = FMA(idx, 2, bit + 1) where bit + 1 comes from (val & 1) + 1

New approach:
1. tmp = val & 1
2. tmp = tmp + v_one  // tmp is now 1 or 2
3. idx = FMA(idx, 2, tmp)  // idx*2 + (1 or 2) = idx*2 + 1 + (val & 1)

This is still 3 ops, but the third op (FMA) has idx as input instead of output
from the previous cycle, potentially improving scheduling.

Alternative: Try double-pumping the branch ops more aggressively across desks.
...
class KernelBuilderH130:
    """
    H130: Fused branch optimization.

    Try to reduce branch computation from 3 ops to fewer by:
    1. Reordering to compute (val & 1) + 1 first
    2. Then using FMA(idx, 2, tmp) where tmp is 1 or 2

    Also try double-pumping branch ops more aggressively.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel using 32 desks per tile with reduced tree preloading.
        H130: Fused branch - try to combine branch ops.
        """
        tmp_scalar = self.alloc_scratch("tmp_scalar")
        tmp_addr = self.alloc_scratch("tmp_addr")

        # H120 CHANGE: Only load the 4 header values we actually use
        ...
        # Preload only tree nodes 0-6 (was 0-14 in earlier versions)
        # Nodes 7-14 are never used because:
        # - Rounds 0-2 and 11-13 use arithmetic selection with nodes 0-6
        # - Rounds 3-10 and 14-15 use gather (not preloaded values)
        NUM_PRELOADED = 7
        v_tree = []
        ...
        print(f"  - H130: Fused branch optimization")
        ...
        def emit_branch(desk_idx):
            """Branch using per-desk addr register as temp.

            H130: Use addr as temp instead of shared temps to enable full parallelism.
            Original (non-fused) approach: val&1 || idx*2+1 can run in parallel.
            """
            d = desks[desk_idx]
            # Use addr as temp - original approach (val&1 and idx*2+1 can run in parallel)
            self.emit("valu", ("&", d['addr'], d['val'], v_one))
            self.emit("valu", ("multiply_add", d['idx'], d['idx'], v_two, v_one))
            self.emit("valu", ("+", d['idx'], d['idx'], d['addr']))

        def emit_branch_all_desks():
            """Double-pumped branch for all desks using per-desk addr as temp.

            Since each desk has its own addr register, all 32 can run in parallel!
            Uses original (non-fused) approach where val&1 and idx*2+1 can overlap.
            """
            # Step 1: tmp = val & 1 for all desks (store in addr)
            for d in range(NUM_DESKS):
                self.emit("valu", ("&", desks[d]['addr'], desks[d]['val'], v_one))
            # Step 2: idx = idx*2 + 1 for all desks (can overlap with step 1!)
            for d in range(NUM_DESKS):
                self.emit("valu", ("multiply_add", desks[d]['idx'], desks[d]['idx'], v_two, v_one))
            # Step 3: idx = idx + tmp for all desks
            for d in range(NUM_DESKS):
                self.emit("valu", ("+", desks[d]['idx'], desks[d]['idx'], desks[d]['addr']))
        ...
        def emit_4way_selection_all_desks():
            """Double-pumped 4-way selection for rounds 2 and 13.

            Indices are in {3, 4, 5, 6}. Uses arithmetic selection with precomputed diffs.

            Key insight: desks are grouped by which temps they share:
            - Desks 0-3 share temp pair 0
            - Desks 4-7 share temp pair 1
            - etc.

            So we need to process each of these sharing groups serially, but within
            a group we can interleave with other sharing groups.

            group_0 = [0, 4, 8, ...] - these use DIFFERENT temp pairs
            group_1 = [1, 5, 9, ...] - these also use different temp pairs
            etc.

            To double-pump correctly, we process all 8 steps for one temp-conflict
            group at a time, but within each step we process all 8 desks that don't
            share temps.
            """
            # Process each step for all desks that don't share temps (group_0)
            # Then move to group_1, etc.

            for group in [group_0, group_1, group_2, group_3]:
                # All desks in this group have different temps, so can be double-pumped
                for d in group:
                    tmp1, _ = get_temps(d)
            ...
            # Step 8: XOR val with node_val for all desks (no temp conflicts)
            for d in range(NUM_DESKS):
        ...
        def emit_tile():
            ...
            emit_branch_all_desks()  # H130: Use double-pumped branch
            ...
            # Serial processing - each desk complete before moving to next
            for d in range(NUM_DESKS):
                tmp1, tmp2 = get_temps(d)
                ...
                self.emit("valu", (">>", desks[d]['addr'], tmp1, v_one))  # bit1 = tmp1 >> 1
            ...
            for _round in range(3, 10):
                ...
                emit_hash_stages_all_desks()
                # Branch for all desks - H130: Use double-pumped branch
                emit_branch_all_desks()
