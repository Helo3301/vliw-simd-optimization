"""
# Experiment H64: Level-Based Preloading with Round Synchronization

**GOAL:** Sub-2000 cycles by exploiting the starting condition (all elements at index 0)

**BREAKTHROUGH INSIGHT** (from H63 analysis):
- All 256 elements start at index 0
- Rounds 0-7 have bounded index diversity (at most 2^N unique indices for round N)
- Tree levels are contiguous in memory - vload loads 8 contiguous values
- Levels 0-7 = 255 nodes = 32 vloads = just 16 cycles to preload!

**ARCHITECTURE**:
Uses H54's proven desk-based pipelining with 16 desks and round fusion.
The tree preloading provides memory locality benefits for early rounds.

**KEY TECHNIQUES**:
1. 16 desks for maximum pipeline depth
2. Round fusion: 2 rounds per iteration
3. Interleaved gather + compute
4. FMA for hash: `a * (1 + 2^n) + C` reduces 3 ops to 1
"""
...
class KernelBuilderH64:
    """
    H64: Level-Based Preloading with Round Synchronization

    Based on H54's successful 16-desk approach with round fusion.
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def build_kernel(
        ...
        """
        H64: 16-Desk Ultra-Deep Pipeline kernel with round fusion.
        Based on H54 but optimized.
        """
