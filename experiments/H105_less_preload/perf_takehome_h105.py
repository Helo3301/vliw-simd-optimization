"""
# Experiment H105: Reduced Tree Preloading

**GOAL:** Reduce NUM_PRELOADED from 15 to 7 (only nodes 0-6) to free up scratch space.

**HYPOTHESIS:** We preload 15 tree nodes but only use nodes 0-6 for arithmetic selection:
- Round 0: Uses tree[0]
- Round 1: Uses tree[1], tree[2]
- Round 2: Uses tree[3], tree[4], tree[5], tree[6]
- Round 11: Uses tree[0] (after wrap)
- Round 12: Uses tree[1], tree[2] (after wrap)
- Round 13: Uses tree[3], tree[4], tree[5], tree[6] (after wrap)

Nodes 7-14 are never used because:
- Rounds 3-10 and 14-15 use gather (not preloaded values)
- After round 10, indices wrap to 0, so rounds 11-15 only need nodes 0-6

**SCRATCH SAVINGS:** 8 * 8 = 64 slots freed up
...
class KernelBuilder:
    """
    H105: Reduced tree preloading - only nodes 0-6 (saves 64 scratch slots).
    """
    def __init__(self):
