"""
# Experiment H134: Inline Diff Computation

**GOAL:** Test whether computing tree diffs inline during rounds improves scheduling.

**HYPOTHESIS:** H120 precomputes v_diff_1_2, v_diff_3_4, v_diff_5_6 during init. These are
only used in rounds 1, 2, 12, and 13. By computing them inline just before their use,
the scheduler might better overlap tree loads with diff computations.
...
class KernelBuilderH134:
    """
    H134: Inline diff computation - compute diffs during rounds instead of init.

    Instead of precomputing v_diff_1_2, v_diff_3_4, v_diff_5_6 during init,
    compute them just before the rounds that need them (rounds 1, 2, 12, 13).
    This might allow better scheduling overlap with tree loads.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel using 32 desks per tile with inline diff computation.
        H134: Compute diffs inline during rounds instead of init phase.
        """
        ...
        # H134 CHANGE: Allocate diff registers but DON'T compute them here
        # They will be computed inline just before the rounds that use them
        ...
        # REMOVED: The precomputation of diffs during init
        ...
        print(f"  - H134: Inline diff computation - diffs computed during rounds")
        ...
        def emit_tile():
            ...
            # H134 CHANGE: Compute v_diff_1_2 inline just before Round 1
            self.emit("valu", ("-", v_diff_1_2, v_tree[2], v_tree[1]))
            ...
            # H134 CHANGE: Compute v_diff_3_4 and v_diff_5_6 inline just before Round 2
            self.emit("valu", ("-", v_diff_3_4, v_tree[4], v_tree[3]))
            self.emit("valu", ("-", v_diff_5_6, v_tree[6], v_tree[5]))
            ...
            # H134 CHANGE: Recompute v_diff_1_2 inline just before Round 12
            # (We could potentially reuse the previous computation, but this tests
            # the scheduling hypothesis more directly)
            self.emit("valu", ("-", v_diff_1_2, v_tree[2], v_tree[1]))
            ...
            # H134 CHANGE: Recompute v_diff_3_4 and v_diff_5_6 inline just before Round 13
            self.emit("valu", ("-", v_diff_3_4, v_tree[4], v_tree[3]))
            self.emit("valu", ("-", v_diff_5_6, v_tree[6], v_tree[5]))
