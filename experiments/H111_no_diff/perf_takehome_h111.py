"""
# Experiment H111: No Precomputed Differences

**GOAL:** Remove precomputed differences (v_diff_1_2, v_diff_3_4, v_diff_5_6) and compute inline.

**HYPOTHESIS:** We allocate 3 vectors (24 slots) for precomputed differences used in FMA:
  result = bit * diff + base
Inline computation would be:
  tmp_diff = tree[high] - tree[low]
  result = bit * tmp_diff + tree[low]

Trade-off: 3 more ops per selection (in rounds 1, 2, 12, 13) vs 24 fewer scratch slots.
...
class KernelBuilderH111:
    """
    H111: No precomputed differences - compute inline (saves 24 scratch slots).
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        # H111: Preload tree nodes 0-6 (same as H105)
        # We no longer precompute differences - compute inline instead
        NUM_PRELOADED = 7
        v_tree = []
        ...
        # H111 CHANGE: NO precomputed differences - we compute them inline
        # This saves 24 scratch slots (3 vectors * 8 lanes)
        ...
        print(f"  - H111: No precomputed diffs (saved 24 slots vs H105)")
        ...
        def emit_tile():
            ...
            # H111: Compute diff inline: result = bit * (tree[2] - tree[1]) + tree[1]
            for group in [group_0, group_1, group_2, group_3]:
                for d in group:
                    tmp1, tmp2 = get_temps(d)
                    self.emit("valu", ("-", tmp1, desks[d]['idx'], v_one))  # bit = 0 or 1
                for d in group:
                    tmp1, tmp2 = get_temps(d)
                    self.emit("valu", ("-", tmp2, v_tree[2], v_tree[1]))  # diff = tree[2] - tree[1]
                for d in group:
                    tmp1, tmp2 = get_temps(d)
                    self.emit("valu", ("multiply_add", desks[d]['node_val'], tmp1, tmp2, v_tree[1]))
            for d in range(NUM_DESKS):
            ...
            # H111: Compute differences inline
            for d in range(NUM_DESKS):
                tmp1, tmp2 = get_temps(d)
                ...
                # Select from low pair: compute diff inline
                self.emit("valu", ("-", tmp1, v_tree[4], v_tree[3]))  # diff_3_4 = tree[4] - tree[3]
                self.emit("valu", ("multiply_add", desks[d]['node_val'], tmp2, tmp1, v_tree[3]))  # low_pair
                # Select from high pair: compute diff inline
                self.emit("valu", ("-", tmp1, v_tree[6], v_tree[5]))  # diff_5_6 = tree[6] - tree[5]
                self.emit("valu", ("multiply_add", tmp1, tmp2, tmp1, v_tree[5]))  # high_pair in tmp1
