"""
# Experiment H69: Global Instruction Scheduling

**GOAL:** Achieve sub-2000 cycles using 16-desk deep pipeline.

**APPROACH:**
This is based on H68 but with optimizations:
1. Process 2 tiles of 128 elements (16 desks per tile)
2. Use selection for rounds 0-1 (indices known: 0 for round 0, {1,2} for round 1)
3. Use gather for rounds 2-15 with deeply interleaved hash operations
4. Skip bounds check for rounds 0-9 (indices guaranteed valid)

**ANALYSIS:**
- H68 achieves 2,775 cycles
- Target: <2,000 cycles
- Key opportunity: Better interleaving of preload rounds with gather pipeline
"""
...
class KernelBuilderH69:
    """
    H69: 16-desk deep pipeline matching H68, then optimizing.
    """
    def __init__(self):
        self.instrs = []
        self.scratch = {}
    ...
    def build_kernel(
        ...
        """
        H69: 16-desk deep pipeline with 2 tiles.
        """
        ...
        # Calculate all offsets for 16 desks
        self.instrs.append({
            "alu": [
        ...
        def emit_preload_round_all_idx_known(tree_idx, skip_bounds=False):
            ...
            # Broadcast tree[tree_idx] to all desk node_vals
        ...
        def emit_preload_round_1(skip_bounds=False):
            ...
            # node_val = tmp1 * (tree[2] - tree[1]) + tree[1]
            for d in range(0, NUM_DESKS, 6):
                ops = []
                for dd in range(d, min(d + 6, NUM_DESKS)):
        ...
        # UNROLLED ROUNDS
        # Round 0: All indices = 0 - skip bounds (max idx = 2)
        emit_preload_round_all_idx_known(0, skip_bounds=True)

        # Round 1: Indices in {1, 2} - skip bounds (max idx = 6)
        ...
        # Rounds 11-15: Use gather with bounds
