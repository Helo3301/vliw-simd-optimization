"""
# Experiment H143a: Reduce ops - attempt to optimize 4-way selection

**GOAL:** Reduce the 4-way selection from 7 VALU ops to 5-6 ops

**APPROACH:**
- Precompute more differences between tree nodes
- Use different formula for 4-way selection

**ANALYSIS:**
Current 4-way selection (7 ops):
1. SUB: offset = idx - 3
2. AND: bit0 = offset & 1
3. SHIFT: bit1 = offset >> 1
4. FMA: low = bit0 * diff_3_4 + tree[3]
5. FMA: high = bit0 * diff_5_6 + tree[5]
6. SUB: diff = high - low
7. FMA: result = bit1 * diff + low

Alternative approach - precompute more:
- tree[3], tree[4], tree[5], tree[6]
- diff_3_4 = tree[4] - tree[3]
- diff_5_6 = tree[6] - tree[5]
- diff_3_5 = tree[5] - tree[3]
- cross_diff = tree[3] - tree[4] - tree[5] + tree[6]

Result = tree[3] + bit0 * diff_3_4 + bit1 * diff_3_5 + bit0 * bit1 * cross_diff

This needs bit0*bit1 (extra MUL), so 6 ops total:
1. SUB: offset = idx - 3
2. AND: bit0 = offset & 1
3. SHIFT: bit1 = offset >> 1
4. MUL: bit01 = bit0 * bit1
5. FMA: tmp = bit0 * diff_3_4 + tree[3]
6. FMA: tmp2 = bit1 * diff_3_5 + tmp
7. FMA: result = bit01 * cross_diff + tmp2

Still 7 ops. Let's try a different approach.

Actually, let's try using vselect for one level:
1. SUB: offset = idx - 3
2. AND: bit0 = offset & 1
3. SHIFT: bit1 = offset >> 1
4. FMA: low = bit0 * diff_3_4 + tree[3]  (tree[3] or tree[4])
5. FMA: high = bit0 * diff_5_6 + tree[5]  (tree[5] or tree[6])
6. vselect: result = bit1 ? high : low

Wait, vselect is a FLOW op, not VALU! Let me check the slot limits.
FLOW limit is 1 per cycle, but it's a separate slot.

So 5 VALU + 1 FLOW = same cycle cost potentially.
...
class KernelBuilderH143a:
    """
    H143a: Optimized 4-way selection using vselect for final stage
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        """
        Build kernel with optimized 4-way selection using vselect.
        """
        tmp_scalar = self.alloc_scratch("tmp_scalar")
        tmp_addr = self.alloc_scratch("tmp_addr")

        # Fast init - only load 4 of 7 header values
        fast_init_vars = [
            ("n_nodes", 1),
            ("forest_values_p", 4),
            ("inp_indices_p", 5),
        ...
        # Precompute tree differences for 2-way selection
        ...
        def emit_round_2_vselect(desk_idx):
            """Round 2: Indices in {3, 4, 5, 6}, use vselect for final selection

            Approach: 5 VALU + 1 FLOW (vselect)
            1. SUB: offset = idx - 3
            2. AND: bit0 = offset & 1
            3. SHIFT: bit1 = offset >> 1
            4. FMA: low = bit0 * diff_3_4 + tree[3]
            ...
            self.emit("valu", ("-", d['tmp1'], d['idx'], v_three))  # offset = idx - 3
            self.emit("valu", ("&", d['tmp2'], d['tmp1'], v_one))   # bit0 = offset & 1
            self.emit("valu", (">>", d['addr'], d['tmp1'], v_one))  # bit1 = offset >> 1
            # Select from pairs using FMA
            self.emit("valu", ("multiply_add", d['node_val'], d['tmp2'], v_diff_3_4, v_tree[3]))  # low
            self.emit("valu", ("multiply_add", d['tmp1'], d['tmp2'], v_diff_5_6, v_tree[5]))  # high
            # Final select using vselect (FLOW)
            self.emit("flow", ("vselect", d['node_val'], d['addr'], d['tmp1'], d['node_val']))  # bit1 ? high : low
            # XOR and hash
            self.emit("valu", ("^", d['val'], d['val'], d['node_val']))
            emit_hash_stages(desk_idx)
            emit_branch(desk_idx)
        ...
        def emit_round_11(desk_idx):
            """Round 11: All indices = 0 after wrap"""
            d = desks[desk_idx]
            self.emit("valu", ("^", d['val'], d['val'], v_tree[0]))
            emit_hash_stages(desk_idx)
            emit_branch(desk_idx)
        ...
        def emit_round_13_vselect(desk_idx):
            """Round 13: Indices in {3, 4, 5, 6}, use vselect for final selection"""
            d = desks[desk_idx]
            # Extract selection bits
            ...
            # Final select using vselect (FLOW)
            self.emit("flow", ("vselect", d['node_val'], d['addr'], d['tmp1'], d['node_val']))
            # XOR and hash
            self.emit("valu", ("^", d['val'], d['val'], d['node_val']))
            emit_hash_stages(desk_idx)
            emit_branch(desk_idx)
        ...
        def emit_round_15_final(desk_idx):
            """Round 15: Gather, XOR, hash - skip branch"""
            d = desks[desk_idx]
            self.emit("valu", ("+", d['addr'], v_forest_p, d['idx']))
            for lane in range(VLEN):
                self.emit("load", ("load", d['node_val'] + lane, d['addr'] + lane))
        ...
        def emit_tile_interleaved(tile_idx):
            """Emit all operations for one tile with interleaved round processing."""
            ...
            for group_desks in all_groups:
                ...
                # Use vselect version for round 2
                for d in group_desks:
                    emit_round_2_vselect(d)
                ...
                # Use vselect version for round 13
                for d in group_desks:
                    emit_round_13_vselect(d)
