"""
# Experiment H143b: Combine vselect optimization + explore hash stage optimization

**GOAL:** Build on H143a and try additional optimizations

**APPROACH:**
1. Keep vselect for 4-way selection from H143a (-27 cycles)
2. Try to optimize hash stage 1/3/5 which are XOR-SHIFT-XOR or ADD-SHIFT-XOR

Looking at hash stages more carefully:
- Stage 1: val = (val ^ 0xC761C23C) ^ (val >> 19)
- Stage 3: val = (val + 0xD3A2646C) ^ (val << 9)
- Stage 5: val = (val ^ 0xB55A4F09) ^ (val >> 16)

The pattern is: val = (val OP1 const) OP2 (val SHIFT n)

Current implementation needs val before we modify it, so we do:
1. tmp1 = val OP1 const
2. tmp2 = val SHIFT n
3. val = tmp1 OP2 tmp2

Can't do better than 3 ops without specialized instructions.

**NEW IDEA:** What if we precompute (val >> n) before the OP1 const step?

For stage 1:
- tmp2 = val >> 19  (need original val)
- tmp1 = val ^ const
- val = tmp1 ^ tmp2

Same as before. No change.

**ANOTHER IDEA:** Merge operations across stages?

Stage 0 outputs: val0 = val * 4097 + const0
Stage 1 needs: val0 and val0 >> 19

Let me see if there's any algebraic simplification...

(val * 4097 + const0) >> 19 is not easily simplifiable.

**SIMPLER APPROACH:** Let's focus on reducing total work by combining vselect +
verifying we haven't regressed anywhere.

Actually, let me try a completely different optimization: What if we can reduce
the number of gathers by using wider loads when possible?

No wait, each lane has a different idx, so we can't vectorize the load.

**TRY THIS:** Reduce the setup overhead at the start of each tile.
Currently we emit const, alu+, vload for each desk. Can these be batched better?

Let me also try: What if we use a DIFFERENT grouping strategy that allows more ILP?
"""
...
class KernelBuilderH143b:
    """
    H143b: vselect for 4-way + try larger group size for better ILP
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        def emit_round_2_vselect(desk_idx):
            """4-way selection with vselect (5 VALU + 1 FLOW instead of 7 VALU)"""
            d = desks[desk_idx]
        ...
        def emit_tile_interleaved(tile_idx):
            ...
            # Try larger group size for potentially better ILP
            GROUP_SIZE = 8  # Try 8 instead of 4
