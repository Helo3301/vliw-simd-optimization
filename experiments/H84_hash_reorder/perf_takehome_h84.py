"""
# Experiment H84: Hash Stage Emission Reordering

**GOAL:** Test if emitting hash stages in a different order affects scheduling.

**KEY CHANGE:** Instead of emitting all 6 hash stages for desk 0, then desk 1, etc.,
emit stage 0 for all desks, then stage 1 for all desks, etc.

**HYPOTHESIS:** The greedy scheduler might pack operations differently based on
emission order (tie-breaking behavior). Stage-first ordering might allow better
VALU packing since operations at the same stage have no dependencies between desks.

**BASE:** H79 (1,904 cycles)
"""
...
class KernelBuilderH84:
    """
    H84: Hash stage emission reordering experiment.

    Key change: emit_hash_stages_for_all_desks emits stage 0 for all desks,
    then stage 1 for all desks, etc. instead of all stages per desk.
    """
    def __init__(self):
    ...
    def build_kernel(
        ...
        # H84 KEY CHANGE: emit_hash_stages_for_all_desks emits stage-by-stage across all desks
        # instead of desk-by-desk
        def emit_hash_stages_for_all_desks(desk_indices):
            """Emit hash stages in stage-first order: stage 0 for all desks, then stage 1, etc."""
            for hi in range(6):
                for desk_idx in desk_indices:
                    d = desks[desk_idx]
                    if hi in v_fma_mult:
        ...
        def emit_branch_for_all_desks(desk_indices):
            for desk_idx in desk_indices:
                d = desks[desk_idx]
        ...
        def emit_bounds_for_all_desks(desk_indices):
            for desk_idx in desk_indices:
                d = desks[desk_idx]
        ...
        def emit_tile(tile_idx):
            ...
            emit_hash_stages_for_all_desks(all_desks)
            emit_branch_for_all_desks(all_desks)
            ...
            emit_hash_stages_for_all_desks(all_desks)
            emit_branch_for_all_desks(all_desks)
            emit_bounds_for_all_desks(all_desks)  # After R10, all indices wrap to 0
