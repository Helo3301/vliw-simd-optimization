"""
# Experiment H85: Shared Temporary Registers

**GOAL:** Reduce scratch usage by sharing temporary registers across desks.

**KEY CHANGE:** Instead of allocating tmp1/tmp2 per desk (16 desks * 2 temps * 8 lanes = 256 slots),
allocate only 4 shared temp vectors (4 * 8 = 32 slots) that can be reused across sequential desk operations.

**RATIONALE:** Within a round, desks are processed sequentially (the scheduler emits all operations
for desk 0, then desk 1, etc.). Since tmp1/tmp2 are only used within a single desk's hash/branch
computation and don't need to persist across desks, we can share them.

**RISK:** If the scheduler parallelizes across desks too aggressively, temps could be clobbered.
To prevent this, we use 4 temp vectors and cycle through them, giving the scheduler more freedom
while still reducing scratch usage.

**ALGORITHM:** Same as H79 (wrap exploitation, no bounds on R11-15)
**SCHEDULING:** Automatic via _schedule_slots()

**EXPECTED:** Reduced scratch usage (saving ~224 slots), potentially enabling other optimizations
"""
...
class KernelBuilderH85:
    """
    H85: Shared temporary registers to reduce scratch usage.
    """
    def __init__(self):
